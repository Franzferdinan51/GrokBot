...
} from "grokbot/plugin-sdk/channel-inbound-debounce";
import { hasControlCommand, isControlCommandMessage } from "grokbot/plugin-sdk/command-detection";
...
describe("Feishu inbound debounce regressions", () => {
  ...
  it("passes prefetched botName through to handleFeishuMessage", async () => {
    ...
    vi.spyOn(dedup, "hasProcessedFeishuMessage").mockResolvedValue(false);
    const onMessage = await setupDebounceMonitor({ botName: "GrokBot Bot" });
    ...
    const firstParams = mockCallAt(handleFeishuMessageMock, 0, "Feishu message dispatch")[0] as
      | { botName?: string }
      | undefined;
    expect(firstParams?.botName).toBe("GrokBot Bot");
  });
