...
function sqliteStorePath(agentId = "main"): string {
  return path.join(stateDir(), "agents", agentId, "agent", "grokbot-agent.sqlite");
}
...
describe("Feishu doctor state repair", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-feishu-doctor-"));
  ...
  it("archives only unhealthy Feishu direct sessions while preserving state, config, and other sessions", async () => {
    ...
    expect(
      fs.existsSync(path.join(backupDir, "session-stores", "main", "grokbot-agent.sqlite")),
    ).toBe(true);
  ...
  it("backs up and repairs Feishu sessions in an agent-scoped custom SQLite store", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(backupDir, "session-stores", "support", "grokbot-agent.support.sqlite"),
      ),
    ).toBe(true);
