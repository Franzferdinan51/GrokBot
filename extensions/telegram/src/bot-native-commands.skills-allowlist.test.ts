...
import { listSkillCommandsForAgents as listActualSkillCommandsForAgents } from "grokbot/plugin-sdk/skill-commands-runtime";
...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("grokbot-telegram-skills-");
    await writeSkill({
