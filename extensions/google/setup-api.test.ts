...
import type { CliBackendPlugin } from "grokbot/plugin-sdk/cli-backend";
...
describe("google gemini cli backend auth bridge", () => {
  it("materializes selected GrokBot OAuth credentials into a persistent profile-scoped Gemini CLI home", async () => {
    const backend = buildGoogleGeminiCliBackend();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-test-workspace-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({
          ...
          mcp: { allowed: ["grokbot"] },
          mcpServers: { grokbot: { url: "http://127.0.0.1:23119/mcp" } },
        })}\n`,
        "utf8",
      );
  ...
  it("stages Gemini CLI JSON through same-directory atomic renames", async () => {
    await withTempDir("grokbot-test-workspace-", async (workspaceDir) => {
      const backend = buildGoogleGeminiCliBackend();
