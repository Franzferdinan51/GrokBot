...
import type { Context, Model } from "grokbot/plugin-sdk/llm";
import type {
...
import { createCapturedThinkingConfigStream } from "grokbot/plugin-sdk/provider-test-contracts";
...
describe("google provider plugin hooks", () => {
  ...
  it("resolves Google Vertex ADC auth evidence to the config marker", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "grokbot-google-vertex-config-key-"));
