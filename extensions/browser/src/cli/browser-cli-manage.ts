...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "grokbot";
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "grokbot"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `grokbot browser start`",
  });
  if (status.graphics) {
    checks.push({
      name: "graphics",
...
type BrowserProfileDriver = "grokbot" | "existing-session" | "extension";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "grokbot" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (grokbot|existing-session). Default: grokbot")
    .action(
      async (
        opts: {
          name: string;
