...
const browserConfigMocks = vi.hoisted(() => ({
  resolveBrowserConfig: vi.fn((browser?: { defaultProfile?: string }) => ({
    enabled: true,
    defaultProfile: browser?.defaultProfile ?? "grokbot",
  })),
}));
...
describe("runBrowserProxyCommand", () => {
  ...
  it("serializes plural action downloads without reading nested page paths", async () => {
    const tempDir = await fs.mkdtemp(nodePath.join(os.tmpdir(), "grokbot-browser-proxy-action-"));
  ...
  it("rejects an aggregate above the proxy transport budget", async () => {
    const tempDir = await fs.mkdtemp(nodePath.join(os.tmpdir(), "grokbot-browser-proxy-limit-"));
  ...
  it("adds profile and browser status details on ws-backed timeouts", async () => {
    ...
    const result = expect(
      ...
      /browser proxy timed out for GET \/snapshot after 5ms; ws-backed browser action; profile=grokbot; status\(running=true, cdpHttp=true, cdpReady=false, cdpUrl=http:\/\/127\.0\.0\.1:18792\)/,
    );
  ...
  it("rejects unauthorized query.profile when allowProfiles is configured", async () => {
    configMocks.loadConfig.mockReturnValue({
      browser: {},
      nodeHost: { browserProxy: { enabled: true, allowProfiles: ["grokbot"] } },
    });
    ...
    await expect(
      runBrowserProxyCommand(
        JSON.stringify({
          method: "POST",
          path: "/reset-profile",
          body: { profile: "grokbot", name: "grokbot" },
          timeoutMs: 50,
        }),
      ),
    ).rejects.toThrow("INVALID_REQUEST: browser.proxy cannot mutate persistent browser profiles");
  ...
  it("canonicalizes an allowlisted body profile into the dispatched query", async () => {
    ...
    expect(request.query).toEqual({ profile: "grokbot" });
  });
