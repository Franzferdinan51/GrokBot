    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      try {
        ...
        if (!wsUrl && hasUrlCredentials && !isWebSocketUrl(normalized)) {
          ...
          // Keep credentialed discovery in GrokBot's guarded fetch path instead.
...
// GrokBot operations short-circuit instead of re-running the SSRF check on a
...
// Quarantine and close a tab that GrokBot itself navigated to a blocked URL.
...
/** Quarantine and close a tab that GrokBot navigated to a blocked URL. */
...
export async function createPageViaPlaywright(
  ...
  if (targetUrl !== "about:blank") {
    ...
    // GrokBot owns this newly-created tab: if the post-navigation safety
