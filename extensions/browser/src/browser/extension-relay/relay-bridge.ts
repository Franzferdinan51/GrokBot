 ...
 * on one side and the GrokBot Chrome extension's chrome.debugger transport on
...
const BROWSER_TARGET_ID = "grokbot-extension-relay";
/** Playwright requires every attached page target to identify its browser context. */
const BROWSER_CONTEXT_ID = "grokbot-extension-context";
...
export class ExtensionRelayBridge {
  ...
  /** Tabs currently shared with GrokBot (the extension's tab group). */
  ...
  private sendToExtension(msg: RelayToExtensionMessage): void {
    if (!this.extension) {
      throw new Error("GrokBot Chrome extension is not connected to the relay");
    }
  ...
  private async ensureTabAttached(tabId: number): Promise<{ targetId: string; sessionId: string }> {
    ...
    if (!tab) {
      throw new Error(`tab ${tabId} is not shared with GrokBot`);
    }
    if (tab.attached) {
      return tab.attached;
    }
    ...
    const attaching = (async () => {
      ...
      const sessionId = `grokbot-tab-${tabId}-${this.nextSessionOrdinal++}`;
   ...
   * "GrokBot is debugging this browser" infobar only spans active automation.
   */
    ...
    switch (request.method) {
      case "Browser.getVersion": {
        ...
        this.respond(client, request, {
          ...
          revision: "grokbot-extension-relay",
      ...
      case "Target.attachToBrowserTarget": {
        const sessionId = `grokbot-browser-${this.nextSessionOrdinal++}`;
      ...
      case "Target.attachToTarget": {
        ...
        if (request.sessionId && this.browserSessions.get(request.sessionId) === client) {
          ...
          const sessionId = `grokbot-tab-${found.tabId}-${this.nextSessionOrdinal++}`;
      ...
      case "Target.createBrowserContext": {
        this.respondError(
          client,
          request,
          "The GrokBot extension relay drives the user's real browser profile; isolated browser contexts are not supported.",
        );
        return;
      }
      default: {
