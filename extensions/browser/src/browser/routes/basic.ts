...
async function runBrowserLiveProbe(profileCtx: ProfileContext, signal: AbortSignal) {
  ...
  try {
    ...
    return {
      ...
      summary: String(err),
      fixHint: "Run grokbot browser start, then retry with grokbot browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    params.profileCtx.profile.driver !== "grokbot" ||
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched grokbot profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post("/profiles/create", async (req, res) => {
    ...
    if (driver && driver !== "grokbot" && driver !== "clawd" && driver !== "existing-session") {
      return jsonError(
        res,
        400,
        `unsupported profile driver "${driver}"; use "grokbot", "clawd", or "existing-session"`,
      );
    }
