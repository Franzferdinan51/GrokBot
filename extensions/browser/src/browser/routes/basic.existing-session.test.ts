    ...
    forProfile: () =>
      ({
        profile: {
          ...
          cdpHost: "127.0.0.1",
          cdpIsLoopback: true,
          userDataDir: "/tmp/grokbot-profile",
          color: "#FF4500",
          headless: false,
          headlessSource: "default",
          attachOnly: false,
...
describe("basic browser routes", () => {
  ...
  it("reports Linux no-display headless fallback for local managed profiles", async () => {
    ...
    try {
      ...
      expect(body.profile).toBe("grokbot");
  ...
  it("retries unavailable graphics diagnostics and caches the first available result", async () => {
    ...
    const first = await callBasicRouteWithState({ query: { profile: "grokbot" }, state });
    const second = await callBasicRouteWithState({ query: { profile: "grokbot" }, state });
    const third = await callBasicRouteWithState({ query: { profile: "grokbot" }, state });
  ...
  it("does not inspect graphics while the managed process is pending reconcile", async () => {
    ...
    state.profiles.set("grokbot", runtime);
  ...
  it("redacts CDP URL credentials from status responses", async () => {
    const response = await callBasicRouteWithState({
      query: { profile: "grokbot" },
      state: createManagedProfileState({
        cdpUrl: "http://grokbot:relay-token@127.0.0.1:18800",
      }),
    });
  ...
  it("passes valid start headless override to local managed profiles", async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual({ ok: true, profile: "grokbot" });
  ...
  it("rejects start headless override for existing-session profiles", async () => {
    ...
    expect(response.statusCode).toBe(400);
    expect(responseBodyRecord(response).error).toBe(
      'Headless start override is only supported for locally launched grokbot profiles. Profile "chrome-live" is attach-only, remote, or existing-session.',
    );
    expect(ensureBrowserAvailable).not.toHaveBeenCalled();
  });
