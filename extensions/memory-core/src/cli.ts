    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["grokbot memory status", "Show index and provider status."],
          [
            "grokbot memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["grokbot memory status --deep", "Probe embedding provider readiness."],
          ["grokbot memory index --force", "Force a full reindex."],
          ['grokbot memory search "meeting notes"', "Quick search using positional query."],
          [
            'grokbot memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `grokbot memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
            "Review weighted short-term candidates for long-term memory.",
          ],
          [
            "grokbot memory promote --apply",
            "Append top-ranked short-term candidates into MEMORY.md.",
          ],
          [
            'grokbot memory promote-explain "router vlan"',
            "Explain why a specific candidate would or would not promote.",
          ],
          [
            "grokbot memory rem-harness --json",
            "Preview REM reflections, candidate truths, and deep promotion output.",
          ],
          [
            "grokbot memory rem-backfill --path ./memory",
          ...
          ["grokbot memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.grokbot.ai/cli/memory")}\n`,
    );
