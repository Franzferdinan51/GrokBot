...
const PROMOTION_MARKER_PREFIX = "grokbot-memory-promotion:";
...
function isContaminatedDreamingSnippet(
  ...
  if (
    /<!--\s*grokbot-memory-promotion:/i.test(snippet) ||
...
const DREAMING_FENCE_START_RE = /<!--\s*grokbot:dreaming:[a-z][a-z0-9-]*:start\s*-->/i;
const DREAMING_FENCE_END_RE = /<!--\s*grokbot:dreaming:[a-z][a-z0-9-]*:end\s*-->/i;
...
function lineRangeOverlapsDreamingFence(
  ...
  for (let i = 0; i < safeEnd; i += 1) {
    ...
    if (isStart || isEnd) {
      ...
      // that includes a `<!-- grokbot:dreaming:*:start/end -->` marker would
...
async function rehydratePromotionCandidate(
  ...
  for (const sourcePath of sourcePaths) {
    ...
    // content. If rehydration lands inside an grokbot:dreaming fence (for example
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*grokbot-memory-promotion:([^\n]*?)\s*-->/gi);
