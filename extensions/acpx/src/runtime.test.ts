...
const DOCUMENTED_OPENCLAW_BRIDGE_COMMAND =
  "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 grokbot acp --url ws://127.0.0.1:18789 --token-file ~/.grokbot/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @agentclientprotocol/codex-acp@1.1.2";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/grokbot/acpx/codex-acp-wrapper.mjs"`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "grokbot" ? "grokbot acp" : agentName),
        list: () => ["codex", "grokbot"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("adds the GrokBot session key to both managed tools MCP bridges", () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    const { runtime } = makeRuntime(baseStore, {
      ...
      mcpServers: [
        ...
        },
        {
          name: "grokbot-tools",
          command: "node",
          args: ["dist/mcp/grokbot-tools-serve.js"],
          env: [],
        },
      ],
    });
    ...
    expect(readScopedMcpEnv("agent:worker:main", "grokbot-plugin-tools")).toContainEqual({
      name: "OPENCLAW_TOOLS_MCP_AGENT_SESSION_KEY",
      value: "agent:worker:main",
    });
    expect(readScopedMcpEnv("agent:research:main", "grokbot-tools")).toContainEqual({
      name: "OPENCLAW_TOOLS_MCP_AGENT_SESSION_KEY",
      value: "agent:research:main",
    });
  });

  it("keeps managed GrokBot tools MCP delegates reachable for fresh sessions", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("uses the no-MCP delegate for startup probes when the GrokBot tools bridge is enabled", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("normalizes GrokBot Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("strips the GrokBot Anthropic provider prefix for Claude ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("keeps Claude ACP model ids intact after stripping the GrokBot provider prefix", () => {
  ...
  it.each([
    ...
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-acpx-runtime-"));
  ...
  it("disables delegate prompt timeout for GrokBot-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(testing.isCodexAcpCommand("grokbot acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/grokbot/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/grokbot/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("grokbot acp")).toBe(false);
  ...
  it("releases managed GrokBot tools MCP delegates after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("cleans up GrokBot-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/grokbot/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        openclawWrapperRoot: "/tmp/grokbot/acpx",
      },
      {
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 900,
              ppid: 1,
              command: 'node "/tmp/grokbot/acpx/codex-acp-wrapper.mjs"',
            },
            {
              pid: 901,
              ppid: 900,
              command:
                "node /tmp/grokbot/plugin-runtime-deps/node_modules/@agentclientprotocol/codex-acp/dist/index.js",
            },
          ]),
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/grokbot/acpx/codex-acp-wrapper.mjs");
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    leaseStore.leases.set("lease-loaded", {
      ...
      sessionKey: "agent:codex:acp:binding:test",
      wrapperRoot: "/tmp/grokbot/acpx",
      wrapperPath: "/tmp/grokbot/acpx/codex-acp-wrapper.mjs",
      rootPid: 777,
      commandHash: "hash",
      startedAt: 1,
      state: "open",
  ...
  it("routes grokbot ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:grokbot:acp:test",
      backend: "acpx",
  ...
  it("routes non-grokbot sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for grokbot sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:grokbot:acp:test",
      backend: "acpx",
      runtimeSessionName: "grokbot-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the grokbot agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "grokbot" ? "codex" : agentName),
        list: () => ["codex", "grokbot"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the grokbot bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "grokbot acp" : agentName),
        list: () => ["codex", "grokbot"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped grokbot bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "grokbot" ? DOCUMENTED_OPENCLAW_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "grokbot"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node grokbot entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "grokbot" ? "env OPENCLAW_HIDE_BANNER=1 node grokbot.mjs acp" : agentName,
        list: () => ["codex", "grokbot"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:grokbot:acp:test",
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:grokbot:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:grokbot:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to grokbot bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
