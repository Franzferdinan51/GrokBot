...
The ACPX extension is a thin GrokBot wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `grokbot/acpx`, not in this extension.
...
Use this flow when GrokBot needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `grokbot/acpx` repo first.
2. In GrokBot, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild GrokBot and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for GrokBot ACP validation.
...
If a change feels like shared ACP runtime behavior instead of GrokBot-specific glue, move it to `grokbot/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
