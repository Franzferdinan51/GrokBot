  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      ...
      staleOpenClawDevices: [
        { deviceId: "DEV123", displayName: "GrokBot Device", current: false },
      ],
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale GrokBot devices detected for @bot:example.org: DEV123. Run 'grokbot matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
