# ClickClack GrokBot channel

Official GrokBot channel plugin for ClickClack.

## Install

```sh
grokbot plugins install @grokbot/clickclack
```
...
`/api/bot-setup-codes/claim#CODE` endpoint. GrokBot validates the versioned
claim response and saves its canonical API base.

For manual token setup:

```sh
grokbot channels add clickclack \
...
Run `grokbot onboard` for guided setup. The workspace value can be a
...
authentication gateway such as Cloudflare Access while the local GrokBot
...
extension publishes GrokBot's native commands for composer autocomplete,
...
ClickClack can create one managed channel for each GrokBot session:
...
concrete GrokBot session id as well as the pinned server and channel. Resetting
...
two GrokBot gateways using the same ClickClack workspace do not adopt each
...
workspace-scoped credential that cannot access the old workspace, GrokBot
...
See `docs/channels/clickclack.md` in the GrokBot repository, or the published docs at `https://docs.grokbot.ai/channels/clickclack`.
