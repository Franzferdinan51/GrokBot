...
const QA_LAB_UI_OVERLAY_DIR = "/opt/grokbot-qa-lab-ui";
...
${params.bindUiDist ? `      - ${yamlDoubleQuoted(`${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro`)}\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - OPENCLAW_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/grokbot-scaffold/grokbot.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://grokbot-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
...
${imageBlock}    pull_policy: never
    ...
    environment:
      OPENCLAW_CONFIG_PATH: /tmp/grokbot/grokbot.json
      OPENCLAW_STATE_DIR: /tmp/grokbot/state
    ...
    volumes:
      - ./state:/opt/grokbot-scaffold:ro
      - ${yamlDoubleQuoted(`${repoMount}:/opt/grokbot-repo:ro`)}
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/grokbot/workspace /tmp/grokbot/state && cp /opt/grokbot-scaffold/grokbot.json /tmp/grokbot/grokbot.json && cp -R /opt/grokbot-scaffold/seed-workspace/. /tmp/grokbot/workspace/ && rm -rf /tmp/grokbot/workspace/repo && ln -s /opt/grokbot-repo /tmp/grokbot/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t grokbot:qa-local-prebaked --build-arg OPENCLAW_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "grokbot:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "grokbot.json"),
  ];
