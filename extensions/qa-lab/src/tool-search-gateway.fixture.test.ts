...
describe("tool search gateway e2e session log scanner", () => {
  it("does not count target mentions from user prompt records", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-tool-search-log-"));
    try {
  ...
  it("counts target mentions from SQLite transcript rows", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-tool-search-sqlite-"));
    const sqlitePath = path.join(stateDir, "agents", "qa", "agent", "grokbot-agent.sqlite");
...
describe("tool search gateway e2e lane result", () => {
  it("preserves surrogate pairs in provider request snippets", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-tool-search-lane-"));
