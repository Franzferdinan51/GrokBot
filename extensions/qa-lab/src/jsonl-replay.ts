...
type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { grokbot: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { grokbot: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      grokbot: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.grokbot.push(parity.cells.grokbot);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# GrokBot JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
