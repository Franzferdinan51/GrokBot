...
export function registerQaLabCli(program: Command) {
  ...
  qa.command("suite")
    ...
    .option("--runtime-pair <pair>", "Run each scenario under both runtimes, e.g. grokbot,codex")
    .option(
  ...
  qa.command("jsonl-replay")
    ...
    .option("--runtime-pair <pair>", "Runtime pair label, e.g. grokbot,codex", "grokbot,codex")
    .option(
      "--provider-mode <mode>",
  ...
  qa.command("docker-scaffold")
    ...
    .option("--image <name>", "Prebaked image name", "grokbot:qa-local-prebaked")
  ...
  qa.command("docker-build-image")
    ...
    .option("--repo-root <path>", "Repository root to target when running from a neutral cwd")
    .option("--image <name>", "Image tag", "grokbot:qa-local-prebaked")
