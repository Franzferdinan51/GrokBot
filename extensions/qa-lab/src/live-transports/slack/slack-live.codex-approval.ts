...
import { listSlackReactions } from "@grokbot/slack/api.js";
...
export function resolveCodexFileApprovalTargetPath(token: string) {
  return path.join(os.homedir(), `.grokbot-qa-codex-file-approval-${token.toLowerCase()}.txt`);
}
...
function findPendingCodexPluginApprovalRecord(params: {
  ...
  for (const entry of list) {
    ...
    if (
      request.pluginId === "grokbot-codex-app-server" &&
