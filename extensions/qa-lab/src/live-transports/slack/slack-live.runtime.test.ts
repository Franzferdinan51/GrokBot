...
describe("Slack live QA runtime helpers", () => {
  ...
  it("extracts typed Slack approval button values from blocks", () => {
    const actionValue =
      'grokbot:approval:v1:{"approvalId":"plugin:abc","approvalKind":"plugin","decision":"allow-once"}';
    expect(
  ...
  it("extracts plugin approval ids from typed Slack approval action values", () => {
    expect(
      testing.extractSlackNativeApprovalId({
        actionValues: [
          'grokbot:approval:v1:{"approvalId":"plugin:abc123","approvalKind":"plugin","decision":"allow-once"}',
          'grokbot:approval:v1:{"approvalId":"plugin:abc123","approvalKind":"plugin","decision":"deny"}',
        ],
        decision: "allow-once",
      }),
    ).toBe("plugin:abc123");
  ...
  it("resolves the Codex file approval target path", () => {
    expect(testing.resolveCodexFileApprovalTargetPath("MARKER")).toMatch(
      /\.grokbot-qa-codex-file-approval-marker\.txt$/u,
    );
  });
