...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "grokbot.json"),
    ...
    expect(compose).toContain("image: grokbot:qa-local-prebaked");
    ...
    expect(services["qa-lab"]?.environment).toMatchObject({
      OPENCLAW_ENABLE_PRIVATE_QA_CLI: "1",
      OPENCLAW_CONFIG_PATH: "/opt/grokbot-scaffold/grokbot.json",
      OPENCLAW_STATE_DIR: "/tmp/grokbot/state",
    });
    expect(services["qa-lab"]?.volumes).toContain("./state:/opt/grokbot-scaffold:ro");
    expect(compose).toContain('      - "127.0.0.1:18889:18789"');
    expect(compose).toContain('      - "127.0.0.1:43124:43123"');
    expect(compose).toContain(":/opt/grokbot-qa-lab-ui:ro");
    expect(compose).toContain("      - sh");
    expect(compose).toContain("      - -lc");
    expect(compose).toContain(
      '        - fetch("http://127.0.0.1:18789/healthz").then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))',
    );
    expect(compose).toContain("--control-ui-proxy-target http://grokbot-qa-gateway:18789/");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/grokbot-qa-lab-ui");
    expect(compose).toContain(":/opt/grokbot-repo:ro");
    expect(compose).toContain("./state:/opt/grokbot-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/grokbot-scaffold/seed-workspace/. /tmp/grokbot/workspace/ && rm -rf /tmp/grokbot/workspace/repo && ln -s /opt/grokbot-repo /tmp/grokbot/workspace/repo",
    );
    expect(compose).toContain("OPENCLAW_CONFIG_PATH: /tmp/grokbot/grokbot.json");
    expect(compose).toContain("OPENCLAW_STATE_DIR: /tmp/grokbot/state");
    ...
    const configText = await readFile(path.join(outputDir, "state", "grokbot.json"), "utf8");
    ...
    expect(configText).toContain('"/tmp/grokbot/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("grokbot:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t grokbot:qa-local-prebaked --build-arg OPENCLAW_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/grokbot",
    ]);
  });
  ...
  it("quotes generated compose paths so shell-sensitive repo paths survive YAML parsing", async () => {
    ...
    expect(services["qa-lab"]?.volumes).toContain(
      "../repo #hash/extensions/qa-lab/web/dist:/opt/grokbot-qa-lab-ui:ro",
    );
    expect(services["grokbot-qa-gateway"]?.volumes).toContain(
      "../repo #hash:/opt/grokbot-repo:ro",
    );
  });
});
