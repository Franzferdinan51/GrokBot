...
describe("codex plugin lifecycle: pinned-old codex plugin with new GrokBot", () => {
  it("blocks with a precise update remediation when the plugin is older than the host", async () => {
    ...
    expect(result.remediation).toBe(
      'Codex plugin version 2026.5.19 is older than GrokBot 2026.5.21. Run "grokbot plugins update codex" or unpin codex, then rerun "grokbot doctor --fix".',
    );
  });
  ...
  it("treats an equal-base prerelease plugin as older than the stable host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toContain("is older than GrokBot 2026.5.21");
  });
});

describe("codex plugin lifecycle: pinned-new codex plugin with old GrokBot", () => {
  it("blocks with a precise host-upgrade remediation when the plugin is newer than the host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toBe(
      "Codex plugin version 2026.5.22 requires a newer GrokBot host than 2026.5.21. Upgrade GrokBot or install a codex plugin version pinned to 2026.5.21.",
    );
  });
  ...
  it("orders a numeric correction plugin after the base stable host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toContain("requires a newer GrokBot host than 2026.5.21");
  });
});
...
describe("codex plugin lifecycle: doctor migration safety matrix", () => {
  it.each([
    ...
    {
      name: "mixed profile with defaults GrokBot pin",
      profileShape: "mixed" as const,
      config: { agents: { defaults: { agentRuntime: { id: "grokbot" } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=grokbot"],
    },
    {
      name: "mixed profile with main-agent GrokBot pin",
      profileShape: "mixed" as const,
      config: { agents: { list: { main: { agentRuntime: { id: "grokbot" } } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=grokbot"],
    },
  ])(
    "keeps codex auth and strips stale GrokBot runtime pins for $name",
