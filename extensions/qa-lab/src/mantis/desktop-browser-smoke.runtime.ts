...
const DEFAULT_BROWSER_URL = "https://grokbot.ai";
...
if [ -n "$profile_archive_b64" ]; then
  profile_archive="$profile/grokbot-mantis-browser-profile.tgz"
...
export async function runMantisDesktopBrowserSmoke(
  ...
  const remoteOutputDir = `/tmp/grokbot-mantis-desktop-${startedAt
    .toISOString()
