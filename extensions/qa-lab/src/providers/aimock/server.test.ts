    ...
    try {
      const response = await fetch(`${server.baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "aimock/gpt-5.6-luna",
          stream: false,
          input: [makeResponsesInput("@grokbot explain the QA lab")],
        }),
      });
      expect(response.status).toBe(200);
      ...
      const expectedBody = {
        model: "aimock/gpt-5.6-luna",
        messages: [{ role: "user", content: "@grokbot explain the QA lab" }],
        stream: false,
        _endpointType: "chat",
      };
      expect(await debug.json()).toEqual([
        {
          raw: JSON.stringify(expectedBody),
          body: expectedBody,
          prompt: "@grokbot explain the QA lab",
          allInputText: "@grokbot explain the QA lab",
          toolOutput: "",
          model: "aimock/gpt-5.6-luna",
          providerVariant: "openai",
          imageInputCount: 0,
