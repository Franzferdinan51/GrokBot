...
describe("token efficiency report", () => {
  it("does not fail live reports solely because Codex uses fewer tokens", () => {
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          "codex-savings",
          makeCell("grokbot", { inputTokens: 120, outputTokens: 80, totalTokens: 200 }),
    ...
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          ...
          makeCell("grokbot", { inputTokens: 72_000, outputTokens: 381, totalTokens: 72_381 }, [
  ...
  it("keeps live zero-usage rows failing instead of passing as neutral", () => {
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity(
          "missing-live-usage",
          makeCell("grokbot", { inputTokens: 0, outputTokens: 0, totalTokens: 0 }),
    ...
    expect(report.pass).toBe(false);
    expect(report.failures).toEqual([
      "missing-live-usage grokbot live usage totalTokens=0",
  ...
  it("excludes explicit non-assistant scenarios from live usage aggregates", () => {
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity(
          "usage-applicable",
          makeCell("grokbot", { inputTokens: 80, outputTokens: 20, totalTokens: 100 }),
    ...
    expect(report.aggregate.grokbot.totalTokens).toBe(100);
  ...
  it("fails live reports with non-integer token usage evidence", () => {
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity(
          ...
          makeCell("grokbot", { inputTokens: 100.5, outputTokens: 0, totalTokens: 100.5 }),
    ...
    expect(report.pass).toBe(false);
    expect(report.failures).toEqual([
      "fractional-live-usage grokbot live usage inputTokens must be a non-negative integer",
      "fractional-live-usage grokbot live usage totalTokens must be a non-negative integer",
    ]);
  });
        ...
        scenarios: [
          {
            ...
            runtimeParity: makeRuntimeParity(
              "mock-regression",
              makeCell("grokbot", { inputTokens: 100, outputTokens: 0, totalTokens: 100 }),
  ...
  it("renders savings and regression classifications in the markdown report", () => {
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          "codex-savings",
          makeCell("grokbot", { inputTokens: 100, outputTokens: 100, totalTokens: 200 }),
