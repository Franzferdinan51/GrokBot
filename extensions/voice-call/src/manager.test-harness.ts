...
import { createPluginStateSyncKeyedStoreForTests } from "grokbot/plugin-sdk/plugin-state-test-runtime";
...
export function createTestStorePath(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-voice-call-test-"));
}
