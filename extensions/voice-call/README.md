...
Official Voice Call plugin for **GrokBot**.
...
Docs: `https://docs.grokbot.ai/plugins/voice-call`
Plugin system: `https://docs.grokbot.ai/tools/plugin`
...
```bash
PLUGIN_HOME=~/.grokbot/extensions
mkdir -p "$PLUGIN_HOME"
...
- Runtime accepts canonical config only. If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `grokbot doctor --fix` to rewrite them.
- advanced webhook, streaming, and tunnel notes: `https://docs.grokbot.ai/plugins/voice-call`
...
`https://docs.grokbot.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.grokbot.ai/plugins/voice-call#tts-for-calls`
