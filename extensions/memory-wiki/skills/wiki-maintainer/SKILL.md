...
description: Maintain the GrokBot memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `grokbot wiki ingest`, `grokbot wiki compile`, and `grokbot wiki lint` as the default maintenance loop.
- In `bridge` mode, run `grokbot wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `grokbot wiki unsafe-local import` only when the user explicitly opted into private local path access.
