...
export function createUsageRecorders(runtime: DiagnosticsRecorderRuntime) {
  ...
  const recordModelUsage = (
    ...
    const attrs = {
      "grokbot.channel": evt.channel ?? "unknown",
      "grokbot.agent": lowCardinalityAttr(evt.agentId),
    ...
    if (usage.input) {
      tokensCounter.add(usage.input, { ...attrs, "grokbot.token": "input" });
    ...
    if (usage.output) {
      tokensCounter.add(usage.output, { ...attrs, "grokbot.token": "output" });
    ...
    if (usage.cacheRead) {
      tokensCounter.add(usage.cacheRead, { ...attrs, "grokbot.token": "cache_read" });
    }
    if (usage.cacheWrite) {
      tokensCounter.add(usage.cacheWrite, { ...attrs, "grokbot.token": "cache_write" });
    }
    if (usage.promptTokens) {
      tokensCounter.add(usage.promptTokens, { ...attrs, "grokbot.token": "prompt" });
    }
    if (usage.total) {
      tokensCounter.add(usage.total, { ...attrs, "grokbot.token": "total" });
    }
    ...
    if (evt.context?.limit) {
      contextHistogram.record(evt.context.limit, {
        ...attrs,
        "grokbot.context": "limit",
      });
    }
    if (evt.context?.used) {
      contextHistogram.record(evt.context.used, {
        ...attrs,
        "grokbot.context": "used",
      });
    }
    ...
    const spanAttrs: Record<string, string | number> = {
      ...attrs,
      "grokbot.tokens.input": usage.input ?? 0,
      "grokbot.tokens.output": usage.output ?? 0,
      "grokbot.tokens.cache_read": usage.cacheRead ?? 0,
      "grokbot.tokens.cache_write": usage.cacheWrite ?? 0,
      "grokbot.tokens.total": usage.total ?? 0,
    };
    ...
    const span = spanWithDuration("grokbot.model.usage", spanAttrs, evt.durationMs, {
  ...
  const recordWebhookReceived = (
    ...
    const attrs = {
      "grokbot.channel": evt.channel ?? "unknown",
      "grokbot.webhook": evt.updateType ?? "unknown",
    };
  ...
  const recordWebhookProcessed = (
    ...
    const attrs = {
      "grokbot.channel": lowCardinalityAttr(evt.channel),
      "grokbot.webhook": lowCardinalityAttr(evt.updateType),
    };
    ...
    const span = spanWithDuration("grokbot.webhook.processed", spanAttrs, evt.durationMs);
    span.end();
  };
  ...
  const recordWebhookError = (evt: Extract<DiagnosticEventPayload, { type: "webhook.error" }>) => {
    ...
    const spanAttrs: Record<string, string | number> = {
      ...attrs,
      "grokbot.error": redactedError,
    };
    const span = tracer.startSpan("grokbot.webhook.error", {
  ...
  const recordMessageQueued = (
    ...
    const attrs = {
      "grokbot.channel": lowCardinalityAttr(evt.channel),
      "grokbot.source": lowCardinalityAttr(evt.source),
    };
  ...
  const recordMessageDispatchStarted = (
    ...
    trackInternalOrTrustedSpan(
      evt,
      metadata,
      spanWithDuration("grokbot.message.processed", attrs, undefined, {
  ...
  const recordMessageProcessed = (
    ...
    const attrs = {
      "grokbot.channel": lowCardinalityAttr(evt.channel),
      "grokbot.outcome": evt.outcome ?? "unknown",
    };
    ...
    const spanAttrs: Record<string, string | number> = { ...attrs };
    if (evt.reason) {
      spanAttrs["grokbot.reason"] = lowCardinalityAttr(evt.reason, "unknown");
    }
    const trackedSpan = getTrackedInternalOrTrustedSpan(evt, metadata);
    const span =
      trackedSpan ??
      spanWithDuration("grokbot.message.processed", spanAttrs, evt.durationMs, {
        parentContext: internalOrTrustedExplicitParentContext(evt, metadata),
        endTimeMs: evt.ts,
      });
  ...
  const messageDeliveryAttrs = (evt: MessageDeliveryDiagnosticEvent): Record<string, string> => ({
    "grokbot.channel": lowCardinalityAttr(evt.channel),
    "grokbot.delivery.kind": lowCardinalityAttr(evt.deliveryKind, "other"),
  });
  ...
  const recordMessageDeliveryCompleted = (
    ...
    const span = spanWithDuration(
      ...
      {
        ...attrs,
        "grokbot.delivery.result_count": evt.resultCount,
      },
      evt.durationMs,
  ...
  const recordMessageDeliveryError = (
    ...
    const span = spanWithDuration("grokbot.message.delivery", attrs, evt.durationMs, {
  ...
  const recordRunStarted = (
    ...
    const span = trackTrustedSpan(
      evt,
      metadata,
      spanWithDuration("grokbot.run", spanAttrs, undefined, {
        parentContext: activeTrustedParentContext(evt, metadata),
        startTimeMs: evt.ts,
      }),
    );
