...
// Shared text-part reading for gen_ai message normalization: GrokBot emits
...
export function assignOtelToolIdentityAttributes(
  ...
  // low-cardinality, and unlike the dropped grokbot.toolCallId passthrough keys
...
export function assignOtelModelContentAttributes(
  ...
  if (policy.inputMessages) {
    assignOtelContentAttribute(
      attributes,
      "grokbot.content.input_messages",
  ...
  if (policy.toolDefinitions) {
    assignOtelContentAttribute(
      attributes,
      "grokbot.content.tool_definitions",
  ...
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "grokbot.content.output_messages",
      content?.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "grokbot.content.system_prompt", content?.systemPrompt);
  }
}
...
export function assignOtelToolContentAttributes(
  ...
  // grokbot.content.* names; normalize once so both copies stay byte-identical.
  if (policy.toolInputs) {
    ...
    if (toolInput) {
      ...
      attributes["grokbot.content.tool_input"] = toolInput;
    }
  }
  if (policy.toolOutputs) {
    ...
    if (toolOutput) {
      ...
      attributes["grokbot.content.tool_output"] = toolOutput;
    }
  }
}
