...
const OTEL_TEST_STATE_DIR = "/tmp/grokbot-diagnostics-otel-test";
...
afterAll(() => {
  ...
  vi.doUnmock("grokbot/plugin-sdk/fetch-runtime");
...
describe("diagnostics-otel service", () => {
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["grokbot.provider"]).toBe("openai");
    for (const key of [
      "grokbot.callId",
      "grokbot.call_id",
      "grokbot.chatId",
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("grokbot.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "grokbot.channel": "telegram",
      "grokbot.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("grokbot.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "grokbot.channel": "telegram",
      "grokbot.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("grokbot.message.queued")?.add).toHaveBeenCalledWith(1, {
      "grokbot.channel": "telegram",
      "grokbot.source": "telegram",
    });
    expect(telemetryState.histograms.get("grokbot.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("grokbot.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "grokbot.channel": "telegram",
      "grokbot.source": "telegram",
    });
    expect(telemetryState.histograms.get("grokbot.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "grokbot.lane": "main",
    });
    expect(telemetryState.counters.get("grokbot.message.processed")?.add).toHaveBeenCalledWith(1, {
      "grokbot.channel": "telegram",
      "grokbot.outcome": "completed",
    });
    expect(telemetryState.counters.get("grokbot.message.received")?.add).toHaveBeenCalledWith(1, {
      "grokbot.channel": "telegram",
      "grokbot.source": "webhook",
    });
    expect(telemetryState.counters.get("grokbot.message.received")?.add).toHaveBeenCalledWith(1, {
      "grokbot.channel": "unknown",
      "grokbot.source": "unknown",
    });
    expect(
      telemetryState.counters.get("grokbot.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("grokbot.message.dispatch.completed")?.add,
    ...
    expect(
      telemetryState.histograms.get("grokbot.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "grokbot.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("grokbot.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "grokbot.channel": "telegram",
      "grokbot.outcome": "completed",
    });
    expect(telemetryState.histograms.get("grokbot.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "grokbot.lane": "main",
      },
    );
    expect(telemetryState.counters.get("grokbot.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("grokbot.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "grokbot.state": "processing",
    });
    expect(
      telemetryState.histograms.get("grokbot.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "grokbot.state": "processing",
    });
    expect(telemetryState.counters.get("grokbot.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "grokbot.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("grokbot.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "grokbot.agent": "agent.default",
        "grokbot.channel": "telegram",
        "grokbot.trigger": "user",
      },
    );

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("grokbot.webhook.processed");
    expect(spanNames).toContain("grokbot.message.processed");
    expect(spanNames).toContain("grokbot.session.stuck");
    const webhookSpanOptions = startedSpanOptions("grokbot.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("grokbot.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("grokbot.message.processed");
    expect(messageSpanOptions?.attributes?.["grokbot.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["grokbot.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["grokbot.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("grokbot.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("grokbot.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("grokbot.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["grokbot.provider"]).toBe("openai");
    expect(runDurationAttributes?.["grokbot.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("grokbot.run");
    expect(runSpanOptions?.attributes?.["grokbot.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("grokbot.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "grokbot.exporter": "diagnostics-otel",
      "grokbot.signal": "logs",
      "grokbot.status": "started",
      "grokbot.reason": "configured",
    });
  ...
  test("exports trusted security events as bounded OTLP logs", async () => {
    ...
    expect(emitCall.body).toBe("grokbot.security.event");
    expect(emitCall.severityText).toBe("WARN");
    expect(emitCall.severityNumber).toBe(13);
    expect(emitCall.attributes).toMatchObject({
      "grokbot.security.event_id": "security-event-1",
      "grokbot.security.category": "tool",
      "grokbot.security.action": "tool.execution.blocked",
      "grokbot.security.outcome": "denied",
      "grokbot.security.severity": "medium",
      "grokbot.security.reason": "tools.deny",
      "grokbot.security.actor.kind": "agent",
      "grokbot.security.actor.id_hash": "agent-hash-1",
      "grokbot.security.actor.role": "operator",
      "grokbot.security.actor.scopes": "operator.read,operator.approvals",
      "grokbot.security.target.kind": "plugin",
      "grokbot.security.target.name": "@acme/security-event-plugin",
      "grokbot.security.target.owner": "plugin-installer",
      "grokbot.security.policy.id": "tools.exec",
      "grokbot.security.policy.decision": "deny",
      "grokbot.security.policy.reason": "allowlist.miss",
      "grokbot.security.control.id": "exec-approval",
      "grokbot.security.control.family": "approval",
      "grokbot.security.attribute.params_kind": "object",
      "grokbot.security.attribute.secretish": "unknown",
    });
    ...
    expect(Object.hasOwn(emitCall.attributes ?? {}, "grokbot.security.attribute.__proto__")).toBe(
      false,
    );
  ...
  test("exports trusted security events as stdout JSONL logs", async () => {
    ...
    try {
      ...
      expect(record.body).toBe("grokbot.security.event");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(record.attributes).toMatchObject({
        "grokbot.security.event_id": "security-event-stdout",
        "grokbot.security.category": "tool",
        "grokbot.security.action": "tool.execution.blocked",
        "grokbot.security.outcome": "denied",
        "grokbot.security.severity": "medium",
      ...
      expect(Object.hasOwn(record.attributes ?? {}, "grokbot.security.attribute.__proto__")).toBe(
        false,
      );
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("grokbot.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "grokbot.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("grokbot.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "grokbot.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("grokbot.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "grokbot.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("grokbot.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["grokbot.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["grokbot.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["grokbot.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "grokbot.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("grokbot.payload.large")?.add).toHaveBeenCalledWith(1, {
      "grokbot.payload.action": "rejected",
      "grokbot.payload.surface": "gateway.frame",
      "grokbot.channel": "web",
      "grokbot.plugin": "none",
      "grokbot.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("grokbot.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "grokbot.exporter": "diagnostics-otel",
      "grokbot.signal": "logs",
      "grokbot.status": "failure",
      "grokbot.reason": "emit_failed",
      "grokbot.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("grokbot.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("grokbot.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["grokbot.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["grokbot.blocked_by"]).toBe("policy-plugin");
  ...
  test("run.completed error span carries the redacted message off the metric attrs", async () => {
    ...
    expect(startedSpanOptions("grokbot.run")?.attributes?.["grokbot.error"]).toBe(
      "upstream model stream stalled then aborted",
    );
    expect(spanByName("grokbot.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "upstream model stream stalled then aborted",
    });
    // The raw message must never widen metric cardinality.
    const runDuration = lastHistogramRecord("grokbot.run.duration_ms");
    expect(runDuration?.[1]?.["grokbot.outcome"]).toBe("error");
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "grokbot.error")).toBe(false);
  ...
  test("run.completed bounds sensitive error text before export", async () => {
    ...
    const status = mockCallArg(spanByName("grokbot.run").setStatus, 0) as {
      message?: string;
    };
  ...
  test("harness.run.completed error span carries the redacted message", async () => {
    ...
    expect(startedSpanOptions("grokbot.harness.run")?.attributes?.["grokbot.error"]).toBe(
      "model run failed during resolve phase",
    );
    expect(spanByName("grokbot.harness.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "model run failed during resolve phase",
    });
    const harnessDuration = lastHistogramRecord("grokbot.harness.duration_ms");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "grokbot.error")).toBe(false);
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("grokbot.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["grokbot.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const modelCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "grokbot.model.call",
    );
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["grokbot.content.input_messages"]).toBe("user prompt");
  ...
  test("preserves OTLP TLS env options when passing env proxy agents", async () => {
    const certDir = mkdtempSync(path.join(tmpdir(), "grokbot-otel-tls-"));
    try {
  ...
  test("exports diagnostic logs as stdout JSONL without constructing the OTLP log exporter", async () => {
    ...
    ctx.config.diagnostics!.otel!.serviceName = "rovoclaw-grokbot";
    const stdout = captureStdoutWrites();
    ...
    try {
      ...
      expect(record.signal).toBe("grokbot.diagnostic.log");
      expect(record["service.name"]).toBe("rovoclaw-grokbot");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(String(record.body)).not.toContain("sk-1234567890abcdef1234567890abcdef");
      expect(String(record.body)).toContain("sk-123");
      expect(record.attributes).toMatchObject({
        "grokbot.log.level": "WARN",
        "grokbot.subsystem": "diagnostic",
      });
      const tokenAttr = record.attributes?.["grokbot.token"];
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["grokbot.token"];
    expect(tokenAttr).not.toBe("ghp_abcdefghijklmnopqrstuvwxyz123456"); // pragma: allowlist secret
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "grokbot.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "grokbot.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "grokbot.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/grokbot/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/grokbot/src/private.ts:42",
      },
    ...
    expect(emitCall.attributes["grokbot.good"]).toBe(`${"y".repeat(4095)}...(truncated)`);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(Object.hasOwn(emitCall.attributes, `grokbot.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "grokbot.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "grokbot.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "grokbot.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "grokbot.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "grokbot.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "grokbot.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("grokbot.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "grokbot.channel": "webchat",
      "grokbot.agent": "ops",
      "grokbot.provider": "openai",
      "grokbot.model": "gpt-5.4",
      "grokbot.token": "input",
    });
  ...
  test("advertises explicit duration buckets on the grokbot run/harness/context histograms", async () => {
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { metrics: true });
    ...
    try {
      ...
      const runDurationOptions = histogramCreateOptions("grokbot.run.duration_ms");
      ...
      const harnessDurationOptions = histogramCreateOptions("grokbot.harness.duration_ms");
      ...
      const contextOptions = histogramCreateOptions("grokbot.context.tokens");
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("grokbot.tokens")?.add).toHaveBeenCalledWith(2, {
      "grokbot.channel": "unknown",
      "grokbot.agent": "unknown",
      "grokbot.provider": "openai",
      "grokbot.model": "gpt-5.4",
      "grokbot.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("grokbot.tokens")?.add.mock.calls),
  ...
  test("drops session-shaped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("grokbot.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "grokbot.lane": "session",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("grokbot.queue.lane.enqueue")?.add.mock.calls),
    ).not.toContain("Agent:qa:otel-trace-smoke");
    await service.stop?.(ctx);
  });
  ...
  test("keeps only the bounded prefix from scoped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("grokbot.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "grokbot.lane": "dreaming-narrative",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("grokbot.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("grokbot.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "grokbot.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "grokbot.sessionId")).toBe(false);
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "grokbot.agent": "main",
      "grokbot.skill.activation": "read",
      "grokbot.skill.name": "tiny-llm-brainstorm",
      "grokbot.skill.source": "workspace",
      "grokbot.toolName": "read",
    };
    expect(telemetryState.counters.get("grokbot.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "grokbot.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("grokbot.run");
    expect(spanNames).toContain("grokbot.model.call");
    expect(spanNames).toContain("grokbot.harness.run");
    expect(spanNames).toContain("grokbot.tool.execution");

    const runOptions = startedSpanOptions("grokbot.run");
    expect(runOptions?.attributes?.["grokbot.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["grokbot.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["grokbot.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["grokbot.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "grokbot.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "grokbot.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "grokbot.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("grokbot.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "grokbot.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "grokbot.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "grokbot.sessionKey")).toBe(false);
    expect(modelOptions?.startTime).toBeTypeOf("number");
    expect(modelOptions?.kind).toBe(2);
    expect(modelCall?.[2]).toBeUndefined();

    const harnessCall = startedSpanCall("grokbot.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["grokbot.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["grokbot.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["grokbot.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["grokbot.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["grokbot.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["grokbot.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["grokbot.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["grokbot.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["grokbot.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["grokbot.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["grokbot.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "grokbot.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "grokbot.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "grokbot.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "grokbot.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("grokbot.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["grokbot.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["grokbot.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["grokbot.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["grokbot.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["grokbot.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "grokbot.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "grokbot.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "grokbot.sessionKey")).toBe(false);
    ...
    const modelCallDuration = lastHistogramRecord("grokbot.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["grokbot.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["grokbot.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("grokbot.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["grokbot.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["grokbot.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("grokbot.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["grokbot.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["grokbot.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("grokbot.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["grokbot.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["grokbot.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("grokbot.model.call");
    expect(modelSpanAttributes["grokbot.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["grokbot.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["grokbot.model_call.time_to_first_byte_ms"]).toBe(45);
    expect(modelSpanAttributes["grokbot.model_call.prompt.input_messages_count"]).toBe(2);
    expect(modelSpanAttributes["grokbot.model_call.prompt.input_messages_chars"]).toBe(3456);
    expect(modelSpanAttributes["grokbot.model_call.prompt.system_prompt_chars"]).toBe(789);
    expect(modelSpanAttributes["grokbot.model_call.prompt.tool_definitions_count"]).toBe(4);
    expect(modelSpanAttributes["grokbot.model_call.prompt.tool_definitions_chars"]).toBe(2345);
    expect(modelSpanAttributes["grokbot.model_call.prompt.total_chars"]).toBe(6590);
    expect(modelSpanAttributes["grokbot.model_call.usage.input_tokens"]).toBe(100);
    expect(modelSpanAttributes["grokbot.model_call.usage.output_tokens"]).toBe(20);
    expect(modelSpanAttributes["grokbot.model_call.usage.cache_read_input_tokens"]).toBe(30);
    expect(modelSpanAttributes["grokbot.model_call.usage.cache_creation_input_tokens"]).toBe(5);
    expect(modelSpanAttributes["grokbot.model_call.usage.reasoning_output_tokens"]).toBe(8);
    expect(modelSpanAttributes["grokbot.model_call.usage.prompt_tokens"]).toBe(135);
    expect(modelSpanAttributes["grokbot.model_call.usage.total_tokens"]).toBe(155);
    ...
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "grokbot.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("grokbot.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["grokbot.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["grokbot.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["grokbot.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "grokbot.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "grokbot.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("grokbot.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["grokbot.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "grokbot.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "grokbot.runId")).toBe(false);

    const toolSpan = spanByName("grokbot.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("grokbot.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("grokbot.model.failover");
    expect(failoverOptions?.attributes?.["grokbot.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["grokbot.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["grokbot.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["grokbot.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["grokbot.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["grokbot.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["grokbot.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["grokbot.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "grokbot.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "grokbot.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("grokbot.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("grokbot.model.failover")).toStrictEqual([
      1,
      {
        "grokbot.failover.reason": "overloaded",
        "grokbot.failover.suspended": "true",
        "grokbot.lane": "main",
        "grokbot.model": "claude-opus-4-6",
        "grokbot.provider": "anthropic",
        "grokbot.failover.to_model": "gpt-5.4",
        "grokbot.failover.to_provider": "openai",
      },
    ]);
    await service.stop?.(ctx);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("grokbot.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "grokbot.toolName": "browser",
        "grokbot.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "grokbot.tool.owner": "browser-tools",
        "grokbot.tool.params.kind": "object",
        "grokbot.deniedReason": "tools.deny",
      },
    ]);
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("grokbot.model.failover");
    expect(failoverOptions?.attributes?.["grokbot.lane"]).toBe("session");
  ...
  test("uses latest GenAI request and agent span shapes only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("grokbot.model.call")).toBeUndefined();
    ...
    expect(agentTurnOptions?.attributes?.["grokbot.model_call.observation_unit"]).toBe("turn");
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("grokbot.model.call");
    expect(modelCallOptions?.attributes?.["grokbot.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "grokbot.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "grokbot.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("grokbot.provider.request", {
      "grokbot.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("grokbot.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["grokbot.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "grokbot.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("grokbot.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "grokbot.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["grokbot.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["grokbot.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["grokbot.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["grokbot.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["grokbot.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["grokbot.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["grokbot.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["grokbot.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["grokbot.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["grokbot.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["grokbot.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["grokbot.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["grokbot.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("grokbot.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "grokbot.toolName": "process",
      "grokbot.loop.level": "critical",
      "grokbot.loop.action": "block",
      "grokbot.loop.detector": "known_poll_no_progress",
      "grokbot.loop.count": 20,
      "grokbot.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("grokbot.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["grokbot.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["grokbot.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["grokbot.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["grokbot.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["grokbot.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["grokbot.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "grokbot.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("grokbot.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("grokbot.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "grokbot.memory.level": "critical",
        "grokbot.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("grokbot.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "grokbot.memory.level": "critical",
      "grokbot.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("grokbot.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["grokbot.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["grokbot.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["grokbot.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["grokbot.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["grokbot.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["grokbot.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["grokbot.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["grokbot.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["grokbot.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["grokbot.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "grokbot.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("grokbot.diagnostic.async_queue.dropped");
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "grokbot.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "grokbot.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "grokbot.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["grokbot.run"]).toBeUndefined();
    expect(parentBySpanName["grokbot.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["grokbot.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("grokbot.message.processed");
    const harnessSpan = spanByName("grokbot.harness.run");
    const runSpan = spanByName("grokbot.run");
    const usageSpan = spanByName("grokbot.model.usage");
    const modelCallSpan = spanByName("grokbot.model.call");
    ...
    expect(parentBySpanName["grokbot.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["grokbot.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["grokbot.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["grokbot.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["grokbot.model.call"]?.spanId).toBe(runSpanContext.spanId);
    await service.stop?.(ctx);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("grokbot.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("grokbot.message.processed")[0]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("grokbot.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("grokbot.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("grokbot.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("grokbot.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("grokbot.message.processed")["grokbot.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
    await service.stop?.(ctx);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("grokbot.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("grokbot.harness.run")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("grokbot.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not parent sibling active runs through shared upstream aliases", async () => {
    ...
    const runContexts = startedSpanParentContextsByName("grokbot.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("grokbot.run").spanContext();
    const modelParentContext = startedSpanParentContexts("grokbot.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("grokbot.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("grokbot.model.usage")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["grokbot.run"]).toBeUndefined();
    expect(parentBySpanName["grokbot.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["grokbot.run"]).toBeUndefined();
    expect(parentBySpanName["grokbot.model.call"]).toBeUndefined();
    expect(parentBySpanName["grokbot.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "grokbot.run"),
    ).toHaveLength(1);
    expect(
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "grokbot.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "grokbot.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("grokbot.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["grokbot.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["grokbot.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["grokbot.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["grokbot.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("grokbot.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["grokbot.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["grokbot.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["grokbot.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["grokbot.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["grokbot.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["grokbot.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["grokbot.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "grokbot.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "grokbot.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "grokbot.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("grokbot.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("grokbot.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("grokbot.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "grokbot.channel": "matrix",
      "grokbot.delivery.kind": "text",
    });
    const deliveryDurationRecords = telemetryState.histograms.get(
      "grokbot.message.delivery.duration_ms",
    )?.record.mock.calls as Array<[unknown, Record<string, unknown>]>;
    expect(deliveryDurationRecords[0]?.[0]).toBe(25);
    expect(deliveryDurationRecords[0]?.[1]["grokbot.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["grokbot.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["grokbot.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["grokbot.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["grokbot.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["grokbot.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["grokbot.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "grokbot.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["grokbot.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["grokbot.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["grokbot.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["grokbot.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["grokbot.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["grokbot.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["grokbot.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["grokbot.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "grokbot.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "grokbot.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "grokbot.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "grokbot.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "grokbot.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "grokbot.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "grokbot.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("grokbot.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["grokbot.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["grokbot.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["grokbot.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("grokbot.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["grokbot.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["grokbot.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["grokbot.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["grokbot.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("grokbot.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["grokbot.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["grokbot.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["grokbot.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("grokbot.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["grokbot.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["grokbot.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["grokbot.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("grokbot.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["grokbot.status"]).toBe("released");
    expect(telemetryState.counters.get("grokbot.talk.event")?.add).toHaveBeenCalledWith(1, {
      "grokbot.talk.brain": "agent-consult",
      "grokbot.talk.event_type": "input.audio.delta",
      "grokbot.talk.mode": "realtime",
      "grokbot.talk.provider": "openai",
      "grokbot.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("grokbot.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "grokbot.talk.brain": "agent-consult",
        "grokbot.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("grokbot.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "grokbot.talk.brain": "agent-consult",
      "grokbot.talk.event_type": "latency.metrics",
      "grokbot.talk.mode": "realtime",
      "grokbot.talk.provider": "openai",
      "grokbot.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("grokbot.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("grokbot.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "grokbot.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "grokbot.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "grokbot.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("grokbot.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "grokbot.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "grokbot.content.tool_output")).toBe(
      false,
    );
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["grokbot.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["grokbot.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["grokbot.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["grokbot.content.tool_input"]).toBe("tool input");
    ...
    expect(toolAttrs?.["gen_ai.tool.call.arguments"]).toBe(
      toolAttrs?.["grokbot.content.tool_input"],
    );
    expect(typeof toolAttrs?.["grokbot.content.tool_output"]).toBe("string");
    expect(String(toolAttrs?.["grokbot.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["grokbot.content.tool_output"])).not.toContain("a".repeat(11));
    expect(toolAttrs?.["gen_ai.tool.call.result"]).toBe(
      toolAttrs?.["grokbot.content.tool_output"],
    );
    await service.stop?.(ctx);
  });
  ...
  test("omits absent model content fields when capture fields are opted in", async () => {
    ...
    expect(attrs["grokbot.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "grokbot.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "grokbot.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "grokbot.content.tool_definitions")).toBe(false);
  ...
  test("exports Claude CLI turn content through the existing Phoenix GenAI keys", async () => {
    ...
    emitTrustedModelCallCompletedWithContent(
      ...
      },
      {
        ...
        systemPrompt: "GrokBot appended instructions",
      },
    );
    await flushDiagnosticEvents();
    ...
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["grokbot.api"]).toBe("claude-code");
    expect(attrs?.["grokbot.transport"]).toBe("stdio-live");
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.input.messages"))).toEqual([
    ...
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.system_instructions"))).toEqual([
      { type: "text", content: "GrokBot appended instructions" },
    ]);
    expect(Object.hasOwn(attrs ?? {}, "gen_ai.tool.definitions")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("exports tool definitions without requiring input message capture", async () => {
    ...
    expect(Object.hasOwn(attrs ?? {}, "grokbot.content.input_messages")).toBe(false);
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.tool.definitions"))).toEqual([
      {
        type: "function",
        name: "lookup",
    ...
    expect(JSON.parse(String(attrs?.["grokbot.content.tool_definitions"]))).toEqual([
      {
        name: "lookup",
        description: "Lookup data",
        parameters: { type: "object" },
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("grokbot.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["grokbot.reason"])).toContain("…");
    expect(typeof attrs?.["grokbot.reason"]).toBe("string");
    expect(String(attrs?.["grokbot.reason"])).not.toContain(
