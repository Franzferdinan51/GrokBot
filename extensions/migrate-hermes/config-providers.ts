    ...
    if (transport && !HERMES_TRANSPORTS[transport]) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Configure an equivalent GrokBot provider plugin or API adapter manually.",
        }),
      );
    ...
    if (readString(raw.api_key) && !readEnvReference(raw.api_key)) {
      items.push(
        createMigrationManualItem({
          ...
          message: `Hermes provider "${id}" contains an inline API key that was not copied into GrokBot config.`,
          recommendation: "Move the key to an environment variable or GrokBot secret provider.",
        }),
      );
    }
    ...
    if (isRecord(raw.extra_body) && Object.keys(raw.extra_body).length > 0) {
      items.push(
        createMigrationManualItem({
          ...
          message: `Hermes provider "${id}" adds request body fields that GrokBot cannot import generically.`,
          recommendation:
    ...
    if (apiKeyEnv && !env[apiKeyEnv]?.trim()) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Configure an GrokBot auth profile for this provider or expose the variable to the GrokBot runtime.",
        }),
      );
    }
  }
