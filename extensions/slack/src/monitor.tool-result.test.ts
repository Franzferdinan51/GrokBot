...
import { CURRENT_MESSAGE_MARKER } from "grokbot/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "grokbot/plugin-sdk/channel-test-helpers";
import { HISTORY_CONTEXT_MARKER } from "grokbot/plugin-sdk/reply-history";
import { resetInboundDedupe } from "grokbot/plugin-sdk/reply-runtime";
...
describe("monitorSlackProvider tool results", () => {
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("grokbot: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("grokbot: hello <@U2>");
  });
