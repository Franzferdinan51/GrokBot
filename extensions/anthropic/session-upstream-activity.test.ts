...
describe("Claude upstream activity", () => {
  it("counts only external user rows after the byte marker", async () => {
    const dir = await makeTempDir("grokbot-claude-upstream-");
  ...
  it("stats without reading when the file did not grow", async () => {
    const dir = await makeTempDir("grokbot-claude-upstream-static-");
  ...
  it("filters GrokBot-authored rows by normalized transcript text", async () => {
    const dir = await makeTempDir("grokbot-claude-upstream-provenance-");
  ...
  it("returns missing for an absent local transcript", async () => {
    const filePath = path.join(
      await makeTempDir("grokbot-claude-upstream-missing-"),
      "gone.jsonl",
    );
  ...
  it("isolates a missing transcript from healthy probes", async () => {
    const dir = await makeTempDir("grokbot-claude-upstream-batch-");
  ...
  it("scans forward across bounded ticks without skipping a middle user row", async () => {
    const dir = await makeTempDir("grokbot-claude-upstream-chunks-");
  ...
  it("treats legacy size and current offset markers as the same scan cursor", async () => {
    const dir = await makeTempDir("grokbot-claude-upstream-marker-");
