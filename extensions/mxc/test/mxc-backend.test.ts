...
import type { CreateSandboxBackendParams } from "grokbot/plugin-sdk/sandbox";
import { isPathInside } from "grokbot/plugin-sdk/security-runtime";
...
vi.mock("grokbot/plugin-sdk/process-runtime", () => ({
...
const baseParams = {
  config: baseConfig,
  runtimeId: "grokbot-mxc-test-abc12345",
  workdir: "/workspace",
};
...
describeOnWindows("createMxcSandboxBackendHandle (Windows-only MXC backend tests)", () => {
  ...
  test("buildExecSpec returns a launcher argv with Windows process containment by default", async () => {
    ...
    expect(processContainer).toEqual({
      name: "grokbot-mxc-test-abc12345",
      leastPrivilege: true,
      capabilities: [],
      ui: {
        isolation: "container",
    ...
    await withProcessEnv(
      {
        ...
        USERPROFILE: "C:\\Users\\grokbot",
        APPDATA: "C:\\Users\\grokbot\\AppData\\Roaming",
        LOCALAPPDATA: "C:\\Users\\grokbot\\AppData\\Local",
      ...
      async () => {
        ...
        expect(env).toContain("USERPROFILE=C:\\Users\\grokbot");
        expect(env).toContain("APPDATA=C:\\Users\\grokbot\\AppData\\Roaming");
        expect(env).toContain("LOCALAPPDATA=C:\\Users\\grokbot\\AppData\\Local");
  ...
  test("Windows process containment caps long AppContainer names", async () => {
    const handle = createMxcSandboxBackendHandle({
      ...baseParams,
      runtimeId: `grokbot-mxc-${"a".repeat(80)}-12345678`,
    });
    const spec = await handle.buildExecSpec({ command: "echo hello", env: {}, usePty: false });
  ...
  test("workspaceAccess rw fails closed when protected skill roots exist", async () => {
    ...
    try {
      ...
      mkdirSync(path.join(workdir, ".grokbot", "sandbox-skills", "skills", "demo"), {
        recursive: true,
      });
  ...
  test("filesystem bridge protects skill overlays when workspaceAccess is rw", async () => {
    ...
    try {
      ...
      await expect(
        bridge?.rename({
          from: "normal.txt",
          to: ".grokbot/sandbox-skills/skills/demo/new.md",
          cwd: workdir,
        }),
      ).rejects.toThrow(/read-only/u);
      await expect(
  ...
  test("runShellCommand uses the inline Windows command line when no args are passed", async () => {
    ...
    const expectedShell = process.env.ComSpec?.trim() || "cmd.exe";
    expect(processConfig?.commandLine).toBe(`${expectedShell} /d /s /c "echo hello"`);
    expect(String(processConfig?.commandLine)).not.toContain(".grokbot-mxc-cmd-");
  });
  ...
  test("runShellCommand uses curated Windows env and passes stdin through unchanged", async () => {
    await withProcessEnv(
      ...
      async () => {
        ...
        expect(commandLine).toMatch(/ \/c ""[^"]*\.grokbot-mxc-cmd-[^"]+\.cmd" /u);
