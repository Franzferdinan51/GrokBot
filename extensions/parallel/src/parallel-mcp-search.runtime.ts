...
import { withTrustedWebSearchEndpoint } from "grokbot/plugin-sdk/provider-web-search";
...
// without this, undici sends a generic `node` UA and GrokBot usage is only
  ...
  const init = await postMcp({
    ...
    body: {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "grokbot-parallel", version: PLUGIN_VERSION },
      },
    },
  });
  if (!init.ok) {
