...
import type { ProviderDefaultThinkingPolicyContext } from "grokbot/plugin-sdk/core";
import type {
  ModelApi,
  ModelProviderConfig,
...
} from "grokbot/plugin-sdk/provider-model-types";
import {
...
const OPENAI_AGENT_RUNTIME_ID = "grokbot";
...
function resolveSingleObservedModelRoute(
  ...
  // and let the GrokBot runtime own the request.
  const platformApi =
...
export function resolveModelRoutes(
  ...
  return {
    ...
    defaultRuntimeId: resolutions.some(
      (resolution) => resolution.kind === "routes" && resolution.defaultRuntimeId === "grokbot",
    )
      ? OPENAI_AGENT_RUNTIME_ID
