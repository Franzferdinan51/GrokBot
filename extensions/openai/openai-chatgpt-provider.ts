...
import { CODEX_CLI_PROFILE_ID, type OAuthCredential } from "grokbot/plugin-sdk/provider-auth";
import { buildOauthProviderAuthResult } from "grokbot/plugin-sdk/provider-auth";
import {
 ...
 * entirely when they were written by older GrokBot versions. When that row wins
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.grokbot.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `grokbot models auth login --provider openai` or `grokbot configure`.";
}
