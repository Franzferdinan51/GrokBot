...
import { resolveExpiresAtMsFromEpochSeconds } from "grokbot/plugin-sdk/number-runtime";
import {
  createProviderHttpError,
  readProviderJsonResponse,
  resolveProviderRequestHeaders,
} from "grokbot/plugin-sdk/provider-http";
import { captureWsEvent } from "grokbot/plugin-sdk/proxy-capture";
  ...
  const payload = await (async () => {
    try {
      if (!response.ok) {
        ...
        // GrokBot auth source won. Keep the status metadata, but give callers
