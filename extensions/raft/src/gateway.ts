...
import { keepHttpServerTaskAlive, waitUntilAbort } from "grokbot/plugin-sdk/channel-outbound";
import { KeyedAsyncQueue } from "grokbot/plugin-sdk/keyed-async-queue";
import { createChannelReplayGuard } from "grokbot/plugin-sdk/persistent-dedupe";
...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. GrokBot passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error("Raft requires GrokBot channel runtime support. Update GrokBot and retry.");
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. GrokBot has no
      ...
      // Raft owns wake metadata and its schema evolution. GrokBot accepts only
