...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/grokbot-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/grokbot-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
vi.mock("grokbot/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("grokbot/plugin-sdk/channel-activity-runtime")
  >("grokbot/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("grokbot/plugin-sdk/plugin-state-test-runtime")>(
      "grokbot/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  return {
    getWhatsAppRuntime: () => ({
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("GrokBot: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "grokbot-auth-"));
    } else {
      authDir = undefined;
    }
  });
