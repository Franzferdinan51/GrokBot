...
vi.mock("grokbot/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("grokbot/plugin-sdk/logging-core")>(
    "grokbot/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("GrokBot WhatsApp listener close");
