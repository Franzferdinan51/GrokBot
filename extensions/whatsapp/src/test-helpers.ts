...
import { formatEnvelopeTimestamp } from "grokbot/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("grokbot:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("grokbot:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[grokbot]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/grokbot-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("grokbot:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("grokbot:lastSocket")];
  if (typeof getter === "function") {
