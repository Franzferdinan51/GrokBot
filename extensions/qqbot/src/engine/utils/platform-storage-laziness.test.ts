...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.grokbot/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".grokbot", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".grokbot", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "grokbot.sqlite");
