...
vi.mock("../utils/platform.js", () => ({
  getQQBotMediaDir: () => "/tmp/grokbot-qqbot-downloads",
}));
...
describe("engine/gateway/inbound-attachments", () => {
  ...
  it("classifies image content types case-insensitively when download succeeds", async () => {
    downloadFileMock.mockResolvedValueOnce("/tmp/grokbot-qqbot-downloads/a.png");
    downloadFileMock.mockResolvedValueOnce("/tmp/grokbot-qqbot-downloads/b.png");
    ...
    expect(result.imageUrls).toEqual([
      "/tmp/grokbot-qqbot-downloads/a.png",
      "/tmp/grokbot-qqbot-downloads/b.png",
    ]);
  ...
  it("does not classify a mixed-case non-image content type as an image", async () => {
    downloadFileMock.mockResolvedValue("/tmp/grokbot-qqbot-downloads/doc.pdf");
    ...
    expect(result.imageUrls).toEqual([]);
    expect(result.attachmentInfo).toBe("\n[Attachment: /tmp/grokbot-qqbot-downloads/doc.pdf]");
  });

  it("prefers voice_wav_url for voice downloads and transcribes with configured STT", async () => {
    downloadFileMock.mockResolvedValue("/tmp/grokbot-qqbot-downloads/voice.wav");
    ...
    expect(transcribeAudioMock).toHaveBeenCalledWith("/tmp/grokbot-qqbot-downloads/voice.wav", {
      channels: { qqbot: { stt: {} } },
    });
    expect(result.voiceAttachmentPaths).toEqual(["/tmp/grokbot-qqbot-downloads/voice.wav"]);
