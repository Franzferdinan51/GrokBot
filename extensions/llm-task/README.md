...
`grokbot.invoke --each`) without adding new GrokBot code per workflow.
...
This extension depends on GrokBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** GrokBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.grokbot/extensions` as a standalone plugin directory.
