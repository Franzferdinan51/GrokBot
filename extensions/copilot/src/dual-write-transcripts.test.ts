...
describe("mirrorCopilotTranscript", () => {
  it("mirrors user, assistant, and tool result messages by SQLite identity", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-basic-");
  ...
  it("preserves gateway user-turn identity across Copilot transcript mirroring", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-user-identity-");
  ...
  it("deduplicates re-emits by idempotency scope", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-dedupe-");
    const messages = [
      makeAgentUserMessage({
        content: [{ type: "text", text: "hello" }],
        timestamp: Date.now(),
  ...
  it("runs before_message_write before appending mirrored messages", async () => {
    ...
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-hook-");
  ...
  it("respects before_message_write blocking decisions", async () => {
    ...
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-block-");
  ...
  it("is a no-op when no mirrorable messages are present", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-empty-");
  ...
  it("uses content fingerprint when no explicit mirror identity is attached", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-fingerprint-");
  ...
  it("uses attached identity instead of content fingerprint when provided", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-identity-");
    ...
    await mirrorCopilotTranscript({
      ...target,
      messages: [tagged],
      idempotencyScope: "copilot:grokbot-session-1",
    });

    const raw = await readMirrorRaw(target);
    expect(raw).toContain(
      '"idempotencyKey":"copilot:grokbot-session-1:sdk-session-1:assistant:0"',
    );
  ...
  it("omits idempotencyKey when no idempotencyScope is provided", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-no-scope-");
  ...
  it("filters out non-mirrorable roles", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-filter-");
    const userMessage = makeAgentUserMessage({
  ...
  it("preserves explicit identity across attachCopilotMirrorIdentity overrides", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-override-");
...
describe("dualWriteCopilotTranscriptBestEffort", () => {
  it("returns normally when mirror succeeds", async () => {
    const target = await createSqliteMirrorTarget("grokbot-copilot-mirror-best-effort-");
    await expect(
      dualWriteCopilotTranscriptBestEffort({
        ...target,
        messages: [
  ...
  it("swallows missing runtime identity and does not write JSONL", async () => {
    const root = await makeRoot("grokbot-copilot-mirror-invalid-");
