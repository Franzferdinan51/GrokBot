...
// Filtering them out of the GrokBot bootstrap injection avoids
...
/**
 * Loads GrokBot workspace bootstrap files (IDENTITY.md, SOUL.md,
...
export async function resolveCopilotWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    // SOUL.md / IDENTITY.md / .grokbot conventions live) onto the
...
function renderCopilotWorkspaceBootstrapInstructions(
  ...
  const lines: string[] = [
    "GrokBot loaded these user-editable workspace files. Treat them as project/user context. The Copilot SDK loads AGENTS.md natively from its instruction directories, so AGENTS.md is not repeated here.",
    "",
    "# Project Context",
    "",
