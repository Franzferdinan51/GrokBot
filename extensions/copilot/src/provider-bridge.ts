...
import { isNonSecretApiKeyMarker } from "grokbot/plugin-sdk/provider-auth";
import { isBlockedHostnameOrIp } from "grokbot/plugin-sdk/ssrf-runtime";
...
const COPILOT_BYOK_TRANSPORT_POLICY_ERROR =
  "[copilot-attempt] BYOK does not support GrokBot provider request proxy, TLS, or private-network policy overrides";
const COPILOT_BYOK_ENDPOINT_POLICY_ERROR =
  "[copilot-attempt] BYOK endpoint is blocked by GrokBot SSRF policy";
...
/**
 * Maps GrokBot's prepared model facts into the Copilot SDK's session-level
 * provider contract. The SDK owns the wire request; GrokBot only supplies
