...
export async function runXaiSearchProviderSetup(
  ...
  await ctx.prompter.note(
    [
      ...
      `You can change this later with ${formatCliCommand("grokbot configure --section web")}.`,
    ].join("\n"),
    "X search",
  );
...
export async function executeXaiWebSearchProviderTool(
  ...
  if (!auth) {
    return {
      ...
      message:
        "web_search (grok) needs xAI credentials. Run `grokbot onboard --auth-choice xai-oauth` to sign in with Grok, run `grokbot onboard --auth-choice xai-api-key`, set `XAI_API_KEY` in the Gateway environment, or configure `plugins.entries.xai.config.webSearch.apiKey`. If you do not want to configure search credentials, use web_fetch for a specific URL or the browser tool for interactive pages.",
