...
vi.mock("grokbot/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: grokbot", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "grokbot" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: grokbot", async () => {
