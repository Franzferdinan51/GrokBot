...
metadata: { "grokbot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == GrokBot `sessions_spawn`
- **File I/O** == GrokBot `read`/`write`
- **Remote fetch** == GrokBot `web_fetch` (or `exec` with curl when POST is required)
