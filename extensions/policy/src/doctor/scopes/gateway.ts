...
import type { HealthCheck, HealthFinding } from "grokbot/plugin-sdk/health";
  ...
  return policyDenied
    ...
    .map((command): HealthFinding => {
      return {
        ...
        severity: "error",
        message: `Gateway node command '${command}' is denied by policy but not denied by GrokBot config.`,
        source: "policy",
        path: "grokbot config",
