...
function previewGatewayLoopbackBind(
  ...
  if (
    finding.ocPath !== "oc://grokbot.config/gateway/bind" &&
    finding.ocPath !== "oc://grokbot.config/gateway/customBindHost"
  ) {
    return [];
  }
  return [
...
function previewGatewayNodeDenyCommand(
  ...
  if (
    command === undefined ||
    command === "" ||
    finding.ocPath !== "oc://grokbot.config/gateway/nodes/denyCommands"
  ) {
    return [];
  }
  return [
