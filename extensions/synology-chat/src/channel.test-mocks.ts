...
vi.mock("grokbot/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("grokbot/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("grokbot/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("grokbot/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("grokbot/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("grokbot/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/grokbot/synology-chat-sessions.json"),
