    ...
    if (settingsPath && settings.hooks !== undefined) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation: "Review hook commands before recreating equivalent GrokBot automation.",
        }),
      );
    }
    ...
    if (settingsPath && settings.env !== undefined) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Move non-secret values manually and store credentials through GrokBot credential flows.",
        }),
      );
    }
  }
