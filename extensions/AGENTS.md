...
- Extension production code should import from `grokbot/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `grokbot.plugin.json` and the package
  `grokbot` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `grokbot/plugin-sdk/*`.
