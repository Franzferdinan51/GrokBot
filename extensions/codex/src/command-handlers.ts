...
import { resolveAgentDir, resolveSessionAgentIds } from "grokbot/plugin-sdk/agent-runtime";
import { expectDefined } from "grokbot/plugin-sdk/expect-runtime";
import {
  isModelSelectionLocked,
  MODEL_SELECTION_LOCKED_MESSAGE,
} from "grokbot/plugin-sdk/model-session-runtime";
...
const CODEX_DIAGNOSTICS_SOURCE = "grokbot-diagnostics";
    ...
    if (!deps.codexPluginsManagementIo) {
      return {
        text:
          ...
          "Edit ~/.grokbot/grokbot.json or use `grokbot config patch` until the runtime exposes the IO.",
      };
    }
...
async function handleNativeGoal(
  ...
  if (!binding?.threadId) {
    return "No Codex thread is attached to this GrokBot session yet.";
  }
...
async function resumeThread(
  ...
  if (!ctx.sessionId) {
    return "Cannot attach a Codex thread because this command did not include an GrokBot session id.";
  }
  const scope = resolveCodexConversationControlScope(ctx);
  ...
  return await deps.bindingStore.withLease(identity, async () => {
    ...
    return `Attached this GrokBot session to Codex thread ${formatCodexDisplayText(
      effectiveThreadId,
    )}.`;
  });
}
...
function formatCodexDiagnosticsTargetBlock(
  ...
  if (target.sessionKey) {
    lines.push(`GrokBot session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
  }
  if (target.sessionId) {
    lines.push(`GrokBot session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
  }
...
function formatCodexDiagnosticsTargetLine(target: CodexDiagnosticsTarget): string {
  ...
  if (sessionLabel) {
    parts.push(`GrokBot session ${formatCodexValueForDisplay(sessionLabel)}`);
  }
