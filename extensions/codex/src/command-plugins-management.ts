...
// Plugin lifecycle changes (enable/disable) write to grokbot.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in grokbot.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
function buildPluginsHelp(): string {
  return [
    "Codex sub-plugin management (writes only to ~/.grokbot/grokbot.json, never to ~/.codex/config.toml):",
...
function formatPluginList(
  ...
  return [
    "Codex sub-plugins in Openclaw config (~/.grokbot/grokbot.json):",
    "",
    ...rows.map(
      (r) =>
