...
import { replaceRuntimeAuthProfileStoreSnapshots } from "grokbot/plugin-sdk/agent-runtime";
...
import { initializeGlobalHookRunner, registerInternalHook } from "grokbot/plugin-sdk/hook-runtime";
import { registerMemoryCapability } from "grokbot/plugin-sdk/memory-core-host-runtime-core";
import { MESSAGE_TOOL_DELIVERY_HINTS } from "grokbot/plugin-sdk/message-tool-delivery-hints";
import { registerPluginCommand } from "grokbot/plugin-sdk/plugin-runtime";
import { createMockPluginRegistry } from "grokbot/plugin-sdk/plugin-test-runtime";
import { GPT5_BEHAVIOR_CONTRACT as CODEX_GPT5_BEHAVIOR_CONTRACT } from "grokbot/plugin-sdk/provider-model-shared";
...
describe("runCodexAppServerAttempt", () => {
  ...
  it("starts active GrokBot sandbox threads with Codex native execution disabled", async () => {
  ...
  it("routes native Codex execution through an GrokBot sandbox exec-server when opted in", async () => {
    const appServer = {
    ...
    try {
      ...
      expect(environmentAddParams?.environmentId).toMatch(/^grokbot-sandbox-/);
  ...
  it("starts Codex threads without duplicate GrokBot workspace tools by default", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("includes Codex app-server scoped plugin command guidance in developer instructions", () => {
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "GrokBot main command guidance.",
    ...
    expect(instructions).not.toContain("GrokBot main command guidance.");
  });

  it("passes GrokBot skills as turn collaboration developer instructions", async () => {
    ...
    Object.assign(params, {
      trajectorySessionFile: `sqlite:main:session-1:${path.join(tempDir, "grokbot-agent.sqlite")}`,
      trajectoryRecorder: {
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).toContain("## GrokBot Skills");
    expect(collaborationInstructions).toContain("<available_skills>");
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).not.toContain("## GrokBot Skills");
    ...
    expect(compiledContext?.data?.systemPrompt).toContain("## GrokBot Skills");
  ...
  it("emits TUI-compatible tool events for Codex dynamic tool calls", async () => {
    ...
    expect(firstResponse).toMatchObject({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown GrokBot tool: python" }],
    });
    ...
    expect(onRunAgentEvent).toHaveBeenCalledWith({
      ...
      data: {
        ...
        isError: true,
        result: {
          content: [{ type: "text", text: "Unknown GrokBot tool: python" }],
        },
      },
    });
  ...
  it("keeps leading delivery hints out of the Codex current user request", async () => {
    for (const [index, deliveryHint] of MESSAGE_TOOL_DELIVERY_HINTS.entries()) {
      ...
      const inputText = turnStartParams.input?.[0]?.text ?? "";
      expect(inputText).toContain("GrokBot delivery metadata:");
  ...
  it("keeps GrokBot control-path tools direct when code-mode-only is enabled", () => {
    const tools = [
      createRuntimeDynamicTool("message"),
    ...
    expect(webSearch?.namespace).toBe("grokbot");
    expect(webSearch?.deferLoading).toBe(true);
    expect(heartbeat?.namespace).toBe("grokbot");
  ...
  it("keeps the heartbeat schema deferred and stable across normal and heartbeat turns", async () => {
    ...
    expect(normalInstructions).not.toContain(
      "Deferred searchable GrokBot dynamic tools available: heartbeat_respond",
    );
  ...
  it("keeps message in the registered schema when disabled for an internal turn", async () => {
    ...
    await expect(
      ...
      contentItems: [
        {
          type: "inputText",
          text: "GrokBot tool is not available for this turn: message",
        },
      ],
    });
  });
  ...
  it("projects bounded continuity when starting Codex without a native thread binding", async () => {
    ...
    const harness = createStartedThreadHarness();
    const params = createParams(sessionFile, workspaceDir);
    params.prompt = "make the default webpage grokbot";
    ...
    expect(inputText).toContain("GrokBot assembled context for this turn:");
    ...
    expect(inputText).toContain("Current user request:");
    expect(inputText).toContain("make the default webpage grokbot");
  });
  ...
  it("does not replay mirrored history already covered by an existing Codex binding", async () => {
    ...
    expect(inputText).not.toContain("GrokBot assembled context for this turn:");
  ...
  it("does not project mirrored messages on consecutive resumes", async () => {
    ...
    const firstInputText =
      (firstTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(firstInputText).toContain("GrokBot assembled context for this turn:");
    ...
    const secondInputText =
      (secondTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(secondInputText).not.toContain("GrokBot assembled context for this turn:");
  ...
  it("passes stable workspace files as Codex developer instructions and routes MEMORY.md through tools", async () => {
    ...
    expect(threadDeveloperInstructions).toContain("GrokBot Workspace Instructions");
    ...
    expect(collaborationInstructions).toContain("GrokBot Agent Soul");
    ...
    expect(collaborationInstructions).toContain("GrokBot Workspace Memory");
    ...
    expect(inputText).not.toContain("GrokBot runtime context for this turn:");
    ...
    expect(inputText).not.toContain("GrokBot Workspace Memory");
  ...
  it("adds memory recall guidance when dated memory notes exist without root MEMORY.md", async () => {
    ...
    expect(collaborationInstructions).toContain("memory_search");
    expect(collaborationInstructions).toContain("memory_get");
    expect(collaborationInstructions).not.toContain("GrokBot Workspace Memory");
  ...
  it("sends workspace bootstrap instructions through Codex app-server payloads", async () => {
    ...
    expect(threadStartParams.developerInstructions).toContain("GrokBot Workspace Instructions");
  ...
  it("omits heartbeat Codex workspace pointers for empty HEARTBEAT.md files", async () => {
    ...
    expect(collaborationInstructions).toContain("This is an GrokBot heartbeat turn");
    expect(collaborationInstructions).not.toContain("HEARTBEAT.md exists");
  });

  it("keeps lightweight cron Codex turns out of GrokBot bootstrap context", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const exactCommand =
      "cd /Users/phaedrus/Projects/grokbot && /Users/phaedrus/clawd/scripts/clawsweeper-related-scan.py";
    await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("keeps lightweight cron delivery hints byte-for-byte without GrokBot prompt context", async () => {
  ...
  it("promotes implicit Codex yolo approval policy when GrokBot tool policy exists", async () => {
    initializeGlobalHookRunner(
    ...
    expect(info).toHaveBeenCalledWith(
      "codex app-server approval policy promoted for GrokBot tool policy",
      {
        from: "never",
        to: "untrusted",
  ...
  it("keeps explicit Codex yolo mode unpromoted when GrokBot tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("keeps normalized full exec mode unpromoted when GrokBot tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
