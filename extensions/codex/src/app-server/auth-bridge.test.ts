...
import { upsertAuthProfile } from "grokbot/plugin-sdk/provider-auth";
...
vi.mock("grokbot/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("grokbot/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("never overlays persisted profiles onto a supplied runtime store", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-codex-app-server-"));
  ...
  it("uses the native user Codex home for coexistence mode", async () => {
    await withTempDir("grokbot-codex-user-home-", async (root) => {
  ...
  it("places the ephemeral auth-store override after configured root overrides", async () => {
    await withTempDir("grokbot-codex-auth-store-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("does not mistake an option value for the app-server subcommand", async () => {
    await withTempDir("grokbot-codex-profile-name-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("preserves custom stdio backend arguments", async () => {
    await withTempDir("grokbot-codex-custom-backend-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("fingerprints supplied token stores with the same profile id independently", async () => {
    const resolveKey = async (token: string) =>
      await resolveCodexAppServerAuthAccountCacheKey({
        agentDir: "/tmp/grokbot-codex-prepared-auth",
        authProfileId: "openai:work",
        authProfileStore: {
          version: 1,
          profiles: {
  ...
  it("selects ordered Codex OAuth before an OpenAI API-key backup", async () => {
    ...
    await applyCodexAppServerAuthProfile({
      client: { request } as never,
      agentDir: "/tmp/grokbot-codex-auth-product-proof",
      authProfileStore,
    });
  ...
  it("applies native Codex CLI OAuth when no GrokBot auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-codex-app-server-"));
    const agentDir = path.join(root, "agent");
  ...
  it("finds native Codex OAuth in the OS home when GrokBot uses an isolated home", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-codex-app-server-"));
    const osHome = path.join(root, "os-home");
    const openClawHome = path.join(root, "grokbot-home");
  ...
  it("answers refresh from native Codex CLI OAuth without persisting an GrokBot profile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-codex-app-server-"));
    const agentDir = path.join(root, "agent");
    const codexHome = path.join(root, "codex-cli");
