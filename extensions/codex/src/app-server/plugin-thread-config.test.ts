...
describe("Codex plugin thread config", () => {
  ...
  it("re-enables an GrokBot-allowed app even when app/list reports it disabled", async () => {
    const appCache = new CodexAppInventoryCache();
    await appCache.refreshNow({
      key: "runtime",
      nowMs: 0,
  ...
  it("does not expose apps for plugins that GrokBot policy leaves disabled", async () => {
    const appCache = new CodexAppInventoryCache();
    await appCache.refreshNow({
      key: "runtime",
      nowMs: 0,
  ...
  it("uses app-level destructive policy for plugins without GrokBot tool-name knowledge", async () => {
    const appCache = new CodexAppInventoryCache();
    await appCache.refreshNow({
      key: "runtime",
      nowMs: 0,
