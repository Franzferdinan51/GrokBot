...
import { hasUsableOAuthCredential } from "grokbot/plugin-sdk/provider-auth";
...
function resolveCodexAppServerEnvApiKeyCacheKey(params: {
  ...
  hash.update("grokbot:codex:app-server-env-api-key:v1");
  hash.update("\0");
...
function fingerprintApiKeyAuthProfileCacheKey(apiKey: string): string {
  const hash = createHash("sha256");
  hash.update("grokbot:codex:app-server-auth-profile-api-key:v1");
  hash.update("\0");
  hash.update(apiKey);
...
function fingerprintTokenAuthProfileCacheKey(accessToken: string): string {
  const hash = createHash("sha256");
  hash.update("grokbot:codex:app-server-auth-profile-token:v1");
  hash.update("\0");
...
function fingerprintCodexCliAuthFileApiKeyCacheKey(apiKey: string): string {
  const hash = createHash("sha256");
  hash.update("grokbot:codex:app-server-cli-auth-json-api-key:v1");
  hash.update("\0");
  hash.update(apiKey);
