...
import { onAgentEvent, type AgentEventPayload } from "grokbot/plugin-sdk/agent-harness-runtime";
import {
...
import { initializeGlobalHookRunner } from "grokbot/plugin-sdk/hook-runtime";
...
describe("runCodexAppServerAttempt dynamic tools", () => {
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    try {
      ...
      expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown GrokBot tool: lookup$/u);
    ...
    expect(resultEvent?.data?.result?.content?.[0]?.text).toBe("Unknown GrokBot tool: lookup");
      ...
      emitDynamicToolTerminalDiagnostic({
        ...
        response: {
          ...
          contentItems: [
            {
              type: "inputText",
              text: "GrokBot dynamic tool call timed out after 1ms while running tool echo.",
            },
          ],
        },
      });
