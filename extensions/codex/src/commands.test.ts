...
import { MODEL_SELECTION_LOCKED_MESSAGE } from "grokbot/plugin-sdk/model-session-runtime";
import type { PluginCommandContext, PluginCommandResult } from "grokbot/plugin-sdk/plugin-entry";
...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`GrokBot session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`GrokBot session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    resetCodexTestBindingStore();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-codex-command-"));
  ...
  it("lists Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(result, "grokbot.json");
  });
  ...
  it("enables and disables Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(disabled, "google-calendar: disabled in grokbot.json");
    ...
    expectResultTextContains(enabled, "google-calendar: enabled in grokbot.json");
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this GrokBot session to Codex thread thread-123.",
    });
  ...
  it("reclaims an unloaded plugin's stale generation before attaching a thread", async () => {
    ...
    expect(result.text).toBe("Attached this GrokBot session to Codex thread thread-new.");
  ...
  it("does not report a resumed thread as attached after a generation conflict", async () => {
    ...
    expect(result.text).not.toContain("Attached this GrokBot session");
  });
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because GrokBot sandboxing is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because GrokBot exec host=node is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
    expect(steerCodexConversationTurn).not.toHaveBeenCalled();
    expect(setCodexConversationModel).not.toHaveBeenCalled();
  ...
  it("blocks config-level exec host=node without a session key", async () => {
    ...
    expect(result.text).toContain(
      "Codex-native /codex bind is unavailable because GrokBot exec host=node is active for this session.",
    );
  });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this GrokBot session yet.",
    });
  });
  ...
  it("previews exec-approved diagnostics upload without exposing Codex ids", async () => {
    ...
    expect(result.text).toBe(
      [
        ...
        "The completed diagnostics reply will list the GrokBot session ids and Codex thread ids that were sent.",
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("GrokBot session key: `agent:first:whatsapp:one`");
    expect(request.text).toContain("GrokBot session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("GrokBot session key: `agent:second:discord:two`");
    expect(request.text).toContain("GrokBot session id: `session-two`");
  ...
  it("uses the host agent for diagnostics inventory sessions with unscoped keys", async () => {
    ...
    expect(request.text).toContain("GrokBot session key: `global`");
  ...
  it("sanitizes diagnostics upload errors before showing them", async () => {
    ...
    await expect(
      ...
      text: [
        ...
        "- channel test, GrokBot session session-1, Codex thread &lt;\uff20U123&gt;: bad??? &lt;\uff20U123&gt; \uff3btrusted\uff3d\uff08https://evil\uff09 \uff20here",
        "Inspect locally:",
  ...
  it("keeps diagnostics upload errors UTF-16 safe at the display boundary", async () => {
    ...
    await expect(
      ...
      text: [
        "Could not send Codex diagnostics:",
        `- channel test, GrokBot session session-1, Codex thread thread-error-boundary: ${expectedError}`,
        "Inspect locally:",
  ...
  it("does not throttle diagnostics retries after upload failures", async () => {
    ...
    await expect(
      ...
      text: [
        "Could not send Codex diagnostics:",
        "- channel test, GrokBot session session-1, Codex thread thread-retry: temporary outage",
        "Inspect locally:",
  ...
  it("explains diagnostics when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: [
        "No Codex thread is attached to this GrokBot session yet.",
