...
const ForkParamsSchema = Type.Object(
  {
    ...
    attach: Type.Optional(
      Type.Boolean({
        default: true,
        description: "Attach the fork to this GrokBot session for its next turn.",
      }),
    ),
  },
  { additionalProperties: false },
...
export function createCodexThreadsTool(options: CodexThreadsToolOptions): AnyAgentTool | null {
  ...
  return {
    ...
    async execute(_toolCallId, rawParams) {
      ...
      if (action === "archive") {
        ...
        await options.bindingStore.withThreadArchiveFence(async () => {
          ...
          if (await options.bindingStore.hasOtherThreadOwner(threadId, identity)) {
            throw new Error(
              "cannot archive a native Codex thread owned by another GrokBot session",
            );
          }
      ...
      if (attach && !session) {
        throw new Error("cannot attach a Codex fork without an active GrokBot session");
      }
