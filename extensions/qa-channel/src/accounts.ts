...
import { createAccountListHelpers } from "grokbot/plugin-sdk/account-helpers";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "grokbot/plugin-sdk/account-id";
import { resolveMergedAccountConfig } from "grokbot/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "grokbot";
  const botDisplayName = merged.botDisplayName?.trim() || "GrokBot QA";
  return {
    accountId,
    enabled,
