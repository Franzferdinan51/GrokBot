...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.grokbot).toBe(
        plan.packageJson.grokbot.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.grokbot.optional).toBe(true);
    }
  });
