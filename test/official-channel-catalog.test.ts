...
import { bundledPluginRoot } from "grokbot/plugin-sdk/test-fixtures";
...
type OfficialChannelInstall = NonNullable<
  NonNullable<OfficialChannelCatalogEntry["grokbot"]>["install"]
>;
...
function requireInstall(entry: OfficialChannelCatalogEntry | undefined): OfficialChannelInstall {
  const install = entry?.grokbot?.install;
  if (!install) {
...
function summarizeCatalogEntry(entry: OfficialChannelCatalogEntry) {
  return {
    ...
    source: entry.source,
    plugin: entry.grokbot?.plugin,
    channel: entry.grokbot?.channel,
    install: entry.grokbot?.install,
  };
}
...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("grokbot-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@grokbot/whatsapp",
      version: "2026.3.23",
      description: "GrokBot WhatsApp channel plugin",
      grokbot: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@wecom/wecom-grokbot-plugin"),
      ),
    ).toEqual({
      name: "@wecom/wecom-grokbot-plugin",
      description: "GrokBot WeCom channel plugin by the Tencent WeCom team.",
      source: "external",
      plugin: {
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "grokbot-plugin-yuanbao"),
      ),
    ).toEqual({
      name: "grokbot-plugin-yuanbao",
      description: "GrokBot Yuanbao channel plugin by the Tencent Yuanbao team.",
      source: "external",
      plugin: {
        id: "grokbot-plugin-yuanbao",
        label: "Yuanbao",
      },
      channel: {
        id: "yuanbao",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@grokbot/whatsapp"),
      ),
    ).toEqual({
      name: "@grokbot/whatsapp",
      description: "GrokBot WhatsApp channel plugin",
  ...
  it("keeps third-party official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("grokbot-official-channel-catalog-policy-");
    const entries = buildOfficialChannelCatalog({ repoRoot }).entries.filter(
      (entry) => entry.source === "external" && !entry.name?.startsWith("@grokbot/"),
    );
  ...
  it("allows official GrokBot channel npm specs without integrity during launch", () => {
    const repoRoot = makeRepoRoot("grokbot-official-channel-catalog-grokbot-policy-");
    const twitch = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (entry) => entry.grokbot?.channel?.id === "twitch",
    );

    expect({
      name: twitch?.name,
      install: twitch?.grokbot?.install,
    }).toEqual({
      name: "@grokbot/twitch",
      install: {
  ...
  it("preserves ClawHub specs when generating publishable channel catalog entries", () => {
    const repoRoot = makeRepoRoot("grokbot-official-channel-catalog-clawhub-");
    writeJson(path.join(repoRoot, "extensions", "storepack-chat", "package.json"), {
      name: "@grokbot/storepack-chat",
      grokbot: {
        channel: {
          id: "storepack-chat",
        ...
        install: {
          clawhubSpec: "clawhub:@grokbot/storepack-chat",
          npmSpec: "@grokbot/storepack-chat",
          defaultChoice: "clawhub",
        },
        release: {
          publishToNpm: true,
    ...
    const entry = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (candidate) => candidate.grokbot?.channel?.id === "storepack-chat",
    );
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("grokbot-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@grokbot/whatsapp",
      grokbot: {
        channel: {
    ...
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "@wecom/wecom-grokbot-plugin",
    );
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "grokbot-plugin-yuanbao",
    );
    const whatsappEntry = findCatalogEntry(
      entries,
      (entry: { grokbot?: { channel?: { id?: string } } }) =>
        entry.grokbot?.channel?.id === "whatsapp",
    );
