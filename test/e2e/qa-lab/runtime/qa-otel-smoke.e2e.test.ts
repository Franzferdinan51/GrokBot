...
describe("qa-otel-smoke receiver bounds", () => {
  ...
  function makePassingSmokeAssertionInput(): Parameters<typeof testing.assertSmoke>[0] {
    return {
      ...
      metrics: [{ name: "grokbot.harness.duration_ms" }],
      requests: [
        {
          path: "/v1/traces",
          signal: "traces",
      ...
      spans: [
        {
          ...
          parent: false,
          attributes: {
            "grokbot.error": "QA OTEL provider stream failed OPENAI_API_KEY=***",
          },
        },
        {
          name: "grokbot.harness.run",
          parent: true,
          attributes: {
            "grokbot.error": "QA OTEL provider stream failed OPENAI_API_KEY=***",
          },
        },
        { name: "grokbot.context.assembled", parent: true, attributes: {} },
        { name: "grokbot.message.delivery", parent: true, attributes: {} },
        {
  ...
  it("ignores inherited OTEL exporter endpoints during direct producer execution", () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "grokbot-qa-otel-env-isolation-"));
    try {
    ...
    input.stdoutLogRecords = [
      {
        ...
        "service.name": "grokbot-qa-lab-otel-smoke",
        severityText: "INFO",
        severityNumber: 9,
        body: "log",
        attributes: {
          "grokbot.log.level": "INFO",
        },
      },
    ];
  ...
  it("moves Docker collector telemetry off the default host port", async () => {
    ...
    const collector = await testing.startDockerOtelCollector(4317, {
      mkdtemp: async () => "/tmp/grokbot-otel-collector-test",
      platform: "linux",
    ...
    expect(stopDockerContainer).toHaveBeenCalledWith(
      "grokbot-otel-smoke-00000000-0000-4000-8000-000000000000",
    );
    expect(removePath).toHaveBeenCalledWith("/tmp/grokbot-otel-collector-test", {
      force: true,
      recursive: true,
    });
  });
  ...
  it("cleans Docker collector containers and temp config after readiness failures", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "grokbot-qa-otel-collector-"));
  ...
  it("reports bounded Docker collector output when readiness exits", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "grokbot-qa-otel-collector-output-"));
    const collectorDir = path.join(tempRoot, "collector");
    const child = new EventEmitter() as EventEmitter & {
      stderr: EventEmitter;
      stdout: EventEmitter;
