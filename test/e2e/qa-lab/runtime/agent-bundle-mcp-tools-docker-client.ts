// GrokBot bundle MCP tools Docker harness.
...
const probeTool = server.tool("docker_probe", "Docker GrokBot MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("grokbot-agent-bundle-mcp-");
