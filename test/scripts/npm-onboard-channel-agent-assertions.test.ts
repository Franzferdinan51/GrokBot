...
function writeConfig(home: string, channels: Record<string, unknown>): void {
  ...
  fs.writeFileSync(path.join(configDir, "grokbot.json"), JSON.stringify({ channels }));
}
...
function runStatusAssert(
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-status-assertions-"));
  try {
...
describe("npm onboard channel agent assertions", () => {
  it("rejects loose mock OpenAI port args", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-onboard-assertions-"));
  ...
  it("validates OpenAI env refs from the SQLite auth profile store", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-onboard-assertions-"));
    const agentDir = path.join(tempDir, ".grokbot", "agents", "main", "agent");
      ...
      writeAuthProfileStoreSqlite(agentDir, {
        ...
        profiles: {
          "openai:api-key": {
            type: "api_key",
            provider: "openai",
            key: "sk-grokbot-npm-onboard-e2e",
          },
        },
      });
  ...
  it("validates channel tokens in their canonical config fields", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-channel-assertions-"));
    try {
      writeConfig(tempDir, {
