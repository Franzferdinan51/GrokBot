// GrokBot Docker E2E tests cover packaged-dist harness wiring.
...
describe("GrokBot Docker E2E scripts", () => {
  it("keeps first-run checks wired to packaged CLI and GrokBot behavior", () => {
    ...
    expect(source).toContain("GrokBot did not fail closed without inference");
    ...
    expect(source).toContain("[grokbot] interpreted: ${plannerCommand}");
    expect(source).toContain("expected one fuzzy setup planner prompt");
    expect(source).toContain("GrokBot did not enable Discord");
    expect(source).toContain("GrokBot did not write Discord token SecretRef");
    expect(source).toContain("GrokBot first-run Docker E2E passed");
    expect(spec).toContain('"auditOperations"');
    expect(spec).toContain('"grokbot.setup"');
  ...
  it("keeps rescue checks wired through auto-reply command handling", () => {
    ...
    expect(source).toContain("[grokbot] done: gateway.restart");
    ...
    expect(source).not.toContain("[grokbot] done: doctor.fix");
    expect(source).toContain("GrokBot rescue Docker E2E passed");
  });
});
