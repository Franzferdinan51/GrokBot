...
function runStopExistingLocalApp(params: { fakeLsof?: string; fakePgrep: string }) {
  const root = mkdtempSync(join(tmpdir(), "grokbot-build-run-mac-test-"));
  tempRoots.push(root);
  ...
  writeFileSync(
    ...
    [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
      'BIN_ABS="/worktree/apps/macos/.build-local/debug/GrokBot"',
      'BIN=".build-local/debug/GrokBot"',
...
describe("scripts/build-and-run-mac.sh", () => {
  ...
  it("stops only the local debug app binary before relaunching", () => {
    ...
    const { killCalls, pgrepCalls, result } = runStopExistingLocalApp({
      fakePgrep: [
        ...
        'if [[ "$2" == "/worktree/apps/macos/.build-local/debug/GrokBot" ]]; then exit 1; fi',
        'if [[ "$2" == ".build-local/debug/GrokBot" && "$count" == "1" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
    ...
    expect(pgrepCalls).toContain("-f /worktree/apps/macos/.build-local/debug/GrokBot");
    expect(pgrepCalls).toContain("-f .build-local/debug/GrokBot");
  ...
  it("fails when the scoped local app process survives cleanup", () => {
    const { result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        'if [[ "$2" == ".build-local/debug/GrokBot" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
