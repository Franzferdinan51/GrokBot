...
describe("website installer sync workflow", () => {
  ...
  it("treats all website installer scripts as GrokBot-owned inputs", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain("bash /tmp/install.sh --version latest && grokbot --version");
    expect(workflow).not.toContain("bash /tmp/install.sh --no-prompt --no-onboard");
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/grokbot");
    ...
    expect(workflow).toContain("grokbot --version");
  ...
  it("syncs verified scripts to grokbot.ai only after all installer checks pass", () => {
    expect(workflow).toContain("needs: [static, linux-docker, macos-installer, windows-installer]");
    expect(workflow).toContain("repository: grokbot/grokbot.ai");
    ...
    expect(workflow).toContain("cp grokbot/scripts/install.sh grokbot.ai/public/install.sh");
    expect(workflow).toContain(
      "cp grokbot/scripts/install-cli.sh grokbot.ai/public/install-cli.sh",
    );
    expect(workflow).toContain("cp grokbot/scripts/install.ps1 grokbot.ai/public/install.ps1");
    expect(workflow).toContain("rm -f grokbot.ai/public/install.cmd");
