...
describe("report-test-temp-creations", () => {
  it("keeps a non-executed warning fixture for changed-gate proof", () => {
    ...
    const warningFixture = 'fs.mkdtempSync("grokbot-warning-fixture-")';
  ...
  it("reports repository-observed mkdtemp call forms", () => {
    const sources = [
      ...
      ["const root = await fsPromises.", "mkdtemp", '("/tmp/grokbot-case-");'].join(""),
  ...
  it("honors explicit allow comments with reasons", () => {
    ...
    const inlineAllowedSource = `const inlineAllowed = ${tmpDirCall}; // grokbot-temp-dir: allow verifies tmp API behavior`;
    const blockedSource = `const blocked = ${mkdtempCall};`;
    const stringMarkerSource = `const stringMarker = ${mkdtempCall}; const note = "grokbot-temp-dir: allow quoted text";`;
  ...
  it("formats GitHub warning annotations for CI report mode", () => {
    expect(
      formatGithubWarning({
        ...
        line: 12,
        reason: "new mkdtemp temp directory creation",
        // grokbot-temp-dir: allow test fixture for GitHub warning formatting
  ...
  it("reads staged source for manual helper scans", () => {
    const root = tempDirs.make("grokbot-temp-report-staged-source-");
  ...
  it("exits non-zero for staged findings when requested", () => {
    const root = tempDirs.make("grokbot-temp-report-");
    const env = createNestedGitEnv();
    execFileSync("git", ["init", "-q", "--initial-branch=main"], { cwd: root, env });
  ...
  it("falls back to a two-dot diff when refs have no merge base", () => {
    const root = tempDirs.make("grokbot-temp-report-no-merge-base-");
    const env = createNestedGitEnv();
