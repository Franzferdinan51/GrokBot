...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/grokbot-pr-maintainer/scripts/github-activity.sh",
);
...
describe("grokbot-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("grokbot/grokbot last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/grokbot\/grokbot\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/grokbot\/grokbot\/commits/g)).toHaveLength(1);
