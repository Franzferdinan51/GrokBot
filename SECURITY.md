...
Security work is shared across a number of GrokBot maintainers, including engineers and security researchers from organizations such as NVIDIA and Tencent. See the [maintainer list](CONTRIBUTING.md#maintainers).

## Report a Security Issue

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [grokbot/grokbot](https://github.com/grokbot/grokbot)
- **macOS desktop app** — [grokbot/grokbot](https://github.com/grokbot/grokbot) (apps/macos)
- **iOS app** — [grokbot/grokbot](https://github.com/grokbot/grokbot) (apps/ios)
- **Android app** — [grokbot/grokbot](https://github.com/grokbot/grokbot) (apps/android)
- **ClawHub** — [grokbot/clawhub](https://github.com/grokbot/clawhub)
- **Trust and threat model** — [grokbot/trust](https://github.com/grokbot/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@grokbot.ai](mailto:security@grokbot.ai)** and we'll route it.

For GrokBot core issues, submit through a private [GitHub Security Advisory](https://github.com/grokbot/grokbot/security/advisories/new). Do not open a public issue or PR that discloses an unpatched vulnerability, exploit path, secret, or security-sensitive proof of concept.
...
For full reporting instructions see our [Trust page](https://trust.grokbot.ai).
For maintainer response workflow, see the [incident response plan](docs/security/incident-response.md).

GrokBot does not currently run a paid bug bounty program. Please still disclose responsibly so we can fix real issues quickly. The best way to help the project right now is to send high-signal reports and, when practical, focused PRs.
...
- The actual impact, including which GrokBot trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses an GrokBot security boundary.
...
- A report whose only primitive is changing the process or child-process environment before running GrokBot or an executable GrokBot invokes.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated GrokBot impact.
...
- Tested version details (GrokBot version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through GrokBot. Showing that GrokBot can reach a native parser is not enough by itself.
- Demonstrated impact tied to GrokBot's documented trust boundaries.
- For exposed-secret reports: proof the credential is GrokBot-owned (or grants access to GrokBot-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example image codec libraries such as libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through GrokBot. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
- Reports whose only impact is transient extra memory, CPU, or allocation work from decoding, base64 expansion, media transcoding, serialization, or other format conversion after the input was already accepted under GrokBot's configured size/trust limits, including base64 decode-before-size-estimate findings. These are performance issues, not vulnerabilities, unless the report demonstrates unauthenticated amplification, bypass of configured limits, crash/process termination, persistent resource exhaustion, data exposure, or another documented boundary bypass.
...
- Reports that depend on attacker-controlled environment variables changing executable behavior, including variables that redirect lookup paths, preload code, select wrappers/interpreters, alter package-manager or runtime hooks, or make one executable call another executable. Control of the process or child-process environment is trusted host/operator control in GrokBot's model; these reports need a separate GrokBot boundary bypass that lets untrusted input set or mutate that environment.
...
GrokBot does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-grokbot-scopes` headers are ignored for that shared-secret path
...
- GrokBot can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need GrokBot, use one VPS (or host/OS user boundary) per user.
...
- This is expected in GrokBot's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of GrokBot's trusted computing base for a gateway.
...
- Using GrokBot in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped GrokBot production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.grokbot`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not GrokBot-owned and not granting access to GrokBot-operated infrastructure/services) without demonstrated GrokBot impact
...
GrokBot security guidance assumes:

- The host where GrokBot runs is within a trusted OS/admin boundary.
- Anyone who can set or mutate the GrokBot process environment, launcher environment, or child-process environment is inside that trusted host/operator boundary.
- Anyone who can modify `~/.grokbot` state/config (including `grokbot.json`) is effectively a trusted operator.
...
GrokBot's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
GrokBot separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when GrokBot can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the GrokBot process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

GrokBot uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/grokbot` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/grokbot` (or `grokbot-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the GrokBot-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use GrokBot temp helpers (`resolvePreferredOpenClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-grokbot-dir.ts`
...
For threat model + hardening guidance (including `grokbot security audit --deep` and `--fix`), see:

- `https://docs.grokbot.ai/gateway/security`
...
GrokBot's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `grokbot gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - GrokBot keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `grokbot security audit` as dangerous findings.
...
GrokBot requires **Node.js 22.22.3+, Node.js 24.15+, or Node.js 25.9+**. Node 24 is the recommended default runtime for new installs. These minimum versions include the upstream SQLite WAL-reset corruption fix; Node 23 is unsupported. The minimum supported Node 22 version also includes important security patches:
...
When running GrokBot in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v grokbot-data:/app/data \
  grokbot/grokbot:latest
```

## Security Scanning

GrokBot uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

GrokBot runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.
