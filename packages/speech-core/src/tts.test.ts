...
import type { OpenClawConfig, TtsConfig } from "grokbot/plugin-sdk/config-contracts";
...
vi.mock("grokbot/plugin-sdk/media-runtime", () => ({
  transcodeAudioBuffer: transcodeAudioBufferMock,
}));

vi.mock("grokbot/plugin-sdk/channel-targets", () => ({
...
vi.mock("grokbot/plugin-sdk/speech-core", async () => {
  const actual = await vi.importActual("grokbot/plugin-sdk/speech-core");
...
describe("speech-core native voice-note routing", () => {
  ...
  it("tells generic TTS guidance to defer to MEMORY voice-delivery instructions", () => {
    const hint = buildTtsSystemPromptHint(createTtsConfig("grokbot-speech-core-tts-hint-test"));
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    await expectTtsPayloadResult({
      channel: "discord",
      prefsName: "grokbot-speech-core-tts-test",
  ...
  it("keeps compatible audio-file synthesis deliverable as a voice memo", async () => {
    await expectTtsPayloadResult({
      ...
      prefsName: "grokbot-speech-core-tts-voice-memo-mp3-test",
  ...
  it("does not mark unsupported audio-file output as a voice memo", async () => {
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "grokbot-speech-core-tts-voice-memo-ogg-test",
  ...
  it("pre-transcodes synthesized mp3 to opus-in-CAF when the host can satisfy preferAudioFileFormat", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "grokbot-speech-core-tts-voice-memo-caf-transcode-test",
  ...
  it("falls back to the original mp3 buffer when the host transcoder fails", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "grokbot-speech-core-tts-voice-memo-caf-fallback-test",
  ...
  it("normalizes non-streaming synthesis text before calling the provider", async () => {
    const result = await synthesizeSpeech({
      ...
      cfg: createTtsConfig("grokbot-speech-core-talk-markdown-test"),
      disableFallback: true,
    });
  ...
  it("speaks stripped code through the explicit textToSpeech conversion path", async () => {
    ...
    try {
      const result = await textToSpeech({
        text: "```ts\nconst answer = 42;\n```",
        cfg: createTtsConfig("grokbot-speech-core-code-convert-test"),
      });
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/grokbot-speech-core-voice-model-default-test.json",
          },
        },
      } as OpenClawConfig,
      disableFallback: true,
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            provider: "openrouter",
            prefsPath: "/tmp/grokbot-speech-core-explicit-model-alias-test.json",
            providers: {
              openrouter: {
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/grokbot-speech-core-voice-model-fallback-test.json",
          },
        },
      } as OpenClawConfig,
    });
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/grokbot-speech-core-same-provider-voice-model-fallback-test.json",
          },
        },
      } as OpenClawConfig,
    });
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            provider: "openai",
            prefsPath: "/tmp/grokbot-speech-core-realtime-voice-model-ignored-test.json",
          },
        },
      } as OpenClawConfig,
      disableFallback: true,
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/grokbot-speech-core-supported-voice-model-provider-test.json",
          },
        },
      } as OpenClawConfig,
    });
  ...
  it.each(["feishu", "whatsapp"] as const)(
    ...
    async (channel) => {
      ...
      await expectTtsPayloadResult({
        channel,
        prefsName: `grokbot-speech-core-tts-${channel}-mp3-test`,
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    await expectTtsPayloadResult({
      channel: "slack",
      prefsName: "grokbot-speech-core-tts-slack-test",
  ...
  it("normalizes voice-note Markdown once before synthesis", async () => {
    ...
    try {
      const result = await maybeApplyTtsToPayload({
        payload: { text },
        cfg: createTtsConfig("grokbot-speech-core-once-normalized-markdown-test"),
        channel: "telegram",
        kind: "final",
      });
  ...
  it("skips channel auto-TTS audio for code-heavy replies", async () => {
    ...
    const result = await maybeApplyTtsToPayload({
      payload: { text },
      cfg: createTtsConfig("grokbot-speech-core-code-heavy-voice-note-test"),
      channel: "telegram",
      kind: "final",
    });
  ...
  it("synthesizes code-heavy explicitly tagged hidden TTS text", async () => {
    const cfg = createTtsConfig("grokbot-speech-core-code-heavy-hidden-tts-test");
    let mediaDir: string | undefined;
    try {
      const result = await maybeApplyTtsToPayload({
        payload: {
  ...
  it("synthesizes explicitly tagged short hidden TTS text", async () => {
    const cfg = createTtsConfig("grokbot-speech-core-short-hidden-tts-test");
    let mediaDir: string | undefined;
    try {
      const result = await maybeApplyTtsToPayload({
        payload: {
  ...
  it("truncates long TTS text on a UTF-16 boundary", async () => {
    const prefsName = "grokbot-speech-core-utf16-truncate-test";
  ...
  it("skips block delivery kind in final mode (accumulated final tail synthesizes instead)", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("grokbot-speech-core-block-kind-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("skips tool delivery kind in final mode", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("grokbot-speech-core-tool-kind-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("keeps skipping untagged short TTS text", async () => {
    const cfg = createTtsConfig("grokbot-speech-core-short-plain-tts-test");
    const result = await maybeApplyTtsToPayload({
      payload: {
        text: "hello",
        audioAsVoice: true,
  ...
  it("skips auto TTS for legacy final media directives", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("grokbot-speech-core-media-directive-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("keeps skipping explicit tagged TTS text that strips to empty markdown", async () => {
    const cfg = createTtsConfig("grokbot-speech-core-empty-hidden-tts-test");
    const result = await maybeApplyTtsToPayload({
      payload: {
  ...
  it("selects persona preferred provider before config fallback", () => {
    ...
    const prefsPath = "/tmp/grokbot-speech-core-persona-provider.json";
  ...
  it("treats provider configuration errors as unconfigured", () => {
    ...
    const cfg = {
      messages: {
        tts: {
          providers: { broken: {} },
          prefsPath: "/tmp/grokbot-speech-core-invalid-provider.json",
        },
      },
    } as OpenClawConfig;
    const config = resolveTtsConfig(cfg);
    ...
    const cfg: OpenClawConfig = {
      messages: {
        tts: {
          enabled: true,
          provider: "mock",
          prefsPath: "/tmp/grokbot-speech-core-persona-merge.json",
          providers: {
            mock: {
              model: "base-model",
              voice: "base-voice",
