...
The Linux companion is a Tauri v2 desktop shell for GrokBot Gateways. It discovers nearby Gateways over Bonjour, installs the CLI when needed, delegates local Gateway service management to `grokbot gateway`, opens the selected Gateway's Control UI, and stays available in the system tray.
...
The app uses `OPENCLAW_DESKTOP_CLI` when set. Otherwise it checks `~/.grokbot/bin/grokbot`, then `grokbot` on `PATH`.
...
The running app gives the headless `grokbot node run` host a single Canvas WebView. The bundled `linux-canvas` plugin advertises `canvas.*` only while the app socket exists. The app listens at `$XDG_RUNTIME_DIR/grokbot-canvas.sock` (or `/tmp/grokbot-canvas-$UID.sock`) with mode `0600`; a headless Linux node without the app does not advertise Canvas.
...
`tauri.conf.json` bundles the repository's canonical `scripts/install-cli.sh` directly as `install-cli.sh`. The app never keeps a forked copy. Stable, beta, and dev installs select `latest`, `beta`, and a managed Git `main` checkout respectively, always under `~/.grokbot`.
...
workflow uploads them as the `grokbot-linux-companion` artifact on pull
