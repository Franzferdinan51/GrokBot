...
GrokBot iOS is the officially released iPhone app. It connects to an GrokBot Gateway as a `role: node` for chat, voice, approvals, sharing, and device-aware automation.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical GrokBot team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical GrokBot team when creating bundle IDs or enabling services
...
- Local development uses the canonical `ai.openclawfoundation.app*` bundle IDs when the GrokBot team is available, and unique `ai.openclawfoundation.app.test.*` bundle IDs only for non-canonical fallback teams.
...
- App Store release builds use the canonical hosted relay at `https://ios-push-relay.grokbot.ai`.
...
8. Expected outputs after a successful run:
   - `apps/ios/build/app-store/GrokBot-<version>.ipa`
   - `apps/ios/build/app-store/GrokBot-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/GrokBot.entitlements` derives `aps-environment` from the active build configuration/signing override.
...
- Recommended gateway-host storage for the APNs `.p8` file is `~/.grokbot/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `OPENCLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `grokbot.json`. `OPENCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine GrokBot official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as GrokBot node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
