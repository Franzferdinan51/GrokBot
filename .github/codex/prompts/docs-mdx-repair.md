# GrokBot Docs MDX Repair Agent

You are repairing generated GrokBot documentation after a fast MDX validation failure.
...
1. Read `.grokbot-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".grokbot-sync/mdx/${LOCALE}.json"`.
