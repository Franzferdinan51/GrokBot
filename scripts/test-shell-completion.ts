 ...
 * `grokbot update`. Use it to verify the completion installation flow
...
\`grokbot update\`, \`grokbot doctor\`, and \`grokbot onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# GrokBot Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`grokbot completion --install\` later to enable.`));
    return;
  }
