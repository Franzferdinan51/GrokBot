...
export OPENAI_API_KEY="sk-grokbot-release-user-journey"
...
fs.writeFileSync(
  path.join(dir, "grokbot.plugin.json"),
...
grokbot plugins install "$plugin_a_dir" --force >"$PLUGIN_A_INSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
  ...
  "$plugin_a_dir"
grokbot journey-a ping >"$PLUGIN_A_CLI_LOG" 2>&1
...
grokbot plugins uninstall journey-plugin-a --force >"$PLUGIN_A_UNINSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
...
grokbot plugins install "$plugin_b_dir" --force >"$PLUGIN_B_INSTALL_LOG" 2>&1
grokbot journey-b ping >"$PLUGIN_B_CLI_LOG" 2>&1
...
grokbot plugins inspect journey-plugin-b --runtime --json >"$PLUGIN_B_AFTER_RESTART_JSON" 2>&1
grokbot channels status --json >"$STATUS_AFTER_RESTART_JSON" 2>"$STATUS_AFTER_RESTART_ERR"
...
grokbot doctor --repair --non-interactive >"$DOCTOR_LOG" 2>&1
