...
function withoutAiRuntimeDependency(value) {
  ...
  const next = value.filter((entry) => entry !== "@grokbot/ai");
...
function prepare(root) {
  ...
  const aiRuntimeSource = path.join(root, "node_modules", "@grokbot", "ai");
  const aiRuntimePackageJson = path.join(aiRuntimeSource, "package.json");
  if (!fs.existsSync(aiRuntimePackageJson)) {
    return;
  }

  const aiRuntimeTarget = path.join(root, ".grokbot-fixture", "packages", "ai");
  ...
  packageJson.dependencies["@grokbot/ai"] = "file:.grokbot-fixture/packages/ai";
