...
if [ -n "$OPENCLAW_TEST_STATE_SCRIPT_B64" ]; then
  ...
  export HOME="$(mktemp -d "${TMPDIR:-/tmp}/grokbot-skill-install-home.XXXXXX")"
...
if [ -n "${OPENCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
  ...
  openclaw_e2e_install_package /tmp/grokbot-skill-install-npm.log
fi

if [ -n "${OPENCLAW_CURRENT_PACKAGE_TGZ:-}" ] && command -v grokbot >/dev/null 2>&1; then
  OPENCLAW_CMD=(grokbot)
elif command -v pnpm >/dev/null 2>&1 && [ -f package.json ]; then
  if [ "${OPENCLAW_SKILL_INSTALL_E2E_BUILD_SOURCE:-0}" = "1" ]; then
    pnpm build >/tmp/grokbot-skill-install-build.log 2>&1
  fi
  OPENCLAW_CMD=(pnpm --silent grokbot)
elif command -v grokbot >/dev/null 2>&1; then
  OPENCLAW_CMD=(grokbot)
else
  echo "grokbot command not found; install package first or run from repo with pnpm" >&2
  exit 1
fi
...
search_json="/tmp/grokbot-skill-install-search.json"
resolve_json="/tmp/grokbot-skill-install-resolved.json"
install_log="/tmp/grokbot-skill-install.log"
info_json="/tmp/grokbot-skill-install-info.json"
...
if ! "${OPENCLAW_CMD[@]}" skills install "$slug" --force >"$install_log" 2>&1; then
  ...
  openclaw_e2e_dump_logs /tmp/grokbot-skill-install-npm.log "$search_json" "$resolve_json" "$install_log"
  exit 1
fi

workspace_dir="$HOME/.grokbot/workspace"
