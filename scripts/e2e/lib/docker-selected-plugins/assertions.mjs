...
for (const [pluginId, expected] of Object.entries(selected)) {
  ...
  assertFile(path.join(pluginRoot, "grokbot.plugin.json"));
  assertFile(path.join(pluginRoot, "package.json"));

  const manifest = readJson(path.join(pluginRoot, "grokbot.plugin.json"));
...
for (const pluginId of ["clickclack", "slack"]) {
  ...
  assert(
    packageJson.grokbot?.build?.bundledDist === false,
