...
if (mode === "assert-release-versions") {
  ...
  const rootAiVersion = rootManifest.dependencies?.["@grokbot/ai"];
  if (
    ...
    throw new Error(
      `candidate version mismatch: grokbot=${String(rootVersion)}, dependency=${String(rootAiVersion)}, @grokbot/ai=${String(aiVersion)}`,
    );
  }
