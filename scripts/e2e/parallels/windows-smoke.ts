...
// Windows Smoke script supports GrokBot repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://grokbot.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-grokbot-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'GrokBot\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-grokbot-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://grokbot.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run grokbot update --channel dev.
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("grokbot-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("grokbot-parallels-windows-tgz.");
    try {
  ...
  private logGuestPreflight(cleanOpenClaw: boolean): void {
    const cleanScript = cleanOpenClaw
      ? "npm.cmd uninstall -g grokbot --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
Invoke-GrokBot --version
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-GrokBot --version");
...
Invoke-GrokBot onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "grokbot onboard failed with exit code $LASTEXITCODE" }
...
Invoke-WithScopedEnv @{ OPENCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; OPENCLAW_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-GrokBot update --channel dev --yes --json
  $script:OpenClawUpdateExit = $LASTEXITCODE
}
if ($script:OpenClawUpdateExit -ne 0) { throw "grokbot update failed with exit code $script:OpenClawUpdateExit" }
Invoke-GrokBot --version
Invoke-GrokBot update status --json`,
...
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-GrokBot gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-GrokBot gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-GrokBot gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-GrokBot gateway status ${suffix}`);
  }
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $output = Invoke-GrokBot @args 2>&1
...
if (-not $agentOk) { throw 'grokbot agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastOpenClawVersion(this.runDir, phaseName, /GrokBot\s+([0-9][^\s]*)/gi);
  }
