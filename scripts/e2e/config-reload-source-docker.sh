...
IMAGE_NAME="$(docker_e2e_resolve_image "grokbot-config-reload-e2e" OPENCLAW_CONFIG_RELOAD_E2E_IMAGE)"
SKIP_BUILD="${OPENCLAW_CONFIG_RELOAD_E2E_SKIP_BUILD:-0}"
PORT="18789"
TOKEN="reload-e2e-token"
CONTAINER_NAME="grokbot-config-reload-e2e-$$"
...
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "source scripts/lib/grokbot-e2e-instance.sh; openclaw_e2e_probe_tcp 127.0.0.1 $PORT"; then
...
docker_e2e_docker_cmd exec "$CONTAINER_NAME" bash -lc "source /tmp/grokbot-test-state-env
