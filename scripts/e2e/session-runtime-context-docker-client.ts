...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".grokbot", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "grokbot.runtime-context",
...
async function verifyDoctorRepair(root: string) {
  ...
  const databasePath = path.join(stateDir, "agents", "main", "agent", "grokbot-agent.sqlite");
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-session-runtime-context-"));
  const stateDir = path.join(root, ".grokbot");
  setEnvValue("HOME", root);
  setEnvValue("OPENCLAW_STATE_DIR", stateDir);
  setEnvValue("OPENCLAW_CONFIG_PATH", path.join(stateDir, "grokbot.json"));
  try {
