...
IMAGE_NAME="$(docker_e2e_resolve_image "grokbot-docker-e2e-functional:local")"
...
PROJECT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/grokbot-compose-proof.XXXXXX")"
PROJECT_NAME="grokbot-compose-proof-$$"
...
cat >"$PROJECT_DIR/config/grokbot.json" <<EOF
{
  "gateway": {
    "mode": "local",
...
"${COMPOSE[@]}" up -d --no-build grokbot-gateway
GATEWAY_ID="$("${COMPOSE[@]}" ps -q grokbot-gateway)"
if [ -z "$GATEWAY_ID" ]; then
  echo "Compose did not create grokbot-gateway" >&2
  exit 1
fi
...
for _ in $(seq 1 180); do
  ...
  if [ "$health" = "unhealthy" ] || [ "$health" = "exited" ] || [ "$health" = "dead" ]; then
    "${COMPOSE[@]}" logs --no-color grokbot-gateway >&2
    exit 1
  fi
  sleep 1
done
...
"${COMPOSE[@]}" exec -T grokbot-gateway node dist/index.js health --token "$TOKEN"
"${COMPOSE[@]}" run -T --no-deps --name "$CLI_NAME" grokbot-cli health --token "$TOKEN"
GATEWAY_VERSION="$("${COMPOSE[@]}" exec -T grokbot-gateway node -p "require('./package.json').version")"
