...
BASE_IMAGE="$(docker_e2e_resolve_image "grokbot-browser-cdp-base-e2e" OPENCLAW_BROWSER_CDP_BASE_E2E_IMAGE)"
if [ -n "${OPENCLAW_BROWSER_CDP_SNAPSHOT_E2E_IMAGE:-}" ]; then
  ...
  IMAGE_NAME="grokbot-browser-cdp-snapshot-e2e:${OPENCLAW_DOCKER_ALL_LANE_NAME:-shared}"
  DERIVED_SHARED_IMAGE="1"
else
  IMAGE_NAME="grokbot-browser-cdp-snapshot-e2e"
  DERIVED_SHARED_IMAGE="0"
fi
...
CONTAINER_NAME="grokbot-browser-cdp-e2e-$$"
...
if [ "$SKIP_BUILD" = "1" ] || { [ "$DERIVED_SHARED_IMAGE" = "0" ] && [ "${OPENCLAW_SKIP_DOCKER_BUILD:-0}" = "1" ]; }; then
  ...
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/grokbot-browser-cdp-build.XXXXXX")"
...
mkdir -p /tmp/grokbot-browser-cdp
find dist -maxdepth 1 -type f -name 'pw-ai-*.js' ! -name 'pw-ai-state-*' -exec mv {} /tmp/grokbot-browser-cdp/ \;
...
set -euo pipefail
source /tmp/grokbot-test-state-env
source scripts/lib/grokbot-e2e-instance.sh
entry=\"\$(openclaw_e2e_resolve_entrypoint)\"
