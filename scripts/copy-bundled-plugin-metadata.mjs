...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "grokbot.plugin.json");
    ...
    if (packageJson.grokbot && "extensions" in packageJson.grokbot) {
      packageJson.grokbot = {
        ...packageJson.grokbot,
        extensions: rewritePackageExtensions(packageJson.grokbot.extensions),
        ...(typeof packageJson.grokbot.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.grokbot.setupEntry) }
          : {}),
      };
    }
