...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.grokbot) ? packageJson.grokbot : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.grokbot?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.grokbot?.channel?.id) || left.name;
    const rightId = trimString(right.grokbot?.channel?.id) || right.name;
