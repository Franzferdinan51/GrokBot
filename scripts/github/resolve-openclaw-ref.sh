...
REMOTE_URL="${OPENCLAW_REF_REMOTE:-https://github.com/grokbot/grokbot.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-grokbot-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves GrokBot branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve GrokBot ref: ${REF}" >&2
exit 1
