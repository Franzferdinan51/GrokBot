#!/usr/bin/env bash
# ClawDock - Docker helpers for GrokBot
# Inspired by Simon Willison's "Running GrokBot in Docker"
# https://til.simonwillison.net/llms/grokbot-docker
#
# Installation:
...
# Common paths to check for GrokBot
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/grokbot"
  "${HOME}/workspace/grokbot"
  "${HOME}/projects/grokbot"
  "${HOME}/dev/grokbot"
  "${HOME}/code/grokbot"
  "${HOME}/src/grokbot"
)
...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port grokbot-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found GrokBot at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/grokbot"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ GrokBot not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/grokbot/grokbot.git ~/grokbot"
    echo "  cd ~/grokbot && ./scripts/docker/setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d grokbot-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart grokbot-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f grokbot-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.grokbot"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show grokbot.json
  if [[ -f "${config_dir}/grokbot.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/grokbot.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/grokbot.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No grokbot.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec grokbot-gateway \
    bash -c 'echo "alias grokbot=\"./grokbot.mjs\"" > /tmp/.bashrc_openclaw && bash --rcfile /tmp/.bashrc_openclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm grokbot-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build grokbot-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d grokbot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build grokbot-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" grokbot-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" grokbot-gateway \
    bash -c './grokbot.mjs config set gateway.remote.token "$TOKEN" && ./grokbot.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec grokbot-gateway \
    bash -c "./grokbot.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart grokbot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps grokbot-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec grokbot-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'grokbot config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for GrokBot${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (grokbot alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to grokbot project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.grokbot)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'grokbot channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'grokbot status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.grokbot.ai${_CLR_RESET}"
  echo ""
}
