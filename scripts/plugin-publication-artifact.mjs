...
const MANIFEST_SCHEMA = "grokbot.plugin-publication-artifact/v1";
...
const META_PACKAGE = "@grokbot/meta-provider";
...
export function inspectPackageTarballBytes(inputBytes, options = {}) {
  ...
  while (offset + TAR_BLOCK_BYTES <= tarBytes.length) {
    ...
    } else if (safePath === "package/grokbot.plugin.json") {
      if (content.length === 0 || content.length > MAX_PLUGIN_MANIFEST_BYTES) {
        throw new Error(
          `Packed grokbot.plugin.json size is outside the allowed range: ${content.length}.`,
        );
      }
  ...
  if (!pluginManifestBytes) {
    throw new Error("Plugin tarball must contain exactly one package/grokbot.plugin.json.");
  }
  ...
  const pluginManifest = parsePackedJson(pluginManifestBytes, "Packed grokbot.plugin.json");
  return {
    inventory,
    packageManifest,
...
export function validatePluginPackageManifest(params, packageManifest) {
  ...
  const release = packageManifest.grokbot?.release;
