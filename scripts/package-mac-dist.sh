...
# Output:
# - dist/GrokBot.app
# - dist/GrokBot-<version>.zip
# - dist/GrokBot-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.grokbot.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/grokbot-sparkle-build.XXXXXX")" || {
...
ZIP="$ROOT_DIR/dist/GrokBot-$VERSION.zip"
DMG="$ROOT_DIR/dist/GrokBot-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/GrokBot-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/GrokBot-$VERSION.dSYM.zip"
