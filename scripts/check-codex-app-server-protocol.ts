...
// Check Codex App Server Protocol script supports GrokBot repository automation.
...
async function main(): Promise<void> {
  ...
  console.log(
    `Codex app-server generated protocol matches GrokBot bridge assumptions: ${source.codexRepo}`,
  );
}

async function checkMaintainedProtocolTypes(sourceRoot: string): Promise<void> {
  // Raw requests go to Codex; raw responses flow into GrokBot. Keep the
  // assignability direction explicit so the probe permits deliberate projections.
  const probePath = path.join(sourceRoot, "grokbot-protocol-compatibility.ts");
