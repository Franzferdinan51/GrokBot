...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.grokbot`, `$OPENCLAW_STATE_DIR`, or active GrokBot state dirs.
...
- Worker must send completion/failure via `grokbot message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/grokbot`; use an isolated checkout.
...
6. Launch the worker in the isolated worktree, never the primary checkout. For GrokBot, the primary checkout under `~/Projects/grokbot` remains forbidden.
...
grokbot message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use grokbot system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t grokbot-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
