...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${OPENCLAW_CONFIG_PATH:-${OPENCLAW_STATE_DIR:-$HOME/.grokbot}/grokbot.json}"
...
```text
${OPENCLAW_STATE_DIR:-$HOME/.grokbot}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with grokbot message send if route provided.
```
