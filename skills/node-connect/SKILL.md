---
name: node-connect
description: "Diagnose GrokBot Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify GrokBot is advertising that route, then fix pairing/auth.
...
- whether `grokbot devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `grokbot qr --json`. It uses the same setup-code payload Android scans.

```bash
grokbot config get gateway.mode
grokbot config get gateway.bind
grokbot config get gateway.tailscale.mode
grokbot config get gateway.remote.url
grokbot config get gateway.auth.mode
grokbot config get gateway.auth.allowTailscale
grokbot config get plugins.entries.device-pair.config.publicUrl
grokbot qr --json
grokbot devices list
grokbot nodes status
```

If this GrokBot instance is pointed at a remote gateway, also run:

```bash
grokbot qr --remote --json
```
...
If `grokbot qr --json` says `Gateway is only bound to loopback`:
...
- `grokbot devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run grokbot qr again, rescan, then approve the pending device pairing.`
