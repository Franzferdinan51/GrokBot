...
GrokBot is the AI that actually does things.
...
GrokBot started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> GrokBot.
...
GrokBot runtime code reads the current configuration schema only.
...
`grokbot doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in GrokBot is a deliberate tradeoff: strong defaults without killing capability.
...
GrokBot has an extensive plugin API.
...
- Code plugins run GrokBot plugin code and are appropriate for deeper runtime extension.
...
GrokBot docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
GrokBot supports MCP as both a server and a runtime integration surface.
...
GrokBot is currently terminal-first by design.
...
GrokBot is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep GrokBot hackable by default.
