...
describe("security audit config include permissions", () => {
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = tempDirs.make("grokbot-include-perms-");
    ...
    const configSnapshot: ConfigFileSnapshot = {
      path: path.join(stateDir, "grokbot.json"),
      exists: true,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = tempDirs.make("grokbot-include-perms-allowed-");
      ...
      const configSnapshot: ConfigFileSnapshot = {
        path: path.join(configDir, "grokbot.json"),
        exists: true,
