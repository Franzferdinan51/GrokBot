    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "grokbot.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "grokbot.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "grokbot.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("grokbot-");
  const acpxRoot = await tempDirs.make("grokbot-acpx-plugin-");
  const helperRoot = await tempDirs.make("grokbot-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("grokbot-");
  const pluginRoot = await tempDirs.make("grokbot-plugin-");
  const outsideDir = await tempDirs.make("grokbot-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("grokbot-");
    const pluginRoot = await tempDirs.make("grokbot-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("grokbot-");
    const pluginRoot = await tempDirs.make("grokbot-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
