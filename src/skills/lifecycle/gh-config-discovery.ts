...
// GrokBot process is running with a different HOME (e.g. the per-agent
...
// See https://github.com/grokbot/grokbot/issues/78063.
...
type GhConfigDirMismatch = {
  ...
  // Suggested env value the operator should set on the GrokBot service to
...
export function formatGhConfigDirMismatchHint(mismatch: GhConfigDirMismatch): string[] {
  ...
  lines.push(
    `  Fix: set GH_CONFIG_DIR=${mismatch.suggestedEnvValue} on the GrokBot service environment, then restart the gateway.`,
  );
  return lines;
}
