...
async function installArchiveResolution(params: {
  ...
  archivePath: string;
  registry: string;
  authority: "official" | "grokbot" | "third-party";
  force?: boolean;
  logger?: Logger;
  config?: OpenClawConfig;
}) {
  return await withExtractedArchiveRoot({
    ...
    tempDirPrefix: "grokbot-skill-clawhub-",
    timeoutMs: 120_000,
...
async function installGitHubResolution(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath: params.archivePath,
    tempDirPrefix: "grokbot-skill-clawhub-github-",
    timeoutMs: 120_000,
...
function assertInstallResolutionAllowed(
  ...
  if (!resolution.ok) {
    if (resolution.reason === "ambiguous_slug") {
      ...
      throw new Error(
        `Skill "${resolution.slug}" is ambiguous on ClawHub. Install an owner-qualified skill, for example: grokbot skills install @owner/${resolution.slug}.${message}`,
      );
    }
