...
const logPathTracker = createSuiteLogPathTracker("grokbot-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-grokbot.log")).toBe(
        path.join(home, "custom-grokbot.log"),
      );
    });
  });
