...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "grokbot/plugin-sdk/core";
import {
  readFileWithinRoot,
  removePathWithinRoot,
  writeFileWithinRoot,
} from "grokbot/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "grokbot/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "grokbot-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps root-bounded file-access helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "grokbot-sdk-file-access-compat-" }, async (root) => {
