...
import { runSetupWizardFinalize } from "grokbot/plugin-sdk/plugin-test-runtime";
...
describe("createOptionalChannelSetupSurface", () => {
  it("returns a matched adapter and wizard for optional plugins", async () => {
    ...
    expect(
      ...
      "Example setup requires @grokbot/example to be installed. Docs: https://docs.grokbot.ai/channels/example",
    );
    ...
    ).rejects.toThrow("@grokbot/example");
  });
});
