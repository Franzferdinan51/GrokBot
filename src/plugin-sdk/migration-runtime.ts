...
import { writeTextAtomic } from "@grokbot/fs-safe/atomic";
...
export async function copyMemoryMigrationFileItem(
  ...
  try {
    ...
    if (replaceExisting) {
      ...
      stagingDir = path.join(".grokbot-memory-import-staging", crypto.randomUUID());
    ...
    if (stagingDir) {
      ...
      await safeRoot.remove(".grokbot-memory-import-staging").catch(() => undefined);
    }
