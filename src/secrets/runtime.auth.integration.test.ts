...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("grokbot-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".grokbot", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".grokbot", "agents", "ops", "agent");
