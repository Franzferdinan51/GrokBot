...
describe("node host invoke", () => {
  ...
  it("lists node-host directories for the folder browser", async () => {
    const root = fs.realpathSync(tempDirs.make("grokbot-node-fs-listdir-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = fs.realpathSync(tempDirs.make("grokbot-node-skill-cwd-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-prepare-env-"));
  ...
  it("forwards suppressNotifyOnExit on completed system.run events", async () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-node-event-suppress-"));
    try {
