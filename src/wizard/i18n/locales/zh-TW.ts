...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 GrokBot 設定",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `grokbot doctor`。",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 GrokBot 工作區？",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密碼存入 GrokBot 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 GrokBot 設定",
    ...
    guided: {
      ...
      custodianIntro: "你好 — 我是 GrokBot，負責維護這套系統。我們開始設定吧。",
      failedOptionLine: "{label}：{reason}",
      failedOptionsIntro: "剛才這些沒有成功：",
      findMeLater: "之後隨時找得到我 — 在終端機執行 `grokbot`，或在儀表板中開啟設定。",
      ...
      appliedTitle: "推理已就緒",
      complete: "GrokBot 已準備就緒。",
      completeWithoutAi: "GrokBot 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      escapeHatches: "如需完整的分步精靈，請執行 `grokbot onboard --classic`。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "GrokBot 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
        "未進行任何變更。請執行 {fixCommand} 修復，再執行 {inspectCommand} 檢查，然後重試 onboarding。",
      invalidConfigUnknown: "- 無法解析設定。",
      manualChoice: "你想如何連接 AI？",
      nextSteps:
        "工作區：{workspace}\n新增頻道：`grokbot channels add`\n偏好聊天？執行 `grokbot setup`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`grokbot dashboard`\n稍後聊天：`grokbot`",
      nextStepsWithoutAi:
        "工作區：{workspace}\n稍後新增 AI：重新執行 `grokbot onboard`\n連接 AI 後新增頻道：`grokbot channels add`\n開啟 dashboard：`grokbot dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要互動式 TTY。自動化請使用 `grokbot onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "設定選項",
    },
    setup: {
    ...
    security: {
      ...
      beta: "GrokBot 是一個興趣專案，仍處於 beta 階段。請預期會有邊緣問題。",
      confirm: "我理解 GrokBot 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉安全加固和存取控制，請不要執行 GrokBot。",
      learnMore: "了解更多",
      notMultitenant: "GrokBot 預設不是敵對多租戶安全邊界。",
      personalAgent: "預設情況下，GrokBot 是個人 agent：一個可信操作者邊界。",
    ...
    skills: {
      ...
      docsLine: "文件：https://docs.grokbot.ai/skills",
      enterEnv: "輸入 {env}",
      ...
      manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `grokbot doctor` 檢查技能和需求。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作區設定 → Integrations → GrokBot → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "GrokBot" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 GrokBot 將你加入允許清單。",
      ...
      separatePhoneLabel: "GrokBot 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 GrokBot 用來向 Chat 傳送回覆的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "GrokBot"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 GrokBot，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 grokbot configure --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.grokbot.ai/web/control-ui",
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 GrokBot。",
      dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 GrokBot。",
      ...
      daemonRuntimeNodeHint: "GrokBot 狀態使用 node:sqlite，因此必須使用 Node；Bun 無法執行閘道。",
      ...
      gatewayTokenStored:
        "儲存位置：$OPENCLAW_CONFIG_PATH（預設 ~/.grokbot/grokbot.json）中的 gateway.auth.token，或 OPENCLAW_GATEWAY_TOKEN。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 GrokBot。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 GrokBot。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。GrokBot 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.grokbot.ai/security",
      ...
      webDocs: "文件：https://docs.grokbot.ai/tools/web",
      ...
      workspaceDocs: "文件：https://docs.grokbot.ai/concepts/agent-workspace",
