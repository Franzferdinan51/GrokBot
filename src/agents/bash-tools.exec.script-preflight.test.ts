...
import { __setFsSafeTestHooksForTest } from "@grokbot/fs-safe/test-hooks";
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("grokbot-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive GrokBot channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", async () => {
    await expect(
      detectUnsafeExecControlShellCommand("grokbot channels login --channel whatsapp"),
    ).resolves.toBe("channel-login");
    expect(
      await detectUnsafeExecControlShellCommand(
        "pnpm exec grokbot channels login --channel whatsapp --verbose",
      ),
    ).toBe("channel-login");
    await expect(
      detectUnsafeExecControlShellCommand("grokbot channels status --deep"),
    ).resolves.toBeNull();
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-grokbot-channel-login", {
        command: "grokbot channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive GrokBot channel login commands/);
    await expect(
      tool.execute("call-wrapped-grokbot-channel-login", {
        command: "sudo -u grokbot bash -lc 'grokbot channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive GrokBot channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'grokbot channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive GrokBot channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env grokbot channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive GrokBot channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'grokbot channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive GrokBot channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("grokbot-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("grokbot-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("grokbot-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("grokbot-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
