...
describe("tool mutation helpers", () => {
  ...
  it.each([
    ...
    ["exec", "gh search prs --repo grokbot/grokbot tool-mutation --json number,title,state"],
    ["bash", "gh pr view 123 --repo grokbot/grokbot --json title,state"],
  ...
  it.each([
    ...
    ["exec", "gh api --method POST repos/grokbot/grokbot/issues"],
