...
describe("agent cleanup timeout", () => {
  ...
  it("uses the trajectory flush timeout environment override for trajectory cleanup", async () => {
    ...
    expect(cleanup).toHaveBeenCalledTimes(1);
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=grokbot-trajectory-flush timeoutMs=25000",
    );
  });
  ...
  it("includes cleanup timeout details when the cleanup step exposes them", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=grokbot-trajectory-flush timeoutMs=5 details=pendingWrites=2 queuedBytes=128 activeOperation=file-append",
    );
  });
  ...
  it("does not fail cleanup when timeout details throw", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=grokbot-trajectory-flush timeoutMs=5 detailsError=details unavailable",
    );
  });
  ...
  it("prefers explicit cleanup timeout values over environment overrides", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-explicit sessionId=session-explicit step=grokbot-trajectory-flush timeoutMs=2000",
    );
  });
  ...
  it("keeps explicit zero cleanup timeouts as a one millisecond timeout", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-zero sessionId=session-zero step=grokbot-trajectory-flush timeoutMs=1",
    );
  });
  ...
  it.each([
    ...
    expect(log.warn).toHaveBeenCalledWith(
      `agent cleanup timed out: runId=${runId} sessionId=${sessionId} step=grokbot-trajectory-flush timeoutMs=10000`,
    );
  });
