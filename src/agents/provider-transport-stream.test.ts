...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to GrokBot transport aliases when needed.
    const cases = [
      {
        ...
        alias: "grokbot-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "grokbot-openai-completions-transport",
      },
      {
        ...
        alias: "grokbot-azure-openai-responses-transport",
      },
      {
        ...
        alias: "grokbot-anthropic-messages-transport",
      },
      {
        ...
        alias: "grokbot-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on GrokBot transport", () => {
    const cases = [
  ...
  it("routes localService models through the GrokBot simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("grokbot-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the GrokBot transport until GrokBot preserves attribution", () => {
