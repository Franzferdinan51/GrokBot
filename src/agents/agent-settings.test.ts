...
describe("isSilentOverflowProneModel", () => {
  ...
  // grokbot#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable GrokBot runtime's secondary compaction path;
  // GrokBot's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of grokbot#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering GrokBot runtime's
  ...
  // state.messages intact; GrokBot's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep GrokBot runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove GrokBot runtime's
