...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    bundleProbeHomeDir = await tempHarness.createTempDir("grokbot-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("grokbot-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("grokbot-cli-bundle-mcp-bundled-");
