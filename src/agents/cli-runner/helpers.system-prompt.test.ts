...
describe("buildCliAgentSystemPrompt", () => {
  ...
  it("uses CLI backend tool fallback instead of GrokBot tool assumptions", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/grokbot",
      tools: [],
      modelDisplay: "test/model",
    ...
    expect(prompt).not.toContain("GrokBot lists the standard tools above");
    expect(prompt).not.toContain("This runtime enables:");
    expect(prompt).not.toContain("For long waits, avoid rapid poll loops");
    expect(prompt).not.toContain("Larger work: use `sessions_spawn`");
    expect(prompt).not.toContain("Do not poll `subagents list` / `sessions_list` in a loop");
    expect(prompt).toContain("No GrokBot tool list is injected");
  });
  ...
  it("uses cwd, not bootstrap workspace, for CLI workspace guidance", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/grokbot-agent",
      cwd: "/tmp/task-repo",
      tools: [],
      modelDisplay: "test/model",
    });
    ...
    expect(prompt).not.toContain("Working directory: /tmp/grokbot-agent");
  });
  ...
  it("includes CLI-scoped plugin command guidance", () => {
    ...
    // GrokBot-main command text into external CLI backends.
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "GrokBot-only command guidance.",
    ...
    expect(prompt).not.toContain("GrokBot-only command guidance.");
  });
