...
import { SYSTEM_PROMPT_CACHE_BOUNDARY } from "@grokbot/ai/internal/shared";
import { MAX_IMAGE_BYTES } from "@grokbot/media-core/constants";
import { expectDefined } from "@grokbot/normalization-core";
import type { ImageContent } from "grokbot/plugin-sdk/llm";
...
describe("prepareCliPromptImagePayload prompt references", () => {
  ...
  it("does not reload GrokBot CLI image cache paths from prior prompt text", async () => {
    const loadImageFromRefSpy = vi.spyOn(promptImageUtils, "loadImageFromRef");
    const sanitizeImageBlocksSpy = vi.spyOn(toolImages, "sanitizeImageBlocks");
    ...
    await expect(
      prepareCliPromptImagePayload({
        backend: { command: "gemini", imagePathScope: "workspace" },
        prompt:
          'Called the Read tool with {"file_path":"/workspace/.grokbot-cli-images/stale.png"}',
        workspaceDir: "/workspace",
      }),
    ).resolves.toStrictEqual({
      prompt: 'Called the Read tool with {"file_path":"/workspace/.grokbot-cli-images/stale.png"}',
    });
  ...
  it("passes the max-byte guardrail through load and sanitize", async () => {
    ...
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-ref-image-"),
    );
  ...
  it("dedupes repeated refs and skips failed loads before sanitizing", async () => {
    ...
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-ref-dedupe-"),
    );
    try {
      const result = await prepareCliPromptImagePayload({
        backend: { command: "gemini", imagePathScope: "workspace" },
...
describe("buildCliArgs", () => {
  ...
  it("passes Codex system prompts via a model instructions file config override", () => {
    expect(
      buildCliArgs({
        ...
        systemPromptFilePath: "/tmp/grokbot/system-prompt.md",
        useResume: false,
      }),
    ).toEqual(["exec", "--json", "-c", 'model_instructions_file="/tmp/grokbot/system-prompt.md"']);
  });
  ...
  it("passes Claude system prompts through its file flag", () => {
    ...
    ).toEqual(["-p", "--append-system-prompt-file", "/tmp/grokbot/system-prompt.md"]);
  });
...
describe("writeCliImages", () => {
  it("uses stable hashed file paths so repeated image hydration reuses the same path", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-write-images-"),
    );
    ...
    try {
      expect(first.imagePaths).toStrictEqual([
        expect.stringMatching(
          new RegExp(
            `^${escapeRegExp(`${resolvePreferredOpenClawTmpDir()}/grokbot-cli-images/`)}.*\\.png$`,
          ),
        ),
      ]);
  ...
  it("uses the shared media extension map for image formats beyond the tiny builtin list", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-write-heic-"),
    );
    const image: ImageContent = {
      type: "image",
      data: "aGVpYy1pbWFnZQ==",
  ...
  it("sweeps stale workspace-scoped CLI image files", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-write-sweep-"),
    );
    const imageRoot = path.join(workspaceDir, ".grokbot-cli-images");
  ...
  it("hydrates prompt media refs into codex image args through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-prompt-image-"),
    );
    ...
    try {
      ...
      expect(argv).toStrictEqual([
        ...
        "--image",
        expect.stringContaining("grokbot-cli-images"),
      ]);
  ...
  it("appends hydrated prompt media refs for stdin backends through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-prompt-image-generic-"),
    );
    ...
    try {
      ...
      expect(promptWithImages).toContain("grokbot-cli-images");
  ...
  it("appends Gemini prompt refs with @-prefixed image paths", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-prompt-image-gemini-"),
    );
    ...
    try {
      ...
      expect(prepared.imagePaths?.[0]?.startsWith(path.join(tempDir, ".grokbot-cli-images"))).toBe(
        true,
      );
  ...
  it("prefers explicit images over prompt refs through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "grokbot-cli-explicit-images-"),
    );
    ...
    try {
      ...
      expect(argv[argv.indexOf("--image") + 1]).toContain("grokbot-cli-images");
  ...
  it("merges inline payloads with offloaded refs in attachment order", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-cli-mixed-images-"));
...
describe("writeCliSystemPromptFile", () => {
  it("writes stripped system prompts to a private temp file", async () => {
    ...
    try {
      expect(written.filePath).toContain("grokbot-cli-system-prompt-");
