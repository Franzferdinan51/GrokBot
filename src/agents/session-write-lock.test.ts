...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("reports live GrokBot-owned stale locks without removing them", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      const owner = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)", "grokbot"], {
        stdio: "ignore",
      });
      if (!owner.pid) {
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-lock-policy-"));
    ...
    try {
      ...
      const envOverride = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/grokbot/grokbot.mjs", "doctor"],
      });
  ...
  it("does not clean live GrokBot locks just because holder max hold expired", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-lock-policy-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/grokbot/grokbot.mjs", "agent"],
      });
  ...
  it("cleans old live .jsonl lock files owned by non-GrokBot processes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    const lockPath = path.join(sessionsDir, "old-non-grokbot.jsonl.lock");
    ...
    try {
      ...
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
          name: "old-non-grokbot.jsonl.lock",
          removed: true,
          stale: true,
          staleReasons: ["too-old", "non-grokbot-owner"],
        },
      ]);
      await expectPathMissing(lockPath);
    } finally {
  ...
  it("cleans fresh live .jsonl lock files owned by a non-GrokBot process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["non-grokbot-owner"],
        },
      ]);
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-GrokBot entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps fresh live .jsonl lock files with GrokBot or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const openclawLock = path.join(sessionsDir, "grokbot-live.jsonl.lock");
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
