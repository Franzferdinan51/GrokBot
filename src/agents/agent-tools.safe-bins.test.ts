...
beforeAll(async () => {
  await withEnvAsync(
    {
      OPENCLAW_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "grokbot-test-no-bundled-extensions"),
    },
    async () => {
...
vi.mock("./grokbot-tools.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./grokbot-tools.js")>();
  return {
    createOpenClawTools: () => [],
...
vi.mock("grokbot/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
describe("createOpenClawCodingTools safeBins", () => {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "grokbot-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "grokbot-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "grokbot-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "grokbot-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "grokbot-safe-bins-grep-",
        safeBins: ["grep"],
