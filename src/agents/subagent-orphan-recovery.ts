 ...
 *
 * @see https://github.com/grokbot/grokbot/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /grokbot\.json|grokbot gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify grokbot.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
