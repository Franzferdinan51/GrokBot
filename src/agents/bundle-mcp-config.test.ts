/** Tests merging bundled MCP defaults with GrokBot user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets GrokBot mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps GrokBot transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled GrokBot MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled GrokBot MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
