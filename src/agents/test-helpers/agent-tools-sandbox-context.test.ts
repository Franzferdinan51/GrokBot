...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("grokbot-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("grokbot-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("grokbot-sbx-");
