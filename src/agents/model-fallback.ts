...
async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  if (isCliAgentRuntime(agentRuntime, params.cfg)) {
    // CLI runtimes own their transport/auth, so stale GrokBot provider
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("grokbot.modelFallbackTestApi")] = {
