...
describe("sandbox ssh helpers", () => {
  it("materializes inline ssh auth data into a temp config", async () => {
    ...
    expect(config).toContain("Host grokbot-sandbox");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-ssh-real-"));
      ...
      const target = path.join(root, "workspace", ".grokbot", "sandbox-skills");
      ...
      await expect(
        fs.stat(path.join(realParent, "runtime", "workspace", ".grokbot", "sandbox-skills")),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = makeTempDir(tempDirs, "grokbot-ssh-real-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "grokbot-ssh-workdir-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "grokbot-ssh-root-");
  ...
  it("builds remote workdir validation commands with quoted literal paths", () => {
    ...
    expect(command).toContain("grokbot-validate-workdir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.symlink(realParent, path.join(root, "workspace", ".grokbot"));
      ...
      await expect(
        execFileAsync("/bin/sh", [
          ...
          ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
          "grokbot-remote-dir",
          path.join(root, "workspace", ".grokbot", "sandbox-skills"),
          root,
        ]),
      ).rejects.toThrow(/unsafe remote directory symlink/);
    },
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-ssh-upload-"));
      ...
      await expect(
        uploadDirectoryToSshTarget({
          session: {
            command: "ssh",
            configPath: "/tmp/grokbot-test-ssh-config",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-ssh-upload-safe-"));
      tempDirs.push(localDir);
