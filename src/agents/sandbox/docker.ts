...
import { markOpenClawExecEnv } from "../../infra/grokbot-exec-env.js";
import {
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; GrokBot will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--init");
  args.push("--label", "grokbot.sandbox=1");
  args.push("--label", `grokbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `grokbot.createdAtMs=${createdAtMs}`);
  args.push("--label", `grokbot.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  args.push("--label", `grokbot.createArgsEpoch=${SANDBOX_DOCKER_CREATE_ARGS_EPOCH}`);
  if (params.configHash) {
    args.push("--label", `grokbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "grokbot.configHash");
}
