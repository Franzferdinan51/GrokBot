...
vi.mock("@grokbot/acp-core/runtime/session-identifiers", () => ({
...
function setupSessionTouchStore(): void {
  ...
  state.storePathMock = "/tmp/grokbot-sessions.json";
}
...
describe("agentCommand – LiveSessionModelSwitchError retry", () => {
  beforeEach(() => {
    ...
    state.prepareInternalSessionEffectsSessionMock.mockResolvedValue({
      ...
      storePath: "/tmp/grokbot-session-store.json",
      sessionFile: "sqlite:default:internal-session:/tmp/grokbot-session-store.json",
  ...
  it("uses a concurrent user override adopted during legacy fallback repair", async () => {
    ...
    state.sessionEntryMock = sessionEntry;
    state.sessionStoreMock = { "agent:main:main": sessionEntry };
    state.storePathMock = "/tmp/grokbot-session-store.json";
    state.persistSessionEntryMock.mockImplementation(async (...args: unknown[]) => {
  ...
  it("does not treat backend CLI session id as GrokBot session identity", async () => {
    setupSingleAttemptFallback();
    setupSessionTouchStore();
    const result = makeSuccessResult("openai", "gpt-5.4") as ReturnType<
      typeof makeSuccessResult
  ...
  it("forwards an explicit GrokBot runtime override into fallback and attempt execution", async () => {
    setupSingleAttemptFallback();
    state.runtimeConfigMock = {
      agents: {
        defaults: {
  ...
  it("passes SQLite transcript markers to visible agent attempts", async () => {
    ...
    const visibleEntry: SessionEntry = {
      sessionId: "session-1",
      updatedAt: 1,
      sessionFile: "sqlite:default:session-1:/tmp/grokbot-session-store.json",
    };
    ...
    expect(attemptCalls[0]?.sessionFile).toBe(
      "sqlite:default:session-1:/tmp/grokbot-session-store.json",
    );
  });
  ...
  it("keeps internal session-effect CLI runs out of visible session state", async () => {
    ...
    expect(attemptCalls).toHaveLength(1);
    expect(attemptCalls[0]?.sessionFile).toBe(
      "sqlite:default:internal-session:/tmp/grokbot-session-store.json",
    );
    ...
    state.runAgentAttemptMock.mockResolvedValue({
      ...
      meta: {
        ...
        agentMeta: {
          ...
          sessionFile: "sqlite:default:rotated-model-run-session:/tmp/grokbot-session-store.json",
        },
      },
    });
