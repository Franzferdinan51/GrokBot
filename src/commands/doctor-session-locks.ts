...
export function sessionLockToHealthFinding(lock: SessionLockInspection): HealthFinding {
  const fixHint = lock.removable
    ? 'Run "grokbot doctor --fix" to remove this stale lock file automatically.'
    : isReportOnlyStaleLock(lock)
      ? "GrokBot is preserving this live owned lock; inspect the owning process if it appears stuck."
      : 'Run "grokbot doctor --fix" after the cleanup grace period if this stale lock remains.';
  return {
...
export async function noteSessionLockHealth(params?: {
  ...
  if (staleCount > 0 && !shouldRepair) {
    ...
    lines.push('- Run "grokbot doctor --fix" to remove stale lock files automatically.');
  }
