...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".grokbot-work");
    const defaultStateDir = path.join(homeDir, ".grokbot");
    const profileConfigPath = path.join(profileStateDir, "grokbot.json");
...
describe("moveToTrash", () => {
  it("uses fs-safe trash instead of resolving a PATH trash command", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-trash-helper-"));
  ...
  it("allows fs-safe trash to move a symlink whose target resolves outside the parent", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-trash-symlink-"));
    const targetPath = path.join(testRoot, "target-link");
    const outsideTarget = path.join(os.tmpdir(), "grokbot-trash-symlink-target");
  ...
  it("moves a dangling symlink instead of treating it as already removed", async () => {
    const testRoot = tempDirs.make("grokbot-trash-dangling-link-");
  ...
  it("canonicalizes a symlinked parent before calling fs-safe trash", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-trash-parent-link-"));
    const lexicalParent = path.join(testRoot, "state-link");
    const realParent = path.join(testRoot, "state-real");
    const targetPath = path.join(lexicalParent, "grokbot.json");
    const sourcePath = path.join(realParent, "grokbot.json");
