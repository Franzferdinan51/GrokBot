// Builds overview table rows for `grokbot status` and `grokbot status --all`.
...
/** Builds the default `grokbot status` overview rows from scan, health, memory, and session inputs. */
  ...
  return buildStatusOverviewRowsFromSurface({
    ...
    middleRows: [
      ...(params.updateRestartValue
        ? [{ Item: "Update restart", Value: params.updateRestartValue }]
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("grokbot security audit --deep")}` },
    ],
