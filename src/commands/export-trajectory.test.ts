...
describe("exportTrajectoryCommand", () => {
  beforeEach(() => {
    ...
    mocks.resolveStorePath.mockReturnValue("/tmp/grokbot/sessions.json");
    mocks.loadSessionEntryReadOnly.mockReturnValue(undefined);
    mocks.exportTrajectoryForCommand.mockResolvedValue({
      outputDir: "/tmp/workspace/.grokbot/trajectory-exports/export",
      displayPath: ".grokbot/trajectory-exports/export",
      sessionId: "session-1",
      eventCount: 2,
      runtimeEventCount: 0,
  ...
  it("points missing session key users at the sessions command", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "--session-key is required. Run grokbot sessions to choose a session.",
    );
  ...
  it("preserves direct options when an encoded request omits them", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "Session not found: agent:main:telegram:direct:123. Run grokbot sessions to see available sessions.",
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it.each([
    ...
    [
      "agent template",
      "/tmp/grokbot/agents/{agentId}/sessions/sessions.json",
      "/tmp/grokbot/agents/work/sessions/sessions.json",
    ],
  ])(
    ...
    async (_name, store, resolvedStore) => {
      ...
      expect(runtime.error).toHaveBeenCalledWith(
        "Session not found: agent:work:telegram:direct:123. Run grokbot sessions to see available sessions.",
      );
      expect(runtime.exit).toHaveBeenCalledWith(1);
    },
  );
  ...
  it("uses configured session.store when no explicit store is provided", async () => {
    const runtime = createRuntime();
    mocks.getRuntimeConfig.mockReturnValue({
      session: { store: "/tmp/grokbot/agents/{agentId}/sessions/sessions.json" },
    });
    mocks.resolveStorePath.mockReturnValue("/tmp/grokbot/agents/work/sessions/sessions.json");
    ...
    expect(mocks.loadSessionEntryReadOnly).toHaveBeenCalledWith({
      agentId: "work",
      sessionKey: "agent:work:telegram:direct:123",
      storePath: "/tmp/grokbot/agents/work/sessions/sessions.json",
    });
    expect(runtime.error).toHaveBeenCalledWith(
      "Session not found: agent:work:telegram:direct:123. Run grokbot sessions to see available sessions.",
    );
  ...
  it("exports SQLite marker sessions without probing a transcript JSONL file", async () => {
    const runtime = createRuntime();
    const sessionFile = "sqlite:main:session-1:/tmp/grokbot/sessions.json";
