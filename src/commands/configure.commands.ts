 ...
 * Mirrors the real `grokbot config <sub>` surface so the message only ever
...
const CONFIGURE_NON_TTY_HINT = [
  ...
  `  ${formatCliCommand("grokbot config set <path> <value>")}  write a config entry`,
  `  ${formatCliCommand("grokbot config get <path>")}          read a config entry`,
  `  ${formatCliCommand("grokbot config patch")}              apply a JSON patch`,
  `  ${formatCliCommand("grokbot config validate")}           validate configuration`,
].join("\n");
...
export async function configureCommandFromSectionsArg(
  ...
  // Fail closed once at the shared entry: both `grokbot configure` and the
  // no-subcommand `grokbot config` route here, so a single guard keeps them
  ...
  if (invalid.length > 0) {
    runtime.error(
      `Invalid --section: ${invalid.join(", ")}. Expected one of: ${CONFIGURE_WIZARD_SECTIONS.join(", ")}. Run ${formatCliCommand("grokbot configure")} without --section to use the full wizard.`,
    );
    runtime.exit(1);
    return;
  }
