...
function appendClawHubRiskAcknowledgementGuidance(params: {
  ...
  return `${params.message} To review and acknowledge this ClawHub package, run \`grokbot plugins install ${shellSpec} --acknowledge-clawhub-risk\` from a trusted shell, then rerun repair.`;
}
  ...
  switch (issue.kind) {
    case "missing-install-record":
      return {
        ...
        target,
        fixHint: `Run \`grokbot doctor --fix\` to install ${issue.installSpec}.`,
      };
    case "missing-installed-payload":
      return {
        ...
        fixHint: "Run `grokbot doctor --fix` to reinstall the configured plugin package.",
      };
    case "repairable-installed-plugin":
      return {
        ...
        target,
        ...(issue.installPath ? { path: issue.installPath } : {}),
        fixHint: "Run `grokbot doctor --fix` to repair the configured plugin package.",
      };
    case "stale-version-bound-runtime":
      return {
        checkId: CONFIGURED_PLUGIN_INSTALLS_CHECK_ID,
        severity: "warning",
        message: `Configured runtime plugin ${issue.pluginId} is older than this GrokBot version.`,
        target,
        ...(issue.installPath ? { path: issue.installPath } : {}),
        fixHint: "Run `grokbot doctor --fix` to refresh the configured runtime plugin.",
      };
    case "stale-channel-config-descriptor":
      return {
        ...
        target,
        ...(issue.installPath ? { path: issue.installPath } : {}),
        fixHint: "Run `grokbot doctor --fix` to repair the configured plugin install metadata.",
      };
    case "deferred-package-manager-repair":
      return {
        ...
        target,
        ...(issue.installPath ? { path: issue.installPath } : {}),
        fixHint: "Rerun `grokbot doctor --fix` after the package update completes.",
      };
  }
...
/** Repair missing installs inferred from the current GrokBot config. */
...
async function repairMissingPluginInstalls(params: {
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      const detail = `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "grokbot doctor --fix" after the update completes.`;
      changes.push(detail);
