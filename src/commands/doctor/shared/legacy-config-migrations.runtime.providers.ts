...
const BUNDLED_DISCOVERY_COMPAT_RULE: LegacyConfigRule = {
  path: ["plugins", "allow"],
  message:
    'plugins.allow now gates bundled provider discovery by default; run "grokbot doctor --fix" to preserve legacy bundled provider compatibility as plugins.bundledDiscovery="compat", or set plugins.bundledDiscovery="allowlist" to keep the stricter behavior.',
...
const X_SEARCH_RULE: LegacyConfigRule = {
  ...
  message:
    'tools.web.x_search.apiKey moved to the xAI plugin; use plugins.entries.xai.config.webSearch.apiKey instead. Run "grokbot doctor --fix".',
};

const X_SEARCH_MODEL_RULE: LegacyConfigRule = {
  path: ["tools", "web", "x_search", "model"],
  message:
    'tools.web.x_search.model uses a retired xAI model; run "grokbot doctor --fix" to repair it.',
  requireSourceLiteral: true,
    ...
    legacyRules: [
      {
        path: ["plugins"],
        message:
          'plugins.entries.codex-supervisor and related plugin policy references are retired; use plugins.entries.codex.config.supervision. Run "grokbot doctor --fix".',
        requireSourceLiteral: true,
        match: (_value, root) =>
    ...
    legacyRules: [
      {
        path: ["plugins"],
        message:
          'plugins.openai-codex references are retired; use the openai plugin id. Run "grokbot doctor --fix".',
        requireSourceLiteral: true,
        match: (_value, root) =>
