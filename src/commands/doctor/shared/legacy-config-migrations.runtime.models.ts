...
const LEGACY_VLLM_QWEN_AGENT_THINKING_FORMAT_RULE: LegacyConfigRule = {
  ...
  message:
    'agents.defaults.models.<vllm-model>.params.qwenThinkingFormat is legacy; run "grokbot doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const LEGACY_VLLM_QWEN_PROVIDER_THINKING_FORMAT_RULE: LegacyConfigRule = {
  ...
  message:
    'models.providers.vllm.params.qwenThinkingFormat is legacy; run "grokbot doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const LEGACY_VLLM_QWEN_PROVIDER_MODEL_THINKING_FORMAT_RULE: LegacyConfigRule = {
  ...
  message:
    'models.providers.vllm.models[*].params.qwenThinkingFormat is legacy; run "grokbot doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const LEGACY_VLLM_QWEN_DEFAULT_PARAMS_THINKING_FORMAT_RULE: LegacyConfigRule = {
  ...
  message:
    'agents.defaults.params.qwenThinkingFormat is legacy; run "grokbot doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const LEGACY_VLLM_QWEN_AGENT_PARAMS_THINKING_FORMAT_RULE: LegacyConfigRule = {
  path: ["agents"],
  message:
    'agents.list[].params.qwenThinkingFormat is legacy; run "grokbot doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const INVALID_THINKING_FORMAT_RULE: LegacyConfigRule = {
  path: ["models", "providers"],
  message:
    'models.providers.<id>.models[*].compat.thinkingFormat has an unrecognized value; run "grokbot doctor --fix" to remove it and restore the runtime default.',
...
const STALE_CONTEXT_WINDOW_RULE: LegacyConfigRule = {
  path: ["models", "providers"],
  message:
    'models.providers.<id>.models[*].contextWindow has a stale catalog value; run "grokbot doctor --fix" to repair it.',
...
const RETIRED_MODEL_REF_MESSAGE =
  'Configured retired model refs are no longer in the bundled catalogs; run "grokbot doctor --fix" to upgrade them.';
    ...
    ...(warningLines.length > 0
      ? {
          warning: [
            ...
            ...warningLines,
            "- Doctor retained matching legacy refs in config, sessions, and cron. These refs will not execute until reconciled: fix the model route/auth metadata, remove the legacy provider entry, then rerun `grokbot doctor --fix`.",
          ].join("\n"),
        }
      : {}),
  };
...
const LEGACY_DEFAULT_MODEL_MIGRATION = defineLegacyConfigMigration({
  ...
  legacyRules: [
    {
      path: ["defaultModel"],
      message: 'defaultModel moved to agents.defaults.model. Run "grokbot doctor --fix".',
    },
  ],
  apply: (raw, changes) => {
    ...
    legacyRules: [
      {
        path: ["models", "providers"],
        message:
          'models.providers.codex and models.providers.openai-codex are legacy; run "grokbot doctor --fix" to move them to models.providers.openai.',
        match: (value, root) => hasAutoFixableLegacyOpenAICodexProvider(value, root),
      },
      {
        path: ["models", "providers"],
        message:
          'openai-codex-responses is legacy; run "grokbot doctor --fix" to use openai-chatgpt-responses.',
        match: (value) => {
    ...
    legacyRules: [
      {
        path: ["agents", "defaults", "models"],
        message:
          'agents.defaults.models no longer restricts model overrides; run "grokbot doctor --fix" to preserve the previous restriction in agents.defaults.modelPolicy.allow.',
