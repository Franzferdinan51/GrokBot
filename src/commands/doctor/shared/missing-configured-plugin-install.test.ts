...
const mocks = vi.hoisted(() => ({
  ...
  getOfficialExternalPluginCatalogManifest: vi.fn(
    (entry: { grokbot?: unknown }) => entry.grokbot,
  ),
  ...
  resolveDefaultPluginExtensionsDir: vi.fn(() => "/tmp/grokbot-plugins"),
  resolveDefaultPluginNpmDir: vi.fn(() => "/tmp/grokbot-npm"),
  resolvePluginNpmProjectsDir: vi.fn((npmDir = "/tmp/grokbot-npm") =>
    path.join(npmDir, "projects"),
  ),
  resolvePluginNpmPackageDir: vi.fn(
    ({ npmDir, packageName }: { npmDir?: string; packageName: string }) =>
      path.join(
        npmDir ?? "/tmp/grokbot-npm",
        "projects",
  ...
  resolvePluginInstallDir: vi.fn(
    (pluginId: string, extensionsDir = "/tmp/grokbot-plugins") => `${extensionsDir}/${pluginId}`,
  ),
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-plugin-stub-repair-"));
  tempDirs.push(dir);
  return dir;
}
...
function writeLegacyNpmDeclarationStub(params: {
  ...
  fs.writeFileSync(
    path.join(params.pluginDir, "grokbot.extension.json"),
    JSON.stringify({
...
describe("repairMissingConfiguredPluginInstalls", () => {
  ...
  beforeEach(() => {
    ...
    mocks.resolveDefaultPluginExtensionsDir.mockReturnValue("/tmp/grokbot-plugins");
    mocks.resolveDefaultPluginNpmDir.mockReturnValue("/tmp/grokbot-npm");
    ...
    mocks.resolveOfficialExternalProviderContractPluginIds.mockImplementation(
      ({ contract, providerIds }: { contract: string; providerIds: ReadonlySet<string> }) => {
        ...
        return entries.flatMap((entry) => {
          ...
          const pluginId = candidate.grokbot?.plugin?.id ?? candidate.id;
          const ownedProviderIds = candidate.grokbot?.contracts?.[contract];
          if (
            !pluginId ||
    ...
    mocks.resolveOfficialExternalProviderPluginIds.mockImplementation(
      ({ providerIds }: { providerIds: ReadonlySet<string> }) => {
        ...
        return entries.flatMap((entry) => {
          ...
          const pluginId = candidate.grokbot?.plugin?.id ?? candidate.id;
          const ownsConfiguredProvider = candidate.grokbot?.providers?.some((provider) =>
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      targetDir: "/tmp/grokbot-plugins/matrix",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@grokbot/plugin-matrix",
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      version: "1.2.3",
      npmResolution: {
        name: "@grokbot/plugin-matrix",
        version: "1.2.3",
        resolvedSpec: "@grokbot/plugin-matrix@1.2.3",
  ...
  it("maps a missing configured plugin install to a structured finding and dry-run effect", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          npmSpec: "@grokbot/plugin-matrix@1.2.3",
    ...
    expect(issue).toEqual({
      ...
      pluginId: "matrix",
      installSpec: "@grokbot/plugin-matrix@1.2.3",
    });
    expect(
      ...
      severity: "warning",
      target: "matrix",
      fixHint: "Run `grokbot doctor --fix` to install @grokbot/plugin-matrix@1.2.3.",
    });
    expect(
    ...
    expect(issues).toEqual([
      {
        ...
        pluginId: "discord",
        installPath: missingDiscordPath,
        installSpec: "@grokbot/discord",
      },
    ]);
  });

  it("installs a missing configured GrokBot channel plugin from npm by default", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "matrix",
        pluginId: "matrix",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: "@grokbot/plugin-matrix@1.2.3",
      extensionsDir: "/tmp/grokbot-plugins",
    ...
    expectRecordFields((records as Record<string, unknown>).matrix, {
      source: "npm",
      spec: "@grokbot/plugin-matrix@1.2.3",
      installPath: "/tmp/grokbot-plugins/matrix",
      version: "1.2.3",
    });
    ...
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from @grokbot/plugin-matrix@1.2.3.',
    ]);
  ...
  it("uses an explicit ClawHub install spec before npm", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@grokbot/plugin-matrix@stable",
          npmSpec: "@grokbot/plugin-matrix@1.2.3",
          expectedIntegrity: "sha512-test",
        },
      },
    ...
    const clawHubCall = expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: "clawhub:@grokbot/plugin-matrix@stable",
      expectedPluginId: "matrix",
    });
    expect(clawHubCall.logger).toEqual(expect.objectContaining({ terminalLinks: false }));
    expect(mocks.installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from clawhub:@grokbot/plugin-matrix@stable.',
    ]);
  ...
  it("adds actionable acknowledgement guidance for risky ClawHub candidate failures", async () => {
    mocks.installPluginFromClawHub.mockResolvedValueOnce({
      ...
      error:
        'ClawHub release "@grokbot/plugin-matrix@stable" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue.',
    });
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "matrix",
    ...
    expect(result.warnings[0]).toContain(
      "grokbot plugins install clawhub:@grokbot/plugin-matrix@stable --acknowledge-clawhub-risk",
    );
  });
  ...
  it("adds repair warnings for blocked ClawHub update outcomes", async () => {
    const records = {
      demo: {
        source: "clawhub",
        spec: "clawhub:@grokbot/plugin-demo@stable",
      ...
      outcomes: [
        {
          ...
          message:
            'Skipped demo ClawHub update: ClawHub release "@grokbot/plugin-demo@1.2.4" cannot be installed because ClawHub flagged it as blocked or malicious. Review the security details above or choose a different version. Existing installed plugin left unchanged.',
        },
      ],
    });
  ...
  it("sanitizes and shell-quotes ClawHub acknowledgement guidance specs before rendering commands", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@grokbot/plugin-matrix\n\u001b[31m@stable;$(touch /tmp/pwned)",
        },
      },
    ]);
    ...
    expect(warning).toContain(
      "grokbot plugins install 'clawhub:@grokbot/plugin-matrix\\n@stable;$(touch /tmp/pwned)' --acknowledge-clawhub-risk",
    );
  ...
  it("falls back to npm when an GrokBot channel plugin artifact is unavailable on ClawHub", async () => {
    mocks.installPluginFromClawHub.mockResolvedValueOnce({
      ok: false,
    ...
    expect(result.changes).toEqual([
      'ClawHub clawhub:@grokbot/plugin-matrix@stable unavailable for "matrix"; falling back to npm @grokbot/plugin-matrix@1.2.3.',
      'Installed missing configured plugin "matrix" from @grokbot/plugin-matrix@1.2.3.',
    ]);
    expect(result.warnings).toStrictEqual([]);
  });
  ...
  it("does not fall back from ClawHub to non-GrokBot npm packages", async () => {
    ...
    expect(result.warnings).toEqual([
      'Failed to install missing configured plugin "matrix" from clawhub:@grokbot/plugin-matrix@stable: ClawHub artifact download is not available yet.',
    ]);
  });

  it("honors npm-first catalog metadata for missing GrokBot channel plugins", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "twitch",
      targetDir: "/tmp/grokbot-plugins/twitch",
      version: "2026.5.2",
      npmResolution: {
        name: "@grokbot/twitch",
        version: "2026.5.2",
        resolvedSpec: "@grokbot/twitch@2026.5.2",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/twitch"),
      expectedPluginId: "twitch",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "twitch" from ${expectedNpmInstallSpec("@grokbot/twitch")}.`,
    ]);
  });
  ...
  it("installs missing configured non-channel plugins from the official external catalog", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      targetDir: "/tmp/grokbot-plugins/diagnostics-otel",
      version: "2026.5.2",
      npmResolution: {
        name: "@grokbot/diagnostics-otel",
        version: "2026.5.2",
        resolvedSpec: "@grokbot/diagnostics-otel@2026.5.2",
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        install: {
          clawhubSpec: "clawhub:@grokbot/diagnostics-otel",
          npmSpec: "@grokbot/diagnostics-otel",
          defaultChoice: "npm",
        },
      },
    ]);
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/diagnostics-otel"),
      expectedPluginId: "diagnostics-otel",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "diagnostics-otel" from ${expectedNpmInstallSpec("@grokbot/diagnostics-otel")}.`,
    ]);
  });
  ...
  it("repairs official plugins at the exact extended-stable core version", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@grokbot/diagnostics-otel",
        version: VERSION,
        resolvedSpec: `@grokbot/diagnostics-otel@${VERSION}`,
      },
    });
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: `@grokbot/diagnostics-otel@${VERSION}`,
      expectedPluginId: "diagnostics-otel",
      trustedSourceLinkedOfficialInstall: true,
    });
  ...
  it("installs the official llama.cpp plugin for configured local memory embeddings", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "llama-cpp",
      targetDir: "/tmp/grokbot-plugins/llama-cpp",
      version: "2026.6.2",
      npmResolution: {
        name: "@grokbot/llama-cpp-provider",
        version: "2026.6.2",
        resolvedSpec: "@grokbot/llama-cpp-provider@2026.6.2",
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        grokbot: {
          ...
          install: {
            npmSpec: "@grokbot/llama-cpp-provider",
            defaultChoice: "npm",
          },
        },
        install: {
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/llama-cpp-provider"),
      expectedPluginId: "llama-cpp",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "llama-cpp" from ${expectedNpmInstallSpec("@grokbot/llama-cpp-provider")}.`,
    ]);
  });
  ...
  it("does not let runtime fallback metadata override official catalog install specs", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "acpx",
      targetDir: "/tmp/grokbot-plugins/acpx",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/acpx"),
      expectedPluginId: "acpx",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "acpx" from ${expectedNpmInstallSpec("@grokbot/acpx")}.`,
    ]);
  });
  ...
  it("uses current bundled discovery to remove records before stale snapshots can reinstall official plugins", async () => {
    const records = {
      "google-meet": {
        source: "npm",
        spec: "@grokbot/google-meet",
        resolvedName: "@grokbot/google-meet",
      ...
      plugins: [
        {
          id: "google-meet",
          origin: "npm",
          packageName: "@grokbot/google-meet",
        },
      ],
      diagnostics: [],
    });
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        id: "google-meet",
        label: "Google Meet",
        install: { npmSpec: "@grokbot/google-meet" },
        grokbot: {
          id: "google-meet",
          install: { npmSpec: "@grokbot/google-meet" },
        },
  ...
  it.each([
    [
      ...
      {
        source: "npm",
        spec: "@grokbot/matrix-fork",
        resolvedName: "@grokbot/matrix-fork",
        resolvedSpec: "@grokbot/matrix-fork@1.2.3",
    ...
    [
      ...
      {
        source: "clawhub",
        spec: "clawhub:@grokbot/matrix-fork@stable",
        clawhubPackage: "@grokbot/matrix-fork",
        installPath: "/missing/matrix-fork",
      },
    ],
  ])(
  ...
  it("updates an existing npm target when stale baseline records miss an installed package", async () => {
    ...
    const packageDir = path.join(npmRoot, "node_modules", "@grokbot", "discord");
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      npmResolution: {
        ...
        version: "1.2.3",
        resolvedSpec: "@grokbot/discord@1.2.3",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/discord"),
      expectedPluginId: "discord",
      npmDir: npmRoot,
      mode: "update",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "discord" from ${expectedNpmInstallSpec("@grokbot/discord")}.`,
    ]);
    expect(result.warnings).toEqual([]);
  ...
  it("prefers an existing npm payload over ClawHub during post-core repair", async () => {
    const npmRoot = makeTempDir();
    const packageDir = path.join(npmRoot, "node_modules", "@grokbot", "matrix");
    fs.mkdirSync(packageDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "@grokbot/matrix", version: "1.2.3" }),
    );
    mocks.resolveDefaultPluginNpmDir.mockReturnValue(npmRoot);
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@grokbot/matrix",
          npmSpec: "@grokbot/matrix",
        },
      },
    ]);
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Plugin "@grokbot/matrix" requires plugin API >=2026.5.18.',
    });
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      ...
      npmResolution: {
        name: "@grokbot/matrix",
        version: "1.2.3",
        resolvedSpec: "@grokbot/matrix@1.2.3",
        integrity: "sha512-matrix",
        resolvedAt: "2026-05-01T00:00:00.000Z",
      },
    });
  ...
  it("passes the post-core compatibility host version to ClawHub repair", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "WhatsApp" },
        install: {
          clawhubSpec: "clawhub:@grokbot/whatsapp",
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "whatsapp",
      targetDir: "/tmp/grokbot-plugins/whatsapp",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@grokbot/whatsapp",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.2.3",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: expectedClawHubInstallSpec("clawhub:@grokbot/whatsapp"),
      env: {
        OPENCLAW_COMPATIBILITY_HOST_VERSION: "2026.5.19",
        OPENCLAW_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    ...
    expectRecordFields(result.records.whatsapp, {
      source: "clawhub",
      spec: "clawhub:@grokbot/whatsapp",
      installPath: "/tmp/grokbot-plugins/whatsapp",
      clawhubPackage: "@grokbot/whatsapp",
    });
  });
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        pluginId: "discord",
        meta: { label: "Discord" },
        install: { npmSpec: "@grokbot/discord" },
      },
    ]);
  ...
  it("installs channel-selected external plugins during a legacy package update doctor pass", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@grokbot/discord",
        version: "2026.5.17",
        resolvedSpec: "@grokbot/discord@2026.5.17",
        integrity: "sha512-discord",
  ...
  it("prefers npm over ClawHub during a legacy package update doctor pass", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        ...
        version: "2026.5.17",
        resolvedSpec: "@grokbot/whatsapp@2026.5.17",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/whatsapp"),
      expectedPluginId: "whatsapp",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "whatsapp" from ${expectedNpmInstallSpec("@grokbot/whatsapp")}.`,
    ]);
    expectRecordFields(result.records.whatsapp, {
      source: "npm",
      installPath: "/tmp/grokbot-plugins/whatsapp",
  ...
  it("installs a missing third-party downloadable plugin from npm only", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "wecom",
      targetDir: "/tmp/grokbot-plugins/wecom",
      version: "2026.4.23",
      npmResolution: {
        name: "@wecom/wecom-grokbot-plugin",
        version: "2026.4.23",
        resolvedSpec: "@wecom/wecom-grokbot-plugin@2026.4.23",
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "WeCom" },
        install: {
          npmSpec: "@wecom/wecom-grokbot-plugin@2026.4.23",
        },
      },
    ]);
    ...
    expectRecordFields(installArg, {
      spec: "@wecom/wecom-grokbot-plugin@2026.4.23",
      expectedPluginId: "wecom",
    });
    expect(installArg).not.toHaveProperty("trustedSourceLinkedOfficialInstall", true);
    expect(result.changes).toEqual([
      'Installed missing configured plugin "wecom" from @wecom/wecom-grokbot-plugin@2026.4.23.',
    ]);
  });
  ...
  it("upgrades v2026.7.1-beta.3 Codex Supervisor config and installs Codex", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "codex",
      targetDir: "/tmp/grokbot-plugins/codex",
      version: "2026.7.2",
      npmResolution: {
        name: "@grokbot/codex",
        version: "2026.7.2",
        resolvedSpec: "@grokbot/codex@2026.7.2",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/codex"),
    ...
    expectRecordFields((records as Record<string, unknown>).codex, {
      source: "npm",
      spec: "@grokbot/codex",
      installPath: "/tmp/grokbot-plugins/codex",
      version: "2026.7.2",
    ...
    expect(result.changes).toEqual([
      `Installed missing configured plugin "codex" from ${expectedNpmInstallSpec("@grokbot/codex")}.`,
    ]);
    expect(result.warnings).toEqual([]);
  ...
  it("installs a missing default Codex runtime plugin from the official external catalog", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@grokbot/codex",
        version: "2026.5.2",
        resolvedSpec: "@grokbot/codex@2026.5.2",
  ...
  it("refreshes a stale managed Codex runtime plugin selected by the OpenAI Codex route", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@grokbot/codex", version: "2026.5.6" }),
    );
    const records = {
      codex: {
        source: "npm",
        spec: "@grokbot/codex",
        resolvedName: "@grokbot/codex",
        resolvedSpec: "@grokbot/codex@2026.5.6",
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@grokbot/codex",
        version: VERSION,
        resolvedSpec: `@grokbot/codex@${VERSION}`,
    ...
    expect(result.changes).toEqual([
      `Refreshed stale configured plugin "codex" from ${expectedNpmInstallSpec("@grokbot/codex")}.`,
    ]);
  ...
  it("does not refresh a converged beta Codex runtime plugin on the second doctor pass", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@grokbot/codex",
        version: codexBetaVersion,
        resolvedSpec: `@grokbot/codex@${codexBetaVersion}`,
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: "@grokbot/codex@beta",
      expectedPluginId: "codex",
      trustedSourceLinkedOfficialInstall: true,
      mode: "update",
    });
    expect(firstPass.changes).toEqual([
      'Refreshed stale configured plugin "codex" from @grokbot/codex@beta.',
    ]);
  ...
  it("does not downgrade a newer managed Codex runtime plugin", async () => {
    ...
    fs.writeFileSync(
      path.join(installDir, "package.json"),
      JSON.stringify({ name: "@grokbot/codex", version: "9999.1.1" }),
    );
    const records = {
      codex: {
        source: "npm",
        spec: "@grokbot/codex",
        resolvedName: "@grokbot/codex",
        resolvedSpec: "@grokbot/codex@9999.1.1",
  ...
  it("still installs a channel catalog plugin when the configured owner is blocked by the allowlist", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "feishu",
      targetDir: "/tmp/grokbot-plugins/feishu",
      version: "2026.5.2",
      npmResolution: {
        name: "@grokbot/feishu",
        version: "2026.5.2",
        resolvedSpec: "@grokbot/feishu@2026.5.2",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/feishu"),
      expectedPluginId: "feishu",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "feishu" from ${expectedNpmInstallSpec("@grokbot/feishu")}.`,
    ]);
  });
  ...
  it("reinstalls a missing configured plugin from its persisted install record", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@grokbot/plugin-demo@1.0.0",
        installPath: "/missing/demo",
      },
    };
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
        ...
        plugins: {
          installs: {
            demo: {
              source: "npm",
              spec: "@grokbot/plugin-demo@1.0.0",
              installPath: "/tmp/grokbot-plugins/demo",
            },
          },
        },
      },
  ...
  it("keeps non-ClawHub updater warnings as persisted-record repair warnings", async () => {
    ...
    const repairWarning =
      'Could not repair grokbot peer link for "demo" at /tmp/grokbot-plugins/demo: permission denied';
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
  ...
  it("adds actionable acknowledgement guidance for risky persisted ClawHub repair failures", async () => {
      ...
      outcomes: [
        {
          ...
          message:
            'Skipped demo ClawHub update: ClawHub release "@grokbot/plugin-demo@1.0.0" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue. Existing installed plugin left unchanged.',
        },
      ],
    });
    ...
    expect(result.warnings[0]).toContain(
      "grokbot plugins install clawhub:@grokbot/plugin-demo@1.0.0 --acknowledge-clawhub-risk",
    );
  });
  ...
  it("prefixes legacy persisted ClawHub package records in acknowledgement guidance", async () => {
      ...
      outcomes: [
        {
          ...
          code: CLAWHUB_INSTALL_ERROR_CODE.CLAWHUB_RISK_ACKNOWLEDGEMENT_REQUIRED,
          message:
            'Skipped demo ClawHub update: ClawHub release "@grokbot/plugin-demo@latest" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue. Existing installed plugin left unchanged.',
        },
      ],
    });
    ...
    expect(result.warnings[0]).toContain(
      "grokbot plugins install clawhub:@grokbot/plugin-demo --acknowledge-clawhub-risk",
    );
  });
  ...
  it("repairs a broken managed package entry from its attributed registry diagnostic", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@grokbot/plugin-demo@1.0.0",
        resolvedName: "@grokbot/plugin-demo",
        resolvedSpec: "@grokbot/plugin-demo@1.0.0",
  ...
  it("reinstalls a known configured plugin from the catalog when its recorded install path is missing", async () => {
    const records = {
      discord: {
        source: "npm",
        spec: "@grokbot/discord",
        installPath: "/tmp/grokbot-missing-discord-install-record",
      },
    };
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      diagnostics: [
        {
          ...
          pluginId: "discord",
          message:
            "channel plugin manifest declares discord without channelConfigs metadata; add grokbot.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads",
        },
      ],
    });
    mocks.updateNpmInstalledPlugins.mockResolvedValue({
  ...
  it("reinstalls a recorded external web search plugin from provider-only config", async () => {
    const records = {
      brave: {
        source: "npm",
        spec: "@grokbot/brave-plugin@beta",
  ...
  it("replaces a configured official web search plugin when its installed package is source-only", async () => {
    ...
    const records = {
      brave: {
        source: "clawhub",
        spec: "clawhub:@grokbot/brave-plugin@2026.5.1-beta.1",
        installPath: installDir,
        clawhubPackage: "@grokbot/brave-plugin",
        clawhubChannel: "official",
        clawhubUrl: "https://clawhub.ai",
      },
    };
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "brave",
      targetDir: "/tmp/grokbot-plugins/brave",
      version: "2026.5.12",
      npmResolution: {
        name: "@grokbot/brave-plugin",
        version: "2026.5.12",
        resolvedSpec: "@grokbot/brave-plugin@2026.5.12",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/brave-plugin"),
    ...
    expect(result).toEqual({
      changes: [
        `Installed missing configured plugin "brave" from ${expectedNpmInstallSpec("@grokbot/brave-plugin")}.`,
      ],
      warnings: [],
  ...
  it("replaces a configured official channel plugin when only its channel is configured", async () => {
    ...
    const records = {
      slack: {
        source: "clawhub",
        spec: "clawhub:@grokbot/slack@2026.5.12-beta.1",
        installPath: installDir,
        clawhubPackage: "@grokbot/slack",
        clawhubChannel: "official",
        clawhubUrl: "https://clawhub.ai",
      },
    };
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "slack",
      targetDir: "/tmp/grokbot-npm/node_modules/@grokbot/slack",
      version: "2026.5.12",
      npmResolution: {
        name: "@grokbot/slack",
        version: "2026.5.12",
        resolvedSpec: "@grokbot/slack@2026.5.12",
    ...
    expect(fs.existsSync(installDir)).toBe(false);
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/slack"),
      expectedPluginId: "slack",
      trustedSourceLinkedOfficialInstall: true,
      mode: "update",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "slack" from ${expectedNpmInstallSpec("@grokbot/slack")}.`,
    ]);
  });
  ...
  it("keeps a broken official install record when replacement install fails", async () => {
    ...
    expect(result).toEqual({
      changes: [],
      warnings: [
        `Failed to install missing configured plugin "brave" from ${expectedNpmInstallSpec("@grokbot/brave-plugin")}: network unavailable`,
      ],
  ...
  it("installs a configured external web search plugin from provider-only config", async () => {
    ...
    mocks.resolveOfficialExternalPluginId.mockImplementation(
      (entry: { id?: string; grokbot?: { plugin?: { id?: string } } }) =>
        entry.grokbot?.plugin?.id ?? entry.id,
    );
    mocks.resolveOfficialExternalPluginInstall.mockImplementation(
      (entry: { install?: unknown; grokbot?: { install?: unknown } }) =>
        entry.grokbot?.install ?? entry.install ?? null,
    );
    mocks.resolveOfficialExternalPluginLabel.mockImplementation(
      (entry: { label?: string; grokbot?: { plugin?: { label?: string } } }) =>
        entry.grokbot?.plugin?.label ?? entry.label ?? "plugin",
    );
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "brave",
      ...
      npmResolution: {
        name: "@grokbot/brave-plugin",
        version: "2026.5.2",
        resolvedSpec: "@grokbot/brave-plugin@2026.5.2",
      },
    });
  ...
  it("installs configured external speech and web-fetch plugins from selected providers", async () => {
    const packages = [
      ["firecrawl", "@grokbot/firecrawl-plugin"],
      ["gradium", "@grokbot/gradium-speech"],
      ["inworld", "@grokbot/inworld-speech"],
    ] as const;
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue(
    ...
    for (const [pluginId, npmSpec] of packages) {
      mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
        ok: true,
        pluginId,
        targetDir: `/tmp/grokbot-plugins/${pluginId}`,
        version: "2026.6.8",
        npmResolution: {
          name: npmSpec,
          version: "2026.6.8",
  ...
  it("installs a configured external model provider without an auth choice", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "groq",
      targetDir: "/tmp/grokbot-plugins/groq",
      version: "2026.6.8",
      npmResolution: {
        name: "@grokbot/groq-provider",
        version: "2026.6.8",
        resolvedSpec: "@grokbot/groq-provider@2026.6.8",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/groq-provider"),
      expectedPluginId: "groq",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "groq" from ${expectedNpmInstallSpec("@grokbot/groq-provider")}.`,
    ]);
  });
  ...
  it("installs an external speech provider selected only by voiceModel", async () => {
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        label: "Gradium",
        install: {
          npmSpec: "@grokbot/gradium-speech",
          defaultChoice: "npm",
        },
        grokbot: {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "gradium",
      targetDir: "/tmp/grokbot-plugins/gradium",
      version: "2026.6.8",
      npmResolution: {
        name: "@grokbot/gradium-speech",
        version: "2026.6.8",
        resolvedSpec: "@grokbot/gradium-speech@2026.6.8",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/gradium-speech"),
      expectedPluginId: "gradium",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "gradium" from ${expectedNpmInstallSpec("@grokbot/gradium-speech")}.`,
    ]);
  });

  it("installs env-only web provider plugins before auto-detection", async () => {
    const packages = [
      ["exa", "@grokbot/exa-plugin", "EXA_API_KEY"],
      ["firecrawl", "@grokbot/firecrawl-plugin", "FIRECRAWL_API_KEY"],
    ] as const;
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue(
  ...
  it("installs configured external web search plugins from beta on the beta channel", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@grokbot/brave-plugin",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@grokbot/brave-plugin@2026.5.4-beta.1",
      },
    });
    ...
    expect(result.changes).toEqual([
      'Installed missing configured plugin "brave" from @grokbot/brave-plugin@beta.',
    ]);
  });
  ...
  it("repairs a configured plugin from a legacy npm declaration stub", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "guardrail-bridge",
      targetDir: "/tmp/grokbot-plugins/guardrail-bridge",
      version: "1.0.0",
      npmResolution: {
    ...
    expectRecordFields((records as Record<string, unknown>)["guardrail-bridge"], {
      source: "npm",
      spec: "@guardrail-bridge/guardrail-bridge@1.0.0",
      installPath: "/tmp/grokbot-plugins/guardrail-bridge",
      version: "1.0.0",
  ...
  it("installs Firecrawl for env-only web fetch when search is disabled", async () => {
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        label: "Firecrawl",
        install: {
          npmSpec: "@grokbot/firecrawl-plugin",
          defaultChoice: "npm",
        },
        grokbot: {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "firecrawl",
      targetDir: "/tmp/grokbot-plugins/firecrawl",
      version: "2026.6.8",
      npmResolution: {
        name: "@grokbot/firecrawl-plugin",
        version: "2026.6.8",
        resolvedSpec: "@grokbot/firecrawl-plugin@2026.6.8",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@grokbot/firecrawl-plugin"),
      expectedPluginId: "firecrawl",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "firecrawl" from ${expectedNpmInstallSpec("@grokbot/firecrawl-plugin")}.`,
    ]);
  });
