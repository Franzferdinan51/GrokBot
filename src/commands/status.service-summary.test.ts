...
describe("readServiceStatusSummary", () => {
  it("marks GrokBot-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["grokbot", "gateway", "run"] })),
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["grokbot", "gateway", "run", "--port", "18789"],
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "grokbot-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "grokbot-gateway.service");
      ...
      expect(layout.sourcePathReal).toBe(path.join(realRoot, "grokbot-gateway.service"));
