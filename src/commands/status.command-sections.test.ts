...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:grokbot security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:grokbot security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:grokbot status --all",
      "  Need to debug live? cmd:grokbot logs --follow",
      "  Fix reachability first: cmd:grokbot gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:grokbot devices approve req-123)",
      "muted(Fallback: cmd:grokbot devices approve --latest)",
      "muted(Inspect: cmd:grokbot devices list)",
    ]);
  });
