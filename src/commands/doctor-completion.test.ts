...
describe("shell completion health mapping", () => {
  it("checks an explicit shell instead of the detected environment shell", async () => {
    const homeDir = tempDirs.make("grokbot-completion-home-");
    const stateDir = tempDirs.make("grokbot-completion-state-");
    setTestEnvValue("HOME", homeDir);
    setTestEnvValue("OPENCLAW_STATE_DIR", stateDir);
    setTestEnvValue("SHELL", "/bin/zsh");

    const current = await checkShellCompletionStatus("grokbot", { shell: "fish" });

    expect(current.shell).toBe("fish");
    expect(current.cachePath).toBe(path.join(stateDir, "completions", "grokbot.fish"));
...
async function setupDoctorCompletionTest(usesSlowPattern: boolean) {
  const homeDir = tempDirs.make("grokbot-doctor-home-");
  const stateDir = tempDirs.make("grokbot-doctor-state-");
  setTestEnvValue("HOME", homeDir);
  setTestEnvValue("OPENCLAW_STATE_DIR", stateDir);
  ...
  if (usesSlowPattern) {
    await fs.writeFile(
      profilePath,
      '# test bashrc\n[ -f "/tmp/nonexistent" ] && source <(grokbot completion bash)\n',
      "utf-8",
    );
    const cacheDir = path.join(stateDir, "completions");
    await fs.mkdir(cacheDir, { recursive: true });
    await fs.writeFile(path.join(cacheDir, "grokbot.bash"), "# completion cache\n", "utf-8");
  }
  return profilePath;
}
...
describe("doctorShellCompletion", () => {
  ...
  it.each([
    ...
    async ({ generationMode, expectedSkipValue }) => {
      ...
      await expect(
        ensureCompletionCacheExists("grokbot", {
          shell: "powershell",
          generationMode,
        }),
      ).resolves.toBe(true);
