...
describe("buildGatewayRuntimeHints", () => {
  ...
  it("surfaces suspicious systemd cgroup hygiene with inspection commands", () => {
    expect(
      ...
      "Run: systemctl --user show grokbot-gateway.service -p KillMode -p TasksCurrent -p MemoryCurrent -p MainPID",
      "Run: systemd-cgls --user-unit grokbot-gateway.service",
      "After reviewing service settings, run: grokbot gateway restart",
    ]);
  });
  ...
  it("guides recovery when systemd hit its restart start limit (crash loop)", () => {
    ...
    expect(text).toContain("grokbot gateway restart");
