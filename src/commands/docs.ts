...
// Implements docs link/search output for `grokbot docs`.
import { readResponseWithLimit } from "../infra/http-body.js";
import type { RuntimeEnv } from "../runtime.js";

const SEARCH_API = "https://docs.grokbot.ai/api/search";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.grokbot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('grokbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.grokbot.ai/");
      runtime.log(`Search: ${formatCliCommand('grokbot docs "your query"')}`);
    }
    return;
  }
