...
async function maybeRemoveLegacyUsageCostCacheFiles(params: {
  ...
  if (!params.shouldRepair) {
    note(
      `${files.length} rebuildable usage-cost cache ${files.length === 1 ? "file remains" : "files remain"}. Run \`grokbot doctor --fix\` to remove ${files.length === 1 ? "it" : "them"}.`,
      "Usage cost cache",
    );
    return;
  }
...
async function maybeRemoveLegacySkillUploadTree(params: {
  ...
  if (!params.shouldRepair) {
    note(
      "Legacy skill-upload staging remains. Run `grokbot doctor --fix` to discard it; active uploads now live in SQLite and must be retried.",
      "Skill uploads",
    );
    return;
  }
