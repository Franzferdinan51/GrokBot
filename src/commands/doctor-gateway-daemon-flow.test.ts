...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.grokbot.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.grokbot.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPENCLAW_STATE_DIR: "/tmp/grokbot-service",
        OPENCLAW_CONFIG_PATH: "/tmp/grokbot-service/grokbot.json",
      },
    });
    ...
    expect(handoffEnv?.OPENCLAW_STATE_DIR).toBe("/tmp/grokbot-service");
    expect(handoffEnv?.OPENCLAW_CONFIG_PATH).toBe("/tmp/grokbot-service/grokbot.json");
    expect(note).toHaveBeenCalledWith(
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "grokbot-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      listeners: [
        ...
        { pid: 5002, commandLine: "grokbot-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("grokbot gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system GrokBot gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        ...
        detail: "unit: /etc/systemd/system/grokbot-gateway.service",
        scope: "system",
        marker: "grokbot",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level GrokBot gateway service detected while the user gateway service is not installed.",
        "- grokbot-gateway.service (unit: /etc/systemd/system/grokbot-gateway.service)",
        "GrokBot will not install a second user-level gateway service automatically.",
        "Run `grokbot gateway status --deep` or `grokbot doctor --deep` to inspect duplicate services.",
