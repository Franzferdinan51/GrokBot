...
const DOCS_HEARTBEAT_TEMPLATE_PAGE_AS_TEMPLATE = [
  "# HEARTBEAT.md template",
  "`HEARTBEAT.md` lives in the agent workspace. Keep the file empty, or with only Markdown comments and headings, when you want GrokBot to skip heartbeat model calls.",
...
function heartbeatTemplateAnalysisToHealthFinding(
  ...
  return {
    ...
    fixHint: 'Run "grokbot doctor --fix" to replace it with the clean heartbeat template.',
  };
}
    ...
    if (!params.shouldRepair) {
      note(
        [
          ...
          'Run "grokbot doctor --fix" to replace it with the clean heartbeat template.',
        ].join("\n"),
        "Heartbeat template",
      );
      continue;
