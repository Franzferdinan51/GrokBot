...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/grokbot/sandbox/containers.json",
        shardedDir: "/tmp/grokbot/sandbox/containers",
        source: "monolithic",
        exists: true,
        valid: true,
        entries: 2,
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- containers monolithic: /tmp/grokbot/sandbox/containers.json (2 entries)",
        "Run grokbot doctor --fix to migrate them to SQLite.",
      ].join("\n"),
      "Sandbox",
    );
  });
  ...
  it("maps legacy registry files to structured findings and dry-run effects", () => {
    ...
    expect(legacySandboxRegistryInspectionToHealthFinding(monolithicFile)).toEqual(
      expect.objectContaining({
        ...
        severity: "warning",
        path: "/tmp/grokbot/sandbox/containers.json",
    ...
    expect(legacySandboxRegistryInspectionToRepairEffect(monolithicFile)).toEqual({
      ...
      target: "/tmp/grokbot/sandbox/containers.json",
      dryRunSafe: false,
    });
    expect(legacySandboxRegistryInspectionToHealthFinding(shardedFile)).toEqual(
      expect.objectContaining({
        path: "/tmp/grokbot/sandbox/containers",
        message: expect.stringContaining(
          "- containers sharded: /tmp/grokbot/sandbox/containers (2 entries)",
        ),
      }),
    );
    expect(legacySandboxRegistryInspectionToRepairEffect(shardedFile)).toEqual(
      expect.objectContaining({
        target: "/tmp/grokbot/sandbox/containers",
      }),
    );
  });
  ...
  it("maps invalid legacy registry files to quarantine effects", () => {
    expect(
      legacySandboxRegistryInspectionToRepairEffect({
        kind: "browsers",
        registryPath: "/tmp/grokbot/sandbox/browsers.json",
        shardedDir: "/tmp/grokbot/sandbox/browsers",
        source: "monolithic",
        exists: true,
        valid: false,
        entries: 0,
      ...
      expect.objectContaining({
        ...
        target: "/tmp/grokbot/sandbox/browsers.json",
      }),
    );
  });
