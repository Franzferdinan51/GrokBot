...
export function formatUpdateRestartStatusValue(
  ...
  if (payload.status === "error") {
    return warn(
      `failed · ${reason ?? "restart failed"} · run grokbot gateway status --deep${age}`,
    );
  }
  ...
  if (payload.status === "skipped") {
    if (reason === CONTROL_PLANE_UPDATE_HANDOFF_STARTED_REASON) {
      ...
      return warn(`handoff running · gateway restart pending · run grokbot update status${age}`);
    }
    if (reason === CONTROL_PLANE_UPDATE_RESTART_HEALTH_PENDING_REASON) {
      ...
      return warn(`restart pending health verification · run grokbot gateway status --deep${age}`);
    }
...
export function formatUpdateRestartActionLines(
  ...
  if (payload.status === "error") {
    return [
      "Update restart failed; run grokbot gateway status --deep.",
  ...
  if (
    ...
    return [
      ...
      "If it stays pending, run grokbot gateway status --deep.",
    ];
  }
  return [];
}
