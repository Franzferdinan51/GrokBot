// Builds the data model for the standard `grokbot status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("grokbot security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("grokbot status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("grokbot tasks maintenance --apply")}`,
