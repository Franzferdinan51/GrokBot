// Builds channel status rows and account details for `grokbot status --all`.
// This layer stays plugin-generic: channel-specific auth rules live in plugin config/status hooks.

import fs from "node:fs";
import { asRecord } from "@grokbot/normalization-core/record-coerce";
...
function formatLoadFailureDetail(message: string): string {
  const reason = isLikelyDependencyTreeCorruption(message)
    ...
    : "registration failed";
  return `plugin load failed: ${reason}; run grokbot doctor --fix`;
}
