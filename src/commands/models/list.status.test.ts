...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/grokbot-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/grokbot-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/grokbot-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/grokbot-dev/grokbot.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/grokbot-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/grokbot-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/grokbot-dev/grokbot.json");
    expect(payload.auth.storePath).toBe("/tmp/grokbot-agent/auth-profiles.json");
  ...
  it("honors OPENCLAW_AGENT_DIR when no --agent override is provided", async () => {
    ...
    await withEnvAsync({ OPENCLAW_AGENT_DIR: "/tmp/grokbot-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/grokbot-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/grokbot-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/grokbot-isolated-agent/auth-profiles.json");
  });
  ...
  it("honors deprecated PI_CODING_AGENT_DIR when OPENCLAW_AGENT_DIR is unset", async () => {
    ...
    await withEnvAsync(
      {
        ...
        PI_CODING_AGENT_DIR: "/tmp/grokbot-legacy-agent",
      },
      async () => {
        await modelsStatusCommand({ json: true }, localRuntime as never);
      },
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/grokbot-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/grokbot-legacy-agent");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/grokbot-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/grokbot-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/grokbot-agent-custom/auth-profiles.json",
        });
      },
    );
  });
    ...
    expect(payload.auth.runtimeAuthRoutes).toEqual([
      {
        ...
        effective: {
          kind: "profiles",
          detail: "/tmp/grokbot-agent/auth-profiles.json",
        },
      },
    ]);
    expect(localRuntime.exit).toHaveBeenCalledWith(1);
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/grokbot-agent/workspace"
          ? {
