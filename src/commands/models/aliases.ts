  ...
  const updated = await updateConfig((cfg) => {
    ...
    if (!found) {
      ...
      if (
        ...
        throw new Error(
          `Cannot remove "${alias}": it is a built-in alias for "${builtinTarget}" provided automatically by GrokBot and is not stored in your config file. To shadow it with a different target, run ${formatCliCommand(`grokbot models aliases add ${alias} <model>`)}.`,
        );
      }
      throw new Error(
        `Alias not found: ${alias}. Run ${formatCliCommand("grokbot models aliases list")} to see configured aliases.`,
      );
    }
    return {
      ...cfg,
