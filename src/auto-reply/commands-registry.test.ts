...
describe("commands registry", () => {
  ...
  it("preserves multiline payloads for skill slash commands", () => {
    ...
    expect(
      normalizeCommandBody("/skill@grokbot: demo_skill first line\nsecond line", {
        botUsername: "grokbot",
      }),
  ...
  it("normalizes telegram-style command mentions for the current bot", () => {
    expect(normalizeCommandBody("/help@grokbot", { botUsername: "grokbot" })).toBe("/help");
    expect(
      normalizeCommandBody("/help@grokbot args", {
        botUsername: "grokbot",
      }),
    ).toBe("/help args");
    expect(
  ...
  it("keeps telegram-style command mentions for other bots", () => {
    expect(normalizeCommandBody("/help@otherbot", { botUsername: "grokbot" })).toBe(
      "/help@otherbot",
    );
  });
...
describe("commands registry args", () => {
  ...
  it.each([
    ...
    { model: "gpt-5.6-luna", agentRuntime: "grokbot", supportsUltra: true },
  ])(
