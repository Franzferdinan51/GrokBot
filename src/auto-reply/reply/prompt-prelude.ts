...
const ROOM_EVENT_PROMPT = "[GrokBot room event]";
...
export function buildReplyPromptEnvelopeBase(
  ...
  // conversation, while the GrokBot marker remains current-turn context only.
  const transcriptBody = params.isHeartbeat
    ? HEARTBEAT_TRANSCRIPT_PROMPT
    : params.isBareSessionReset
      ? softResetTail || `[GrokBot session ${params.startupAction}]`
      : isRoomEvent
