...
export function resolveAcpInstallCommandHint(cfg: OpenClawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `grokbot plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^grokbot plugins install /u, "");
    ...
    return "grokbot plugins install acpx";
  }
