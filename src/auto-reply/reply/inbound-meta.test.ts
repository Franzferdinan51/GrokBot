...
describe("buildInboundMetaSystemPrompt", () => {
  it("includes stable routing fields and omits chat ids", () => {
    ...
    expect(payload["schema"]).toBe("grokbot.inbound_meta.v2");
...
describe("buildInboundUserContextPrefix", () => {
  ...
  it("does not treat group chats as direct based on sender id", () => {
    ...
    expect(conversationInfo["sender"]).toEqual({ id: "grokbot-control-ui" });
  ...
  it("renders chat window structured context as compact transcript text", () => {
              ...
              messages: [
                {
                  ...
                  media_path: "/home/user/.grokbot/media/inbound/sticker.webp",
    ...
    expect(text).not.toContain("/home/user/.grokbot/media/inbound/sticker.webp");
  ...
  it("includes inbound history media metadata without leaking paths or URLs", () => {
      ...
      InboundHistory: [
        {
          ...
          media: [
            {
              path: "/tmp/grokbot-secret-image.png",
    ...
    expect(text).not.toContain("/tmp/grokbot-secret-image.png");
  ...
  it("preserves every media content type for a history message with multiple attachments", () => {
      ...
      InboundHistory: [
        {
          ...
          media: [
            {
              path: "/tmp/grokbot-secret-image-1.png",
            ...
            {
              path: "/tmp/grokbot-secret-image-2.jpg",
    ...
    expect(text).not.toContain("/tmp/grokbot-secret-image-1.png");
    expect(text).not.toContain("/tmp/grokbot-secret-image-2.jpg");
