...
function makeWorkspace(): string {
  return tempDirs.make("grokbot-session-export-");
}
...
describe("writeSessionExportFile", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = makeWorkspace();
      const aliasParent = tempDirs.make("grokbot-session-export-alias-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = makeWorkspace();
      const outsideDir = tempDirs.make("grokbot-session-export-outside-");
