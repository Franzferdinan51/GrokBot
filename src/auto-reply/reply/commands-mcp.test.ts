...
const workspaceHarness = createCommandWorkspaceHarness("grokbot-command-mcp-");
...
describe("handleCommands /mcp", () => {
  ...
  it("writes MCP config and shows it back", async () => {
    await withTempHome("grokbot-command-mcp-home-", async () => {
  ...
  it("routes group /mcp show privately and redacts the delivered config", async () => {
    await withTempHome("grokbot-command-mcp-home-", async () => {
      ...
      expect(namedGroupText).not.toContain("/tmp/grokbot.json");
      ...
      expect(allGroupText).not.toContain("/tmp/grokbot.json");
  ...
  it.each([
    ...
    await withTempHome("grokbot-command-mcp-home-", async () => {
      ...
      expect(groupText).not.toContain("/tmp/grokbot.json");
