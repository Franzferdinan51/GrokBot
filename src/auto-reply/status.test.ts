...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("GrokBot");
    ...
    expect(normalized).toContain("Runtime: GrokBot Default");
  ...
  it("shows the default GrokBot harness as the model runtime", () => {
    const text = buildStatusMessage({
      agent: {
        model: "openai/gpt-5.4",
      },
      sessionEntry: {
        sessionId: "grokbot-harness",
        updatedAt: 0,
        fastMode: true,
      },
      sessionKey: "agent:main:main",
    ...
    expect(normalized).toContain("Fast");
    expect(normalized).toContain("Runtime: GrokBot Default");
    expect(normalized).not.toContain("· grokbot");
  });
