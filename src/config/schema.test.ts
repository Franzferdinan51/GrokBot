import { SENSITIVE_URL_HINT_TAG } from "@grokbot/net-policy/redact-sensitive-url";
...
describe("config schema", () => {
  ...
  it("validates MCP OAuth client metadata URLs against the SDK contract", () => {
        ...
        mcp: {
          servers: {
            docs: {
              ...
              auth: "oauth",
              oauth: {
                clientMetadataUrl: "https://client.example.com/grokbot-mcp.json",
              },
            },
          },
        },
    ...
    for (const clientMetadataUrl of [
      "http://client.example.com/grokbot-mcp.json",
  ...
  it("accepts install policy exec config in the runtime zod schema", () => {
    ...
    expect(parsed.security?.installPolicy?.exec?.command).toBe(
      "/usr/local/bin/grokbot-install-policy",
    );
  });
