...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the GrokBot agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus GrokBot, or the agent id plus GrokBot. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the GrokBot agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which GrokBot conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing GrokBot Discord conversation. */
