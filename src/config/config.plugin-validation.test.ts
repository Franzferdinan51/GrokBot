...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "grokbot.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      OPENCLAW_STATE_DIR: path.join(suiteHome, ".grokbot"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-config-plugin-validation-"));
  ...
  describe("missing Codex plugin diagnostics", () => {
    ...
    it("still warns when only one provider model route is pinned to GrokBot", () => {
    ...
    it("still reports explicit Codex allowlist entries for custom OpenAI-compatible base URLs", () => {
      ...
      expect(res.warnings ?? []).toContainEqual(
        expect.objectContaining({
          path: "plugins.allow",
          message:
            "plugin not installed: codex — install the official external plugin with: grokbot plugins install @grokbot/codex",
        }),
      );
    });
  ...
  it("deduplicates catalog install hints for missing configured official external plugins", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: brave — install the official external plugin with: grokbot plugins install @grokbot/brave-plugin";
  ...
  it("warns instead of failing when an official external memory slot plugin is not installed", () => {
    ...
    expect(res.ok).toBe(true);
    const slotMessage =
      "plugin not installed: memory-lancedb — gateway will run without persistent memory until installed; install the official external plugin with: grokbot plugins install @grokbot/memory-lancedb";
    const entryMessage =
      "plugin not installed: memory-lancedb — install the official external plugin with: grokbot plugins install @grokbot/memory-lancedb";
  ...
  it("deduplicates yuanbao missing-plugin warnings across entries and allow", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: yuanbao — install the official external plugin with: grokbot plugins install grokbot-plugin-yuanbao@2.15.0";
            ...
            diagnostics: [
              {
                ...
                source: path.join(suiteHome, "extensions", "broken-local", "grokbot.plugin.json"),
  ...
  it("warns instead of failing for stale channel config backed by missing plugin refs", () => {
    ...
    expect(res.warnings).toContainEqual({
      ...
      message:
        "unknown channel id: missing-chat (stale channel plugin config ignored; run grokbot doctor --fix to remove stale config, or install the plugin)",
    });
    expect(res.warnings).toContainEqual({
      path: "plugins.allow",
      message:
    ...
    expect(res.warnings ?? []).toEqual([
      {
        path: "plugins.allow",
        message:
          "plugin not installed: discord — install the official external plugin with: grokbot plugins install @grokbot/discord",
      },
    ]);
  });

  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    const stateDir = path.join(suiteHome, ".grokbot");
    ...
    try {
      ...
      expect(res.warnings).toContainEqual({
        ...
        message:
          "unknown channel id: missing-sms (stale channel plugin config ignored; run grokbot doctor --fix to remove stale config, or install the plugin)",
      });
    } finally {
      await writePersistedInstalledPluginIndex(
        {
  ...
  it("warns for removed skill-workshop plugin id instead of failing validation", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin removed: skill-workshop (stale plugin config ignored; Skill Workshop is built into GrokBot skills now. Use skills.workshop settings and grokbot skills workshop commands, then remove this plugins config entry)";
  ...
  it("ignores standalone helper scripts in auto-discovered global extensions", async () => {
    const helperPath = path.join(suiteHome, ".grokbot", "extensions", "my-helper.mjs");
  ...
  it("accepts known plugin ids and valid channel/heartbeat enums", () => {
    const res = validateInSuite({
      agents: {
        ...
        list: [{ id: "grokbot", heartbeat: { directPolicy: "allow" } }],
      },
      channels: {
        modelByChannel: {
          openai: {
  ...
  it("accepts plugin heartbeat targets", () => {
    const res = validateInSuite({
      agents: { defaults: { heartbeat: { target: "chat" } }, list: [{ id: "grokbot" }] },
