...
import { normalizeConfiguredProviderCatalogModelId } from "@grokbot/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  grokbot config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  grokbot config set ${policyPath} "pairing"`,
  ].join("\n");
}
