...
function resolveCustomStoreSqlitePath(params: {
  ...
  const sqliteBaseName =
    params.sqliteBaseName ?? (path.basename(resolved, path.extname(resolved)) || "grokbot-agent");
...
export function resolveSqliteTargetFromSessionStorePath(
  ...
  if (path.basename(resolved) === "grokbot-agent.sqlite" || resolved.endsWith(".sqlite")) {
  ...
  return {
    ...
    path: path.join(agentDir, "agent", "grokbot-agent.sqlite"),
  };
}
...
function resolveAgentIdFromSqliteDatabasePath(databasePath: string): string | undefined {
  if (path.basename(databasePath) !== "grokbot-agent.sqlite") {
    return undefined;
  }
