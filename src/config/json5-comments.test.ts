...
describe("warnIfJSON5CommentsWillBeStripped", () => {
  it.each([
    ...
    warnIfJSON5CommentsWillBeStripped({ raw, filePath: "/tmp/grokbot.json", warn });

    expect(warn).toHaveBeenCalledWith(
      "Config write will strip JSON5 comments from /tmp/grokbot.json.",
    );
  });
