...
const ETC_OPENCLAW_DIR = path.join(ROOT_DIR, "etc", "grokbot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "grokbot.json");
...
describe("collectIncludePathsRecursive", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "grokbot-include-scan-" }, async (tempRoot) => {
        ...
        const includePaths = await collectIncludePathsRecursive({
          configPath: path.join(configDir, "grokbot.json"),
          parsed: {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "grokbot-include-root-swap-" }, async (tempRoot) => {
        ...
        expect(() =>
          resolveConfigIncludes(
            { $include: "./outer.json5" },
            path.join(configLink, "grokbot.json"),
            resolver,
          ),
  ...
  it("honors explicitly allowed include roots", async () => {
    await withTempDir({ prefix: "grokbot-include-scan-roots-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("continues past rejected nested includes to later safe siblings", async () => {
    await withTempDir({ prefix: "grokbot-include-scan-nested-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("revisits an include reached later at a shallower depth", async () => {
    await withTempDir({ prefix: "grokbot-include-scan-depth-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
...
describe("resolveConfigIncludeWritePath", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "grokbot-include-write-path-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTempDir({ prefix: "grokbot-includes-symlink-" }, async (tempRoot) => {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "grokbot.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTempDir({ prefix: "grokbot-includes-hardlink-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        const outsideDir = path.join(tempRoot, "outside");
        await fs.mkdir(configDir, { recursive: true });
        await fs.mkdir(outsideDir, { recursive: true });
    ...
    it("rejects include files larger than the guarded read limit", async () => {
      await withTempDir({ prefix: "grokbot-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        await fs.writeFile(
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "grokbot.json")),
...
describe("OPENCLAW_INCLUDE_ROOTS allowlist", () => {
  ...
  it("resolves a symlinked include whose realpath lands inside an allowed root", async () => {
    await withTempDir({ prefix: "grokbot-includes-allowed-symlink-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const sharedDir = path.join(tempRoot, "shared");
      await fs.mkdir(configDir, { recursive: true });
      await fs.mkdir(sharedDir, { recursive: true });
  ...
  it("rejects a symlinked include that escapes both the config directory and every allowed root", async () => {
    await withTempDir({ prefix: "grokbot-includes-allowed-escape-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const allowedDir = path.join(tempRoot, "allowed");
