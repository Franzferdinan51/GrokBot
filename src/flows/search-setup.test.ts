...
const mockGrokProvider = vi.hoisted(() => ({
  ...
  docsUrl: "https://docs.grokbot.ai/tools/web",
        ...
        plugins: {
          ...
          installs: {
            ...
            [entry.pluginId]: {
              ...
              installPath: `/tmp/grokbot-plugins/${entry.pluginId}`,
            },
          },
        },
      },
...
describe("runSearchSetupFlow", () => {
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            label: "暂时跳过",
            hint: "稍后可用 grokbot configure --section web 配置",
          }),
        ]),
      }),
    );
  ...
  it("shows provider credential notes before SecretRef setup notes", async () => {
    ...
    expect(note).toHaveBeenNthCalledWith(
      3,
      [
        "Secret references enabled — GrokBot will store a reference instead of the API key.",
        "Env var: XAI_API_KEY.",
  ...
  it("installs an external catalog search provider before enabling it", async () => {
    ...
    expect(installRequest.entry?.install?.npmSpec).toBe("@grokbot/brave-plugin");
    ...
    expect(next.plugins?.installs?.brave?.spec).toBe("@grokbot/brave-plugin");
  });
