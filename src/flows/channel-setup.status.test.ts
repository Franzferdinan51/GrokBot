...
describe("resolveChannelSetupSelectionContributions", () => {
  ...
  it("sanitizes channel metadata before primer notes", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        'For multi-user DMs, isolate sessions with: grokbot config set session.dmScope "per-channel-peer" (or "per-account-channel-peer" for multi-account channels).',
        "Docs: https://docs.grokbot.ai/channels/pairing",
        "",
        "bad\\nid: Blurb\\nline",
      ].join("\n"),
      "How channels work",
  ...
  it("sanitizes channel metadata before selection notes", () => {
    ...
    expect(docsLink("/channels/zalo", "Docs")).toBe("https://docs.grokbot.ai/channels/zalo");
