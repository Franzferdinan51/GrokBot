// GrokBot CLI runner selects JSON, one-shot, or interactive setup-helper mode.
...
/** Options accepted by the GrokBot command runner. */
...
/** Run GrokBot in JSON, one-shot message, or interactive TUI mode. */
export async function runSystemAgent(
  ...
  if (boundOpts.message?.trim()) {
    ...
    const overview = await withProgress(
      {
        label: "Loading GrokBot overview…",
        indeterminate: true,
        delayMs: 0,
        fallback: "none",
      },
  ...
  if (!inputIsTty || !outputIsTty) {
    // Without a TTY, GrokBot cannot safely ask for confirmation; require --message instead.
