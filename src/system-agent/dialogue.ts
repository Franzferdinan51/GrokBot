// GrokBot dialogue parses direct commands and optionally asks the assistant planner.
...
/**
 * Dialogue helpers for turning user text into GrokBot operations.
 *
...
/** Resolve user input to an GrokBot operation, optionally using the assistant planner. */
...
function logAssistantPlan(
  ...
  runtime.log(`[grokbot] planner: ${modelLabel}`);
  if (plan.reply) {
    runtime.log(plan.reply);
  }
  runtime.log(`[grokbot] interpreted: ${plan.command}`);
}
