...
describe("avatar policy", () => {
  it("classifies avatar URI and path helpers directly", () => {
    ...
    expect(isWindowsAbsolutePath("C:\\\\avatars\\\\grokbot.png")).toBe(true);
  });
  ...
  it("detects avatar-like path strings", () => {
    expect(looksLikeAvatarPath("avatars/grokbot.svg")).toBe(true);
    expect(looksLikeAvatarPath("grokbot.webp")).toBe(true);
