...
This directory contains hooks that ship with GrokBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.grokbot/workspace`)
...
```bash
grokbot hooks enable bootstrap-extra-files
```
...
**Output**: `~/.grokbot/logs/commands.log`

**Enable**:

```bash
grokbot hooks enable command-logger
```
...
homepage: https://docs.grokbot.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.grokbot/hooks/` (shared across workspaces)
...
```bash
grokbot hooks enable session-memory
grokbot hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.grokbot/grokbot.json`:
...
2. Restart gateway: `pkill -9 -f 'grokbot.*gateway' && pnpm grokbot gateway`
3. Enable the hook: `grokbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.grokbot.ai/automation/hooks
