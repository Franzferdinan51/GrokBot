...
homepage: https://docs.grokbot.ai/automation/hooks#command-logger
metadata:
  {
    "grokbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.grokbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.grokbot/logs/commands.log ~/.grokbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/grokbot`:
   ```
   /home/username/.grokbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.grokbot/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.grokbot/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.grokbot/logs/commands.log | jq .
```
