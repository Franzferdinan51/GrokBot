...
const fixtureRoot = path.join(process.cwd(), ".tmp", `grokbot-hook-install-${randomUUID()}`);
...
const npmPackHooksBuffer = await createTarGzHookPackBuffer({
  packageName: "@grokbot/test-hooks",
  hookName: "one-hook",
...
function writeHookPackFiles(params: {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      'metadata: {"grokbot":{"events":["command:new"]}}',
      "---",
      "",
      `# ${params.heading}`,
    ].join("\n"),
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    grokbot: { hooks: [`./hooks/${params.hookName}`] },
  });
...
describe("installHooksFromPath", () => {
  ...
  ])("returns a stable code for $error", async ({ grokbot, error, code }) => {
    const pkgDir = makeTempDir();
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@grokbot/test-hooks", grokbot }),
    );
  ...
  it("blocks a staged single hook before publishing the target", async () => {
    ...
    expect(scanCall.packageDir).toContain(".grokbot-install-stage-");
  ...
  it.each([".codex-plugin/plugin.json", "hooks/hooks.json", "grokbot.plugin.json"])(
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "grokbot.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "grokbot.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
          ...
          expect(params.archiveInstallParams).toEqual(
            expect.objectContaining({
              installPolicyRequest: {
                kind: "plugin-npm",
                requestedSpecifier: "@grokbot/test-hooks@0.0.1",
                source: {
                  kind: "npm",
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.packageKind).toBe("hook-only");
    expect(result.npmResolution?.resolvedSpec).toBe("@grokbot/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@grokbot/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@grokbot/test-hooks@0.0.2-beta.1",
      name: "@grokbot/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@grokbot/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@grokbot/test-hooks@beta"');
    }
  });
});
