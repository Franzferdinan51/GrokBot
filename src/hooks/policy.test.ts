...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "grokbot-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "grokbot-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "grokbot-bundled");
      const plugin = makeHookEntry("shared", "grokbot-plugin");
      const managed = makeHookEntry("shared", "grokbot-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("grokbot-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "grokbot-managed");
      const workspace = makeHookEntry("shared", "grokbot-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "grokbot-workspace");
      const second = makeHookEntry("shared", "grokbot-workspace");
      second.hook.handlerPath = "/tmp/grokbot-workspace/shared/handler-2.js";
