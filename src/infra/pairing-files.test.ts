...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/grokbot-state", "devices")).toEqual({
      dir: path.join("/tmp/grokbot-state", "devices"),
      pendingPath: path.join("/tmp/grokbot-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/grokbot-state", "devices", "paired.json"),
    });
  });
