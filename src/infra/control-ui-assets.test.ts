...
let openclawRoot: typeof import("./grokbot-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openclawRoot = await import("./grokbot-root.js");
  });
  ...
  it("keeps a truncated build failure diagnostic within its UTF-16 limit", async () => {
    ...
    setFile(path.join(root, "package.json"), '{"name":"grokbot"}\n');
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/grokbot-bundle");
    (
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/GrokBot.app/Contents/MacOS/GrokBot");
    const bundledUiDir = abs("fixtures/GrokBot.app/Contents/Resources/control-ui");
  ...
  it("resolves control-ui root for symlinked argv1 via realpath", () => {
    const pkgRoot = abs("fixtures/bun-global/grokbot");
    const wrapperArgv1 = abs("fixtures/bin/grokbot");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/grokbot-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
