...
const TEMP_DIR_PREFIX = "grokbot-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("grokbot-install-source-utils-");
  const archivePath = path.join(cwd, "grokbot-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("grokbot-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-grokbot-archive.tgz",
...
describe("resolveNpmSpecMetadata", () => {
  ...
  it.each([
    ...
    const result = await resolveNpmSpecMetadata({ spec: "@grokbot/codex" });
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "@grokbot/codex",
        version: "2026.6.11",
        resolvedSpec: "@grokbot/codex@2026.6.11",
  ...
  it("selects the newest multi-version entry satisfying the requested range", async () => {
    ...
    await expect(resolveNpmSpecMetadata({ spec: "@grokbot/codex@^2026.6.0" })).resolves.toEqual({
      ok: true,
  ...
  it("uses the last multi-version entry when the selector is not a semver range", async () => {
    ...
    const result = await resolveNpmSpecMetadata({ spec: "@grokbot/codex@latest" });
  ...
  it("accepts metadata without an grokbot block", async () => {
    const { grokbot: _openclaw, ...withoutOpenClaw } = npmViewMetadata;
  ...
  it("reports which required metadata fields are missing", async () => {
    ...
    await expect(resolveNpmSpecMetadata({ spec: "@grokbot/codex" })).resolves.toEqual({
      ok: false,
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "grokbot-plugin@1.2.3",
          name: "grokbot-plugin",
          version: "1.2.3",
          filename: "grokbot-plugin-1.2.3.tgz",
    ...
    const result = await runPack("grokbot-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "grokbot-plugin",
        version: "1.2.3",
        resolvedSpec: "grokbot-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "grokbot-plugin@1.2.3", "--ignore-scripts", "--json"],
      {
        cwd,
        timeoutMs: 300_000,
        env: {
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "grokbot-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\ngrokbot-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"grokbot-plugin@1.2.3","name":"grokbot-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "grokbot-plugin",
        version: "1.2.3",
        resolvedSpec: "grokbot-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@grokbot/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@grokbot/whatsapp", cwd);
    expectPackError(result, [
      "Package not found on npm",
      "@grokbot/whatsapp",
      "docs.grokbot.ai/tools/plugin",
    ]);
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("grokbot-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "grokbot-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@grokbot/plugin-demo@2.0.0",
            filename: "grokbot-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@grokbot/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "grokbot-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@grokbot/plugin-demo@2.0.0",
      },
    });
  });
