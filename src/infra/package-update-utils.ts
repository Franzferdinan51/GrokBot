...
import { readRootJsonObjectSync } from "@grokbot/fs-safe/json";
...
/** Return true when an installed package needs an grokbot peer link repair. */
export function installedPackageNeedsOpenClawPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "grokbot"));
    return false;
  } catch (error) {
