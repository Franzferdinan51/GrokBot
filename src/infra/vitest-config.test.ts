    ...
    expect(
      externalPatterns.some(
        (pattern) =>
          ...
          pattern.test("/tmp/grokbot-pnpm-node-modules/some-dep/dist/index.mjs"),
      ),
    ).toBe(true);
    expect(
      externalPatterns.some(
        (pattern) =>
          pattern instanceof RegExp &&
          pattern.test("/tmp/grokbot-pnpm-node-modules/vite/dist/client/env.mjs"),
      ),
    ).toBe(false);
  });
});
