...
// update.
//
// `runGatewayUpdate` (git mode) runs `grokbot doctor --fix` with
...
// `shouldDeferConfiguredPluginInstallRepair`). The `grokbot update` CLI resumes
...
// binary's hidden `grokbot update finalize` entrypoint — the designed
...
// re-enters the full `grokbot update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-update-post-core-"));
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "grokbot update finalize",
