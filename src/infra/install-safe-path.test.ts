...
const canCreateDirectorySymlinks = (() => {
  ...
  try {
    probeDir = fsSync.mkdtempSync(
      path.join(os.tmpdir(), "grokbot-install-safe-dir-symlink-probe-"),
    );
...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@grokbot/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@grokbot/matrix", id: "matrix", expected: true },
    { packageName: "@grokbot/matrix", id: "@grokbot/matrix", expected: true },
    { packageName: "@grokbot/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@grokbot/matrix", id: " ", expected: false },
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "grokbot-install-safe-" }, async (baseDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "grokbot-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "grokbot-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "grokbot-install-safe-" }, async (parentDir) => {
