...
import { isRecord as isObjectRecord } from "@grokbot/normalization-core/record-coerce";
...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".grokbot-install-hidden-npmrc-";
...
export async function installPackageDir(params: {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".grokbot-install-stage-"));
  ...
  if (params.mode === "update" && (await pathExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".grokbot-install-backups");
