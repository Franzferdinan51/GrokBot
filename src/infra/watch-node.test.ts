...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "grokbot.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTempDir({ prefix: "grokbot-watch-node-" }, async (cwd) => {
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
    const spawnEnv = requireSpawnEnv(spawn, 0);
    expect(spawnEnv.LAUNCH_JOB_LABEL).toBe("ai.grokbot.gateway");
    expect(spawnEnv.OPENCLAW_NO_RESPAWN).toBe("1");
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/grokbot/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/grokbot/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        [""],
        [
          "[grokbot] gateway:watch could not start because a dependency package config looks corrupted.",
        ],
        ["[grokbot] Invalid package config: /tmp/grokbot/.pnpm/chokidar/package.json"],
        ["[grokbot] This usually means a file in node_modules is empty or truncated."],
        ["[grokbot] Recommended recovery:"],
        ["[grokbot]   rm -rf node_modules"],
        ["[grokbot]   pnpm store prune"],
        ["[grokbot]   pnpm install"],
        [""],
        ["[grokbot] Original error:"],
        [error],
      ]);
    } finally {
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTempDir({ prefix: "grokbot-watch-node-lock-" }, async (cwd) => {
