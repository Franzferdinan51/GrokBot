...
describe("package dist inventory", () => {
  it("tracks missing and stale dist files", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-" }, async (packageRoot) => {
  ...
  it("keeps the pending install guard outside the expected inventory", async () => {
    await withTempDir({ prefix: "grokbot-dist-install-guard-" }, async (packageRoot) => {
  ...
  it("keeps npm-omitted dist artifacts out of the inventory", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-pack-" }, async (packageRoot) => {
  ...
  it("honors package files exclusions when writing the dist inventory", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-package-files-" }, async (packageRoot) => {
      ...
      const omittedAppBundle = path.join(packageRoot, "dist", "GrokBot.app");
  ...
  it("keeps transient plugin dependency trees out of the inventory", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-plugin-deps-" }, async (packageRoot) => {
  ...
  it("omits packaged extension node_modules while keeping extension runtime files", async () => {
    await withTempDir(
      { prefix: "grokbot-dist-inventory-extension-node-modules-" },
      async (packageRoot) => {
  ...
  it("keeps publishable externalized bundled plugin dist trees out of the inventory", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-externalized-" }, async (packageRoot) => {
  ...
  it("keeps publishable core-package runtime plugin dist trees in the inventory", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-core-runtime-" }, async (packageRoot) => {
  ...
  it("matches install-stage paths case-insensitively across path segments", () => {
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/brave/.grokbot-install-stage/node_modules/typebox/package.json",
      ),
    ).toBe(true);
    expect(
      isLegacyPluginDependencyInstallStagePath(
    ...
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/browser/.grokbot-runtime-deps-copy-AbC123/package.json",
      ),
    ).toBe(false);
    expect(
      isLegacyPluginDependencyInstallStagePath("dist/extensions/.grokbot-install-stage"),
    ).toBe(false);
  });

  it("rejects pre-populated install-stage debris before writing an inventory", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-stage-" }, async (packageRoot) => {
      for (const relativePath of [
        "dist/extensions/brave/.grokbot-install-stage/package.json",
        "dist/extensions/browser/.grokbot-install-stage-AbC123/node_modules/playwright-core/package.json",
      ]) {
  ...
  it("rejects mixed-case install-stage debris on case-sensitive builders", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-stage-case-" }, async (packageRoot) => {
  ...
  it("returns null when the inventory is missing", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-missing-" }, async (packageRoot) => {
  ...
  it("rejects symlinked dist entries", async () => {
    await withTempDir({ prefix: "grokbot-dist-inventory-symlink-" }, async (packageRoot) => {
