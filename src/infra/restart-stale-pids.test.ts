...
function createOpenClawBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "grokbot-gateway" }]),
    ...overrides,
  });
}
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses grokbot-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "grokbot-gateway" },
          { pid: process.pid, cmd: "grokbot-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of grokbot", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/grokbot/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: grokbot-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "grokbot-gateway" },
          { pid: unrelatedStalePid, cmd: "grokbot-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "grokbot-gateway" },
            { pid: grandparentPid, cmd: "grokbot-gateway" },
            { pid: benignStalePid, cmd: "grokbot-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "grokbot-gateway" },
            { pid: gatewayGrandparentPid, cmd: "grokbot-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "grokbot-gateway" },
              { pid: gatewayGrandparentPid, cmd: "grokbot-gateway" },
              { pid: benignStalePid, cmd: "grokbot-gateway" },
            ]),
            stderr: "",
    ...
    it("excludes pids whose command does not include 'grokbot'", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "grokbot-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["grokbot", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple grokbot pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "grokbot-gateway" },
          { pid: pid2, cmd: "grokbot-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-grokbot pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks grokbot processes — non-grokbot listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty grokbot stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty grokbot stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + grokbot pid in stdout — container-restricted lsof reports partial results
          return createOpenClawBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "grokbot-gateway" }]),
          stderr: "",
        };
      });
    ...
    it("continues cleanup and port polling when individual process signals fail", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: termDeniedPid, cmd: "grokbot-gateway" },
                { pid: killDeniedPid, cmd: "grokbot-gateway" },
              ]),
            })
          : createLsofResult({ status: 1 });
      });
    ...
    it("does not kill a protected gateway pid after reparenting", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: protectedPid, cmd: "grokbot-gateway" },
                { pid: stalePid, cmd: "grokbot-gateway" },
              ]),
            })
          : createLsofResult({ status: 1 });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'grokbot'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("grokbot")
      // inside the mid-loop flush: a non-grokbot cmd between two entries must not
      // be pushed, but the following grokbot entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-grokbot entry first, then grokbot entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — parse false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid grokbot entry must still be found.
      const stdout = `p0\ncgrokbot-gateway\np${stalePid}\ncgrokbot-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  describe("pollPortOnce — status 1 + non-empty stdout", () => {
    it("keeps polling when status 1 includes a non-grokbot listener record", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return lsofCall === 2
              ? createLsofResult({
                  stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "grokbot-gateway" }]),
                })
              : createLsofResult({ status: 1 });
          }
          return createLsofResult();
