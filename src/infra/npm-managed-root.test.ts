...
const fixtureRootTracker = createSuiteTempRootTracker({
  prefix: "grokbot-npm-managed-root-",
});
...
describe("managed npm root", () => {
  ...
  it("syncs GrokBot-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
    ...
    expect(
      resolveManagedNpmRootDependencySpec({
        ...
        resolution: {
          name: "@grokbot/discord",
          version: "2026.5.2",
          resolvedSpec: "@grokbot/discord@2026.5.2",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@grokbot/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@grokbot/discord/-/discord-2026.5.2.tgz",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for grokbot@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
  ...
  it("repairs stale managed grokbot peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    await fs.mkdir(path.join(npmRoot, "node_modules", "grokbot"), { recursive: true });
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
        {
    ...
    await fs.writeFile(
      path.join(npmRoot, "node_modules", "grokbot", "package.json"),
      `${JSON.stringify({ name: "grokbot", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "grokbot"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "grokbot.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "grokbot.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    expect(lockfile.packages?.["node_modules/@grokbot/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.grokbot).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "grokbot"));
    for (const binName of ["grokbot", "grokbot.cmd", "grokbot.ps1"]) {
  ...
  it("does not repair the active GrokBot host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/grokbot-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "grokbot", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "grokbot"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "grokbot"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "grokbot.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "grokbot.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/grokbot-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "grokbot"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/grokbot"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/grokbot-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.grokbot).toBeUndefined();
    for (const binName of ["grokbot", "grokbot.cmd", "grokbot.ps1"]) {
