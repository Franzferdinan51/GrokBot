...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*grokbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "grokbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "grokbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "grokbot-gw");
  ...
  records.push(`_grokbot-gw._tcp IN PTR ${instanceLabel}._grokbot-gw._tcp`);
  records.push(`${instanceLabel}._grokbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._grokbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; grokbot-content-hash: ${contentHash}\n${contentBody}`;
}
