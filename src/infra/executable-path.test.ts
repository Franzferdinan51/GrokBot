...
describe("executable path helpers", () => {
  it("detects regular files and rejects directories", async () => {
    await withTempDir({ prefix: "grokbot-exec-path-" }, async (base) => {
  ...
  it("does not treat drive-less rooted windows paths as cwd-relative executables", () => {
    ...
    expect(
      resolveExecutablePath(String.raw`:\Users\demo\AI\system\grokbot\git.exe`, {
        cwd: String.raw`C:\Users\demo\AI\system\grokbot`,
      }),
    ).toBeUndefined();
    expect(
      resolveExecutablePath(String.raw`:/Users/demo/AI/system/grokbot/git.exe`, {
