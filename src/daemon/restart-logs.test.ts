...
describe("restart log conventions", () => {
  it("resolves profile-aware gateway logs and restart attempts together", () => {
    ...
    expect(resolveGatewayLogPaths(env)).toEqual({
      logDir: "/Users/test/.grokbot-work/logs",
      stdoutPath: "/Users/test/.grokbot-work/logs/gateway.log",
      stderrPath: "/Users/test/.grokbot-work/logs/gateway.err.log",
    });
    expect(resolveGatewayRestartLogPath(env)).toBe(
      "/Users/test/.grokbot-work/logs/gateway-restart.log",
    );
  });
  ...
  it("honors OPENCLAW_STATE_DIR for restart attempts", () => {
    ...
    expect(resolveGatewayRestartLogPath(env)).toBe("/tmp/grokbot-state/logs/gateway-restart.log");
  });

  it("keeps macOS LaunchAgent stdout outside the state directory", () => {
    const env = {
      HOME: "/Users/test",
      OPENCLAW_STATE_DIR: "/Volumes/External/grokbot",
    };
    ...
    expect(resolveGatewayRestartLogPath(env)).toBe(
      "/Volumes/External/grokbot/logs/gateway-restart.log",
    );
  });
  ...
  it("keeps macOS LaunchAgent logs profile-aware in the shared user log directory", () => {
    ...
    expect(resolveGatewaySupervisorLogPaths(env, { platform: "darwin" })).toEqual({
      logDir: "/Users/test/Library/Logs/grokbot",
      stdoutPath: "/Users/test/Library/Logs/grokbot/gateway-work.log",
      stderrPath: "/Users/test/Library/Logs/grokbot/gateway-work.err.log",
    });
  });
  ...
  it("renders best-effort POSIX log setup with escaped paths", () => {
    ...
    expect(setup).toContain(
      "if mkdir -p '/Users/test'\\''s/.grokbot/logs' 2>/dev/null && : >>'/Users/test'\\''s/.grokbot/logs/gateway-restart.log' 2>/dev/null; then",
    );
    expect(setup).toContain("exec >>'/Users/test'\\''s/.grokbot/logs/gateway-restart.log' 2>&1");
  });
  ...
  it("renders CMD log setup with quoted paths", () => {
    ...
    expect(setup.quotedLogPath).toBe('"C:\\Users\\Test User/.grokbot/logs/gateway-restart.log"');
    expect(setup.lines).toContain(
      'if not exist "C:\\Users\\Test User/.grokbot/logs" mkdir "C:\\Users\\Test User/.grokbot/logs" >nul 2>&1',
    );
  });

  it("appends a profile-aware lifecycle audit line with stable key-value fields", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-lifecycle-audit-"));
    ...
    expect(line).toMatch(/^\[[^\]]+\] grokbot gateway lifecycle /);
  ...
  it("does not throw when lifecycle audit logging fails", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-lifecycle-audit-fail-"));
    tempDirs.push(stateDir);
