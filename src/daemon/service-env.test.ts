...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENCLAW_SERVICE_MARKER).toBe("grokbot");
    expect(env.OPENCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENCLAW_SERVICE_VERSION).toBe("string");
    expect(env.OPENCLAW_SYSTEMD_UNIT).toBe("grokbot-gateway.service");
    expect(env.OPENCLAW_WINDOWS_TASK_NAME).toBe("GrokBot Gateway");
    expect(env.OPENCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    if (process.platform === "darwin") {
      expect(env.OPENCLAW_LAUNCHD_LABEL).toBe("ai.grokbot.gateway");
    }
  });

  it("sets the GrokBot-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through OPENCLAW_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPENCLAW_WRAPPER: " /usr/local/bin/grokbot-doppler ",
      },
      port: 18789,
    });

    expect(env.OPENCLAW_WRAPPER).toBe("/usr/local/bin/grokbot-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".grokbot", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.OPENCLAW_SYSTEMD_UNIT).toBe("grokbot-gateway-work.service");
    expect(env.OPENCLAW_WINDOWS_TASK_NAME).toBe("GrokBot Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.OPENCLAW_LAUNCHD_LABEL).toBe("ai.grokbot.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        OPENCLAW_SYSTEMD_UNIT: "grokbot-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.OPENCLAW_SYSTEMD_UNIT).toBe("grokbot-gateway-maintenance.service");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the GrokBot-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.OPENCLAW_LAUNCHD_LABEL).toBe("ai.grokbot.node");
  });
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.OPENCLAW_WINDOWS_TASK_NAME).toBe("GrokBot Node");
    expect(env.OPENCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".grokbot"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".grokbot-rescue"));
  });
  ...
  it("uses OPENCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENCLAW_STATE_DIR: "/var/lib/grokbot" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/grokbot"));
  });

  it("expands ~ in OPENCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENCLAW_STATE_DIR: "~/grokbot-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/grokbot-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENCLAW_STATE_DIR: "C:\\State\\grokbot" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\grokbot");
  });
});
