...
function resolveStartupEntryPath(env: Record<string, string>, extension = "cmd") {
  const taskName = env.OPENCLAW_WINDOWS_TASK_NAME ?? "GrokBot Gateway";
  return path.join(
...
async function writeNodeScript(env: Record<string, string>, port = "18789") {
  ...
  await fs.writeFile(
    ...
    [
      ...
      `"C:\\bin\\grokbot.cmd" node run --host 127.0.0.1 --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
...
function makeNodeServiceEnv(env: Record<string, string>): Record<string, string> {
  return {
    ...
    OPENCLAW_WINDOWS_TASK_NAME: "GrokBot Node",
  };
}
        ...
        stdout: JSON.stringify(
          processAlive
            ? [
                {
                  ProcessId: processId,
                  CommandLine: "C:\\bin\\grokbot.cmd node run --host 127.0.0.1 --port 18789",
                },
...
function installNodeScheduledTask(env: Record<string, string>, stdout = new PassThrough()) {
  return installScheduledTask({
    ...
    stdout,
    programArguments: ["node", "grokbot", "node", "run", "--host", "127.0.0.1", "--port", "18789"],
    environment: {
      OPENCLAW_SERVICE_KIND: "node",
...
describe("Windows startup fallback", () => {
  it("skips task ownership probes when no Startup fallback exists", async () => {
    await withWindowsEnv("grokbot-win-startup-", async ({ env }) => {
    ...
    await withWindowsEnv("grokbot-win-startup-", async ({ env }) => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          '"C:\\bin\\grokbot-doppler.exe" gateway --port 18789',
          "",
        ].join("\r\n"),
        "utf8",
      );
            ...
            stdout: JSON.stringify(
              processAlive
                ? [
                    {
                      ProcessId: 4242,
                      CommandLine: '"C:\\bin\\grokbot-doppler.exe" gateway --port 18789',
                    },
                    { ProcessId: 9999, CommandLine: "powershell.exe" },
                  ]
                : [{ ProcessId: 9999, CommandLine: "powershell.exe" }],
        ...
        if (
          ...
          return makeSpawnSyncResult({
            stdout: JSON.stringify([
              {
                ProcessId: 3131,
                CommandLine: "C:\\manual\\grokbot.cmd gateway --port 18789",
              },
              {
                ProcessId: 4242,
                CommandLine:
            ...
            ? [
                {
                  pid: 4242,
                  command: "node.exe",
                  commandLine: 'node "C:\\grokbot\\dist\\index.js" gateway --port 18789',
                },
              ]
            : [],
        hints: [],
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 5151,
                CommandLine: "node grokbot node run --host 127.0.0.1 --port 18789",
              },
            ]),
            stderr: "",
            status: 0,
  ...
  it("does not report a node task as running from a gateway listener", async () => {
    await withWindowsEnv("grokbot-win-startup-", async ({ env }) => {
      ...
      env.OPENCLAW_WINDOWS_TASK_NAME = "GrokBot Node";
      findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4242]);
      schtasksResponses.push(
        { code: 0, stdout: "", stderr: "" },
        { code: 0, stdout: notYetRunTaskQueryOutput(), stderr: "" },
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 4242,
                CommandLine: "C:\\manual\\grokbot.cmd node run --host 127.0.0.1 --port 18789",
              },
              {
                ProcessId: 5151,
                CommandLine: "C:\\bin\\grokbot.cmd node run --host 127.0.0.1 --port 18789",
