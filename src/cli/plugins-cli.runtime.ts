// Runtime implementations for `grokbot plugins` subcommands. Heavy plugin modules stay
...
function formatBlockedRuntimePluginGuidance(params: {
  ...
  const alternative =
    pluginId === "acpx"
      ...
      : 'change the runtime policy to "grokbot"';
...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "grokbot doctor --fix" to install ${installSpec}, or install it manually with "grokbot plugins install ${installSpec}".`,
    ];
  });
}
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(`${theme.muted("Repair:")} ${theme.command("grokbot plugins registry --refresh")}`);
  }
...
export async function runPluginsDoctorCommand(): Promise<void> {
  ...
  if (shadowed.length > 0) {
    ...
    for (const diag of shadowed) {
      ...
      lines.push("  repair:");
      lines.push("    grokbot plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
      lines.push("    edit or remove the config-selected plugin source");
      lines.push("    grokbot plugins registry --refresh");
      lines.push("    grokbot gateway restart --force");
    }
  }
  ...
  const docs = formatDocsLink("/plugin", "docs.grokbot.ai/plugin");
  lines.push("");
...
/** List entries from the configured GrokBot marketplace feed. */
...
/** Refresh the configured GrokBot marketplace feed snapshot. */
