...
describe("cli program (nodes basics)", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        ...
        "grokbot nodes approve request-approval",
      ],
    },
    {
      ...
      expectedOutput: [
        ...
        "grokbot nodes approve request-reapproval",
      ],
    },
  ...
  it("runs nodes describe and calls node.describe", async () => {
    ...
    expect(out).toContain("grokbot nodes approve request-approval");
  ...
  it("keeps explicit gateway options in node reapproval guidance without leaking auth", async () => {
    ...
    const output = getRuntimeOutput();
    expect(output).toContain("grokbot nodes approve request-reapproval --timeout 3000");
  ...
  it("does not recommend approval from a stale pending request id alone", async () => {
    ...
    expect(output).not.toContain("grokbot nodes approve stale-request");
