...
export function registerNodeCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "grokbot node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["grokbot node status", "Check node host service status."],
          ["grokbot node install", "Install the node host service."],
          ["grokbot node start", "Start the installed node host service."],
          ["grokbot node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.grokbot.ai/cli/node")}\n`,
    );
  ...
  node
    ...
    .option("--context-path <path>", "Gateway WebSocket context path (e.g. /grokbot-gw)")
