...
const DEFAULT_CLI_NAME = "grokbot";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(grokbot)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `grokbot`. */
...
/** Replace a leading `grokbot` command prefix with the active CLI name. */
