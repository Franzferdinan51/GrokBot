...
const SANDBOX_EXAMPLES = {
  main: [
    ["grokbot sandbox list", "List all sandbox containers."],
    ["grokbot sandbox list --browser", "List only browser containers."],
    ["grokbot sandbox recreate --all", "Recreate all containers."],
    ["grokbot sandbox recreate --session main", "Recreate a specific session."],
    ["grokbot sandbox recreate --agent mybot", "Recreate agent containers."],
    ["grokbot sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["grokbot sandbox list", "List all sandbox containers."],
    ["grokbot sandbox list --browser", "List only browser containers."],
    ["grokbot sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["grokbot sandbox recreate --all", "Recreate all containers."],
    ["grokbot sandbox recreate --session main", "Recreate a specific session."],
    ["grokbot sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["grokbot sandbox recreate --browser --all", "Recreate only browser containers."],
    ["grokbot sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["grokbot sandbox explain", "Show effective sandbox config."],
    ["grokbot sandbox explain --session agent:main:main", "Explain a specific session."],
    ["grokbot sandbox explain --agent work", "Explain an agent sandbox."],
    ["grokbot sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.grokbot.ai/cli/sandbox")}\n`,
    )
    .action(() => {
