...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# GrokBot Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# GrokBot Completion\n${sourceLine}`;
  const next = trimmed ? `${trimmed}\n\n${block}\n` : `${block}\n`;
  return { next, changed: next !== content, hadExisting };
}

/** Resolves the shell startup profile path that should contain the GrokBot completion block. */
...
/** Returns whether a shell profile already contains an GrokBot completion block or source line. */
 ...
 * Returns true if profile has `source <(grokbot completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "grokbot") {
