...
export function writeClaudeMcpConfig(mcpConfig: AttachGrant["mcpConfig"]): {
  ...
  const dir = mkdtempSync(join(tmpdir(), "grokbot-attach-"));
...
export async function registerAttachCli(program: Command, _argv: string[] = process.argv) {
  program
    ...
    .addHelpText(
      "after",
      "\nExamples:\n  grokbot attach                       Attach Claude Code to the main session\n  grokbot attach --session agent:main:telegram:123 --ttl 600000\n  grokbot attach --print-config        Set up the grant + config and print how to launch it yourself\n",
    )
