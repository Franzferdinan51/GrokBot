...
const STEP_LABELS: Record<string, string> = {
  ...
  "grokbot doctor entry": "Checking doctor entrypoint",
  "grokbot doctor": "Running doctor checks",
...
function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g grokbot@latest");
    hints.push(
      "System install outline: grokbot gateway stop -> sudo <system-npm> i -g grokbot@latest -> grokbot gateway install --force -> grokbot gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g grokbot@latest --omit=optional");
  }
