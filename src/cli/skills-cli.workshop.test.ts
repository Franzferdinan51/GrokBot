...
vi.mock("../terminal/links.js", () => ({
  formatDocsLink: () => "docs.grokbot.ai/cli/skills",
}));
...
describe("skills workshop cli", () => {
  ...
  beforeEach(async () => {
    testState = await createOpenClawTestState({
      layout: "state-only",
      prefix: "grokbot-skills-cli-workshop-state-",
    });
    mocks.workspaceDir = await tempDirs.make("grokbot-skills-cli-workshop-");
  ...
  it("scopes list and inspect to the selected workspace", async () => {
    ...
    mocks.workspaceDir = await tempDirs.make("grokbot-skills-cli-workshop-second-");
    await runCommand(["skills", "workshop", "list"]);
