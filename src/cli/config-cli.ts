...
import { readByteStreamWithLimit } from "@grokbot/media-core/read-byte-stream-with-limit";
...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "grokbot config set secrets.providers.vault --provider-source file --provider-path /etc/grokbot/secrets.json --provider-mode json",
);
...
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("grokbot config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("grokbot doctor --fix")}\` ${message}`;
}
...
function formatConfigUnsetMissingPathMessage(params: {
  ...
  if (params.runtimeOnly) {
    return `Config path not found in authored config: ${params.path}. It only exists after runtime defaults are applied, so there is nothing for config unset to remove. Use ${formatCliCommand("grokbot config set <path> <value>")} to override the inherited value.`;
  }
  return `Config path not found: ${params.path}. Nothing was changed. Run ${formatCliCommand("grokbot config get <path>")} first if you are unsure of the path.`;
}
...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`GrokBot config is invalid: ${shortenHomePath(snapshot.path)}`);
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("grokbot plugins install <spec>")}`,
    `  ${formatCliCommand("grokbot plugins update <plugin-id>")}`,
    `  ${formatCliCommand("grokbot plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
function formatAutoManagedMetaError(paths: readonly PathSegment[][]): string {
  ...
  return [
    `${subject} is auto-managed by GrokBot and cannot be edited; the value would be overwritten on the next config write.`,
    "",
    "These fields are stamped on every config write to record the GrokBot version and timestamp that produced the file.",
  ].join("\n");
}
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      ...
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("grokbot config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "grokbot.json";
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("grokbot onboard")} or run ${formatCliCommand("grokbot doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(danger(`GrokBot config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("grokbot config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.grokbot.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing grokbot.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing grokbot.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for grokbot.json")
    .action(async () => {
