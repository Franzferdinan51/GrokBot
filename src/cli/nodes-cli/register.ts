    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["grokbot nodes status", "List known nodes with live status."],
          ["grokbot nodes pairing pending", "Show pending node pairing requests."],
          ["grokbot nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'grokbot nodes invoke --node <id> --command system.which --params \'{"bins":["uname"]}\'',
            "Invoke a node command directly.",
          ],
          ["grokbot nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.grokbot.ai/cli/nodes")}\n`,
    );
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `grokbot nodes` listing should still surface plugin-provided subcommands.
    return true;
  }
