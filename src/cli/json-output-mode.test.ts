...
describe("json output mode", () => {
  ...
  it("detects json output flags before argv terminators", () => {
    expect(hasJsonOutputFlag(["node", "grokbot", "nodes", "list", "--json"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "grokbot", "nodes", "list", "--json=true"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "grokbot", "nodes", "--", "--json"])).toBe(false);
  });
