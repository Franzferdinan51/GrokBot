...
async function signalGatewayRestart(
  ...
  if (pids.length > 1) {
    throw new Error(
      `multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "grokbot gateway status --deep" before retrying restart`,
    );
  }
    ...
    postRestartCheck: async ({ warnings, fail, stdout, warn }) => {
      if (restartedWithoutServiceManager) {
        ...
        fail(
          ...
          [formatCliCommand("grokbot gateway status --deep"), formatCliCommand("grokbot doctor")],
        );
