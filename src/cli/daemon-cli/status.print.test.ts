...
vi.mock("../../daemon/restart-logs.js", () => ({
  ...
  resolveGatewaySupervisorLogPaths: () => ({
    logDir: "/Users/test/Library/Logs/grokbot",
    stdoutPath: "/Users/test/Library/Logs/grokbot/gateway.log",
    stderrPath: "/Users/test/Library/Logs/grokbot/gateway.err.log",
  }),
...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expectMockLineContains(runtime.error, formatCliCommand("grokbot gateway restart"));
  });
  ...
  it("prints established gateway client guidance gathered by deep status", () => {
          ...
          established: [
            {
              ...
              ppid: 1,
              command: "node",
              commandLine: "/tmp/newer-grokbot/bin/grokbot logs --follow",
    ...
    expectMockLineContains(runtime.log, "newer-grokbot");
  ...
  it("prints stale updater launchd job guidance", () => {
    ...
    expectMockLineContains(runtime.error, "Stale GrokBot updater launchd job(s) detected.");
    expectMockLineContains(runtime.error, "ai.grokbot.update.2026.5.12");
    expectMockLineContains(runtime.error, "ai.grokbot.manual-update.1717168800");
  ...
  it("prints macOS launchd stdout and suppressed stderr when gateway is not listening", () => {
    ...
    expectMockLineContains(runtime.error, "/Users/test/Library/Logs/grokbot/gateway.log");
  ...
  it("prints CLI and gateway versions with readable guidance when they differ", () => {
    printDaemonStatus(
      {
        cli: {
          version: "2026.4.23",
          entrypoint: "/usr/local/bin/grokbot",
        },
        service: {
          label: "LaunchAgent",
          loaded: true,
    ...
    expectMockLineContains(runtime.log, "CLI version: 2026.4.23 (/usr/local/bin/grokbot)");
    expectMockLineContains(runtime.log, "Gateway version: 2026.5.6");
    expectMockLineContains(runtime.error, "this GrokBot command is version 2026.4.23");
    expectMockLineContains(
      runtime.error,
      "if this mismatch is unexpected, update PATH so `grokbot` points to the version you want",
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/grokbot-cli/grokbot.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/grokbot-daemon/grokbot.json",
            exists: true,
            valid: true,
    ...
    printDaemonStatus(
      {
        ...
        extraServices: [{ label: "ai.grokbot.gateway.rescue", scope: "user", detail: "loaded" }],
      },
      { json: false },
    );

    expectMockLineContains(runtime.log, "Other gateway-like services detected");
    expectMockLineContains(runtime.log, "ai.grokbot.gateway.rescue");
  ...
  it("prints a terse plugin drift warning outside deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "grokbot gateway status --deep");
  ...
  it("prints detailed plugin drift entries in deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "grokbot plugins update whatsapp");
    expectMockLineContains(runtime.log, "grokbot gateway restart");
  });
  ...
  it("prints exact package update commands for pinned npm plugin drift in deep mode", () => {
          ...
          drifts: [
            {
              ...
              gatewayVersion: "2026.6.10-beta.1",
              source: "npm",
              packageName: "@grokbot/brave-plugin",
              spec: "@grokbot/brave-plugin@2026.6.9",
            },
          ],
        },
        extraServices: [],
    ...
    expectMockLineContains(
      runtime.log,
      "grokbot plugins update @grokbot/brave-plugin@2026.6.10-beta.1",
    );
    expectMockLineContains(runtime.log, "grokbot gateway restart");
  });
