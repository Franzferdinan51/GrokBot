...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["grokbot status", "Show channel health + session summary."],
          ["grokbot status --all", "Full diagnosis (read-only)."],
          ["grokbot status --json", "Machine-readable output."],
          ["grokbot status --usage", "Show model provider usage/quota snapshots."],
          [
            "grokbot status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["grokbot status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.grokbot.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.grokbot.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["grokbot sessions", "List all sessions."],
          ["grokbot sessions --agent work", "List sessions for one agent."],
          ["grokbot sessions --all-agents", "Aggregate sessions across agents."],
          ["grokbot sessions --active 120", "Only last 2 hours."],
          ["grokbot sessions --limit 25", "Show the newest 25 sessions."],
          ["grokbot sessions --json", "Machine-readable output."],
          ["grokbot sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.grokbot.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["grokbot sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "grokbot sessions cleanup --dry-run --fix-missing",
          ...
          ["grokbot sessions cleanup --enforce", "Apply maintenance now."],
          ["grokbot sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["grokbot sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "grokbot sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .grokbot/trajectory-exports")
  ...
  sessionsCmd
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `grokbot sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["grokbot commitments", "List pending inferred follow-ups."],
          ["grokbot commitments --all", "List all inferred follow-ups."],
          ["grokbot commitments --agent work", "List one agent's inferred follow-ups."],
          ["grokbot commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
