...
function resolveHelpCommand(argv: string[] | undefined, options?: { root?: boolean }): string {
  if (options?.root || !argv) {
    return formatCliCommand("grokbot --help");
  }
  const commandPath = getCommandPathWithRootOptions(argv, 2);
  if (commandPath.length === 0) {
    return formatCliCommand("grokbot --help");
  }
  return formatCliCommand(`grokbot ${commandPath.join(" ")} --help`);
}
...
function formatDocsHint(): string {
  return `${theme.muted("Docs:")} ${formatDocsLink("/cli", "docs.grokbot.ai/cli")}`;
}

/** Convert Commander parse errors into GrokBot-specific help and docs guidance. */
export function formatCliParseErrorOutput(
  ...
  if (unknownCommand) {
    ...
    return lines(
      theme.error(`GrokBot does not know the command ${quote(command)}.`),
      formatCliCommandSuggestions(command),
      formatHelpHint(options.argv, { root: true }),
      `${theme.muted("Plugin command?")} ${theme.command(formatCliCommand("grokbot plugins list"))}`,
      formatDocsHint(),
    );
  }
  ...
  if (unknownOption) {
    ...
    return lines(
      theme.error(`GrokBot does not recognize option ${quote(option)}.`),
  ...
  return lines(
    theme.error(`GrokBot could not parse this command: ${message}`),
    formatHelpHint(options.argv),
  );
}
