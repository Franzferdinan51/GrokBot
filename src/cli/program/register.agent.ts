...
export function registerAgentsCommands(program: Command): void {
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.grokbot.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ['grokbot agents set-identity --agent main --name "GrokBot" --emoji "🦞"', "Set name + emoji."],
  ["grokbot agents set-identity --agent main --avatar avatars/grokbot.png", "Set avatar path."],
  [
    "grokbot agents set-identity --workspace ~/.grokbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "grokbot agents set-identity --identity-file ~/.grokbot/workspace/IDENTITY.md --agent main",
