// Commander wiring for `grokbot update`, its status/finalize subcommands, and help text.
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["grokbot update repair", "Rerun post-update doctor and plugin convergence."],
          ["grokbot update repair --channel beta", "Repair against the beta update channel."],
          ["grokbot update repair --json", "JSON output for automation."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.grokbot.ai/cli/update")}`,
    )
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["grokbot update", "Update a source checkout (git)"],
        [
          "grokbot update --channel extended-stable",
          "Switch to the monthly supported npm channel",
        ],
        ["grokbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["grokbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["grokbot update --tag beta", "One-off update to a dist-tag or version"],
        ["grokbot update --tag main", "One-off package update from GitHub main"],
        ["grokbot update --dry-run", "Preview actions without changing anything"],
        ["grokbot update --no-restart", "Update without restarting the service"],
        ["grokbot update --json", "Output result as JSON"],
        ["grokbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["grokbot update repair", "Repair stranded post-update plugin state"],
        ["grokbot update --acknowledge-clawhub-risk", "Acknowledge ClawHub plugin trust warnings"],
        ["grokbot update wizard", "Interactive update wizard"],
        ["grokbot --update", "Shorthand for grokbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run grokbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.grokbot.ai/cli/update")}`;
    })
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["grokbot update status", "Show channel + version status."],
          ["grokbot update status --json", "JSON output."],
          ["grokbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
