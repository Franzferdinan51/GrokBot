/**
 * Shared realtime voice controls for active GrokBot agent runs.
 *
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal GrokBot voice control result.",
    "Do not call openclaw_agent_consult or any other tool for this message.",
    "Speak this exact GrokBot status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "GrokBot is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "GrokBot has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "GrokBot could not queue that follow-up."
    : "GrokBot could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "GrokBot finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `GrokBot is starting ${name}.` : "GrokBot is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `GrokBot finished ${name} and is continuing.`
        : "GrokBot finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `GrokBot is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "GrokBot is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `GrokBot is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "GrokBot is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "GrokBot is working on the current voice request.";
  }
