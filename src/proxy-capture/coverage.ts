...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[grokbot proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[grokbot proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`grokbot proxy coverage\` for details.`,
  );
}
