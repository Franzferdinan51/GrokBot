...
describe("sessions.files RPC handlers", () => {
  ...
  beforeEach(() => {
    ...
    workspaceRoot = fs.mkdtempSync(path.join(tempRoot, "grokbot-session-files-test-"));
    ...
    writeWorkspaceFile(workspaceRoot, "package.json", '{"name":"grokbot-test"}\n');
  ...
  it("does not read absolute or parent-relative paths outside the configured workspace", async () => {
    const outsidePath = path.join(os.tmpdir(), `grokbot-outside-${Date.now()}.txt`);
  ...
  it("omits transcript paths outside the workspace without hiding missing workspace files", async () => {
    const outsidePath = path.join(os.tmpdir(), `grokbot-outside-list-${Date.now()}.txt`);
    fs.writeFileSync(outsidePath, "outside\n", "utf8");
    hoisted.visitSessionMessagesAsync.mockImplementation(async (_scope, visit) => {
      [
        outsidePath,
        "../outside.txt",
        "~/.grokbot-external.txt",
  ...
  it("does not follow workspace symlinks for file previews", async () => {
    const outsidePath = path.join(os.tmpdir(), `grokbot-linked-${Date.now()}.txt`);
  ...
  it("does not follow symlinked parent directories for file previews", async () => {
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-linked-parent-"));
  ...
  it("does not derive a workspace root from transcript cwd", async () => {
    ...
    const transcriptCwd = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-transcript-cwd-"));
  ...
  it("rejects escaped and symlinked write targets without touching outside files", async () => {
    const tempRoot = fs.realpathSync(os.tmpdir());
    const outsidePath = path.join(tempRoot, `grokbot-session-write-outside-${Date.now()}.txt`);
    const escapedName = `grokbot-session-write-escape-${Date.now()}.txt`;
