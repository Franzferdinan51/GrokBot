...
function emptySkillStatusReport() {
  return {
    ...
    managedSkillsDir: "/tmp/grokbot/skills",
    skills: [],
  };
}
...
function expectEmptySecurityVerdicts(response: unknown): void {
  expect(response).toEqual({
    schema: "grokbot.skills.security-verdicts.v1",
    items: [],
  });
}
...
describe("skills gateway handlers (clawhub)", () => {
      ...
      items: [
        {
          ...
          version: "1.2.3",
          securityAuditUrl:
            "https://clawhub.ai/grokbot/skills/agentreceipt/security-audit?version=1.2.3",
  ...
  it("loads local Skill Card content for a known installed skill", async () => {
    ...
    expect(response).toEqual({
      schema: "grokbot.skills.skill-card.v1",
      skillKey: "agentreceipt",
      path: "/tmp/workspace/skills/agentreceipt/skill-card.md",
      sizeBytes: 34,
  ...
  it("returns ClawHub skill update trust warnings in Gateway error details", async () => {
    updateSkillsFromClawHubMock.mockResolvedValue([
      {
        ...
        code: "clawhub_download_blocked",
        warning: "Latest skill version is marked malicious; GrokBot will not download it.",
      },
    ]);
    ...
    expect(error).toEqual({
      ...
      details: {
        ...
        warnings: ["Latest skill version is marked malicious; GrokBot will not download it."],
      },
    });
  });
