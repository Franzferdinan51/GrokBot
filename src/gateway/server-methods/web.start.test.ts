...
describe("webHandlers web.login.start", () => {
  ...
  it("surfaces the missing official external plugin hint when no web-login provider is loaded", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      repairHint:
        "Install the official external plugin with: grokbot plugins install clawhub:@grokbot/whatsapp, or run: grokbot doctor --fix.",
    });
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message:
          "web login provider is not available. Install the official external plugin with: grokbot plugins install clawhub:@grokbot/whatsapp, or run: grokbot doctor --fix.",
      }),
    );
  ...
  it("joins multiple missing official external plugin hints when more than one configured channel is missing", async () => {
        ...
        channelId === "whatsapp"
          ...
          : channelId === "signal"
            ? {
                ...
                channelId: "signal",
                label: "Signal",
                installSpec: "clawhub:@grokbot/signal",
                installCommand: "grokbot plugins install clawhub:@grokbot/signal",
                doctorFixCommand: "grokbot doctor --fix",
                repairHint:
                  "Install the official external plugin with: grokbot plugins install clawhub:@grokbot/signal, or run: grokbot doctor --fix.",
              }
            : null,
    );
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        code: "INVALID_REQUEST",
        message:
          "web login provider is not available. Configured official external channel plugins are missing for WhatsApp, Signal. Install them with: grokbot plugins install clawhub:@grokbot/whatsapp; grokbot plugins install clawhub:@grokbot/signal, or run: grokbot doctor --fix.",
      }),
    );
  });
