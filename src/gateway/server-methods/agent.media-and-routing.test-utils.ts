...
describe("gateway agent handler", () => {
  ...
  it("recovers a failed session when its default transcript exists", async () => {
    ...
    await withTempDir({ prefix: "grokbot-gateway-failed-default-session-file-" }, async (root) => {
  ...
  it.each([
    ...
    await withTempDir({ prefix: "grokbot-gateway-stale-failed-session-" }, async (root) => {
      const sessionsDir = `${root}/sessions`;
  ...
  it("recovers a failed session when its relative transcript resolves and exists", async () => {
    ...
    await withTempDir({ prefix: "grokbot-gateway-failed-session-file-" }, async (root) => {
      const sessionsDir = `${root}/sessions`;
      await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("recovers terminal failed agent API sessions without rotating the session id", async () => {
    ...
    await withTempDir({ prefix: "grokbot-gateway-terminal-recovery-" }, async (root) => {
      const sessionsDir = `${root}/sessions`;
      await fs.mkdir(sessionsDir, { recursive: true });
