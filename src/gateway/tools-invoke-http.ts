...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-grokbot-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-grokbot-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-grokbot-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-grokbot-thread-id"));
