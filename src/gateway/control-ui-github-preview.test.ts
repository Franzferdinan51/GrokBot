...
function previewPayload(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    base: { repo: { url: "https://api.github.com/repos/grokbot/grokbot" } },
    repository_url: "https://api.github.com/repos/grokbot/grokbot",
    user: {
...
describe("parseControlUiGitHubPreviewTarget", () => {
  it("accepts bounded GitHub issue and pull request targets", () => {
    ...
    expect(target).toEqual({ kind: "pull", number: 99816, owner: "grokbot", repo: "grokbot" });
    expect(
      parseControlUiGitHubPreviewTarget({
        kind: "issue",
        number: 1,
    ...
    for (const repo of [
      ...
      ).toEqual({ kind: "issue", number: 1, owner: "grokbot", repo });
    }
  });
...
describe("loadControlUiGitHubPreview", () => {
  ...
  it("normalizes public metadata and embeds a bounded GitHub avatar", async () => {
    ...
    const target = { kind: "pull" as const, number: 99816, owner: "grokbot", repo: "grokbot" };
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "https://api.github.com/repos/grokbot/grokbot/pulls/99816",
    );
  ...
  it.each([
    ...
    const preview = await loadControlUiGitHubPreview(
      { kind: "issue", number, owner: "grokbot", repo },
      fetchMock,
    );
  ...
  it("discards rejected avatar response bodies", async () => {
    ...
    const preview = await loadControlUiGitHubPreview(
      { kind: "pull", number: 70009, owner: "grokbot", repo: "bad-avatar" },
      fetchMock,
    );
  ...
  it("returns token-backed metadata only after public repository proofs", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        githubJson(
          previewPayload({
            repository_url: "https://api.github.com/repos/grokbot/public",
            user: { login: "octocat" },
          }),
        ),
      )
      .mockResolvedValueOnce(githubJson({ private: false }));
    const target = { kind: "issue" as const, number: 70003, owner: "grokbot", repo: "public" };

    await loadControlUiGitHubPreview(target, fetchMock);

    expect(fetchMock).toHaveBeenCalledTimes(3);
    expect(fetchMock.mock.calls[0]?.[0]).toBe("https://api.github.com/repos/grokbot/public");
    expect(fetchMock.mock.calls[1]?.[0]).toBe(
      "https://api.github.com/repos/grokbot/public/issues/70003",
    );
    expect(fetchMock.mock.calls[2]?.[0]).toBe("https://api.github.com/repos/grokbot/public");
  ...
  it("follows GitHub API redirects for renamed public repositories", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(null, {
          status: 301,
          headers: { Location: "/repos/grokbot/renamed/issues/70007" },
        }),
      )
      .mockResolvedValueOnce(githubJson({ private: false }))
      .mockResolvedValueOnce(
        githubJson(
          previewPayload({
            repository_url: "https://api.github.com/repos/grokbot/renamed",
            user: { login: "octocat" },
          }),
        ),
      )
    ...
    const preview = await loadControlUiGitHubPreview(
      { kind: "issue", number: 70007, owner: "grokbot", repo: "old-name" },
      fetchMock,
    );
    ...
    expect(requestUrl(fetchMock.mock.calls[3]?.[0])).toBe(
      "https://api.github.com/repos/grokbot/renamed/issues/70007",
    );
    expect(requestUrl(fetchMock.mock.calls[4]?.[0])).toBe(
      "https://api.github.com/repos/grokbot/renamed",
    );
    for (const call of fetchMock.mock.calls) {
  ...
  it("rejects cross-origin GitHub API redirects before forwarding credentials", async () => {
    ...
    const redirectResponse = new Response("discard me", {
      status: 301,
      headers: { Location: "https://example.com/repos/grokbot/private" },
    });
    const fetchMock = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(githubJson({ private: false }))
    ...
    await expect(
      loadControlUiGitHubPreview(
        { kind: "pull", number: 70008, owner: "grokbot", repo: "unsafe-redirect" },
        fetchMock,
      ),
  ...
  it("stops private and missing repositories before fetching item metadata", async () => {
    ...
    for (const [repo, number] of [
      ...
      await expect(
        loadControlUiGitHubPreview({ kind: "issue", number, owner: "grokbot", repo }, fetchMock),
    ...
    expect(fetchMock.mock.calls.map((call) => requestUrl(call[0]))).toEqual([
      "https://api.github.com/repos/grokbot/private",
      "https://api.github.com/repos/grokbot/missing",
    ]);
  });
  ...
  it("does not expose metadata transferred into a private repository", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(null, {
          status: 301,
          headers: { Location: "/repos/grokbot/private/issues/70004" },
        }),
      )
      .mockResolvedValueOnce(githubJson({ private: true }));
    ...
    await expect(
      loadControlUiGitHubPreview(
        { kind: "issue", number: 70004, owner: "grokbot", repo: "public-source" },
        fetchMock,
      ),
    ).rejects.toMatchObject({ statusCode: 404 } satisfies Partial<ControlUiGitHubError>);
    expect(fetchMock).toHaveBeenCalledTimes(3);
  ...
  it("rechecks public visibility for every authenticated preview cache miss", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        githubJson(
          previewPayload({
            repository_url: "https://api.github.com/repos/grokbot/visibility-change",
            user: { login: "octocat" },
          }),
        ),
      )
    ...
    await loadControlUiGitHubPreview(
      { kind: "issue", number: 70005, owner: "grokbot", repo: "visibility-change" },
      fetchMock,
    );
    await expect(
      loadControlUiGitHubPreview(
        { kind: "issue", number: 70006, owner: "grokbot", repo: "visibility-change" },
        fetchMock,
      ),
    ).rejects.toMatchObject({ statusCode: 404 } satisfies Partial<ControlUiGitHubError>);
  ...
  it("maps missing GitHub items to a safe not-found error", async () => {
    ...
    await expect(
      loadControlUiGitHubPreview(
        { kind: "issue", number: 70002, owner: "grokbot", repo: "missing-preview" },
        fetchMock,
      ),
    ).rejects.toMatchObject({ statusCode: 404 } satisfies Partial<ControlUiGitHubError>);
