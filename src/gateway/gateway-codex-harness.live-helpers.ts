...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current GrokBot session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsOpenClawStatus =
    normalized.includes("grokbot is running on") ||
    /grokbot\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("grokbot status:") ||
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from grokbot status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live grokbot config shows") ||
