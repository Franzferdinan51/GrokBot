...
import { createOpenClawTools } from "../agents/grokbot-tools.js";
  ...
  const allTools = includeNodeExecTool
    ? [
        ...
        createLazyExecTool(
          ...
          {
            description:
              "Execute a shell command on a connected GrokBot node. This tool is node-only; use the CLI native shell for Gateway-local commands. Commands run synchronously. Set node when multiple nodes are available.",
