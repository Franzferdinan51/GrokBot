...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "grokbot", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `grokbot` or `grokbot/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "grokbot", input: "hi" },
        { "x-grokbot-agent-id": "beta" },
      );
      ...
      const resSessionOverride = await postResponses(
        ...
        {
          "x-grokbot-agent-id": "beta",
          "x-grokbot-session-key": "agent:beta:openresponses:custom",
        },
      );
      ...
      const resReservedSessionOverride = await postResponses(
        port,
        { model: "grokbot", input: "hi" },
        { "x-grokbot-session-key": "agent:main:subagent:spoofed" },
      );
      ...
      expect(reservedSessionJson.error?.message).toBe(
        "`x-grokbot-session-key` cannot use reserved internal session namespaces.",
      );
      expect(agentCommand).toHaveBeenCalledTimes(0);
      ...
      const resHarnessSessionOverride = await postResponses(
        ...
        { model: "grokbot", input: "hi" },
        {
          "x-grokbot-session-key": "agent:main:harness:codex:supervision:spoofed-native-thread",
        },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "grokbot/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "grokbot/default", input: "hi" });
...

      {
        ...
        const res = await postResponses(
          port,
          { model: "grokbot", input: "hi" },
          { "x-grokbot-agent-id": "missing-agent" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
      ...
      {
        agentCommand.mockClear();
        const res = await postResponses(port, { model: "grokbot/missing-agent", input: "hi" });
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Unknown agent 'missing-agent'.");
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "grokbot", input: "hi" },
        { "x-grokbot-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-grokbot-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("accepts write-scoped and admin-scoped HTTP callers", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "grokbot", input: "hi" },
      { "x-grokbot-scopes": "operator.admin, operator.write" },
    );
