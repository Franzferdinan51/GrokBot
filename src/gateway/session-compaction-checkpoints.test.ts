...
import { CURRENT_SESSION_VERSION, SessionManager } from "grokbot/plugin-sdk/agent-sessions";
...
describe("session-compaction-checkpoints", () => {
  ...
  test("checkpoint store branches and restores SQLite marker checkpoints from rows", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-checkpoint-sqlite-branch-"));
    tempDirs.push(dir);
    const storePath = path.join(dir, "grokbot-agent.sqlite");
  ...
  test("checkpoint store branches row-backed checkpoints when entry sessionFile is stale", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-checkpoint-sqlite-stale-"));
    tempDirs.push(dir);
    const storePath = path.join(dir, "grokbot-agent.sqlite");
  ...
  test("checkpoint store does not fork retired legacy snapshots for SQLite marker entries", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-checkpoint-sqlite-legacy-"));
    tempDirs.push(dir);
    const storePath = path.join(dir, "grokbot-agent.sqlite");
  ...
  test("leaf state follows terminal controls while retaining the append cursor", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-checkpoint-leaf-control-"));
    tempDirs.push(dir);
  ...
  test("async leaf scans ignore controls with dangling references", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-checkpoint-invalid-leaf-"));
    tempDirs.push(dir);
    const sessionFile = path.join(dir, "session.jsonl");
    await fs.writeFile(
      sessionFile,
  ...
  test("file-backed checkpoint store branches active state and restores source management state", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-checkpoint-store-"));
    tempDirs.push(dir);
  ...
  test("file-backed checkpoint store rejects identity changes for model-selection-locked sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-checkpoint-locked-"));
    tempDirs.push(dir);
