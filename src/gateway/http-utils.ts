...
export const OPENCLAW_MODEL_ID = "grokbot";
/** Default OpenAI-compatible model alias that targets the default GrokBot agent. */
export const OPENCLAW_DEFAULT_MODEL_ID = "grokbot/default";
...
class GatewaySessionKeyOverrideError extends Error {
  constructor() {
    super("`x-grokbot-session-key` cannot use reserved internal session namespaces.");
...
function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-grokbot-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-grokbot-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  const m =
    raw.match(/^grokbot[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
/** Validates and resolves the `x-grokbot-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `grokbot` or `grokbot/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-grokbot-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-grokbot-model`." };
  }
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-grokbot-session-key")?.trim();
  if (explicit) {
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-grokbot-message-channel")) ??
