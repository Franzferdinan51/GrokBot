...
const suiteTempRootTracker = createSuiteTempRootTracker("grokbot-plugin-install-path");
...
describe("installPluginFromPath", () => {
  it("rejects managed plain file plugin installs through path install", async () => {
    ...
    expect(result.error).toBe(
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains grokbot.plugin.json, or list standalone plugin files in plugins.load.paths.",
    );
  });
