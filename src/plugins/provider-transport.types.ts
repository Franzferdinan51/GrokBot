...
/**
 * Provider-owned stream wrapper hook after GrokBot applies its generic
 ...
 *
 * Return `true` or `false` to override GrokBot's built-in provider cache TTL
 ...
 *
 * Runs only after GrokBot exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that GrokBot should append to the generic `Unknown model`
 * error.
 */
