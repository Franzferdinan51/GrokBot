...
function createWebSearchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    OPENCLAW_HOME: "/tmp/grokbot-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/grokbot.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/grokbot.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  it("resolves current config contents when config changes in place", () => {
    ...
    const env = createWebSearchEnv({ OPENCLAW_HOME: "/tmp/grokbot-home-a" });
  ...
  it("resolves current env contents when env changes in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.OPENCLAW_HOME = "/tmp/grokbot-home-b";
      },
      expectedLoaderCalls: 2,
    });
  });
