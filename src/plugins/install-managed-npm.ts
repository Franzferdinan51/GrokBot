  ...
  const runManagedNpmInstall = async (
    ...
    if (params.packageName !== "grokbot") {
      ...
      if (repairedOpenClawPeer) {
        logger.info?.(`Repaired stale grokbot peer dependency in ${npmRoot}`);
      }
    }
    ...
    const quarantineForRecovery = async (
      ...
      try {
        ...
        return await rollbackFailedManagedNpmInstall({
          ok: false,
          error: `${cause.error}, but GrokBot could not quarantine ${npmRoot} for rebuild: ${String(error)}`,
        });
      }
      logger.warn?.(
    ...
    if (omittedPlatformPackages.length > 0) {
      ...
      try {
        freshCacheDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-npm-cache-"));
    ...
    if (params.packageName !== "grokbot") {
      ...
      if (repairedOpenClawPeer) {
        logger.info?.(`Repaired stale grokbot peer dependency in ${npmRoot} after npm install`);
      }
    }
    try {
      ...
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: `Failed to repair grokbot peer links after npm install: ${String(error)}`,
      });
    }
