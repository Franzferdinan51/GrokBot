...
function resolvePluginLoaderJitiNativeModules(): string[] {
  try {
    ...
    const nativeModules = Array.isArray(configured)
      ...
      : [];
    return [...new Set([...nativeModules, "grokbot"])];
  } catch {
    return ["grokbot"];
  }
}
...
function hasTrustedOpenClawRootIndicator(params: {
  ...
  const hasOpenClawBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("grokbot")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.grokbot === "string");
  const hasOpenClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "grokbot.mjs"));
...
const PLUGIN_SDK_PACKAGE_NAMES = ["grokbot/plugin-sdk", "@grokbot/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@grokbot/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  {
    packageName: "@grokbot/gateway-client",
  ...
  {
    packageName: "@grokbot/gateway-protocol",
  ...
  {
    packageName: "@grokbot/markdown-core",
  ...
  {
    packageName: "@grokbot/media-generation-core",
  ...
  {
    packageName: "@grokbot/media-core",
  ...
  {
    packageName: "@grokbot/terminal-core",
  ...
  {
    packageName: "@grokbot/net-policy",
  ...
  {
    packageName: "@grokbot/model-catalog-core",
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@grokbot/") ? name : null;
}
  ...
  const workspacePackageAliasEntries = [
    ...
    ...["normalization-core", "acp-core"].flatMap((packageDir) =>
      listWorkspacePackageExportAliasEntries({
        packageRoot,
        packageName: `@grokbot/${packageDir}`,
        packageDir,
      }),
    ),
  ];
...
function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `grokbot/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderJitiOptions(
  ...
  return {
    ...
    tryNative: true,
    // When jiti must transform a plugin entry, keep GrokBot's own package
