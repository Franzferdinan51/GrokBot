...
import { hasHttpUrlPrefix } from "@grokbot/net-policy/url-protocol";
...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-marketplace-"));
...
> {
  ...
  try {
    ...
    try {
      ...
      tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "grokbot-marketplace-download-"));
