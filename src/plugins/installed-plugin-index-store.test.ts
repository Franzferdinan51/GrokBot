...
function makeTempDir() {
  return makeTrackedTempDir("grokbot-installed-plugin-index-store", tempDirs);
}
  ...
  it("preserves startup config paths across persisted index roundtrips", async () => {
    ...
    const index = createIndex({
      plugins: [
        {
          pluginId: "browser",
          manifestPath: "/plugins/browser/grokbot.plugin.json",
      ...
      plugins: [
        {
          ...
          manifestPath: "/plugins/provider-owner/grokbot.plugin.json",
