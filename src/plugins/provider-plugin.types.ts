...
export type ProviderPlugin = {
   ...
   *
   * GrokBot calls this only from async model resolution paths. After it
   ...
   *
   * Runs after GrokBot performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before GrokBot registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic GrokBot wrappers.
   *
   ...
   *
   * Called after GrokBot resolves the raw configured credential but before the
   ...
   * hint than GrokBot's generic auth-store guidance.
   */
   ...
   * GrokBot's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when GrokBot should surface a provider-specific hint for
   ...
   * merging. GrokBot deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. GrokBot stores the
   ...
   * without replacing the full GrokBot-owned system prompt.
   */
   ...
   *
   * Runs after GrokBot's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep GrokBot's default fallback policy.
   */
   ...
   * rewrites after GrokBot has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before GrokBot handles
   ...
   *
   * GrokBot uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `grokbot doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `grokbot doctor` should offer.
   *
   ...
   *
   * GrokBot calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep GrokBot's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * GrokBot falls back to that stored profile.
   */
