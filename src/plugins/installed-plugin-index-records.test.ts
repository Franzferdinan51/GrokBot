...
function makeStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-plugin-index-records-"));
  tempDirs.push(dir);
  return dir;
}
...
function createPluginCandidate(stateDir: string, pluginId: string): PluginCandidate {
  ...
  fs.writeFileSync(
    path.join(rootDir, "grokbot.plugin.json"),
    JSON.stringify({
      id: pluginId,
...
describe("plugin index install records store", () => {
  it("writes machine-managed install records outside config", async () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        twitch: {
          source: "npm",
          spec: "@grokbot/plugin-twitch@1.0.0",
          installPath: "plugins/npm/@grokbot/plugin-twitch",
        },
      },
      {
        stateDir,
    ...
    expect(indexPath).toBe(path.join(stateDir, "state", "grokbot.sqlite"));
  ...
  it.each([
    ...
    {
      expectedSpec: "@grokbot/discord@latest",
      label: "latest",
      persistedVersion: "2026.7.1",
      recoveredVersion: "2026.7.1",
    ...
    {
      expectedSpec: "@grokbot/discord@beta",
      label: "dist-tag",
      persistedVersion: "2026.7.1",
      recoveredVersion: "2026.7.1",
      spec: "@grokbot/discord@beta",
    ...
    {
      expectedSpec: "@grokbot/discord@2026.7.1",
      label: "obsolete exact-version",
      persistedVersion: "2026.6.4",
      recoveredVersion: "2026.7.1",
      spec: "@grokbot/discord@2026.6.4",
    },
    {
      expectedSpec: "@grokbot/discord@2027.1.0",
      label: "unsupported legacy range",
      persistedVersion: "2026.6.4",
      recoveredVersion: "2027.1.0",
      spec: "@grokbot/discord@^2026.6.0",
    },
    {
      expectedSpec: "@grokbot/discord@2026.7.2-beta.1",
    ...
    async ({ expectedSpec, persistedVersion, recoveredVersion, spec }) => {
      const stateDir = makeStateDir();
      const packageName = "@grokbot/discord";
  ...
  it("recovers managed npm metadata when the persisted record points at an older package version", async () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        codex: {
          source: "npm",
          spec: "@grokbot/codex@2026.5.16-beta.1",
          installPath: codexDir,
          version: "2026.5.16-beta.1",
          resolvedName: "@grokbot/codex",
          resolvedVersion: "2026.5.16-beta.1",
          resolvedSpec: "@grokbot/codex@2026.5.16-beta.1",
          integrity: "sha512-stale",
          shasum: "stale",
    ...
    const record = expectRecordFields(loaded.codex, {
      source: "npm",
      spec: "@grokbot/codex@2026.5.18-beta.1",
      installPath: codexDir,
      version: "2026.5.18-beta.1",
      resolvedName: "@grokbot/codex",
      resolvedVersion: "2026.5.18-beta.1",
      resolvedSpec: "@grokbot/codex@2026.5.18-beta.1",
    });
    expect(record.integrity).toBeUndefined();
  ...
  it("keeps recovered managed npm records cached until cache clear after package changes", () => {
    ...
    expectRecordFields(loadInstalledPluginIndexInstallRecordsSync({ stateDir }).codex, {
      ...
      resolvedSpec: "@grokbot/codex@2026.5.19-beta.1",
    });
  });
