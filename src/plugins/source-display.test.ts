...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "grokbot", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".grokbot", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".grokbot", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "grokbot-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".grokbot", "extensions"),
      },
    });
  });
});
