...
function writeFakeOpenClawPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "grokbot.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installOpenClawPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-dev-source-"));
    ...
    try {
      ...
      expect(installedAliases).toContain("grokbot/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("grokbot/plugin-sdk/agent-runtime"))).toBe(
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("grokbot/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("grokbot/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-source-resolver-"));
    ...
    expect(installedAliases).toContain("grokbot/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("grokbot/plugin-sdk/channel-outbound"))).toBe(
      fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")),
    );
  });

  it("lets built external plugins resolve GrokBot SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-resolver-"));
    ...
    try {
      ...
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("grokbot/plugin-sdk/channel-outbound")),
      ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
      const sdk = requireFromPlugin("grokbot/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("grokbot/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"grokbot/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"grokbot/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installOpenClawPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeOpenClawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("grokbot/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("grokbot/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for GrokBot-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeOpenClawPackage(root);
    ...
    expect(installedAliases).toContain("@grokbot/normalization-core/string-coerce");
    expect(installedAliases).toContain("@grokbot/normalization-core/boolean-coercion");
    expect(installedAliases).toContain("@grokbot/normalization-core/result");
    expect(installedAliases).toContain("@grokbot/normalization-core/agent-id");
    expect(installedAliases).toContain("@grokbot/media-core/mime");
    expect(installedAliases).toContain("@grokbot/markdown-core/code-spans");
    expect(installedAliases).toContain("@grokbot/ai/internal/retry-after");
    expect(installedAliases).toContain("@grokbot/ai/internal/runtime");
    expect(installedAliases).toContain("@grokbot/acp-core/runtime/types");
    expect(installedAliases).toContain("@grokbot/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@grokbot/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(
      fs.realpathSync(
        requireFromCoreSource.resolve("@grokbot/normalization-core/boolean-coercion"),
      ),
    ).toBe(fs.realpathSync(booleanCoercionSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@grokbot/normalization-core/result")),
    ).toBe(fs.realpathSync(resultSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@grokbot/normalization-core/agent-id")),
    ).toBe(fs.realpathSync(agentIdSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@grokbot/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@grokbot/markdown-core/code-spans")),
    ).toBe(fs.realpathSync(markdownCoreSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@grokbot/ai/internal/retry-after")),
    ).toBe(fs.realpathSync(aiRetryAfterSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@grokbot/ai/internal/runtime"))).toBe(
      fs.realpathSync(aiRuntimeSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@grokbot/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@grokbot/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@grokbot/normalization-core/string-coerce")).toThrow();
    expect(() =>
      requireFromPlugin.resolve("@grokbot/normalization-core/boolean-coercion"),
    ).toThrow();
    expect(() => requireFromPlugin.resolve("@grokbot/normalization-core/result")).toThrow();
    expect(() => requireFromPlugin.resolve("@grokbot/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@grokbot/markdown-core/code-spans")).toThrow();
    expect(() => requireFromPlugin.resolve("@grokbot/ai/internal/retry-after")).toThrow();
    expect(() => requireFromPlugin.resolve("@grokbot/ai/internal/runtime")).toThrow();
    expect(() => requireFromPlugin.resolve("@grokbot/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@grokbot/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeOpenClawPackage(root);
    ...
    expect(installedAliases).toContain("grokbot/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("grokbot/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("grokbot/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "grokbot-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeOpenClawPackage(root);
    ...
    expect(installedAliases).toContain("grokbot/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("grokbot/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("grokbot/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("grokbot/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("grokbot/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("grokbot/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});
