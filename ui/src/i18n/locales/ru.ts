...
export const ru: TranslationMap = {
  ...
  lazyView: {
    ...
    staleSubtitle:
      "GrokBot был обновлён в фоновом режиме. Перезагрузите страницу, чтобы получить последнюю версию панели.",
    retry: "Повторить",
  },
  nodes: {
    pairing: {
      ...
      title: "GrokBot для мобильных устройств",
      ...
      qrAlt: "QR-код сопряжения GrokBot для мобильных устройств",
      ...
      review: "Обзор",
      waiting:
        "Официальные мобильные приложения GrokBot подключаются автоматически после сканирования.",
  ...
  worktrees: {
    ...
    subtitle: "Изолированные копии репозиториев, принадлежащие GrokBot.",
    cleanNow: "Очистить сейчас",
    cleanupTitle: "Очистка",
    cleanupSubtitle:
      "GrokBot периодически удаляет старые рабочие деревья, чтобы освободить место на диске. Настройте интенсивность очистки.",
  ...
  configView: {
    ...
    retry: "Повторить",
    applyBannerText: "Сохранено в grokbot.json — перезапустите gateway для применения.",
  ...
  subtitles: {
    ...
    config: "Редактировать grokbot.json.",
  ...
  modelSetup: {
    ...
    intro:
      "GrokBot использует уже имеющийся у вас доступ к ИИ — вход через CLI, ключ API или авторизацию у провайдера.",
    ...
    access: {
      ...
      gatewayTooOld: "На Gateway установлена устаревшая версия GrokBot",
    },
    candidates: {
  ...
  memoryImport: {
    ...
    confirmBackup: "Перед копированием памяти GrokBot создаёт проверенную резервную копию.",
    ...
    subtitle:
      "Проверьте консолидированную память Codex и автоматическую память Claude Code перед копированием в GrokBot.",
    agent: "Целевой агент",
  ...
  onboarding: {
    memoryImport: {
      ...
      body: "GrokBot обнаружил память других ассистентов для программирования. Импортировать её в рабочее пространство вашего агента?",
      ...
      doneBody:
        "Перенесено: {migrated}, пропущено: {skipped}. Можно продолжить настройку GrokBot.",
    },
  },
  mcpPage: {
  ...
  pluginsPage: {
    ...
    noInstalledBody:
      "Откройте для себя рекомендуемый плагин или выполните поиск в ClawHub, чтобы расширить GrokBot.",
    ...
    optionalCapability: "Необязательная возможность GrokBot.",
    enabled: "Включено",
    disabled: "Отключено",
    available: "Доступно",
  ...
  aboutPage: {
    ...
    license: "© 2026 GrokBot Foundation — лицензия MIT.",
  ...
  skillWorkshop: {
    ...
    selfLearning: {
      ...
      pitchBody:
        "GrokBot анализирует исправления и значимые завершённые запуски, а затем создаёт для этой доски черновики предложений по навыкам. При этом используются дополнительные токены в фоновом режиме, а черновики поступают как ожидающие рассмотрения предложения.",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "Вставьте токен из grokbot dashboard --no-open или введите настроенный пароль.",
        stepGenerate:
          "Если токен не настроен, выполните grokbot doctor --generate-gateway-token на хосте шлюза.",
      ...
      authFailed: {
        ...
        stepDashboard:
          "Запустите grokbot dashboard --no-open и откройте новый URL или вставьте его токен.",
        stepReplace:
      ...
      pairing: {
        ...
        stepList: "Выполните grokbot devices list на хосте Gateway.",
        stepApproveId: "Подтвердите этот запрос: grokbot devices approve {requestId}.",
      ...
      protocol: {
        ...
        stepDashboard:
          "Повторно откройте обслуживаемую панель командой grokbot dashboard, чтобы UI и Gateway были из одной установки.",
        stepDevUi:
          "Если используется pnpm ui:dev, пересоберите или перезапустите dev UI для текущей рабочей копии.",
        stepRestart:
          "Перезапустите Gateway после обновления GrokBot, чтобы он обслуживал текущий протокол.",
      },
      network: {
        ...
        stepGateway:
          "Убедитесь, что Gateway запущен, с помощью grokbot status или grokbot gateway run.",
        stepUrl:
          "Проверьте URL WebSocket и используйте wss://, когда Gateway находится за HTTPS/Tailscale Serve.",
        stepDashboard:
          "Снова откройте панель управления с помощью grokbot dashboard --no-open, чтобы повторно скопировать текущий URL и данные аутентификации.",
      },
    },
  },
  chat: {
  ...
  cron: {
    ...
    list: {
      ...
      emptyHint:
        "Опишите, что и когда должен делать GrokBot — задача будет выполняться по расписанию.",
    ...
    detail: {
      ...
      newSubtitle: "Опишите, что должен делать GrokBot, затем выберите время запуска.",
    ...
    form: {
      ...
      promptPlaceholder: "Опишите, что должен делать GrokBot...",
      repeat: "Повтор",
