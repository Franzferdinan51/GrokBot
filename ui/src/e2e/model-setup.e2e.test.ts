...
describeControlUiE2e("Control UI Model Setup mocked Gateway E2E", () => {
  ...
  it("hands first-run model setup to the custodian in onboarding chrome", async () => {
    ...
    try {
      ...
      const detect = await gateway.waitForRequest("grokbot.setup.detect");
      expect(detect.params).toEqual({});
      const activate = await gateway.waitForRequest("grokbot.setup.activate");
      ...
      const chatRequest = await gateway.waitForRequest("grokbot.chat");
  ...
  it("completes device-code sign-in and re-detects the configured model", async () => {
    ...
    const gateway = await installMockGateway(page, {
      ...
      methodResponses: {
        "grokbot.setup.detect": initialDetection,
        "grokbot.setup.auth.start": {
    ...
    try {
      ...
      const start = await gateway.waitForRequest("grokbot.setup.auth.start");
      ...
      await gateway.setMethodResponse("grokbot.setup.detect", {
        ...initialDetection,
        authOptions: [],
        configuredModel: "provider/verified-model",
        setupComplete: true,
      });
      const detectCountBeforeCompletion = (await gateway.getRequests("grokbot.setup.detect"))
        .length;
      ...
      await expect
        .poll(async () => (await gateway.getRequests("grokbot.setup.detect")).length)
  ...
  it("verifies the current model connection", async () => {
    ...
    try {
      ...
      const verify = await gateway.waitForRequest("grokbot.setup.verify");
      expect(verify.params).toEqual({});
      await page.getByText("Answered in 1234 ms").waitFor();
      const detectCountBeforeRefresh = (await gateway.getRequests("grokbot.setup.detect")).length;
