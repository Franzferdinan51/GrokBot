...
async function selectGatewayOnNextLoad(
  ...
  const settingsKey = `grokbot.control.settings.v1:${normalizeGatewayTokenScope(gatewayUrl)}`;
  const selectionKey =
    `grokbot.control.currentGateway.v1:` +
...
describeControlUiE2e("Control UI device-token reconnect E2E", () => {
  ...
  it("isolates device tokens across gateways, origins, and revocation", async () => {
    ...
    expect(await rositaReconnect.page.locator("grokbot-login-gate").count()).toBe(0);
    ...
    expect(await wilfredReconnect.page.locator("grokbot-login-gate").count()).toBe(0);

    const identity = await wilfredSource.page.evaluate(() => {
      const raw = localStorage.getItem("grokbot-device-identity-v1");
    ...
    const wilfredStoreKey =
      `grokbot.device.auth.v1:` + normalizeGatewayCredentialScope(WILFRED_GATEWAY_URL);
    await expect
      .poll(() =>
