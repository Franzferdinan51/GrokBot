...
metadata:
  grokbot:
    homepage: https://github.com/grokbot/discrawl
    requires:
      bins:
        - discrawl
    install:
      - kind: go
        module: github.com/grokbot/discrawl/cmd/discrawl@latest
        bins:
          - discrawl
---
...
- Repo: `grokbot/discrawl`; use `~/GIT/_Perso/discrawl` only after verifying
  its remote targets `grokbot/discrawl`, otherwise use a fresh checkout
...
`grokbot/discrawl` checkout before concluding the feature is missing.
...
This reader imports `https://github.com/grokbot/discord-store.git` into
