---
name: clawsweeper
description: "Use for all ClawSweeper work: GrokBot issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one GrokBot
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `grokbot/clawsweeper` for comment-router
...
pnpm commit-reports -- --repo grokbot/grokbot --author steipete --since 7d
...
```bash
gh workflow run commit-review.yml --repo grokbot/clawsweeper \
  -f target_repo=grokbot/grokbot \
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/grokbot-grokbot/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/grokbot/inbox/clawsweeper-grokbot-grokbot-123.md
pnpm run repair:dispatch -- jobs/grokbot/inbox/clawsweeper-grokbot-grokbot-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo grokbot/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo grokbot/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo grokbot/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo grokbot/clawsweeper --body 1
```
...
@grokbot-clawsweeper fix ci
@grokbot-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo grokbot/grokbot --lookback-minutes 180
pnpm run repair:comment-router -- --repo grokbot/grokbot --execute --wait-for-capacity
```
...
leave the local GrokBot checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central GrokBot security handling.
...
```bash
gh run list --repo grokbot/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo grokbot/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo grokbot/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo grokbot/grokbot --workflow "ClawSweeper Dispatch" \
...
```bash
gh api "repos/grokbot/grokbot/commits/<sha>/check-runs?per_page=100" \
