---
name: verify-release
description: "Verify an GrokBot release is fully published across GitHub, npm, plugins, ClawHub, package smoke, and live Gateway agent turns."
---

# Verify Release

Use this when asked whether an GrokBot release is fully released, published,
promoted, smoke-tested, or live-verified. This is a verification skill, not a
publish skill; use `$release-grokbot-maintainer` before changing release state.
...
1. GitHub release:
   - `gh release view v<VERSION> --repo grokbot/grokbot --json tagName,name,publishedAt,isDraft,isPrerelease,targetCommitish,url,body,assets`
...
2. Root npm:
   - `npm view grokbot@<VERSION> version dist-tags.latest dist.tarball dist.integrity time.<VERSION> --json`
...
3. Plugin publish set:
   - Get exact tag metadata from GitHub, not the local checkout when dirty:
     download `https://api.github.com/repos/grokbot/grokbot/tarball/v<VERSION>`
     into `/tmp/grokbot-v<VERSION>-src`.
   - Count `extensions/*/package.json` with
     `grokbot.release.publishToNpm === true` and
     `grokbot.release.publishToClawHub === true`.
   - Compare expected counts to workflow job counts:
     `gh api repos/grokbot/grokbot/actions/runs/<RUN>/jobs --paginate`.
...
6. Published package smoke:
   - In `/tmp`, isolated HOME:
     `npm exec --yes --package grokbot@<VERSION> -- grokbot --version`.
...
7. Dev Gateway live model smoke:
   - Use temp HOME/workspace, not the user's normal state:
     `HOME=/tmp/grokbot-release-smoke/home OPENCLAW_WORKSPACE=/tmp/grokbot-release-smoke/work pnpm grokbot --dev gateway run --auth none --force --verbose`.
   - Health check via CLI: `grokbot --dev gateway health --json`.
