...
export OPENCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.grokbot/grokbot.json' | tr -d '\n'
)"
...
- Use the `grokbot` wrapper for guest `message send/read`; `node grokbot.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/grokbot-parallels-smoke.*`
...
- host inbound nonce appears in `grokbot message read` output
