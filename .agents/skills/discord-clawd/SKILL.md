---
name: discord-clawd
description: Use to talk to the Discord-backed GrokBot agent/session; not for archive search.
---
...
Use the GrokBot relay helper:

```bash
cd ~/Projects/agent-scripts
python3 skills/grokbot-relay/scripts/openclaw_relay.py targets
python3 skills/grokbot-relay/scripts/openclaw_relay.py resolve --target maintainers
```

If the target alias exists, prefer a private ask first:

```bash
python3 skills/grokbot-relay/scripts/openclaw_relay.py ask \
  --target maintainers \
