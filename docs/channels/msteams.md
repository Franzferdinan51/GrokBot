...
Microsoft Teams ships as a bundled plugin in current GrokBot releases; no separate install is required in the normal packaged build.

On an older build or a custom install that excludes bundled Teams, install the npm package directly:

```bash
grokbot plugins install @grokbot/msteams
```
...
```bash
grokbot plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-grokbot-bot --allow-anonymous
devtunnel port create my-grokbot-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-grokbot-bot
...
**4. Configure GrokBot** using the credentials from the output:
...
- Talk to GrokBot via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists; they can change. GrokBot disables direct name matching by default; opt in with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, GrokBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow) and logs the mapping. Unresolved names are kept as typed but ignored for routing unless `channels.msteams.dangerouslyAllowNameMatching: true` is set.
...
5. Configure `msteams` in `~/.grokbot/grokbot.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `grokbot-msteams` (must be unique) |
...
For production, GrokBot supports **federated authentication** as an alternative to client secrets, via `channels.msteams.authType: "federated"`. Two methods:
...
3. At runtime, GrokBot uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint.
...
**Default:** when `authType` is unset, GrokBot uses client-secret authentication (`appPassword`). Existing configs keep working unchanged.
...
These auth-related config keys can be set via environment variables instead of `grokbot.json` (other config keys, such as `groupPolicy` or `historyLimit`, are config-only):
...
GrokBot exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve verified roster details for a configured conversation.
...
```json5
{
  ...
  description: { short: "GrokBot in Teams", full: "GrokBot in Teams" },
...
Teams delivers messages via HTTP webhook. GrokBot applies fixed HTTP server
...
| Teams environment | GrokBot config                                             | Proactive `serviceUrl`                             |
| ----------------- | ----------------------------------------------------------- | -------------------------------------------------- |
...
By default, GrokBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Per-user sharing is more secure since only chat participants can access the file. GrokBot requires a successful member lookup for group chats; timeouts, transport failures, empty results, and Graph API denials fail the send instead of widening access to the organization.
...
GrokBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `grokbot message poll --channel msteams --target conversation:<id> --poll-question "..." --poll-option "..." --poll-option "..."`.
- Votes are recorded by the gateway in GrokBot plugin-state SQLite under `state/grokbot.sqlite`.
- Existing `msteams-polls.json` files are imported by `grokbot doctor --fix`, not by the running plugin.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries, and there is no poll-results CLI yet.

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. GrokBot renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer, so GrokBot downgrades them to readable text before delivery.
...
```bash
grokbot message send --channel msteams \
...
grokbot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
grokbot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
grokbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
grokbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- Proactive messages are only possible **after** a user has interacted, because GrokBot stores conversation references at that point.
...
- **Ignore** the `groupId` query parameter for GrokBot routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
