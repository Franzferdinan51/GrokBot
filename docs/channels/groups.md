...
GrokBot applies the same group rules across group-capable channels, including Discord, iMessage, Matrix, Microsoft Teams, QQBot, Signal, Slack, Telegram, WhatsApp, and Zalo.
...
GrokBot "lives" on your own messaging accounts. There is no separate WhatsApp bot user: if **you** are in a group, GrokBot can see that group and respond there.
...
Translation: allowlisted senders can trigger GrokBot by mentioning it.
...
By default GrokBot keeps context as received: allowlists decide who can trigger actions, not what quoted or historical snippets the model sees. To also filter supplemental context, set `contextVisibility`:
    ...
    list: [
      {
        id: "main",
        groupChat: {
          mentionPatterns: ["@grokbot", "grokbot", "\\+15555550123"],
          historyLimit: 50,
        },
      },
    ],
...
as `grokbot:` to count as a mention. Native platform mentions are separate:
...
- `toolsBySender`: per-sender overrides within the group. Use explicit key prefixes: `channel:<channelId>:<senderId>`, `id:<senderId>`, `e164:<phone>`, `username:<handle>`, `name:<displayName>`, and `"*"` wildcard. Channel ids use canonical GrokBot channel ids; aliases such as `teams` normalize to `msteams`. Legacy unprefixed keys are still accepted, matched as `id:` only, and log a deprecation warning.
