...
| Request signing              | Not used (auth is the App-Level Token)                                                                                                               | Slack signs every request; GrokBot verifies with `signingSecret`                                              |
| Recovery on connection drop  | Slack SDK auto-reconnect is enabled; GrokBot also restarts failed Socket Mode sessions with bounded backoff. Pong-timeout transport tuning applies. | No persistent connection to drop; retries are per-request from Slack                                           |
...
<Warning>
  Slack can maintain multiple Socket Mode connections for one app and may deliver each payload to any connection. Separate GrokBot gateways that share a Slack app therefore need equivalent routing and authorization configuration. Otherwise, use a separate Slack app per gateway, a single relay ingress, or HTTP Request URLs behind a load balancer. See [Using Socket Mode](https://docs.slack.dev/apis/events-api/using-socket-mode#using-multiple-connections).
</Warning>

### Relay mode

Relay mode separates Slack ingress from the GrokBot gateway. A trusted router owns the single Slack Socket Mode connection, chooses a destination gateway, and forwards a typed event over an authenticated websocket. The gateway still uses its own bot token for outbound Slack Web API calls.
...
```json
{
  "display_information": {
    "name": "GrokBot",
    "description": "Slack connector for GrokBot"
  },
  "features": {
    "bot_user": { "display_name": "GrokBot", "always_online": true }
  },
  "oauth_config": {
    "scopes": {
      "bot": [
...
GrokBot. Generate an app-level token with `connections:write` for Socket Mode,
...
At startup, GrokBot verifies `enterpriseOrgInstall` with Slack `auth.test`.
...
granted the installation; GrokBot then applies the configured channel, user,
...
`channel:<id>` form. GrokBot normalizes either form to the raw channel ID for
...
```bash
grokbot plugins install @grokbot/slack
```
...
JSON5
grokbot config patch --file ./slack.socket.patch.json5 --dry-run
grokbot config patch --file ./slack.socket.patch.json5
```
...
```
        ...
        <Info>
          The three URL fields (`slash_commands[].url`, `event_subscriptions.request_url`, and `interactivity.request_url` / `message_menu_options_url`) all point at the same GrokBot endpoint. Slack's manifest schema requires them named separately, but GrokBot routes by payload type so a single `webhookPath` (default `/slack/events`) is enough. Slash commands without `slash_commands[].url` silently no-op in HTTP mode.
        </Info>
...
JSON5
grokbot config patch --file ./slack.http.patch.json5 --dry-run
grokbot config patch --file ./slack.http.patch.json5
```
...
User identity lets GrokBot read and post as the human who authorizes the Slack app. The `userToken` is the acting identity; a companion Slack app carries Events API traffic over Socket Mode or an HTTP Request URL. The companion app does not need a bot user or bot token.
...
3. Choose one event transport:
   ...
   - **HTTP Request URL:** point Event Subscriptions at the public GrokBot Slack endpoint and copy **Basic Information -> App Credentials -> Signing Secret**. Configure it as `signingSecret`.
...
<Warning>
  DMs and group DMs work only through the user-scope event subscription above. A bot cannot join a human 1:1 DM or be inserted into an existing group DM. The companion app is invisible plumbing: other Slack members see messages from the authorizing human, not from an GrokBot bot.
</Warning>

GrokBot automatically drops user-scope message events authored by the resolved human identity, so messages it sends do not trigger self-replies.

## Socket Mode transport tuning

GrokBot sets the Slack SDK client pong timeout to 15 seconds by default for Socket Mode. Override the transport settings only when you need workspace- or host-specific tuning:
...
- Only `clientPingTimeout` has an GrokBot default (`15000`). `serverPingTimeout` and `pingPongLoggingEnabled` are passed to the Slack SDK only when configured.
...
<AccordionGroup>
  <Accordion title="Optional native slash commands">
    ...
    GrokBot registers handlers for enabled native commands, but Slack manifest entries remain administrator-managed and are not synchronized at runtime. Add `/login` to the manifest manually; the example below includes it instead of the optional `/side` alias to remain at 25 commands. `/login` can be surfaced anywhere, but it issues pairing codes only in private chats or the Web UI.
...
<Tabs>
  <Tab title="DM policy">
    ...
    Legacy `channels.slack.dm.policy` and `channels.slack.dm.allowFrom` still read for compatibility. `grokbot doctor --fix` migrates them to `dmPolicy` and `allowFrom` when it can do so without changing access.

    Pairing in DMs uses `grokbot pairing approve slack <code>`.
...
- GrokBot seeds an eligible top-level channel root into `agent:<agentId>:slack:channel:<channelId>:thread:<rootTs>` when that root is expected to start a visible Slack thread, so the root and later thread replies share one GrokBot session. This applies to `app_mention` events, explicit bot or configured mention-pattern matches, and `requireMention: false` channels with non-`off` `replyToMode`.
...
- `channels.slack.implicitMentions.threadParticipation` controls whether follow-ups in a thread where the bot has replied bypass mention gating (default `true`). Set it to `false` to require a new explicit mention in those follow-ups. `grokbot doctor --fix` migrates the former `channels.slack.thread.requireExplicitMention` key to this positive canonical flag.
...
`ackReaction` sends an acknowledgement emoji while GrokBot is processing an inbound message. `ackReactionScope` decides _when_ that emoji is actually sent.
...
- Top-level Slack DMs stay off-thread by default, so they do not show Slack's thread-style native stream/status preview; GrokBot posts and edits a draft preview in the DM instead.
...
- If streaming fails mid-reply, GrokBot falls back to normal delivery for remaining payloads.
...
- Legacy aliases are not read at runtime; run `grokbot doctor --fix` to rewrite persisted Slack streaming config to the canonical keys.

## Typing reaction fallback

`typingReaction` adds a temporary reaction to the inbound Slack message while GrokBot is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
<AccordionGroup>
  <Accordion title="Inbound attachments">
    ...
    Downloads use bounded idle and total timeouts. If Slack file retrieval stalls or fails, GrokBot keeps processing the message and falls back to the file placeholder.
...
renders line, bar, area, and pie charts in messages. GrokBot maps the portable
...
block. Standard presentation sends to other GrokBot channels receive that same
deterministic chart data as text unless they advertise native chart support. If
Slack rejects the chart with `invalid_blocks` during a phased rollout, GrokBot
...
a presentation contains more than two valid charts, GrokBot keeps their order
...
renders structured rows and columns in messages. GrokBot maps an explicit
...
GrokBot maps header and string cells to Slack `raw_text` cells. Numeric cells
...
If Slack rejects native chart or table blocks with `invalid_blocks`, GrokBot
...
Markdown pipe tables remain authored text; GrokBot does not guess at table
structure or cell types. Existing trusted Slack-native producers can continue
to pass raw blocks through `channelData.slack.blocks`; GrokBot derives fallback
...
- The interactive callback values are GrokBot-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, GrokBot falls back to the original text reply instead of sending an invalid blocks payload.

### Plugin-owned modal submissions

Slack plugins that register an interactive handler can also receive modal
`view_submission` and `view_closed` lifecycle events before GrokBot compacts
the payload for the agent-visible system event. Use one of these routing
patterns when opening a Slack modal:

- Set `callback_id` to `grokbot:<namespace>:<payload>`.
...
When those buttons are present, they are the primary approval UX; GrokBot
...
Define global or message shortcuts in your Slack app configuration and use any non-empty callback ID. GrokBot acknowledges matching shortcut payloads, applies the same DM/channel sender policy as other Slack interactions, and queues the sanitized event for the routed agent session. Trigger IDs and response URLs are redacted from agent context.

### Presence events

Slack does not send presence changes through the Events API or Socket Mode. GrokBot can instead poll [`users.getPresence`](https://docs.slack.dev/reference/methods/users.getPresence/) for human participants whose messages passed normal Slack access and routing checks.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `grokbot channels status --probe --json` shows `botTokenStatus` or
...
1. GrokBot downloads the file from Slack's private URL using the bot token.
...
| Forwarded/shared attachments                  | Text and Slack-hosted image/file media are best-effort                             | Re-share directly in the GrokBot thread                                      |
