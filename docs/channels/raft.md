...
read_when:
  - You want to connect GrokBot to a Raft workspace
...
Raft connects an GrokBot agent to a Raft External Agent through the local
...
```bash
grokbot plugins install @grokbot/raft
...
- The Raft CLI installed on the same host as the GrokBot Gateway, on the
  service's `PATH`.
...
```bash
grokbot channels add --channel raft
```
...
7. Starts one serialized GrokBot agent turn per accepted wake.

The bridge owns Raft delivery retries and reconnects. The GrokBot turn
...
```bash
raft --profile grokbot message check
raft --profile grokbot message send
```

<Note>
Raft is not a push-message transport. GrokBot does not automatically send the model's final text back through the bridge, so the agent must use the Raft CLI after processing a wake.
</Note>

## Verify

Check that GrokBot can find the CLI and has a configured profile:

```bash
grokbot channels status --probe
grokbot plugins inspect raft --runtime --json
```
