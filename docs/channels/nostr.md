...
read_when:
  - You want GrokBot to receive DMs via Nostr
  - You're setting up decentralized messaging
title: "Nostr"
---

Nostr is a downloadable channel plugin (`@grokbot/nostr`) that lets GrokBot receive and answer NIP-04 encrypted direct messages over Nostr relays. One account per gateway; DMs only.

## Install

```bash
grokbot plugins install @grokbot/nostr
```
...
```bash
grokbot plugins install --link <path-to-local-nostr-plugin>
```

Restart the gateway after installing or enabling plugins. Onboarding (`grokbot onboard`) and `grokbot channels add` surface Nostr from the shared channel catalog once the plugin is installed.

### Non-interactive setup

```bash
grokbot channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
grokbot channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "grokbot@example.com",
        lud16: "grokbot@example.com",
      },
    },
  },
}
...
1. Note the bot pubkey from gateway logs or `grokbot channels status` (hex; convert to npub in your client if needed).
