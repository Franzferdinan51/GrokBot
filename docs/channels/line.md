...
read_when:
  - You want to connect GrokBot to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to GrokBot via the LINE Messaging API. The plugin runs as a webhook
...
```bash
grokbot plugins install @grokbot/line
```

Local checkout (when running from a git repo):

```bash
grokbot plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so GrokBot applies a strict pre-auth body limit (64 KB) and read timeout before verification.
- GrokBot processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
grokbot pairing list line
grokbot pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.grokbot/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs of at most 2000 characters. GrokBot
