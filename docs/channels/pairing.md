...
read_when:
  ...
  - Reviewing GrokBot security posture
title: "Pairing"
---

"Pairing" is GrokBot's explicit access approval step.
...
Supported channels (any installed channel plugin that declares pairing; external plugins such as `grokbot-weixin` can add more): `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `signal`, `slack`, `sms`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
`<channel>-<accountId>-allowFrom.json` under `~/.grokbot/credentials/`.
Startup migration and `grokbot doctor --fix` import those files into SQLite and
...
5. On your phone, open the GrokBot app → **Settings** → **Gateway**.
6. Scan the QR code or paste the setup code, then connect.

Official GrokBot iOS and Android apps are approved automatically when their
...
3. On your phone, open the GrokBot iOS app → Settings → Gateway.
...
- the default handed-off `operator` token includes `operator.admin`,
  ...
  `operator.write`
- Control UI **Limited access** and `grokbot qr --limited` omit
...
For `gateway.bind=lan` setup URLs, GrokBot detects persistent Tailscale Serve
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, GrokBot keeps the existing approval as-is and creates a fresh pending upgrade request. Use `grokbot devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
Stored in the shared SQLite state database at `~/.grokbot/state/grokbot.sqlite`:

- pending device pairing requests (short-lived; they expire after 5 minutes)
- paired devices + tokens

Older gateways kept this state in `~/.grokbot/devices/*.json`; those files are
imported into SQLite at gateway startup and archived with a `.migrated` suffix.

### Notes

- The `node.pair.*` API (CLI: `grokbot nodes pending|approve|reject|remove|rename`) manages
