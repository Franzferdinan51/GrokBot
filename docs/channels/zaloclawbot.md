---
summary: "Zalo ClawBot channel setup through the external grokbot-zaloclawbot plugin"
read_when:
  - You want a personal Zalo assistant bot with QR-code login
  - You are installing or troubleshooting the grokbot-zaloclawbot channel plugin
title: "Zalo ClawBot"
---

GrokBot connects to Zalo ClawBot through the catalog-listed external `@zalo-platforms/grokbot-zaloclawbot` plugin. Login uses a Zalo Mini App QR code; the plugin id in config is `grokbot-zaloclawbot`.

## Compatibility

| Plugin Version | GrokBot Version | npm dist-tag | Status        |
| -------------- | ---------------- | ------------ | ------------- |
...
- Node.js >= 22
- [GrokBot](https://docs.grokbot.ai/install) installed (`grokbot` CLI available)
...
```bash
grokbot plugins install "@zalo-platforms/grokbot-zaloclawbot@0.1.4"
```

Use the exact pinned version so GrokBot verifies the package against the catalog integrity hash during install.

### 2. Enable the plugin in config

```bash
grokbot config set plugins.entries.grokbot-zaloclawbot.enabled true
```

### 3. Generate a QR code and log in

```bash
grokbot channels login --channel grokbot-zaloclawbot
```
...
The plugin manages bot credentials under the GrokBot state directory. Treat that directory as sensitive and cover it under the same access-control and backup policy as the rest of GrokBot state.

This plugin's runtime lives entirely in the external `@zalo-platforms/grokbot-zaloclawbot` package; behavior details below beyond install/config are as reported by the plugin's maintainers and are not verified against GrokBot core source.
...
- **Gateway fails to load:** confirm your GrokBot host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger this ID requires.
