...
Goal: let GrokBot sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
    ...
    list: [
      {
        id: "main",
        groupChat: {
          mentionPatterns: ["@?grokbot", "\\+?15555550123"],
        },
      },
    ],
  },
...
1. Add your WhatsApp account (the one running GrokBot) to the group.
2. Say `@grokbot ...` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@grokbot` ping in the group and confirm a reply that references the sender name.
