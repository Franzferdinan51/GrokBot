...
Nextcloud Talk is a downloadable channel plugin (`@grokbot/nextcloud-talk`) that connects GrokBot to a self-hosted Nextcloud instance through a Talk webhook bot. Direct messages, rooms, reactions, and markdown messages are supported; media goes out as URLs.

## Install

```bash
grokbot plugins install @grokbot/nextcloud-talk
```
...
```bash
grokbot plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "GrokBot" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure GrokBot:
   ...
   ```bash
   grokbot channels add --channel nextcloud-talk \
...
- With `apiUser`/`apiPassword` and `webhookPublicUrl` set, `grokbot channels status` probes the bot and warns when the `response` feature is missing.

## Access control (DMs)

- Default: `channels.nextcloud-talk.dmPolicy = "pairing"`. Unknown senders get a pairing code.
- Approve via:
  - `grokbot pairing list nextcloud-talk`
  - `grokbot pairing approve nextcloud-talk <CODE>`
