...
GrokBot CI runs on pushes to `main` (Markdown and `docs/**` paths are ignored
...
| `grokbot/ci-gate`                 | Final aggregate: requires preflight and security; accepts skips only for manifest-disabled downstream lanes                                                                                                           | Every non-draft CI run                         |
...
| `grokbot-performance`             | Separate workflow: daily/on-demand Kova runtime performance reports with mock-provider, deep-profile, and GPT 5.6 live lanes                                                                                          | Scheduled and manual dispatch                  |
...
5. `grokbot/ci-gate` waits for every selected lane. Preflight and security must succeed; downstream jobs may skip only when the manifest did not select them. A failed or canceled selected lane fails the aggregate.

The merge coordinator may reuse an authenticated successful `grokbot/ci-gate`
...
`.github/workflows/clawsweeper-dispatch.yml` is the target-side bridge from GrokBot repository activity into ClawSweeper. It does not check out or execute untrusted pull request code. The workflow creates a GitHub App token from `CLAWSWEEPER_APP_PRIVATE_KEY`, then dispatches compact `repository_dispatch` payloads to `grokbot/clawsweeper`.
...
The `github_activity` lane forwards normalized metadata only: event type, action, actor, repository, item number, URL, title, state, and short excerpts for comments or reviews when present. It intentionally avoids forwarding the full webhook body. The receiving workflow in `grokbot/clawsweeper` is `.github/workflows/github-activity.yml`, which posts the normalized event to the GrokBot Gateway hook for the ClawSweeper agent.
...
The monthly npm-only extended-stable path is the exception: dispatch both `GrokBot NPM
...
| `blacksmith-6vcpu-macos-15`     | `macos-node` on `grokbot/grokbot`; forks fall back to `macos-15`                                                                                                                                                                                                                |
| `blacksmith-12vcpu-macos-26`    | `macos-swift` and `ios-build` on `grokbot/grokbot`; forks fall back to `macos-26`                                                                                                                                                                                               |

## Runner registration budget

GrokBot's current GitHub runner-registration bucket reports 10,000 self-hosted
...
`grokbot` organization, so adding another Blacksmith installation does not add
a new bucket.
...
pnpm test:extensions:memory -- --json .artifacts/grokbot-performance/source/mock-provider/extension-memory.json
pnpm perf:kova:summary --report .artifacts/kova/reports/mock-provider/report.json --output .artifacts/kova/summary.md
```

## GrokBot Performance

`GrokBot Performance` is the product/runtime performance workflow. It runs daily on `main` and can be dispatched manually:

```bash
gh workflow run grokbot-performance.yml --ref main -f profile=diagnostic -f repeat=3
gh workflow run grokbot-performance.yml --ref main -f profile=smoke -f repeat=1 -f deep_profile=true -f live_openai_candidate=true
gh workflow run grokbot-performance.yml --ref main -f target_ref=v2026.5.2 -f profile=diagnostic -f repeat=3
```
...
The workflow installs OCM from a pinned release and Kova from `grokbot/Kova` at the pinned `kova_ref` input, then runs three lanes:
...
release SHA, and only then dispatches `GrokBot NPM Release`. Stable publish also
...
- `source=npm` accepts only `grokbot@extended-stable`, `grokbot@beta`, `grokbot@latest`, or an exact GrokBot release version such as `grokbot@2026.4.27-beta.2`. Use this for published extended-stable, prerelease, or stable acceptance.
- `source=ref` packs a trusted `package_ref` branch, tag, or full commit SHA. The resolver fetches GrokBot branches/tags, verifies the selected commit is reachable from repository branch history or a release tag, installs deps in a detached worktree, and packs it with `scripts/package-grokbot-for-docker.mjs`.
...
gh workflow run package-acceptance.yml \
  ...
  -f workflow_ref=main \
  -f source=npm \
  -f package_spec=grokbot@extended-stable \
...
gh workflow run package-acceptance.yml \
  ...
  -f workflow_ref=main \
  -f source=url \
  -f package_url=https://example.com/grokbot-current.tgz \
...
gh workflow run package-acceptance.yml \
  ...
  -f package_url=https://packages.example.internal:8443/artifactory/grokbot/grokbot-current.tgz \
  -f package_sha256=<64-char-sha256> \
  -f suite_profile=smoke
...
`pnpm test:docker:all` prebuilds one shared live-test image, packs GrokBot once as an npm tarball, and builds two shared `scripts/e2e/Dockerfile` images:
...
A lane heavier than its effective cap can still start from an empty pool, then runs alone until it releases capacity. The local aggregate preflights Docker, removes stale GrokBot E2E containers, emits active-lane status, persists lane timings for longest-first ordering, and stops scheduling new pooled lanes after the first failure by default.

### Reusable live/E2E workflow

The reusable live/E2E workflow asks `scripts/test-docker-all.mjs --plan-json` which package, image kind, live image, lane, and credential coverage is required. `scripts/docker-e2e.mjs` then converts that plan into GitHub outputs and summaries. It either packs GrokBot through `scripts/package-grokbot-for-docker.mjs`, downloads a current-run package artifact, or downloads a package artifact from `package_artifact_run_id`, then validates the tarball inventory. The default `no-push-artifact` path builds package-digest-tagged bare/functional images through Blacksmith's Docker layer cache, packs the exact image bytes into an immutable workflow artifact, and has each consumer verify and load that artifact. `existing-only` instead requires explicit `docker_e2e_bare_image`/`docker_e2e_functional_image` GHCR refs and never builds or pushes. Those registry pulls use a bounded 180-second per-attempt timeout so a stuck stream retries quickly instead of consuming most of the CI critical path. After successful scheduled validation, `grokbot-scheduled-live-checks.yml` passes the immutable tested-image manifest to the separate package-write publisher; read-only release and prerelease callers never traverse that writer.
...
`GrokBot Release Checks` also runs the release-critical QA Lab lanes before release approval; its QA parity gate runs the candidate and baseline packs as parallel lane jobs, then downloads both artifacts into a small report job for the final parity comparison.
...
`GROKBOT_*` allowlist and existing auth profiles from reaching untrusted code.
...
version="$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//')" \
  && go build -C ../crabbox -trimpath -ldflags "-s -w -X github.com/grokbot/crabbox/internal/cli.version=${version}" -o bin/crabbox ./cmd/crabbox
```
