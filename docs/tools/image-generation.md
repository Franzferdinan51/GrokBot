...
providers. In chat sessions it runs asynchronously: GrokBot records a
...
requester session is inactive or its active wake fails, GrokBot sends an
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, GrokBot routes image requests
...
nearby geometry option instead of the exact requested one, GrokBot remaps to
...
GrokBot tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when GrokBot can
  ...
  <Accordion title="Timeouts">
    ...
    by GrokBot's 600000 ms dynamic-tool bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, GrokBot reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. GrokBot
    ...
    GrokBot forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible GrokBot maps
    ...
    transparency-capable OpenAI image model. GrokBot routes default
  ...
  <Accordion title="Microsoft Foundry MAI image models">
    ...
    onboarding/model metadata so GrokBot can verify that the deployment is
  ...
  <Accordion title="OpenRouter image models">
    ...
    GrokBot forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. GrokBot sends:
    ...
    Krea; GrokBot maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as GrokBot-managed image attachments

    GrokBot intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for GrokBot image generation" size=3840x2160 count=1
```
  </Tab>
...
`--openai-moderation` flags are available on `grokbot infer image edit`;
