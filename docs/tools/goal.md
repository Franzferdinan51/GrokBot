...
read_when:
  - You want GrokBot to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current GrokBot session.
...
stale or unknown token snapshot when the goal starts, GrokBot waits for the
...
GrokBot usage and model controls.

## Model tools

GrokBot exposes three goal tools to agent harnesses:
...
GrokBot keeps the line compact by truncating long objectives. Paused,
...
`/goal` works in command-capable GrokBot sessions, including the TUI and
...
fresh token snapshot yet. Usage refreshes as GrokBot records session usage
