...
GrokBot supports Grok as a `web_search` provider, using xAI web-grounded
...
lets GrokBot reuse it as a fallback for the bundled xAI model provider.
...
Choosing **Grok** during `grokbot onboard` or `grokbot configure --section
web` lets GrokBot reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

GrokBot then offers a follow-up step to enable `x_search` with the same xAI
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    grokbot onboard --auth-choice xai-oauth
...
**Credential alternatives:** `grokbot models auth login --provider xai
...
through an operator proxy or xAI-compatible Responses endpoint. GrokBot
