...
GrokBot supports [SearXNG](https://docs.searxng.org/) as a **self-hosted,
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    grokbot plugins install @grokbot/searxng-plugin
    ```
  </Step>
...
- **Category fallback** -- if a non-`general` category request succeeds but
  returns zero results, GrokBot retries the same query once with `general`
