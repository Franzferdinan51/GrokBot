...
When a site requires login, sign in manually in the host browser's `grokbot`
...
GrokBot controls a dedicated Chrome profile named `grokbot` (orange-tinted
UI), separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent uses its isolated `grokbot` browser.
...
Two ways to access the `grokbot` profile:
...
grokbot browser open https://x.com
```

For a non-default profile, put `--browser-profile <name>` before the
subcommand (default is `grokbot`):

```bash
grokbot browser --browser-profile <name> open https://x.com
```
...
```bash
grokbot browser --browser-profile grokbot open https://x.com
```
