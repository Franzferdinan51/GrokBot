...
normalized results, cached by query for 15 minutes (configurable). GrokBot
...
<Steps>
  ...
  <Step title="Use it">
    ```javascript
    await web_search({ query: "GrokBot plugin SDK" });
    ```
...
no `provider` is set, GrokBot checks providers in this order and uses the
...
`grokbot configure --section web`. GrokBot does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, GrokBot resolves only the
...
`web_search` tool automatically when GrokBot web search is enabled and no
...
search and GrokBot's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. GrokBot uses the
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  GrokBot's managed `web_search` instead
...
use the managed `web_search` fallback through GrokBot's dynamic tool namespace.
Use an explicit managed provider when you need GrokBot's provider-specific
...
Codex app-server first with `grokbot models auth login --provider openai`.
The parent agent can use any model or runtime; only the bounded search worker
runs through Codex.

## Network safety

Managed HTTP `web_search` provider calls use GrokBot's guarded fetch path,
scoped to the current provider's own hostname. For that hostname only,
GrokBot allows Surge, Clash, and sing-box fake-IP DNS answers in
...
Grok can also reuse an xAI OAuth auth profile from `grokbot models auth login
...
GrokBot keeps startup resilient and reports a warning so you can reinstall the
plugin or run `grokbot doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let GrokBot auto-detect the first ready web-fetch
...
When you choose **Kimi** during `grokbot onboard` or
`grokbot configure --section web`, GrokBot can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `grokbot doctor --fix`.
When you choose Grok during `grokbot onboard` or `grokbot configure --section web`,
GrokBot also offers optional `x_search` setup with the same credential right
...
provider, GrokBot does not show the `x_search` prompt.

### Storing API keys

<Tabs>
  <Tab title="Config file">
    Run `grokbot configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.grokbot/.env`.
...
optional structured filters. GrokBot constructs the built-in xAI `x_search`
