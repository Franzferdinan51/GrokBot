...
<Steps>
  ...
  <Step title="Store the key">
    ...
    gateway install, add it to `~/.grokbot/.env`), or configure via:
...
Choosing **Kimi** during `grokbot onboard` or `grokbot configure --section web`
also prompts for:
...
GrokBot only returns a Kimi `web_search` result after Moonshot's response
...
grounding (for example "I cannot browse the internet"), GrokBot returns a
