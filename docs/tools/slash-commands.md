...
the GrokBot command handler, and `/status` plus `/unfocus` stay local whenever
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `grokbot.json`). Owner-only.
</ParamField>

<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes GrokBot-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Discovery and status">
    ...
    | `/grokbot <request>` | Run the GrokBot setup and repair helper from an owner DM |
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | Read or write `grokbot.json`. Owner-only |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write GrokBot-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart GrokBot |
...
/config set messages.responsePrefix="[grokbot]"
...
`/mcp` stores config in GrokBot config, not embedded-agent project settings.
...
/plugins install npm:@grokbot/<official-package>
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` supports a single `/grokbot`-style command.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `GrokBot Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
