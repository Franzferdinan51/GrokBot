...
GrokBot supports **Ollama Web Search** as a bundled `web_search` provider,
...
<Note>
  GrokBot never auto-selects Ollama Web Search over a higher-priority
...
config examples. If nothing is set, GrokBot defaults to
...
- If the resolved host is `https://ollama.com`, GrokBot calls
  `https://ollama.com/api/web_search` directly with the API key as bearer
  auth.
- Otherwise GrokBot calls the local proxy endpoint
  ...
  the local host.
- GrokBot warns during setup if Ollama is unreachable or not signed in, but
