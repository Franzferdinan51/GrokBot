...
summary: "Build, test, and publish custom SKILL.md workspace skills for your GrokBot agents."
read_when:
...
GrokBot loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.grokbot/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.grokbot/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    GrokBot watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ```bash
    grokbot agent --message "give me a greeting"
    ```
...
metadata: { "grokbot": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `grokbot.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `grokbot.json`:
...
grokbot skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.grokbot` gating fields
...
<Tip>
  ...
  - **Test locally** — use `grokbot agent --message "..."` before sharing.
