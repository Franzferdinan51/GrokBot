...
read_when:
  - Running GrokBot Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, GrokBot Gateway runs inside WSL2, Chrome runs
on Windows, and browser control must cross the WSL2/Windows boundary. Several
independent problems can surface at once (see
[issue #39369](https://github.com/grokbot/grokbot/issues/39369)): CDP
...
- GrokBot points a browser profile at the address reachable from WSL2
...
```powershell
chrome.exe --remote-debugging-port=9222 --user-data-dir="$env:LOCALAPPDATA\GrokBot\ChromeCDP"
```
...
If this fails, diagnose the Windows listeners below. GrokBot is not the
problem yet.
...
that before touching GrokBot config.

### Layer 3: configure the correct browser profile

Point GrokBot at the address reachable from WSL2:
...
- use HTTP(S) when you want GrokBot to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools
  socket URL
- test the same URL with `curl` before expecting GrokBot to succeed
...
```bash
grokbot browser --browser-profile remote open https://example.com
grokbot browser --browser-profile remote tabs
```
...
| stale viewport / dark-mode / locale / offline overrides after a remote session          | run `grokbot browser --browser-profile remote stop` to close the session and release the cached Playwright/CDP connection without restarting the Gateway or the external browser |
...
3. GrokBot config: does `browser.profiles.<name>.cdpUrl` use that exact
...
from WSL2 second, and only then debug GrokBot config or Control UI auth.
