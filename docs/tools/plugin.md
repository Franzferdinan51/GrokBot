---
summary: "Install, configure, and manage GrokBot plugins"
read_when:
...
Plugins extend GrokBot with channels, model providers, agent harnesses, tools,
...
- an GrokBot checkout or installation with the `grokbot` CLI available
...
<Steps>
  <Step title="Find the plugin">
    ...
    they match an official plugin id. Raw `@grokbot/*` specs that match a
  ...
  <Step title="Install the plugin">
    ...
    # From npm.
    grokbot plugins install npm:<package>

    # From git.
    grokbot plugins install git:github.com/<owner>/<repo>@<ref>

    # From a local development checkout.
    grokbot plugins install ./my-plugin
    grokbot plugins install --link ./my-plugin
    ```

    Treat plugin installs like running code. Prefer pinned versions for
    reproducible production installs. ClawHub packages and GrokBot's
  ...
  <Step title="Configure and enable it">
    ...
    ```bash
    grokbot plugins enable <plugin-id>
    ```

    If `plugins.allow` is set, the installed plugin id must be in that list
    before the plugin can load. `grokbot plugins install` adds the installed
  ...
  <Step title="Verify runtime registration">
    ```bash
    grokbot plugins inspect <plugin-id> --runtime --json
    ```
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want GrokBot-native discovery, scans, version metadata, and install hints | `grokbot plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `grokbot plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `grokbot plugins install git:github.com/<owner>/<repo>@<ref>` |
| local path  | You are developing or testing a plugin on the same machine                     | `grokbot plugins install --link ./my-plugin`                  |
| marketplace | You are installing a Claude-compatible marketplace plugin                      | `grokbot plugins install <plugin> --marketplace <source>`     |
...
bare spec installs through npm during the launch cutover. Raw `@grokbot/*`
specs that match bundled plugins also resolve to the bundled copy before npm
fallback. Use `npm:@grokbot/<plugin>@<version>` to deliberately install the
external npm package instead of the bundled copy. Use `clawhub:`, `npm:`,
`git:`, or `npm-pack:` for deterministic source selection. See
[`grokbot plugins`](/cli/plugins#install) for the full command contract.

For npm installs, unpinned specs and `@latest` choose the newest stable
package that advertises compatibility with this GrokBot build. If npm's
current latest release declares a newer `grokbot.compat.pluginApi` or
`grokbot.install.minHostVersion` than this build supports, GrokBot scans
...
later, and only in GrokBot processes where plugin hooks are loaded, so use
...
compatibility but is a no-op: it does not bypass install policy or GrokBot's
...
snippet. Run [`grokbot plugins list --enabled --verbose`](/cli/plugins#list)
or [`grokbot plugins inspect <id>`](/cli/plugins#inspect) on the listed
plugin id before copying trusted plugins into `grokbot.json`. The same
...
`plugins.allow` or reinstall from a trusted source so GrokBot records install
provenance.

Run `grokbot doctor` or `grokbot doctor --fix` when config validation
reports stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin
paths.

## Understand plugin formats

GrokBot recognizes two plugin formats:

| Format                 | How it loads                                                                 | Use when                                                               |
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native GrokBot plugin | `grokbot.plugin.json` plus a runtime module loaded in process               | You are installing or building GrokBot-specific runtime capabilities  |
| Compatible bundle      | Codex, Claude, or Cursor plugin layout mapped into GrokBot plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `grokbot plugins list`, `grokbot plugins inspect`,
`grokbot plugins enable`, and `grokbot plugins disable`. See
...
Plugin-managed internal hooks show up in `grokbot hooks list` with
`plugin:<id>`. You cannot enable or disable them through `grokbot hooks`;
enable or disable the plugin instead.

## Verify the active Gateway

`grokbot plugins list` and plain `grokbot plugins inspect` read cold config,
...
sure any manual restart targets the actual `grokbot gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `grokbot plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `grokbot plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `grokbot doctor --fix` if it points to stale plugin state                                             | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `grokbot plugins registry --refresh`                    |
...
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `grokbot plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |

When an enabled managed plugin fails payload verification during Gateway
startup, GrokBot quarantines that exact installed plugin root for the boot and
continues serving other plugins. `grokbot status --all`, `grokbot health`,
and `grokbot doctor` report it as `configured-unavailable`. Fix or reinstall
...
`grokbot doctor --fix` to remove stale plugin and channel entries. Unknown
...
plugin id. If both plugins are explicitly enabled, GrokBot keeps that request
...
GrokBot needs at runtime. Update or reinstall after the publisher ships
...
and validation follows with `plugin present but blocked`, GrokBot found
plugin files owned by a different Unix user than the process loading them.
Keep the plugin config in place; fix the filesystem ownership or run GrokBot
as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted GrokBot config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run GrokBot as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/grokbot-config/npm
```

After fixing ownership, rerun `grokbot doctor --fix` or
`grokbot plugins registry --refresh` so the persisted plugin registry
...
```bash
grokbot config set logging.level trace
grokbot logs --follow
```
...
GrokBot caches successful plugin tool factory results for repeated
...
- [`grokbot plugins`](/cli/plugins) - full CLI reference
