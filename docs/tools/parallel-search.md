      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; GrokBot appends /v1/search
          },
        },
      },
    },
...
environment. For a gateway install, put it in `~/.grokbot/.env`.
...
Cloudflare AI Gateway). GrokBot normalizes bare hosts by prepending
...
- Both providers return a `session_id`; GrokBot surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- GrokBot always forwards a resolved result count to Parallel as
  ...
  GrokBot's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.
