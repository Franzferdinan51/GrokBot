...
frontmatter and a markdown body. GrokBot loads bundled skills plus any local
...
GrokBot loads from these sources, **highest precedence first**. When the same
...
| 4           | Managed / local skills | `~/.grokbot/skills`                    |
| 5           | Bundled skills         | shipped with the install                |
| 6 — lowest  | Extra directories      | `skills.load.extraDirs` + plugin skills |

Skill roots support grouped layouts. GrokBot discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an GrokBot
  skill root. Use `grokbot migrate plan codex` to inventory those skills, then
  `grokbot migrate codex` to copy them into your GrokBot workspace.
</Note>

## Node-hosted skills

A connected headless node can publish skills installed in its active GrokBot
skills directory (`~/.grokbot/skills` by default; profile environment overrides
...
| Shared managed | `~/.grokbot/skills`         | All agents on this machine  |
...
`grokbot.plugin.json` (paths relative to the plugin root). Plugin skills load
...
`metadata.grokbot.requires` in its frontmatter, same as any other skill.
...
`grokbot skills` commands for install and update, or the `clawhub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `grokbot skills install @owner/<slug>`                |
| Install from a Git repository      | `grokbot skills install git:owner/repo@ref`           |
| Install a local skill directory    | `grokbot skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `grokbot skills install @owner/<slug> --global`       |
| Update all workspace skills        | `grokbot skills update --all`                         |
| Update a shared managed skill      | `grokbot skills update @owner/<slug> --global`        |
| Update all shared managed skills   | `grokbot skills update --all --global`                |
| Verify a skill's trust envelope    | `grokbot skills verify @owner/<slug>`                 |
| Print the generated Skill Card     | `grokbot skills verify @owner/<slug> --card`          |
| Publish / sync via ClawHub CLI     | `clawhub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `grokbot skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.grokbot/skills` directory, visible to all local agents unless agent
    ...
    `grokbot skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `grokbot skills verify @owner/<slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `grokbot.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.grokbot/skills` and personal `~/.agents/skills` may contain
...
<Note>
  GrokBot follows the [AgentSkills](https://agentskills.io) spec. Frontmatter
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, GrokBot keeps the skill's instructions out of the agent's normal
...
GrokBot filters skills at load time using `metadata.grokbot` (JSON5 object
embedded in the frontmatter, see the parsing note above). A skill with no
`metadata.grokbot` block is always eligible unless explicitly disabled.
...
<ParamField path="requires.config" type="string[]">
  Each `grokbot.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.grokbot` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, GrokBot lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `grokbot.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** GrokBot does not auto-install Homebrew or translate brew
    ...
    - **Go:** GrokBot requires Go 1.21 or newer for automatic skill installs.
      If `go` is missing and Homebrew is available, GrokBot installs Go via
      ...
      dependency always targets a dedicated GrokBot-managed bin directory
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.grokbot/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.grokbot.primaryEnv`.
...
<Note>
  ...
  `metadata.grokbot.skillKey`, use that key under `skills.entries` instead.
  Quote hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, GrokBot:

<Steps>
  <Step title="Reads skill metadata">
    GrokBot resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, GrokBot also materializes the same
...
GrokBot snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, GrokBot refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, GrokBot watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, GrokBot can treat macOS-only skills as eligible when
    ...
    answering bin probes, GrokBot clears its cached bin matches.
...
When skills are eligible, GrokBot injects a compact XML block into the system
...
(`skills.limits.maxSkillsPromptChars`), GrokBot first preserves as many skill
...
note pointing at `grokbot skills check` whenever compact formatting or list
