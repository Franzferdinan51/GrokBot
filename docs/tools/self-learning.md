---
summary: "Let GrokBot propose reusable skills from corrections and substantial completed work"
read_when:
  - You want GrokBot to learn reusable procedures from completed conversations
...
Self-learning lets GrokBot turn useful evidence from conversations into pending
...
```bash
grokbot config set skills.workshop.autonomous.enabled true --strict-json
```

Or edit `~/.grokbot/grokbot.json`:
...
```bash
grokbot config set skills.workshop.autonomous.enabled false --strict-json
```
...
GrokBot persists only cursor and coverage metadata in the shared state database;
it does not create a second transcript archive.

Sessions are scanned only when GrokBot can prove their ownership and exclude
...
## What GrokBot can learn

Self-learning has two conservative paths:

1. **Direct instructions and corrections.** GrokBot detects durable language
...
2. **Experience review.** After a successful, substantial foreground turn,
   GrokBot can review the completed work for a reusable recovery technique or
...
- GrokBot waits 30 seconds after completion. A later foreground completion in
...
when necessary, GrokBot keeps the first message and the newest evidence and
...
Before starting, GrokBot reloads current runtime configuration and rechecks the
...
grokbot skills workshop reject <proposal-id> --reason "Too specific"
...
When self-learning is enabled, `grokbot doctor` checks whether the default
