...
- allowlist entries live in the host-local approvals file under `agents.<id>.allowlist` (or via Control UI / `grokbot approvals allowlist ...`).
- `grokbot security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `grokbot doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
- If GrokBot cannot identify exactly one concrete local file for an interpreter/runtime command
...
session, GrokBot also resumes that session with an internal followup so the agent observes that
...
After an approved async exec finishes, GrokBot sends a followup `agent` turn to the same session.
...
channel, GrokBot keeps the local deterministic `/approve` prompt visible. If the native runtime is
active and attempts delivery but no target receives the card, GrokBot sends a same-chat fallback
...
- Telegram defaults to approver DMs (`target: "dm"`). Switch to `channel` or `both` to also show
  approval prompts in the originating Telegram chat/topic. For Telegram forum topics, GrokBot
...
- Matrix native DM/channel routing and reaction shortcuts handle both exec and plugin approvals;
  ...
  include `com.grokbot.approval` custom event content on the first prompt event so GrokBot-aware
...
owner routing for approval prompts and final results. GrokBot first tries a private route on the
