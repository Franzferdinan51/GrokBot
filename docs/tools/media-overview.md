...
read_when:
  - Looking for an overview of GrokBot's media capabilities
...
GrokBot generates images, videos, and music, understands inbound media
...
GrokBot until the shared realtime-voice contract can represent it.
</Note>
...
For async tools, GrokBot submits the request to the provider, returns a task
...
GrokBot wakes the agent with the generated media paths so it can tell the
...
GrokBot sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    other native model types; GrokBot has no provider contract for those
  ...
  <Accordion title="xAI">
    ...
    not registered in GrokBot until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
