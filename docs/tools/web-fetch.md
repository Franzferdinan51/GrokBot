...
`plugins.entries.firecrawl.config.webFetch` via `grokbot doctor --fix`.
...
- If `provider` is omitted, GrokBot auto-detects the first ready web-fetch
...
In this mode, GrokBot still applies hostname-based SSRF checks before sending
