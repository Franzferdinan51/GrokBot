---
summary: "Typed workflow runtime for GrokBot with resumable approval gates."
title: Lobster
read_when:
...
see [Task Flow](/automation/taskflow) (`grokbot tasks flow`); for the task
...
→ grokbot calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ grokbot calls gmail.send
...
GrokBot runs Lobster workflows **in-process** using the bundled
...
[Lobster repo](https://github.com/grokbot/lobster) and put `lobster` on
`PATH`.
...
gog.gmail.search --query 'newer_than:1d' \
  | grokbot.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
### Important limitation: embedded Lobster vs `grokbot.invoke`

The bundled Lobster plugin runs workflows **in-process** inside the gateway.
In that embedded mode, `grokbot.invoke` does **not** automatically inherit a
gateway URL/auth context for nested GrokBot CLI tool calls.

That means this pattern is **not currently reliable in the embedded runner**:

```lobster
grokbot.invoke --tool llm-task --action json --args-json '{ ... }'
```

Use the example below only when running the **standalone Lobster CLI** in an
environment where `grokbot.invoke` is already configured with the correct
...
- non-`grokbot.invoke` steps inside the Lobster pipeline until a supported
...
a bare envelope: GrokBot creates or resumes a durable flow record, applies the
...
- **No secrets** - Lobster doesn't manage OAuth; it calls GrokBot tools that
  do.
