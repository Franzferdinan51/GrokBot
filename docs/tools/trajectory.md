---
summary: "Export redacted trajectory bundles for debugging an GrokBot agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an GrokBot session
...
Trajectory capture is GrokBot's per-session flight recorder. It records a
...
GrokBot writes the bundle under the workspace:

```text
.grokbot/trajectory-exports/grokbot-trajectory-<session>-<timestamp>/
```
...
The name resolves inside `.grokbot/trajectory-exports/`. Absolute paths and
`~` paths are rejected.
...
bundle; do not use allow-all. In group chats, GrokBot sends the approval
...
`.grokbot/trajectory-exports`), `--store <path>` (session store override),
...
Trajectory capture is on by default for GrokBot agent runs.
...
| `metadata.json`       | GrokBot version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
This disables runtime trajectory capture before starting GrokBot.
...
GrokBot flushes runtime trajectory rows during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`OPENCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting GrokBot:

```bash
export OPENCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when GrokBot logs an `grokbot-trajectory-flush` timeout and
...
Trajectory bundles are for support and debugging, not public posting. GrokBot
...
- confirm GrokBot was started without `OPENCLAW_TRAJECTORY=0`
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.grokbot/trajectory-exports/`
