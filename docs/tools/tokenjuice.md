...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in GrokBot
...
Today this applies to GrokBot embedded runs and GrokBot dynamic tools in the Codex
app-server harness. Tokenjuice hooks GrokBot's tool-result middleware and
...
```bash
grokbot plugins install clawhub:@grokbot/tokenjuice
```
...
```bash
grokbot config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
grokbot plugins disable tokenjuice
```
