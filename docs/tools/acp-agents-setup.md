...
read_when:
  ...
  - Enabling the plugin-tools or GrokBot-tools MCP bridge
...
| `grokbot`   | GrokBot ACP bridge (native `grokbot acp`)                                                                     |
...
When GrokBot uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal GrokBot `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by GrokBot before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, GrokBot/acpx cannot force a model selection.
...
Packaged installs use the official `@grokbot/acpx` runtime plugin for ACP.
...
```bash
grokbot plugins install ./path/to/local/acpx-plugin
```
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before GrokBot passes it through the current acpx command-string registry.
...
manually, and there is no separate postinstall step for GrokBot itself. If an
adapter download or spawn fails, `/acp doctor` reports the failure.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose GrokBot plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
GrokBot plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
grokbot config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `grokbot-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled GrokBot
  plugins.
...
### GrokBot tools MCP bridge

By default, ACPX sessions also do **not** expose built-in GrokBot tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
grokbot config set plugins.entries.acpx.config.openClawToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `grokbot-tools` into ACPX session
  bootstrap.
- Exposes selected built-in GrokBot tools. The initial server exposes `cron`.
...
```bash
grokbot config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use GrokBot agent/run timeouts, including `/acp timeout`.
...
```bash
grokbot config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from GrokBot exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between GrokBot `tools.exec.mode`, Codex Guardian
...
```bash
grokbot config set plugins.entries.acpx.config.permissionMode approve-all
grokbot config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
GrokBot defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `PermissionPromptUnavailableError: Permission prompt unavailable in non-interactive mode`.
