...
GrokBot agents generate videos from text prompts, reference images, or
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    grokbot config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. GrokBot submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, GrokBot wakes the same session with an internal completion event.
4. The agent reports it through the session's normal visible-reply mode:
   ...
   generated media is still missing from the completion reply, GrokBot sends
...
generation, or `grokbot tasks list` / `grokbot tasks show <lookup>` from the
...
Generated video files save under GrokBot-managed media storage when the
...
provider also returns a hosted output URL, GrokBot delivers that URL instead
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. GrokBot normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `360P`, `480P`, `540P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. GrokBot normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, GrokBot uses `agents.defaults.videoGenerationModel.timeoutMs` if configured, otherwise the plugin-authored provider default when one exists.</ParamField>
...
<Note>
Not all providers support all parameters. GrokBot normalizes duration to
...
GrokBot resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5 plugin">
    Requires the [`@grokbot/byteplus-modelark`](https://www.npmjs.com/package/@grokbot/byteplus-modelark)
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. GrokBot waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. GrokBot submits the
...
Set the default video-generation model in your GrokBot config:
...
```bash
grokbot config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
