---
summary: "Chrome extension: let GrokBot drive your signed-in Chrome with no remote-debugging prompt"
read_when:
...
The GrokBot Chrome extension lets an agent control your **signed-in Chrome
tabs** without launching a separate managed browser, and **without** Chrome's
blocking "Allow remote debugging?" prompt.

This matters when you drive GrokBot from a phone (Telegram, WhatsApp, etc.):
the [`user` profile](/tools/browser#profiles-grokbot-user-chrome) attaches over
...
so the only in-page hint is Chrome's dismissible "GrokBot started debugging
this browser" banner.
...
- **Extension relay** (loopback WebSocket): a small server the control service
  ...
  GrokBot and speaks to the extension. Both sides authenticate with a
  host-local token (see below).
- **GrokBot Chrome extension** (MV3): attaches to tabs with `chrome.debugger`,
  forwards CDP traffic, and manages the **GrokBot tab group**.

GrokBot only sees and controls tabs that are in the **GrokBot tab group**. The
...
1. Print the unpacked extension path:
   ...
   ```bash
   grokbot browser extension path
   ```
...
3. Print the pairing string:
   ...
   ```bash
   grokbot browser extension pair
   ```

4. Click the GrokBot toolbar icon and paste the pairing string into the popup.
...
```bash
grokbot config set browser.defaultProfile chrome
```
...
- Share a tab: click the GrokBot toolbar button on that tab (it joins the
  GrokBot tab group), or drag any tab into the group.
...
GrokBot configures `sidepanel.html` for that exact Chrome tab; the manifest has
...
## Send a page to GrokBot

Use **Send page to GrokBot** in the toolbar popup to share readable page text
with your main GrokBot session. You can add an optional note, use the page or
selection right-click menu, or press `Alt+Shift+S`. GrokBot prefers your current
selection when one exists, enqueues the share as a system event, and wakes the
main session immediately.

The tab does not need to be in the GrokBot tab group. This is a one-shot,
...
Page text is wrapped in GrokBot's external-content safety boundary. Your
...
- **Same host** (Gateway + Chrome on one machine): pair on that machine with
  `grokbot browser extension pair`. The relay is loopback-only.
...
- **Direct to a remote Gateway** (Chrome on your laptop, Gateway on a VPS, and
  **nothing else on the laptop**): on the Gateway, run
  `grokbot browser extension pair --gateway-url wss://your-gateway.example.com`.
  ...
  over `wss://` — no GrokBot install, Node, CLI, or open inbound port on the
  laptop. This is the managed-hosting path.
- **Via a browser node host** (Chrome on a machine already running an GrokBot
...
```bash
grokbot browser status --browser-profile chrome
grokbot browser doctor --browser-profile chrome
```
...
- The agent can only see and drive tabs in the **GrokBot tab group**. Your
...
managed `grokbot` and Chrome MCP `user` profiles.
