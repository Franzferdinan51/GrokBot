...
generated audio is still missing from the reply, GrokBot sends an
...
<Note>
Not all providers support all parameters. GrokBot still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, GrokBot
...
Provider request timeouts are operator configuration only. GrokBot uses
...
- **Status lookup:** `grokbot tasks list` or `grokbot tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** GrokBot injects an internal completion event back
...
```bash
grokbot tasks list
grokbot tasks show <taskId>
grokbot tasks cancel <taskId>
```
