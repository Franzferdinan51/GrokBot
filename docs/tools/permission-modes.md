...
read_when:
  ...
  - Comparing GrokBot exec approvals with ACPX harness permissions
...
```bash
grokbot config set tools.exec.mode auto
...
Set ACPX permissions separately from GrokBot exec approvals:
...
| Use Codex/GrokBot auto-review before humans  | `tools.exec.mode: "auto"`                                   |
...
Host exec uses the stricter result of GrokBot config and the host-local approvals file. ACPX harness permissions do not loosen host exec approvals, and host exec approvals do not loosen ACPX harness prompts.
