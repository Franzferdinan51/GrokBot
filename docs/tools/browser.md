...
read_when:
  ...
  - Debugging why grokbot is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (GrokBot-managed)"
---

GrokBot can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls. It runs through a small local control service inside the Gateway (loopback only) and is isolated from your personal browser.

- Think of it as a **separate, agent-only browser**. The `grokbot` profile never touches your personal browser profile.
...
- A separate browser profile named **grokbot** (orange accent by default).
...
If `grokbot browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `grokbot browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `grokbot browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `grokbot`, `user`, `chrome`

- `grokbot`: managed, isolated browser (no extension required).
- `user`: built-in Chrome DevTools MCP attach profile for your **real
  ...
  prompt the first time GrokBot attaches, so someone must be at the computer.
- `chrome`: built-in [Chrome extension](/tools/chrome-extension) profile for
  ...
  desk because it drives tabs through the GrokBot browser extension instead of
  the remote-debugging port, so there is no "Allow remote debugging?" prompt.

For agent browser tool calls:

- Default: use the isolated `grokbot` browser.
...
Set `browser.defaultProfile: "grokbot"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.grokbot/grokbot.json`.
...
Session tab cleanup applies only to tabs created by the GrokBot browser tool
with `action: "open"`. GrokBot does not adopt tabs that were already open,
...
safely after restart. This durable path can cover GrokBot-managed profiles,
regular remote CDP profiles, and existing-session profiles with an explicit
`cdpUrl`, provided GrokBot can resolve both the native target and a stable
browser identity. Before closing a durable record, GrokBot verifies that the
...
but GrokBot does not automatically attach them to channel replies. To share a
...
understanding model is configured, GrokBot keeps the normal image result so the
...
- Local `grokbot` profiles auto-assign `cdpPort`/`cdpUrl` from a range starting 9 ports above the control port (default `18800`-`18899`); set those only for
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, GrokBot pauses new launch
...
- Before each fresh GrokBot-managed Chrome launch, GrokBot best-effort disables network prediction, suppressing Chromium's observed speculative preconnect for those denied loads. This is defense in depth, not a policy boundary: a browser reused across a control-service restart and other browser backends may not share the hardening. Playwright routing is still not a network firewall and does not intercept redirect hops, a popup's first request, Service Worker traffic, page code that runs after the bounded guard window, or every background/subresource path. Complete egress isolation requires owner-side isolation or a policy-enforcing proxy.
- In strict SSRF mode, remote CDP endpoint discovery and `/json/version` probes (`cdpUrl`) are checked too.
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the GrokBot-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
- GrokBot-managed local CDP readiness probes and DevTools WebSocket connections bypass the managed network proxy for the exact launched loopback endpoint, so `grokbot browser start` still works when an operator proxy blocks loopback egress.
...
- `POST /start?headless=true` and `grokbot browser start --headless` request a
  ...
  remote CDP profiles reject the override because GrokBot does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  ...
  `grokbot browser --json status`; trailing `grokbot browser status --json`
...
- For a running local managed profile, status and doctor query Chrome's
  ...
  `grokbot browser --json status`. A passive status call does not launch Chrome.
...
- Default profile is `grokbot` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
- `driver: "extension"` drives your signed-in Chrome through the [GrokBot Chrome extension](/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
GrokBot uses it automatically. Set `browser.executablePath` to override
auto-detection. Top-level and per-profile `executablePath` values accept `~`
for your OS home directory:

```bash
grokbot config set browser.executablePath "/usr/bin/google-chrome"
grokbot config set browser.profiles.work.executablePath "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
```
...
Per-profile `executablePath` only affects local managed profiles that GrokBot
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, GrokBot will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local GrokBot-managed browser profile.
- `headless` only affects local managed profiles that GrokBot launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `grokbot browser stop` stops the browser process that
  GrokBot launched
- attach-only and remote CDP profiles: `grokbot browser stop` closes the active
  ...
  though no browser process was launched by GrokBot
...
GrokBot preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, GrokBot can
...
- If you set `nodeHost.browserProxy.allowProfiles`, GrokBot treats it as a least-privilege boundary limiting which profile names the proxy will target.
...
CDP connection URLs over HTTPS and WebSocket. GrokBot can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let GrokBot
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and GrokBot runs on the host, treat
...
GrokBot process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-GrokBot WebSocket base, such
...
an address GrokBot cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, GrokBot treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by GrokBot.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). GrokBot accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  GrokBot calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. GrokBot connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). GrokBot tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise GrokBot
  ...
  accepts it, GrokBot falls back to that root as well. This lets a bare `ws://`
...
`grokbot browser doctor` uses the same discovery-first, WebSocket-fallback
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-grokbot-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, GrokBot
...
GrokBot supports multiple named profiles (routing configs). Profiles can be:

- **grokbot-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `grokbot` profile is auto-created if missing.
...
GrokBot can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when GrokBot attaches.
...
```bash
grokbot browser --browser-profile user start
grokbot browser --browser-profile user status
grokbot browser --browser-profile user tabs
grokbot browser --browser-profile user snapshot --format ai
```
...
- `grokbot doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `grokbot` profile because it can
  act inside your signed-in browser session.
- GrokBot does not launch the browser for this driver; it only attaches.
- GrokBot uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, GrokBot skips
...
Compared to the managed `grokbot` profile, existing-session drivers are more constrained:
...
- **Dialog visibility** - Managed browser action responses include `blockedByDialog` and `browserState.dialogs.pending` when an action opens a modal dialog; snapshots also include pending dialog state. Respond with `browser dialog --accept/--dismiss --dialog-id <id>` while a dialog is pending. Dialogs handled outside GrokBot appear under `browserState.dialogs.recent`.
...
When launching locally, GrokBot picks the first available:
...
control API** plus a matching `grokbot browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means GrokBot cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "grokbot" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by grokbot` when a
...
- For the local loopback `grokbot` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for GrokBot's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (grokbot, chrome, or remote CDP).
