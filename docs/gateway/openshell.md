---
summary: "Use OpenShell as a managed sandbox backend for GrokBot agents"
title: OpenShell
read_when:
...
locally, GrokBot delegates sandbox lifecycle to the `openshell` CLI, which
...
- OpenShell plugin installed (`grokbot plugins install @grokbot/openshell-sandbox`)
- `openshell` CLI on `PATH` (or a custom path via
  `plugins.entries.openshell.config.command`)
- An OpenShell account with sandbox access
- GrokBot Gateway running on the host
...
Restart the Gateway. On the next agent turn GrokBot creates an OpenShell
...
- Before `exec`, GrokBot syncs the local workspace into the sandbox.
- After `exec`, GrokBot syncs the remote workspace back to local.
- File tools go through the sandbox bridge, but local stays source of truth
  between turns.

Best for development workflows: local edits outside GrokBot show up on the
...
- On first sandbox creation, GrokBot seeds the remote workspace from local
  once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. GrokBot does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside GrokBot after the initial seed is invisible to the remote sandbox. Run `grokbot sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"grokbot"`  | Sandbox source for first-time create                                                   |
...
1. GrokBot runs `sandbox get` for the sandbox name (with any configured
...
2. GrokBot runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
...
- [Sandbox CLI](/cli/sandbox) - `grokbot sandbox` commands
