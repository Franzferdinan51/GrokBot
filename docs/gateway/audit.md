...
The Gateway keeps a bounded, metadata-only audit ledger in the shared GrokBot
...
Records live in the shared state database (`state/grokbot.sqlite`) and are
...
schema automatically at startup (or via `grokbot doctor --fix`); existing
rows and their ledger sequences are preserved.

## Querying

- CLI: [`grokbot audit`](/cli/audit) with filters for agent, session, run,
