...
GrokBot can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the Gateway dashboard and WebSocket port. This keeps the gateway bound to loopback while Tailscale provides HTTPS, routing, and (for Serve) identity headers.
...
Status and audit output use **Tailscale exposure** for this GrokBot Serve/Funnel mode. `off` means GrokBot is not managing Serve or Funnel; it does not mean the local Tailscale daemon is stopped or logged out.
...
Startup then reports the Service URL as `https://grokbot.<tailnet-name>.ts.net/` instead of the device hostname. Tailscale Services require the host to be an approved tagged node in your tailnet — configure the tag and approve the Service in Tailscale before enabling this, otherwise `tailscale serve --service=...` fails during gateway startup.
...
grokbot gateway --tailscale funnel --auth password
```
...
- `gateway.tailscale.serviceName` applies only to Serve mode and is passed to `tailscale serve --service=<name>`. The value must use Tailscale's `svc:<dns-label>` format, for example `svc:grokbot`. Tailscale requires Service hosts to be tagged nodes, and the Service may need admin-console approval before Serve can publish it.
- `gateway.tailscale.resetOnExit` undoes `tailscale serve`/`tailscale funnel` configuration on shutdown.
- `gateway.tailscale.preserveFunnel: true` keeps an externally configured `tailscale funnel` route alive across gateway restarts. With `mode: "serve"`, GrokBot checks `tailscale funnel status` before re-applying Serve and skips it when a Funnel route already covers the gateway port. The GrokBot-managed Funnel password-only policy is unchanged.
