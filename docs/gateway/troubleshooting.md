...
- `grokbot gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `grokbot doctor` reports no blocking config/service issues.
- `grokbot channels status --probe` shows live per-account transport status and, where supported, `works` or `audit ok`.
...
- `Update restart` in `grokbot status` / `grokbot status --all`. Pending or failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run grokbot doctor --fix` under Channels: the channel config still exists, but plugin registration failed before the channel could load.
- Provider 401s after re-auth: `grokbot doctor --fix` checks for stale per-agent OAuth auth shadows and removes old copies so all agents resolve the current shared profile.

## Split brain installs and newer config guard

Use when a gateway service unexpectedly stops after an update, or logs show one `grokbot` binary is older than the version that last wrote `grokbot.json`.

GrokBot stamps config writes with `meta.lastTouchedVersion`. Read-only commands can inspect a config written by a newer GrokBot, but process and service mutations refuse to run from an older binary. Blocked actions: gateway service start/stop/restart/uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which grokbot
grokbot --version
grokbot gateway status --deep
grokbot config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `grokbot` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `grokbot` binary.
  </Step>
</Steps>
...
- `Established clients:` in `grokbot gateway status --deep` or `Gateway clients` in `grokbot doctor --deep`: active TCP clients connected to the Gateway port, with PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer GrokBot install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale GrokBot client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed GrokBot: local dashboards, editors, app-server helpers, or long-running `grokbot logs --follow` shells.
3. Re-run `grokbot gateway status --deep` or `grokbot doctor --deep` and confirm the stale client PID is gone.
...
Every skill root is a containment boundary. A symlink under `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or `~/.grokbot/skills` is skipped when its real target resolves outside that root, unless the target is explicitly trusted.
...
grokbot config get skills.load
```
...
grokbot config get agents.defaults.models
```
...
Do not assume this is always an GrokBot configuration issue. The response can come from an upstream security layer such as a CDN, WAF, bot-management rule, or reverse proxy in front of an OpenAI-compatible endpoint.
...
Fix the provider-side filtering first when evidence points to a WAF/CDN block. Prefer a narrowly scoped allow or skip rule for the API path GrokBot uses, and avoid disabling protection for the whole site.
...
- GrokBot model runs fail only on normal agent turns.
...
grokbot infer model run --model <provider/model> --prompt "hi" --json
grokbot logs --follow
```

Look for:

- Direct tiny calls succeed, but GrokBot runs fail only on larger prompts.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0`: the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. GrokBot retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - Direct tiny requests succeed, but GrokBot agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds): GrokBot transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle GrokBot's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while GrokBot agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
grokbot status
grokbot channels status --probe
grokbot pairing list --channel <channel> [--account <id>]
grokbot config get channels
grokbot logs --follow
```
...
If `curl` returns GrokBot HTML, the Gateway is working and the remaining issue is likely browser cache, an old deep link, or stale tab state. Open `http://127.0.0.1:18789` directly and navigate from the dashboard. If restart does not leave the service running, run `grokbot gateway start` and recheck `grokbot gateway status`.
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `grokbot config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `grokbot devices list` then `grokbot devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `grokbot devices rotate` / `revoke` / `remove` is denied unexpectedly:

- Paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`.
- `grokbot devices rotate --scope ...` can only request operator scopes that the caller session already holds.
...
grokbot logs --follow
grokbot doctor
grokbot gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `grokbot onboard --mode local` / `grokbot setup` to restamp the expected local-mode config. If you are running GrokBot via Podman, the default config path is `~/.grokbot/grokbot.json`.
    ...
    - `System-level GrokBot gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `OPENCLAW_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `grokbot doctor --fix` or `grokbot gateway install --force`, then restart the gateway service.
...
Use when channels (Telegram, WhatsApp, etc.) on a macOS host go quiet for minutes to hours at a time, and the gateway appears to come back the moment you open the Control UI, SSH in, or otherwise interact with the host. There is usually no obvious symptom in `grokbot status` because by the time you look the gateway is alive again.

```bash
ls ~/.grokbot/logs/stability/ | tail -5
grokbot gateway stability --bundle latest
pmset -g log | grep -iE "sleep|wake|maintenance" | tail -50
launchctl print gui/$UID/ai.grokbot.gateway | grep -E "state|last exit|runs"
```

Look for:

- One or more `*-uncaught_exception.json` bundles in `~/.grokbot/logs/stability/` with `error.code` set to a transient network code such as `ENETDOWN`, `ENETUNREACH`, `EHOSTUNREACH`, or `ECONNREFUSED`.
...
- `runs` count incrementing across the day with no corresponding `received SIG*; shutting down` line in `~/Library/Logs/grokbot/gateway.log`: clean shutdowns log a signal; transient crashes do not.
...
3. **Add a liveness watchdog** so a future crash burst that gets parked by launchd is caught quickly:
   ...
   state=$(launchctl print gui/$UID/ai.grokbot.gateway 2>/dev/null | awk -F'= ' '/state =/ {print $2; exit}')
...
Use this when a macOS install keeps restarting every few seconds, `grokbot`
...
This was observed on older installs where both `ai.grokbot.gateway` and
`ai.grokbot.node` LaunchAgents were active and each injected
`OPENCLAW_LAUNCHD_LABEL`. In that state GrokBot can detect launchd
...
```bash
for i in 1 2 3 4; do
  ps aux | grep 'grokbot.*index.js' | grep -v grep | awk '{print $2}'
  sleep 10
done
...
grokbot node status
launchctl print gui/$UID/ai.grokbot.gateway | grep -E 'state|last exit|runs'
tail -n 80 ~/Library/Logs/grokbot/gateway.log
```
...
- Both `~/Library/LaunchAgents/ai.grokbot.gateway.plist` and
  `~/Library/LaunchAgents/ai.grokbot.node.plist` loaded at the same time on a
...
1. If this host should only run the Gateway service, remove the managed node
   service through GrokBot. **Skip this step** if you actively rely on the node
   service for remote node features; uninstalling it stops those features on
   this host:

   ```bash
   grokbot node uninstall
   ```

2. Install a persistent Gateway wrapper that clears the inherited launchd
   markers before starting GrokBot. Use the supported `--wrapper` option; do
   not edit the generated file under `~/.grokbot/service-env/`, because service
   reinstall, update, and doctor repair regenerate that file:

   ```bash
   mkdir -p ~/.local/bin
   cat >~/.local/bin/grokbot-launchd-workaround <<'EOF'
   #!/bin/sh
   set -eu
   unset OPENCLAW_LAUNCHD_LABEL LAUNCH_JOB_LABEL LAUNCH_JOB_NAME XPC_SERVICE_NAME || true
   exec grokbot "$@"
   EOF
   chmod 700 ~/.local/bin/grokbot-launchd-workaround

   grokbot gateway install \
     --wrapper ~/.local/bin/grokbot-launchd-workaround \
     --force
   ```
...
3. Verify that the Gateway is stable and serving RPC, not merely listening:
   ...
   ```bash
   grokbot gateway status --deep --require-rpc
...
4. After upgrading to a release where the underlying dual-LaunchAgent loop is
   ...
   rm ~/.local/bin/grokbot-launchd-workaround
   ```
...
- `critical memory pressure bundle written` appears shortly before restart → GrokBot captured a pre-OOM stability bundle. Inspect it with `grokbot gateway stability --bundle latest`.
- `memory pressure: level=critical` appears in gateway logs → GrokBot detected critical memory pressure and recorded the available in-process memory facts.
...
- `V8 heap:` used bytes are close to the heap limit → lower prompt/session pressure or reduce concurrent work first. For a managed service, inspect `Gateway heap:` in `grokbot gateway status`; if it says `not set`, regenerate old service metadata with `grokbot gateway install --force`. Ambient shell `NODE_OPTIONS` is intentionally ignored. Use an explicit supervisor-level heap override only after confirming the sustained workload and leaving enough native-memory headroom.
...
- Critical memory pressure appears in logs but no bundle exists → capture `grokbot gateway diagnostics export` after the event for the available operational evidence.
...
- A timestamped `grokbot.json.rejected.*` file beside the active config.
- A timestamped `grokbot.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit.
- GrokBot keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an GrokBot-owned write.
    - Gateway startup fails closed instead of rewriting `grokbot.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - GrokBot-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `grokbot doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, GrokBot rotates older `.clobbered.*` files so the newest repaired payload is still available.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(grokbot config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an GrokBot-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an GrokBot-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `grokbot doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `grokbot config set` or `config.patch`.
    3. Run `grokbot config validate` before restarting.
...
Use when `grokbot gateway probe` reaches something, but still prints a warning block.
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or GrokBot could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
...
grokbot cron status
grokbot cron list
grokbot cron runs --id <jobId> --limit 20
grokbot system heartbeat last
grokbot logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank, comment, header, fence, or empty-checklist scaffolding, so GrokBot skips the model call.
...
grokbot nodes describe --node <idOrNameOrIp>
grokbot approvals get --node <idOrNameOrIp>
grokbot logs --follow
grokbot status
```
...
grokbot browser start --browser-profile grokbot
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - Stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `grokbot browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (GrokBot-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    grokbot gateway status
    grokbot config get gateway.mode
    grokbot config get gateway.remote.url
    grokbot config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    grokbot config get gateway.bind
    grokbot config get gateway.auth.mode
    grokbot config get gateway.auth.token
    grokbot gateway status
    grokbot logs --follow
    ```
