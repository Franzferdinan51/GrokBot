---
summary: "Start local model servers on demand before GrokBot model and embedding requests"
read_when:
  - You want GrokBot to start a local model server only when its model or embedding provider is selected
...
`models.providers.<id>.localService` starts a provider-owned local model server on demand. When a model or embedding request selects that provider, GrokBot probes the health endpoint, starts the process if it is down, waits for readiness, then sends the request. Use it to avoid keeping expensive local servers running all day.

## How it works

1. A model or embedding request resolves to a configured provider.
2. If that provider has `localService`, GrokBot probes `healthUrl`.
3. On a successful probe, GrokBot uses the already-running server.
4. On a failed probe, GrokBot spawns `command` with `args`.
5. GrokBot polls the health endpoint until `readyTimeoutMs` expires.
6. The request goes through the normal model or embedding transport.
7. If GrokBot started the process and `idleStopMs` is set, it stops the process after the last in-flight request has been idle that long.

GrokBot does not install launchd, systemd, Docker, or any daemon for this. The server is a plain child process of whichever GrokBot process first needed it.
...
| `env`            | no       | Environment variables merged over the GrokBot process environment.                                                                  |
...
| `idleStopMs`     | no       | Idle shutdown delay for an GrokBot-started process. `0` or omitted keeps it alive until GrokBot exits.                             |
...
Replace `command` with the result of `which inferrs` on the machine running GrokBot. Full inferrs setup: [Inferrs](/providers/inferrs).
...
<CardGroup cols={2}>
  ...
  <Card title="Inferrs" href="/providers/inferrs" icon="cpu">
    Run GrokBot through the inferrs OpenAI-compatible local server.
  </Card>
</CardGroup>
