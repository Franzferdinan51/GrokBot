...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to GrokBot
...
External apps talk to GrokBot through the Gateway protocol: WebSocket
...
<Note>
  ...
  [Building a Gateway client](https://docs.grokbot.ai/gateway/clients). If your
  app supervises the Gateway as a child process, also read
  [Embedding GrokBot](https://docs.grokbot.ai/gateway/embedding). During the
  initial package rollout, npm may return `E404` until the first package-bearing
  GrokBot release is published.
</Note>

<Note>
  This page is for code outside the GrokBot process. Plugin code that runs
  inside GrokBot should use documented `grokbot/plugin-sdk/*` subpaths instead.
</Note>
...
| ---------------------------------------------------------------- | ------------- | --------------------------------------------------------------------------------------------- |
| [Gateway client guide](https://docs.grokbot.ai/gateway/clients) | Release train | npm packages, auth, reconnect, history, events, approvals, and version policy.                |
| [Embedding guide](https://docs.grokbot.ai/gateway/embedding)    | Release train | Child-process environment, readiness, lifecycle, recovery, RPC ownership, and packaging.      |
...
| [`grokbot agent`](/cli/agent)                                   | Ready         | One-shot script integration when shelling out to the CLI is enough.                           |
| [`grokbot message`](/cli/message)                               | Ready         | Sending messages or channel actions from scripts.                                             |
...
4. Pin the GrokBot version you test against.
5. Recheck the RPC reference when upgrading GrokBot.
...
fail-closed, and the host must not freeze or snapshot it. GrokBot retries the
...
Preparation is refuse-only: GrokBot closes new root/session/command admission,
...
<Tip>
  For host wake scheduling, keep the GrokBot-facing part in an in-process
...
Use Gateway RPC when code lives outside GrokBot:
...
Use the Plugin SDK when code runs inside GrokBot:
...
External apps should not import `grokbot/plugin-sdk/*`; those subpaths are for
plugins loaded by GrokBot.
