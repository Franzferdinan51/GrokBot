...
GrokBot supports OAuth and API keys for model providers. For an always-on gateway host, an API key is the most predictable option; subscription/OAuth flows work too when they match your provider account model.
...
2. Put it on the **gateway host** (the machine running `grokbot gateway`):
...
3. If the gateway runs under systemd/launchd, put the key in `~/.grokbot/.env` so the daemon can read it:
...
`grokbot onboard` can also store API keys for daemon use if you don't want to manage env vars yourself. See [Environment variables](/help/environment) for the full env-loading precedence (`env.shellEnv`, `~/.grokbot/.env`, systemd/launchd).
...
This is two steps: log Claude Code into Anthropic on the host, then tell GrokBot to route Anthropic model selection through the local `claude-cli` backend and store the matching GrokBot auth profile.
...
```bash
grokbot models auth paste-token --provider openrouter
```

GrokBot reads auth profiles from each agent's `grokbot-agent.sqlite`. Endpoint details (`baseUrl`, `api`, model ids, headers, timeouts) belong under `models.providers.<id>` in `grokbot.json` or `models.json`, not in auth profiles.

If an older install still has `auth-profiles.json`, `auth-state.json`, or a flat shape like `{ "openrouter": { "apiKey": "..." } }`, run `grokbot doctor --fix` to import it into SQLite; doctor keeps timestamped backups beside the original JSON files.

External auth routes such as Bedrock `auth: "aws-sdk"` aren't credentials. For a named Bedrock route, set `auth.profiles.<id>.mode: "aws-sdk"` in `grokbot.json` — don't write `type: "aws-sdk"` into the auth profile store. `grokbot doctor --fix` migrates legacy AWS SDK markers from the credential store into config metadata.
...
- If auth exists but GrokBot can't resolve a probeable model for that provider, probe reports `status: no_model`.
...
GrokBot rotates to the next key only when the error message matches: `rate_limit`, `rate limit`, `429`, `quota exceeded`/`quota_exceeded`, `resource exhausted`/`resource_exhausted`, or `too many requests`. Other errors are not retried with alternate keys. If all keys fail, the final error from the last attempt is returned.
...
```bash
grokbot models auth login --provider anthropic --force
```
...
```bash
grokbot models auth order get --provider anthropic
grokbot models auth order set --provider anthropic anthropic:default
grokbot models auth order clear --provider anthropic
```

Use `--agent <id>` to target a specific agent; omit it to use the configured default agent. `grokbot models status --probe` shows omitted stored profiles as `excluded_by_auth_order` rather than silently skipping them.
...
Run `grokbot models status` to see which profile is expiring. If an Anthropic token profile is missing or expired, refresh it via setup-token or migrate to an Anthropic API key.
