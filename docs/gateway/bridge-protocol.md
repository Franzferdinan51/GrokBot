...
<Warning>
The TCP bridge has been **removed**. Current GrokBot builds do not ship the bridge listener, and `bridge.*` config keys are no longer in the schema. This page is historical reference only. Use the [Gateway protocol](/gateway/protocol) for all node/operator clients.
</Warning>
...
- Bind the bridge to a tailnet IP: `bridge.bind: "tailnet"` in `~/.grokbot/grokbot.json` (historical only; `bridge.*` is no longer valid config).
