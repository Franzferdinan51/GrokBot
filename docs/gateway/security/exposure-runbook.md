---
summary: "Pre-flight and rollback checklist before exposing an GrokBot Gateway beyond loopback"
...
- Gateway host, OS user, and state directory (default `~/.grokbot`).
...
- Backup location for `~/.grokbot/grokbot.json` and credentials.
...
```bash
grokbot gateway probe --url ws://127.0.0.1:18789 --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
Run `grokbot security audit --deep` after proxy changes. Trusted-proxy
...
1. Re-run `grokbot security audit --deep`.
...
5. Re-run `grokbot security audit --deep`.
