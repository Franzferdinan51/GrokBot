---
summary: "Supervise the GrokBot Gateway as a child process from Electron or another host app"
read_when:
  - Embedding GrokBot in a desktop or server application
...
title: "Embedding GrokBot"
---

An embedding host should supervise the installed `grokbot` executable, use the
...
and upgrades explicit without depending on GrokBot's private state layout.
...
const packageEntry = fileURLToPath(import.meta.resolve("grokbot"));
const openclawEntry = resolve(dirname(packageEntry), "..", "grokbot.mjs");
...
Resolve GrokBot through the installed package as shown; do not assume that a
project-local `grokbot` binary is on the host process's `PATH`. The example
...
| `OPENCLAW_NO_RESPAWN=1`          | In an unmanaged embedding child, prevents GrokBot from handing an update restart to a detached child. Routine restarts remain in process, so the host keeps ownership of the tracked PID. |
...
1. Run `grokbot doctor --fix --yes --non-interactive` against the same config and
...
`@grokbot/gateway-protocol/startup-unavailable` or the reference client's built-in
...
Keep the Gateway as the only owner of GrokBot state. Common embedding operations
...
under `~/.grokbot` to implement app features. Those layouts are private runtime
implementation details and can move or change without protocol compatibility.

## Install; do not flatten

The root `grokbot` package is not a single-file vendoring target. Bundled runtime
files under `dist/extensions` retain bare self-imports such as
`grokbot/plugin-sdk/*`, while the npm package intentionally excludes
per-extension `node_modules` trees.

Install GrokBot through npm, pnpm, or another normal Node package installation so
Node can resolve the package exports and root dependency tree. Spawn the installed
`grokbot` executable. Do not copy only `dist`, flatten the package into an app
bundle, or vendor selected extension files.

## Related

- [Building a Gateway client](https://docs.grokbot.ai/gateway/clients)
- [Gateway protocol](https://docs.grokbot.ai/gateway/protocol)
- [Gateway CLI](https://docs.grokbot.ai/cli/gateway)
- [Gateway integrations for external apps](https://docs.grokbot.ai/gateway/external-apps)
