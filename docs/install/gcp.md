---
summary: "Run GrokBot Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want GrokBot running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent GrokBot Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.
...
4. Clone the GrokBot repository
...
<Steps>
  ...
  <Step title="Create a GCP project">
    ```bash
    gcloud projects create my-grokbot-project --name="GrokBot Gateway"
    gcloud config set project my-grokbot-project
  ...
  <Step title="Create the VM">
    ...
    ```bash
    gcloud compute instances create grokbot-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    ```bash
    gcloud compute ssh grokbot-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the GrokBot repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    OPENCLAW_IMAGE=grokbot:latest
    OPENCLAW_GATEWAY_TOKEN=
    OPENCLAW_GATEWAY_BIND=lan
    OPENCLAW_GATEWAY_PORT=18789

    OPENCLAW_CONFIG_DIR=/home/$USER/.grokbot
    OPENCLAW_WORKSPACE_DIR=/home/$USER/.grokbot/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.grokbot
    ```
    ...
    across restarts. If neither is set, GrokBot uses a runtime-only token for
    ...
    **Do not commit this file.** It holds container/runtime env such as
    ...
    mounted `~/.grokbot/agents/<agentId>/agent/auth-profiles.json`.
  ...
  <Step title="GCP-specific launch notes">
    ...
    ```bash
    docker compose run --rm grokbot-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh grokbot-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop grokbot-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type grokbot-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start grokbot-gateway --zone=us-central1-a
```
...
```bash
gcloud iam service-accounts create grokbot-deploy \
  --display-name="GrokBot Deployment"

gcloud projects add-iam-policy-binding my-grokbot-project \
  --member="serviceAccount:grokbot-deploy@my-grokbot-project.iam.gserviceaccount.com" \
