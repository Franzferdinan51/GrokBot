---
summary: "Host GrokBot on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying GrokBot to Upstash Box
  - You want a managed Linux environment for GrokBot with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent GrokBot Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current GrokBot Box walkthrough at
[GrokBot Setup](https://upstash.com/docs/box/guides/grokbot-setup).

## Connect with an SSH tunnel

Forward the GrokBot dashboard port to your local machine. Use your Box API key
...
```bash
grokbot config set gateway.bind lan
nohup grokbot gateway > gateway.log 2>&1 &
```
...
- [Updating GrokBot](/install/updating)
