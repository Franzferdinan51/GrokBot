---
summary: "Deploy GrokBot on Railway with one-click template"
read_when:
  - Deploying GrokBot to Railway
  - You want a one-click cloud deploy with browser-based Control UI
title: "Railway"
---

Deploy GrokBot on Railway with a one-click template and access it through the web Control UI. This is the easiest "no terminal on the server" path: Railway runs the Gateway for you.
...
This creates a portable backup archive with GrokBot state plus any configured workspace. See [Backup](/cli/backup) for details.
