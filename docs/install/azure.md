---
summary: "Run GrokBot Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want GrokBot running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on GrokBot Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

Set up an Azure Linux VM with the Azure CLI, apply Network Security Group (NSG) hardening, configure Azure Bastion for SSH access, and install GrokBot.
...
- Install GrokBot with the installer script
- Verify the gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-grokbot"
    LOCATION="westus2"
    VNET_NAME="vnet-grokbot"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-grokbot-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-grokbot-vm"
    VM_NAME="vm-grokbot"
    ADMIN_USERNAME="grokbot"
    BASTION_NAME="bas-grokbot"
    BASTION_PIP_NAME="pip-grokbot-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install GrokBot (in the VM shell)">
    ```bash
    curl -fsSL https://grokbot.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node and dependencies if not already present, installs GrokBot, and launches onboarding. See [Install](/install) for details.
...
- More detail on Azure deployment with the GitHub Copilot model provider: [GrokBot on Azure with GitHub Copilot](https://github.com/johnsonshi/grokbot-azure-github-copilot)
