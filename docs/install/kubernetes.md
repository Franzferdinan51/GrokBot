---
summary: "Deploy GrokBot Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run GrokBot on a Kubernetes cluster
  - You want to test GrokBot in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running GrokBot on Kubernetes, not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm

GrokBot is a single container with some config files. The interesting customization is in agent content (Markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. Layer a Helm chart on top of these manifests if your deployment grows more complex.
...
kubectl port-forward svc/grokbot 18789:18789 -n grokbot
...
```bash
kubectl get secret grokbot-secrets -n grokbot -o jsonpath='{.data.OPENCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```text
Namespace: grokbot (configurable via OPENCLAW_NAMESPACE)
├── Deployment/grokbot        # Single pod, init container + gateway
├── Service/grokbot           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/grokbot-config  # grokbot.json + AGENTS.md
└── Secret/grokbot-secrets    # Gateway token + API keys
```
...
Edit `grokbot.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret grokbot-secrets -n grokbot \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/grokbot -n grokbot
```
...
```yaml
image: ghcr.io/grokbot/grokbot:slim # primary; official Docker Hub mirror: grokbot/grokbot
```
...
└── manifests/
    ...
    ├── configmap.yaml          # grokbot.json + AGENTS.md
