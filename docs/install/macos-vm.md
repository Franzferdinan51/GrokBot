---
summary: "Run GrokBot in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want GrokBot isolated from your main macOS environment
...
Run GrokBot in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume). This gives you:
...
Once you have SSH access to a macOS VM, continue at [Install GrokBot](#6-install-grokbot) below.

## Quick path (Lume, experienced users)

1. Install Lume.
2. `lume create grokbot --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH).
4. `lume run grokbot --no-display`
5. SSH in, install GrokBot, configure channels.
6. Done.
...
```bash
lume create grokbot --os macos --ipsw latest
```
...
```bash
lume stop grokbot
lume run grokbot --no-display
```

The VM runs in the background; GrokBot's daemon keeps the gateway running. To check status:

```bash
ssh youruser@192.168.64.X "grokbot status"
```

## Bonus: iMessage integration

This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to GrokBot.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running GrokBot/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your GrokBot config:
...
```bash
lume stop grokbot
lume clone grokbot grokbot-golden
```

Reset anytime:

```bash
lume stop grokbot && lume delete grokbot
lume clone grokbot-golden grokbot
lume run grokbot --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get grokbot` again                            |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                     |
| WhatsApp QR not scanning | Ensure you are logged into the VM (not host) when running `grokbot channels login` |
