---
summary: "Updating GrokBot safely (global install or source), plus rollback strategy"
read_when:
  - Updating GrokBot
...
## Recommended: `grokbot update`

Detects your install type (npm, pnpm, Bun, or git), fetches the latest version, runs `grokbot doctor`, and restarts the gateway.
...
```bash
grokbot update --channel beta
grokbot update --channel extended-stable
grokbot update --channel dev
grokbot update --dry-run   # preview without applying
```

`grokbot update` has no `--verbose` flag (the installer does). For diagnostics use
`--dry-run` to preview planned actions, `--json` for structured results, or
`grokbot update status --json` to inspect channel and availability state.
...
foreground-only. GrokBot reads the public npm `extended-stable` selector,
...
successful core update; a direct `npm install -g grokbot@extended-stable`
...
Catalog installs created by current GrokBot versions retain that default
intent. Older records that contain only an exact version remain pinned because
GrokBot cannot safely distinguish an old automatic pin from a user pin; run
`grokbot plugins update @grokbot/name` once on the extended-stable channel
to opt that plugin back into exact-core tracking.

`--channel dev` gives a persistent moving GitHub `main` checkout. For a one-off
package update, `--tag main` maps to the `github:grokbot/grokbot#main` package
...
credentials, and workspace in `~/.grokbot`; it only changes which GrokBot
...
```bash
grokbot update --channel dev --dry-run
grokbot update --channel stable --dry-run
```
...
converting it. If the gateway is already installed, `grokbot update` refreshes
the service metadata and restarts it unless you pass `--no-restart`.

For package installs with a managed Gateway service, `grokbot update` targets
the package root used by that service. If the shell `grokbot` command comes
...
If `grokbot update` fails after the npm package install phase, re-run the
...
```bash
curl -fsSL https://grokbot.ai/install.sh | bash -s -- --install-method npm
```

Pin the recovery to a specific version or dist-tag with `--version`:

```bash
curl -fsSL https://grokbot.ai/install.sh | bash -s -- --install-method npm --version <version-or-dist-tag>
```

## Alternative: manual npm, pnpm, or bun

```bash
npm i -g grokbot@latest
```

Prefer `grokbot update` for supervised installs: it can coordinate the package
...
For a root-owned Linux system-global install, if `grokbot update` fails with
...
sudo /usr/bin/npm i -g grokbot@latest
...
grokbot gateway status --deep --json
grokbot doctor --lint --json
```

When `grokbot update` manages a global npm install, it installs the target
into a temporary npm prefix first. The candidate package validates the host
Node version during `preinstall`; only then does GrokBot verify the packaged
...
swap. On npm 12 and newer, the updater approves only the candidate GrokBot
...
command fails, GrokBot retries once with `--omit=optional`, which helps hosts
where native optional dependencies cannot compile.

GrokBot-managed npm update and plugin-update commands also clear npm's
...
explicit GrokBot update means "install the selected release now."

```bash
pnpm add -g grokbot@latest
```

If pnpm 11 installed GrokBot 2026.7.1, run that manual command once. That
...
or when the invoking package is orphaned or not the only active GrokBot
install there.

If GrokBot shares a pnpm 11 global install group with another package, the
...
<AccordionGroup>
  <Accordion title="Read-only package tree">
    GrokBot treats packaged global installs as read-only at runtime, even when the global package directory is writable by the current user. Plugin package installs live in GrokBot-owned npm/git roots under the user config directory, and Gateway startup does not mutate the GrokBot package tree.

    Some Linux npm setups install global packages under root-owned directories such as `/usr/lib/node_modules/grokbot`. GrokBot supports that layout because plugin install/update commands write outside that global package directory.

  </Accordion>
  <Accordion title="Hardened systemd units">
    Give GrokBot write access to its config/state roots so explicit plugin installs, plugin updates, and doctor cleanup can persist their changes:

    ```ini
    ReadWritePaths=/var/lib/grokbot /home/grokbot/.grokbot /tmp
    ```

  </Accordion>
  <Accordion title="Disk-space preflight">
    Before package updates and explicit plugin installs, GrokBot tries a best-effort disk-space check for the target volume. Low space produces a warning with the checked path, but does not block the update because filesystem quotas, snapshots, and network volumes can change after the check. The actual package-manager install and post-install verification remain authoritative.
  </Accordion>
</AccordionGroup>

## Auto-updater

Off by default. Enable it in `~/.grokbot/grokbot.json`:
...
| `dev`             | No automatic apply. Use `grokbot update` manually.                                                                           |
...
exits, and lets the normal `grokbot update --yes --json` CLI path stop the
...
app runs `grokbot update --tag <app-version> --json`, restarts its Gateway,
...
1. Reinstall older GrokBot code while keeping the current state.
...
`grokbot update` preserves an automatic pre-update config copy, but it does not
create a full state recovery point. Before a significant update, create one
explicitly:

```bash
mkdir -p ~/Backups/grokbot
grokbot backup create --output ~/Backups/grokbot --verify
```

The archive manifest records the GrokBot version and the source paths included
...
```bash
npm view grokbot versions --json
grokbot update --tag <known-good-version> --dry-run
grokbot update --tag <known-good-version>
```

`grokbot update --tag` is preferred over a direct package-manager install. It
...
filesystem swap or command-shim replacement fails, GrokBot restores the old
...
```bash
grokbot gateway stop
npm i -g grokbot@<known-good-version>
grokbot gateway install --force
grokbot gateway restart
```
...
Before starting an older file-backed GrokBot release, use the current CLI to
...
Broad `grokbot backup create` archives support creation and verification, but
...
restore. `grokbot backup sqlite restore` likewise writes a verified database
...
- Run `grokbot doctor` again and read the output carefully.
- For `grokbot update --channel dev` on source checkouts, the updater auto-bootstraps `pnpm` when needed. If you see a pnpm/corepack bootstrap error, install `pnpm` manually (or re-enable `corepack`) and rerun the update.
