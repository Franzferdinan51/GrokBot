---
summary: "Deploy GrokBot on Render with Infrastructure-as-Code"
read_when:
  - Deploying GrokBot to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy GrokBot on [Render](https://render.com) using the repo's `render.yaml` Blueprint. It declares the service, disk, and environment variables in one file.
...
The Blueprint defaults to `starter`. To use the free tier, change `plan: free` in your fork's `render.yaml` — note that with no persistent disk, GrokBot state resets on each deploy.
...
- **Vertical**: change the plan for more CPU/RAM. Usually sufficient for GrokBot.
- **Horizontal**: increase instance count (Standard plan and above). Requires sticky sessions or external state management since GrokBot keeps runtime state on the local disk.
