---
summary: "Host GrokBot on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up GrokBot on Oracle Cloud
  - Looking for free VPS hosting for GrokBot
  - Want 24/7 GrokBot on a small server
title: "Oracle Cloud"
---

Run a persistent GrokBot Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname grokbot
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=grokbot
    ```

    From now on, connect via Tailscale: `ssh ubuntu@grokbot`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    grokbot config set gateway.bind loopback
    ...
    grokbot config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://grokbot.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.grokbot` to restrict credential file permissions.
- `grokbot security audit` for an GrokBot-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most GrokBot features work fine; a small number of native binaries need ARM builds:
...
- `~/.grokbot/` -- `grokbot.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@grokbot
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=grokbot --reset` to re-authenticate.
