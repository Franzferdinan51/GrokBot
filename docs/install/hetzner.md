---
summary: "Run GrokBot Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want GrokBot running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running GrokBot in Docker on Hetzner or a similar provider
title: "Hetzner"
---

Run a persistent GrokBot Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.
...
2. Install Docker
3. Clone the GrokBot repository
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.grokbot/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.grokbot
    ```
  ...
  <Step title="Configure environment variables">
    ...
    OPENCLAW_CONFIG_DIR=/root/.grokbot
    OPENCLAW_WORKSPACE_DIR=/root/.grokbot/workspace
...
- Infrastructure: [grokbot-terraform-hetzner](https://github.com/andreesg/grokbot-terraform-hetzner)
- Docker config: [grokbot-docker-config](https://github.com/andreesg/grokbot-docker-config)
