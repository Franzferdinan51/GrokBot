...
read_when:
  - You want to understand `grokbot.ai/install.sh`
  - You want to automate installs (CI / headless)
  - You want to install from a GitHub checkout
title: "Installer internals"
---

GrokBot ships three installer scripts, served from `grokbot.ai`.

| Script                             | Platform             | What it does                                                                                   |
| ---------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL  | Installs Node if needed, installs GrokBot via npm (default) or git, can run onboarding.       |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | Installs Node + GrokBot into a local prefix (`~/.grokbot`) via npm or git. No root required. |
| [`install.ps1`](#installps1)       | Windows (PowerShell) | Installs Node if needed, installs GrokBot via npm (default) or git, can run onboarding.       |
...
<Tabs>
  <Tab title="install.sh">
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install.sh | bash -s -- --help
    ```

  </Tab>
  <Tab title="install-cli.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install-cli.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install-cli.sh | bash -s -- --help
    ```
  ...
  <Tab title="install.ps1">
    ...
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://grokbot.ai/install.ps1))) -Tag beta -NoOnboard -DryRun
    ```
...
<Note>
If install succeeds but `grokbot` is not found in a new terminal, see [Node.js troubleshooting](/install/node#troubleshooting).
</Note>
...
<Steps>
  ...
  <Step title="Install GrokBot">
    - `npm` method (default): global npm install
    - `git` method: clone/update repo, install deps with pnpm, build, then install wrapper at `~/.local/bin/grokbot`

  </Step>
  <Step title="Post-install tasks">
    - Resolves the just-installed `grokbot` binary for follow-up commands
...
If run inside an GrokBot checkout (`package.json` + `pnpm-workspace.yaml`), the script offers:
...
<Tabs>
  ...
  <Tab title="Dry run">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install.sh | bash -s -- --dry-run
    ```
  </Tab>
...
| `OPENCLAW_HOME=<path>`                            | Base directory for GrokBot state and default git/onboarding paths |
...
(default `~/.grokbot`) and no system Node dependency. Supports npm installs
...
<Steps>
  ...
  <Step title="Install GrokBot under prefix">
    - `npm` method (default): installs under the prefix with npm, then writes wrapper to `<prefix>/bin/grokbot`
    - `git` method: clones/updates a checkout (default `~/grokbot`) and still writes the wrapper to `<prefix>/bin/grokbot`

  </Step>
  <Step title="Refresh loaded gateway service">
    If a gateway service is already loaded from that same prefix, the script runs
    `grokbot gateway install --force`, which activates the replacement service,
...
<Tabs>
  ...
  <Tab title="Custom prefix + version">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install-cli.sh | bash -s -- --prefix /opt/grokbot --version latest
    ```
  </Tab>
  <Tab title="Git install">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install-cli.sh | bash -s -- --install-method git --git-dir ~/grokbot
    ```
  </Tab>
  <Tab title="Automation JSON output">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install-cli.sh | bash -s -- --json --prefix /opt/grokbot
    ```
  </Tab>
  <Tab title="Run onboarding">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install-cli.sh | bash -s -- --onboard
    ```
  </Tab>
</Tabs>
...
| Flag                                    | Description                                                                     |
| --------------------------------------- | ------------------------------------------------------------------------------- |
| `--prefix <path>`                       | Install prefix (default: `~/.grokbot`)                                         |
| `--install-method \| --method npm\|git` | Choose install method (default: `npm`)                                          |
| `--npm`                                 | Shortcut for npm method                                                         |
| `--git \| --github`                     | Shortcut for git method                                                         |
| `--git-dir \| --dir <path>`             | Git checkout directory (default: `~/grokbot`)                                  |
| `--version <ver>`                       | GrokBot version or dist-tag (default: `latest`)                                |
| `--node-version <ver>`                  | Node version (default: `24.15.0`; `22.22.3` on Linux ARMv7)                     |
| `--json`                                | Emit NDJSON events                                                              |
| `--onboard`                             | Run `grokbot onboard` after install                                            |
...
| `OPENCLAW_INSTALL_METHOD=git\|npm`          | Install method                                                     |
| `OPENCLAW_VERSION=<ver>`                    | GrokBot version or dist-tag                                       |
...
<Note>
`grokbot@main` and other GitHub source specs are not valid `--version` targets for npm installs. Use `--install-method git --version main` instead.
</Note>
...
<Steps>
  ...
  <Step title="Ensure Node.js 24 by default">
    If missing, attempts install via winget, then Chocolatey, then Scoop. If no package manager is available, the script downloads the official Node.js 24 Windows zip into `%LOCALAPPDATA%\GrokBot\deps\portable-node` and adds it to the current process and user PATH. Node 22.22.3+, Node 24.15+, and Node 25.9+ are supported; Node 23 is unsupported.
  </Step>
  <Step title="Install GrokBot">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\grokbot.cmd`. If Git is missing, the script bootstraps user-local MinGit under `%LOCALAPPDATA%\GrokBot\deps\portable-git` and adds it to the current process and user PATH.

  </Step>
  <Step title="Post-install tasks">
    - Adds needed bin directory to user PATH when possible
    - Refreshes a loaded gateway service best-effort (`grokbot gateway install --force`, then restart)
    - Runs `grokbot doctor --non-interactive` on upgrades and git installs (best effort)
...
<Tabs>
  ...
  <Tab title="Git install">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://grokbot.ai/install.ps1))) -InstallMethod git
    ```
  </Tab>
  <Tab title="GitHub main checkout">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://grokbot.ai/install.ps1))) -InstallMethod git -Tag main
    ```
  </Tab>
  <Tab title="Custom git directory">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://grokbot.ai/install.ps1))) -InstallMethod git -GitDir "C:\grokbot"
    ```
  </Tab>
  <Tab title="Dry run">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://grokbot.ai/install.ps1))) -DryRun
    ```
  </Tab>
</Tabs>
...
| `-GitDir <path>`            | Checkout directory (default: `%USERPROFILE%\grokbot`)     |
...
<Tabs>
  <Tab title="install.sh (non-interactive npm)">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://grokbot.ai/install.sh | bash -s -- --no-prompt --no-onboard
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title='Windows: "grokbot is not recognized"'>
  ...
  <Accordion title="grokbot not found after install">
