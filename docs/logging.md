...
read_when:
  - You need a beginner-friendly overview of GrokBot logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

GrokBot has two main log surfaces:
...
The date uses the gateway host's local timezone. When `/tmp/grokbot` is unsafe
or unavailable (and always on Windows), GrokBot uses a user-scoped
`grokbot-<uid>` directory under the OS temp dir instead. Dated log files are
pruned after 24 hours.

Each file rotates when the next write would exceed `logging.maxFileBytes`
(default: 100 MB). GrokBot keeps up to five numbered archives beside the
active file, such as `grokbot-YYYY-MM-DD.1.log`, and keeps writing to a fresh
active log instead of suppressing diagnostics.

You can override the path in `~/.grokbot/grokbot.json`:

```json
{
  "logging": {
    "file": "/path/to/grokbot.log"
  }
}
```
...
or times out before `logs.tail` answers, `grokbot logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `grokbot logs --follow` is stricter: on Linux it uses the active
...
```bash
grokbot channels logs --channel whatsapp
```
...
GrokBot preserves the original structured log arguments alongside these fields
...
`grokbot gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
grokbot gateway
grokbot gateway --verbose --ws-log compact
grokbot gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.grokbot/grokbot.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/grokbot/grokbot-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`OPENCLAW_LOG_LEVEL`** environment variable (e.g. `OPENCLAW_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `grokbot.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `grokbot --log-level debug gateway run`), which overrides the environment variable for that command.
...
These flags log through normal GrokBot logging, so `grokbot logs --follow`
...
GrokBot writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
GrokBot can redact sensitive tokens before they hit console output, file logs,
...
policy. GrokBot still redacts safety-boundary payloads that can be shown to UI
...
- **Gateway not reachable?** Run `grokbot doctor` first.
