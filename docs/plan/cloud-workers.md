...
- No external coding harnesses (Claude Code, Codex CLI) on workers. Worker sessions run GrokBot's embedded runner only. Harness support is a v2 opt-in because harnesses do their own inference with their own credentials.
...
### 2. Worker bootstrap: install GrokBot on the box
...
- `npm i -g grokbot@<exact gateway version>` is an optimization when the gateway runs a released version; never `latest`.
