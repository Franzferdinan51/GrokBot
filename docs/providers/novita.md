---
summary: "Use NovitaAI's OpenAI-compatible API with GrokBot"
read_when:
  - You want to run GrokBot with NovitaAI models
...
It ships as a bundled GrokBot provider (no separate plugin install), so
...
```bash
grokbot onboard --auth-choice novita-api-key
```
...
```bash
grokbot models list --provider novita
```
...
- `401`/`403`: verify the key in Novita's key management page and re-run
  `grokbot onboard --auth-choice novita-api-key` if the stored profile is
  stale.
