---
summary: "fal image, video, and music generation setup in GrokBot"
title: "Fal"
read_when:
  - You want to use fal image generation in GrokBot
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate, video_generate, or music_generate
---

GrokBot ships a bundled `fal` provider for hosted image, video, and music
generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    grokbot onboard --auth-choice fal-api-key
    ```
...
Krea 2 models use fal's native Krea payload schema. GrokBot sends
...
`aspectRatio`; GrokBot maps `size` to the closest supported Krea aspect ratio
...
control in GrokBot, so `background: "transparent"` is reported as an ignored
override for fal models.
Krea 2 endpoints do not expose an `output_format` request field through fal, so
GrokBot rejects `outputFormat` overrides for Krea requests.
