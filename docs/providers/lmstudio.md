---
summary: "Run GrokBot with LM Studio"
read_when:
  - You want to run GrokBot with open source models via LM Studio
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    On a fresh guided setup, GrokBot first queries `/api/v1/models` on the
...
```bash
grokbot models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys use an `author/model-name` format (e.g. `qwen/qwen3.5-9b`); GrokBot model refs
...
```bash
grokbot onboard --non-interactive --accept-risk --auth-choice lmstudio
```
...
servers; omit it for unauthenticated servers and GrokBot stores a local non-secret marker instead.
...
LM Studio doesn't always emit an OpenAI-shaped `usage` object on streamed responses. GrokBot
...
When LM Studio's `/api/v1/models` discovery reports model-specific reasoning options, GrokBot
...
"on"]`) while rejecting those literal values on `/v1/chat/completions`; GrokBot normalizes that
...
LM Studio supports just-in-time (JIT) model loading, loading models on first request. GrokBot
...
disable GrokBot's preload step:
