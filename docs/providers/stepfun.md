---
summary: "Use StepFun models with GrokBot"
read_when:
  - You want StepFun models in GrokBot
  - You need StepFun setup guidance
title: "StepFun"
---

StepFun ships as an external official plugin (`@grokbot/stepfun-provider`) with two provider ids:
...
```bash
grokbot plugins install @grokbot/stepfun-provider
...
<Tabs>
  <Tab title="Standard">
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        grokbot onboard --auth-choice stepfun-standard-api-key-intl
        ```

        China endpoint:

        ```bash
        grokbot onboard --auth-choice stepfun-standard-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        grokbot models list --provider stepfun
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Step Plan">
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        grokbot onboard --auth-choice stepfun-plan-api-key-intl
        ```

        China endpoint:

        ```bash
        grokbot onboard --auth-choice stepfun-plan-api-key-cn
        ```
      </Step>
      <Step title="Non-interactive alternative">
        ```bash
      ...
      <Step title="Verify models are available">
        ```bash
        grokbot models list --provider stepfun-plan
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Notes">
    - `step-3.7-flash` accepts text and image input through GrokBot. StepFun's API also supports video, which is not yet a model input modality in GrokBot.
    ...
    - Use `grokbot models list` and `grokbot models set <provider/model>` to inspect or switch models.
