...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in GrokBot
---

ClawRouter gives GrokBot one policy-scoped key for multiple upstream model
...
the key's budget and aggregate usage on GrokBot usage surfaces.
...
GrokBot host. The plugin ships bundled with GrokBot (`enabledByDefault: true`);
you only need an issued ClawRouter credential.

| Property      | Value                                    |
| ------------- | ---------------------------------------- |
| Provider      | `clawrouter`                             |
| Plugin        | bundled (included in GrokBot)           |
| Auth          | `CLAWROUTER_API_KEY`                     |
| Default URL   | `https://clawrouter.grokbot.ai`         |
...
<Steps>
  ...
  <Step title="Configure GrokBot">
    ...
    grokbot onboard --auth-choice clawrouter-api-key
    grokbot plugins enable clawrouter
    ```
    ...
    ClawRouter origin; the default is `https://clawrouter.grokbot.ai`.

  </Step>
  <Step title="List granted models">
    ```bash
    grokbot models list --all --provider clawrouter
    ```
  ...
  <Step title="Select a model">
    ```bash
    grokbot models set clawrouter/<provider>/<model>
    ```

    You can also select a returned model for one run with
    `grokbot agent --model clawrouter/<provider>/<model> --message "..."`.
...
SecretRef in `grokbot.json`. The canonical managed fields are:
...
```bash
grokbot config patch --file ./clawrouter.patch.json5 --dry-run --json
grokbot config patch --file ./clawrouter.patch.json5
```
...
# GrokBot gateway startup readiness only; no model call is made.
curl -fsS http://127.0.0.1:18789/readyz

# Credential-scoped catalog discovery.
grokbot models list --all --provider clawrouter --json

# Minimal real inference probe through the configured ClawRouter provider.
grokbot models status --probe --probe-provider clawrouter --probe-max-tokens 8 --json

# Workload canary using an exact granted model ref.
grokbot agent --agent main \
...
`X-Request-ID`, so an GrokBot model-call event can be joined to ClawRouter's
...
supported request routes. GrokBot does not ship a second, fixed list of
ClawRouter models. A catalog model is advertised as an GrokBot model when:
...
Adding a model to a supported ClawRouter provider needs no GrokBot release:
...
ClawRouter owns upstream credentials; its catalog tells GrokBot which
transport to use, so you never install every upstream company's auth plugin.

| Catalog capability / route                               | GrokBot transport     |
| -------------------------------------------------------- | ---------------------- |
...
unsupported request format is intentionally not advertised as an GrokBot
...
ClawRouter's `/v1/usage` response feeds the normal GrokBot provider-usage
...
```bash
grokbot status --usage
grokbot models status
```

The same provider snapshot is available to `/status` in chat and GrokBot's
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter credential; GrokBot does not fall back to upstream provider keys.                                          |
...
<CardGroup cols={2}>
  ...
  <Card title="Usage tracking" href="/concepts/usage-tracking" icon="chart-line">
    GrokBot usage and status surfaces.
  </Card>
</CardGroup>
