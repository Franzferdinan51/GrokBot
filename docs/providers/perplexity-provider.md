...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    grokbot config set plugins.entries.perplexity.config.webSearch.apiKey "pplx-xxxxxxxxxxxx"
    ```
  ...
  <Step title="Start searching">
    ...
    ```bash
    grokbot config set tools.web.search.provider perplexity
    ```
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    ...
    imported. Set the key in `~/.grokbot/.env` or via `env.shellEnv` so the
  ...
  <Accordion title="OpenRouter proxy setup">
    ...
    (prefix `sk-or-`) instead of a native Perplexity key. GrokBot detects the
