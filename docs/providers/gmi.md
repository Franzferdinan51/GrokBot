---
summary: "Use GMI Cloud's OpenAI-compatible API with GrokBot"
read_when:
  - You want to run GrokBot with GMI Cloud models
  - You need the GMI provider id, key, or endpoint
title: "GMI Cloud"
---

GMI Cloud is a hosted inference platform for frontier and open-weight models
behind an OpenAI-compatible API. In GrokBot it is an official external provider
...
primary provider does. GrokBot owns the provider id, auth profile, aliases,
...
```bash
grokbot onboard --auth-choice gmi-api-key
```
...
```bash
grokbot models list --provider gmi
```

## Troubleshooting

- `401` or `403`: check that `GMI_API_KEY` is set for the process running
  GrokBot, or re-run onboarding to store the key in the provider auth profile.
- Unknown model errors: confirm the model exists in your GMI account and use the
  full `gmi/<route-id>` ref shown by `grokbot models list --provider gmi`.
