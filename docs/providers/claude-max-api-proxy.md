...
**claude-max-api-proxy** is a community npm package (not an GrokBot plugin) that
...
plan articles, and [Anthropic provider](/providers/anthropic) for GrokBot's
...
<Steps>
  ...
  <Step title="Configure GrokBot">
    Point GrokBot at the proxy as a custom OpenAI-compatible endpoint:
...
<Note>
The model ids below are the proxy's own catalog, not GrokBot's Anthropic
...
<AccordionGroup>
  <Accordion title="Proxy-style OpenAI-compatible notes">
    This uses GrokBot's generic custom `/v1` OpenAI-compatible route, the same
    ...
    - No Responses `store`, prompt-cache hints, or OpenAI reasoning-compat
      payload shaping.
    - GrokBot's OpenAI/Codex attribution headers (`originator`, `version`,
...
<CardGroup cols={2}>
  <Card title="Anthropic provider" href="/providers/anthropic" icon="bolt">
    Native GrokBot integration with Claude CLI or API keys.
  </Card>
