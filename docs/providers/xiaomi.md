---
summary: "Use Xiaomi MiMo pay-as-you-go and Token Plan models with GrokBot"
read_when:
  - You want Xiaomi MiMo models in GrokBot
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    ```bash
    grokbot onboard --auth-choice xiaomi-api-key
    ```

    Token Plan:

    ```bash
    grokbot onboard --auth-choice xiaomi-token-plan-sgp
    ```

    Or pass the keys directly:

    ```bash
    grokbot onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    grokbot onboard --auth-choice xiaomi-token-plan-sgp --xiaomi-token-plan-api-key "$XIAOMI_TOKEN_PLAN_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    grokbot models list --provider xiaomi
    grokbot models list --provider xiaomi-token-plan
    ```
  </Step>
</Steps>
...
GrokBot's [`/think` directive](/tools/thinking) with levels `off`,
...
GrokBot sends `speakerVoice` for that model.
...
the desired voice description; GrokBot sends it as the `user` message, sends
...
Matrix, Telegram, and WhatsApp), GrokBot transcodes Xiaomi output to 48kHz
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.grokbot/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.grokbot/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full GrokBot configuration reference.
  </Card>
