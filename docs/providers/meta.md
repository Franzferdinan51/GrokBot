...
title: "Meta"
read_when:
  - You want to use Meta with GrokBot
...
for the `muse-spark-1.1` reasoning model. The provider ships as a bundled GrokBot
plugin.
...
```bash Onboarding
grokbot onboard --auth-choice meta-api-key
```
...
```
  ...
  <Step title="Verify models are available">
    ```bash
    grokbot models list --provider meta
    ```

    Lists the static `muse-spark-1.1` catalog entry. If `MODEL_API_KEY` is unresolved,
    `grokbot models status --json` reports the missing credential under
...
<Warning>
`muse-spark-1.1` does not accept `reasoning.effort: "none"`. GrokBot maps
...
`~/.grokbot/.env` or through `env.shellEnv`. A key exported only in an
