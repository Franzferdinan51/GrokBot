---
summary: "Sign in to GitHub Copilot from GrokBot using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `grokbot models auth login-github-copilot` flow
  - You are choosing between the built-in Copilot provider, Copilot SDK harness, and Copilot Proxy
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. GrokBot can use Copilot as a model
provider or agent runtime in three different ways.

## Three ways to use Copilot in GrokBot

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when GrokBot runs. This is the **default** and simplest path
    ...
    <Steps>
      ...
      <Step title="Set a default model">
        ```bash
        grokbot models set github-copilot/claude-opus-4.7
        ```
  ...
  <Tab title="Copilot SDK harness plugin (copilot)">
    Install the external `@grokbot/copilot` plugin when you want GitHub's
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. GrokBot talks to
    the proxy's `/v1` endpoint (default `http://localhost:3000/v1`) and uses the
    model list you configure.

    The `copilot-proxy` plugin ships with GrokBot and is enabled by default.
    Configure the base URL and model ids with:

    ```bash
    grokbot models auth login --provider copilot-proxy --set-default
    ```
...
endpoints rather than public `github.com`. GrokBot exposes this as a
first-class auth choice so you do not have to hand-edit URLs.

<Steps>
  <Step title="Pick the Enterprise auth choice">
    In onboarding or `grokbot models auth`, choose
    **GitHub Copilot (Enterprise / data residency)**. You will be prompted for
    ...
    GrokBot derives those endpoints from the tenant root automatically.

    ```bash
    grokbot models auth login --provider github-copilot --method device-enterprise
    ```
...
tenant, or one tenant to another), GrokBot will not reuse the existing token —
...
`grokbot models auth login-github-copilot` shortcut, token refresh, embeddings,
...
| ---------------------------------------------------------------------- | --------------- | ---------------------------------------------------- |
| `grokbot models auth login-github-copilot`                            | `--yes`         | Overwrite an existing auth profile without prompting |
| `grokbot models auth login --provider github-copilot --method device` | `--set-default` | Also apply the provider's recommended default model  |

```bash
# Skip the re-login confirmation
grokbot models auth login-github-copilot --yes

# Login and set the default model in one step
grokbot models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

The device-login flow requires an interactive TTY. For headless setup, import
an existing GitHub OAuth access token with `grokbot onboard --non-interactive`:

```bash
grokbot onboard --non-interactive --accept-risk \
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    GrokBot refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    churn. Newly published Copilot models become visible without an GrokBot
  ...
  <Accordion title="Transport selection">
    ...
    models keep the OpenAI Responses transport. GrokBot selects the correct
    transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    GrokBot sends Copilot IDE-style request headers on Copilot transports
  ...
  <Accordion title="Environment variable resolution order">
    GrokBot resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, GrokBot uses the highest-priority one.
    The device-login flow (`grokbot models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    token when GrokBot runs. You do not need to manage the token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, GrokBot can use it for embeddings without a separate API key.

### Config

Set `memorySearch.provider` explicitly to use GitHub Copilot embeddings. If a
GitHub token is available, GrokBot discovers available embedding models from
...
1. GrokBot resolves your GitHub token (from env vars or auth profile).
...
available, GrokBot skips Copilot and tries the next provider.
