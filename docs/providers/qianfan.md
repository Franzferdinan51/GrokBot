---
summary: "Use Qianfan's unified API to access many models in GrokBot"
read_when:
...
Qianfan is Baidu's MaaS platform: a unified, OpenAI-compatible API that routes requests to many models behind a single endpoint and API key. GrokBot ships it as the official external plugin `@grokbot/qianfan-provider`.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    grokbot onboard --auth-choice qianfan-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    grokbot models list --provider qianfan
    ```
  </Step>
</Steps>
