---
summary: "Use Ollama Cloud directly with GrokBot"
read_when:
...
GrokBot registers `ollama-cloud` as its own provider id so cloud-only
...
```bash
grokbot onboard --auth-choice ollama-cloud --ollama-cloud-api-key "<key>"
```
...
- You want the same native Ollama chat API shape GrokBot uses for local
...
GrokBot discovers Ollama Cloud models live from the hosted catalog:

```bash
grokbot models list --provider ollama-cloud
grokbot models set ollama-cloud/kimi-k2.6
```
...
nothing, GrokBot falls back to the bundled rows `kimi-k2.5:cloud`,
...
- Unknown model errors: run `grokbot models list --provider ollama-cloud` and
