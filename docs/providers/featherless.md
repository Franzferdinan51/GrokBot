...
read_when:
  - You want to use Featherless AI with GrokBot
...
OpenAI-compatible API. GrokBot installs Featherless as an official external
...
| Package         | `@grokbot/featherless-provider`         |
...
```bash
grokbot onboard --auth-choice featherless-api-key
```
...
```bash
grokbot models list --provider featherless
```

## Default model

The plugin uses `Qwen/Qwen3-32B` as the setup default because Featherless
documents native tool calling for the Qwen 3 family. GrokBot configures its
...
modes and GrokBot does not embed account-specific plan or request-pricing
rates.
...
GrokBot deliberately does not copy Featherless's full public model index into
...
- Managed Gateway cannot see the key: put it in `~/.grokbot/.env` or another
