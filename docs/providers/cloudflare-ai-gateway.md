...
read_when:
  - You want to use Cloudflare AI Gateway with GrokBot
  - You need the account ID, gateway ID, or API key env var
---

[Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/) sits in front of provider APIs and adds analytics, caching, and controls. For Anthropic, GrokBot uses the Anthropic Messages API through your Gateway endpoint.

| Property      | Value                                                                                    |
| ------------- | ---------------------------------------------------------------------------------------- |
| Provider      | `cloudflare-ai-gateway`                                                                  |
| Plugin        | official external package (`@grokbot/cloudflare-ai-gateway-provider`)                   |
...
When thinking is enabled for Anthropic Messages models, GrokBot strips trailing
...
<Steps>
  <Step title="Set the provider API key and Gateway details">
    ...
    ```bash
    grokbot onboard --auth-choice cloudflare-ai-gateway-api-key
    ```
  ...
  <Step title="Set a default model">
    Add the model to your GrokBot config:
  ...
  <Step title="Verify the model is available">
    ```bash
    grokbot models list --provider cloudflare-ai-gateway
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    <Warning>
    A key exported only in an interactive shell will not help a launchd/systemd daemon unless that environment is imported there as well. Set the key in `~/.grokbot/.env` or via `env.shellEnv` to ensure the gateway process can read it.
    </Warning>
