---
summary: "Use NVIDIA's OpenAI-compatible API in GrokBot"
read_when:
  - You want to use open models in GrokBot for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). GrokBot
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    grokbot onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    grokbot models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
grokbot onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
model-selection surfaces without waiting for an GrokBot release. When the
...
GrokBot falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in GrokBot. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    GrokBot prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="OpenAI-compatible endpoint">
    GrokBot talks to NVIDIA with the `openai-completions` adapter against the
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. GrokBot's bundled Ultra row
