---
summary: "Use Venice AI privacy-focused models in GrokBot"
read_when:
  - You want privacy-focused inference in GrokBot
...
<Steps>
  ...
  <Step title="Configure GrokBot">
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        grokbot onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    grokbot agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
```bash
grokbot models set venice/kimi-k2-5
grokbot models list --all --provider venice
```

You can also run `grokbot configure` and pick **Model/auth provider > Venice AI**.
...
The bundled catalog above is a manifest-backed seed list. At runtime GrokBot
...
GrokBot catalog advertises only the canonical model IDs returned by `/models`.

## DeepSeek V4 replay behavior

If Venice exposes DeepSeek V4 models such as `deepseek-v4-pro` or
`deepseek-v4-flash`, GrokBot fills the required `reasoning_content` replay
...
grokbot agent --model venice/kimi-k2-5 --message "Quick health check"

# Claude Opus via Venice (anonymized)
grokbot agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Uncensored model
grokbot agent --model venice/venice-uncensored-1-2 --message "Draft options"

# Vision model with image
grokbot agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Coding model
grokbot agent --model venice/qwen3-coder-480b-a35b-instruct-turbo --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    grokbot models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    Run `grokbot models list --all --provider venice` to see currently
