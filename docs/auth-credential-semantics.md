...
- `grokbot doctor` auth checks (`doctor-auth`)
...
Agent auth inheritance is read-through. When an agent has no local profile, it resolves profiles from the default/main agent store at runtime without copying secret material into its own credential store (`agents/<agentId>/agent/grokbot-agent.sqlite`).

Explicit copy flows, such as `grokbot agents add`, use this portability policy:
...
Do not write `type: "aws-sdk"` into the credential store; stored credentials are only `api_key`, `token`, or `oauth`. If a legacy `auth-profiles.json` has such a marker, `grokbot doctor --fix` moves it to `auth.profiles` and removes the marker from the store.
...
- If a provider has credentials but GrokBot cannot resolve a probeable model candidate for it, `models status --probe` reports `status: no_model` with `reasonCode: no_model`.
