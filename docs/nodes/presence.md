...
read_when:
  - You want GrokBot to identify the active Mac
...
the most recent physical mouse or keyboard input. GrokBot uses that signal to
...
- The GrokBot macOS app is paired and connected in node mode.
- **Accessibility** permission is granted to the signed GrokBot app.
...
3. Move the mouse or press a key on that Mac, then run:
   ...
   grokbot nodes describe --node <node-id-or-name>
   ```
...
GrokBot sends idle duration, not input content. It does not send key values,
...
GrokBot waits 750 milliseconds so the connecting Mac can submit its first
...
- If no active Mac is available or primary delivery fails, GrokBot waits five
...
| Symptom                                   | Check                                                                                                                                                                |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No row is marked `active`                 | Confirm a native macOS node is connected and `grokbot nodes describe --node <id>` shows `permissions.accessibility: true`.                                          |
| The wrong Mac remains active              | Use that Mac physically, wait for the coalescing window, then rerun `grokbot nodes status`. Synthetic computer-control actions do not count.                        |
