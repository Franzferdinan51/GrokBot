...
GrokBot supports camera capture for agent workflows on paired **iOS**, **Android**, **macOS**, and **Linux** nodes: capture a photo (`jpg`) or a short video clip (`mp4`, with optional audio) via Gateway `node.invoke`.
...
```bash
grokbot nodes camera snap --node <id>                 # default: both front + back (2 MEDIA lines)
grokbot nodes camera snap --node <id> --facing front
grokbot nodes camera clip --node <id> --duration 3000
grokbot nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`grokbot.cameraEnabled`).
  - Default: **off**.
  - When off: camera requests return `CAMERA_DISABLED: enable Camera in Settings`.

### CLI helper (node invoke)

Use the main `grokbot` CLI to invoke camera commands on the macOS node.

```bash
grokbot nodes camera list --node <id>                     # list camera ids
grokbot nodes camera snap --node <id>                     # prints saved path
grokbot nodes camera snap --node <id> --max-width 1280
grokbot nodes camera snap --node <id> --delay-ms 2000
grokbot nodes camera snap --node <id> --device-id <id>
grokbot nodes camera clip --node <id> --duration 10s       # prints saved path
grokbot nodes camera clip --node <id> --duration-ms 3000   # prints saved path (legacy flag)
grokbot nodes camera clip --node <id> --device-id <id>
grokbot nodes camera clip --node <id> --no-audio
```

- `grokbot nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
The bundled Linux Node plugin adds camera capture to the CLI `grokbot node` service. It works on a headless host and does not require the Linux desktop app.
...
```bash
grokbot nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```
