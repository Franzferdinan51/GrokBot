...
WS server as one node (so `grokbot nodes …` works against this Mac). The app
adds native Canvas, camera, screen, notification, and computer-control commands
to the same node-host command surface used by `grokbot node run`. Do not start a
...
grokbot devices reject <requestId>
grokbot nodes status
grokbot nodes describe --node <idOrNameOrIp>
```

Pending pairing requests expire 5 minutes after the device's last retry — a device that keeps reconnecting keeps its one pending request (and `requestId`) alive instead of minting a new prompt every few minutes; see [Node pairing](/gateway/pairing) for the full request/approve lifecycle. If a node retries with changed auth details (role/scopes/public key), the prior pending request is superseded and a new `requestId` is created — clients get a `device.pair.resolved` event for the superseded request, and you should re-run `grokbot devices list` before approving.
...
| Approvals    | Enforced on the node host via `~/.grokbot/exec-approvals.json`. |
...
- For direct shell/runtime file executions, GrokBot also best-effort binds one concrete local file operand and denies the run if that file changes before execution.
- If GrokBot cannot identify exactly one concrete local file for an interpreter/runtime command, approval-backed execution is denied instead of pretending full runtime coverage. Use sandboxing, separate hosts, or an explicit trusted allowlist/full workflow for broader interpreter semantics.
...
```bash
grokbot node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
grokbot node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `grokbot node run` supports token or password auth.
...
```bash
grokbot node install --host <gateway-host> --port 18789 --display-name "Build Node"
grokbot node start
...
If the node retries with changed auth details, re-run `grokbot devices list` and approve the current `requestId`.

Naming options:

- `--display-name` on `grokbot node run` / `grokbot node install` (persists in the shared `node_host_config` SQLite row alongside the client instance ID and Gateway connection metadata).
- `grokbot nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).

### Node-hosted MCP servers

Configure MCP servers in `grokbot.json` on the node machine, not on the
Gateway:
...
older GrokBot version may request a one-time command-surface upgrade after the
...
`grokbot node run` or `grokbot node restart` to apply node MCP config changes;
...
Install skills under the node machine's active GrokBot skills directory,
`~/.grokbot/skills` by default. `OPENCLAW_HOME`, `OPENCLAW_STATE_DIR`, and
...
`grokbot config file`. The headless node host publishes valid `SKILL.md` files
...
change. Restart `grokbot node run` or `grokbot node restart` after changing
...
its active GrokBot state directory, so relative paths resolve on the node rather
...
- `~/.grokbot/state/grokbot.sqlite` (`node_host_config`): the client instance ID, display name, and Gateway connection metadata.
- `~/.grokbot/state/grokbot.sqlite` (`device_identities`, key `primary`): the signed device keypair and derived cryptographic device ID.
- `~/.grokbot/identity/device-auth.json`: paired device auth tokens keyed by cryptographic device ID and role.
...
identity use. Stop the node host and run `grokbot doctor --fix`; Doctor imports
...
```bash
grokbot approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
grokbot approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.grokbot/exec-approvals.json`.
...
```bash
grokbot config set tools.exec.host node
grokbot config set tools.exec.security allowlist
grokbot config set tools.exec.node "<id-or-name>"
```
...
The relay does not provide the full GrokBot harness continuation and archive ownership contracts. **Continue** and **Archive** are therefore unavailable for remote rows. On the Gateway computer, stored and idle
...
This is a takeover of the native session; unlike GrokBot adoption, it does not
...
Chat composer: GrokBot imports bounded visible history, resumes with
...
row on that node becomes continuable: GrokBot imports bounded history, binds
...
```bash
grokbot nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
grokbot nodes canvas snapshot --node <idOrNameOrIp> --format png
grokbot nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
grokbot nodes canvas present --node <idOrNameOrIp> --target https://example.com
grokbot nodes canvas hide --node <idOrNameOrIp>
grokbot nodes canvas navigate https://example.com --node <idOrNameOrIp>
grokbot nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
grokbot nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
grokbot nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
grokbot nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
grokbot nodes camera list --node <idOrNameOrIp>
grokbot nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
grokbot nodes camera snap --node <idOrNameOrIp> --facing front
grokbot nodes camera snap --node <idOrNameOrIp> --device-id <id> --max-width 1200 --quality 0.9 --delay-ms 2000
```

Video clips (`mp4`):

```bash
grokbot nodes camera clip --node <idOrNameOrIp> --duration 10s
grokbot nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
grokbot nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
grokbot nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
grokbot nodes location get --node <idOrNameOrIp>
grokbot nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
For read-only SMS search, opt in explicitly in `grokbot.json`:
...
```bash
grokbot nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from GrokBot"}'
```
...
```bash
grokbot nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
grokbot nodes invoke --node <idOrNameOrIp> --command device.apps --params '{"limit":10}'
grokbot nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
grokbot nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
grokbot nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
grokbot nodes invoke --node <idOrNameOrIp> --command system.which --params '{"bins":["git"]}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.grokbot/exec-approvals.json`); on macOS specifically, see the exec-host routing env vars under [Headless node host](#headless-node-host-cross-platform) below.
...
```bash
grokbot config set tools.exec.node "node-id-or-name"
```

Per-agent override:

```bash
grokbot config get agents.list
grokbot config set 'agents.list[0].tools.exec.node' "node-id-or-name"
```

Unset to allow any node:

```bash
grokbot config unset tools.exec.node
grokbot config unset 'agents.list[0].tools.exec.node'
```
...
GrokBot can run a **headless node host** (no UI) that connects to the Gateway WebSocket and exposes `system.run` / `system.which`. This is useful on Linux/Windows or for running a minimal node alongside a server.

Start it:

```bash
grokbot node run --host <gateway-host> --port 18789
```
...
- Exec approvals are enforced locally via `~/.grokbot/exec-approvals.json` (see [Exec approvals](/tools/exec-approvals)).
...
- The macOS menubar app connects to the Gateway WS server as a node (so `grokbot nodes …` works against this Mac).
