---
summary: "Delegate architecture: running GrokBot as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Run GrokBot as a **named delegate**: an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human - it sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate

A delegate is an GrokBot agent that:
...
GrokBot's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of GrokBot's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.grokbot/workspace-delegate",
  tools: {
...
- Cron run history: GrokBot's shared SQLite state database.
- Session transcripts: `~/.grokbot/agents/delegate/sessions`.
- Identity provider audit logs (Exchange, Google Workspace).

All delegate actions flow through GrokBot's session store. For compliance, retain and review these logs.
...
```bash
grokbot agents add delegate --workspace ~/.grokbot/workspace-delegate
```

This creates:

- Workspace: `~/.grokbot/workspace-delegate`
- Agent state: `~/.grokbot/agents/delegate/agent`
- Sessions: `~/.grokbot/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.grokbot/workspace" },
      {
        id: "delegate",
        workspace: "~/.grokbot/workspace-delegate",
        tools: {
...
~/.grokbot/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.grokbot/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.grokbot/workspace-org",
        agentDir: "~/.grokbot/agents/org-assistant/agent",
