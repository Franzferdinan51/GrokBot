...
`grokbot/plugin-sdk/channel-outbound` / `channel-inbound` subpaths. For the
...
Public surface: `grokbot/plugin-sdk/channel-outbound` (send/receipt/durable/live/reply-pipeline
helpers) and `grokbot/plugin-sdk/channel-inbound` (inbound context, `runChannelInboundEvent`,
...
GrokBot only advances the persisted restart watermark past updates that
...
- **No `MessageOrigin` / `shouldDropOpenClawEcho`.** The original plan called
  for a `source: "grokbot"` origin tag on gateway-failure messages plus a
...
- **iMessage** (`extensions/imessage/src/monitor/echo-cache.ts`,
  ...
  cache, or GrokBot can re-ingest its own replies as inbound user messages.
...
- Whether Telegram should eventually replace the grammY (`1.43.0`) polling
  ...
  redelivery, not only GrokBot's persisted restart watermark
