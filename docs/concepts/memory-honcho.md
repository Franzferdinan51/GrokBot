...
[Honcho](https://honcho.dev) adds AI-native memory to GrokBot through an
...
```bash
grokbot plugins install @honcho-ai/grokbot-honcho
grokbot honcho setup
...
Settings live under `plugins.entries["grokbot-honcho"].config`:
    ...
    entries: {
      "grokbot-honcho": {
        config: {
          ...
          workspaceId: "grokbot", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `grokbot honcho setup` detects and
...
During conversation, Honcho tools query the service during GrokBot's
...
```bash
grokbot honcho setup                        # Configure API key and migrate files
grokbot honcho status                       # Check connection status
grokbot honcho ask <question>               # Query Honcho about the user
grokbot honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/grokbot-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho GrokBot integration guide](https://docs.honcho.dev/v3/guides/integrations/grokbot)
