---
summary: "How GrokBot remembers things across sessions"
...
GrokBot remembers things by writing plain Markdown files in your agent's
workspace (default `~/.grokbot/workspace`). The model only remembers what gets
...
If `MEMORY.md` grows past the bootstrap file budget, GrokBot keeps the file on
...
prompt budget. Use `/context list`, `/context detail`, or `grokbot doctor` to
...
detected files, and confirm the import. GrokBot copies only Markdown memory:
...
GrokBot approval settings, sandboxing, and scheduled tasks for hard
...
memories for that case. GrokBot infers them in a hidden background pass,
...
<Info>
GrokBot uses OpenAI embeddings by default. Set
...
GrokBot runs a silent turn that reminds the agent to save important context
...
```bash
grokbot memory rem-backfill --rollback
grokbot memory rem-backfill --rollback-short-term
```

## CLI

```bash
grokbot memory status          # Check index status and provider
grokbot memory search "query"  # Search from the command line
grokbot memory index --force   # Rebuild the index
```
