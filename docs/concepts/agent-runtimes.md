---
summary: "How GrokBot separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between GrokBot, Codex, ACP, or another native agent runtime
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `anthropic`, `github-copilot`, `openai`      | How GrokBot authenticates, discovers models, and names model refs. |
| Model         | `claude-opus-4-6`, `gpt-5.6-sol`             | The model selected for the agent turn.                              |
| Agent runtime | `claude-cli`, `codex`, `copilot`, `grokbot` | The low-level loop or backend that executes the prepared turn.      |
| Channel       | Discord, Slack, Telegram, WhatsApp           | Where messages enter and leave GrokBot.                            |
...
runtime keys are legacy and ignored. `grokbot doctor --fix` removes old
...
- **Embedded harnesses** run inside GrokBot's prepared agent loop: the
  built-in `grokbot` runtime, plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | GrokBot name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
makes native app-server features available; `grokbot doctor --fix` owns
...
That means GrokBot selects an OpenAI model ref, then asks the Codex
...
GrokBot session store becomes Codex.
...
3. **GrokBot explicitly chosen for an OpenAI model** -> keep the model ref as `openai/<model>` and set provider/model runtime policy to `agentRuntime.id: "grokbot"`. A selected `openai` OAuth profile is routed internally through GrokBot's Codex-auth transport.
4. **Legacy Codex model refs in config** -> repair with `grokbot doctor --fix` to `openai/<model>`; doctor keeps the Codex auth route by adding provider/model-scoped `agentRuntime.id: "codex"` where the old model ref implied it. Legacy **`codex-cli/*`** model refs repair to the same `openai/<model>` Codex app-server route; GrokBot no longer keeps a bundled Codex CLI backend.
...
| Surface                     | GrokBot embedded                              | Codex app-server                                                            |
| --------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | GrokBot, through the GrokBot embedded runner | Codex app-server                                                            |
| Canonical thread state      | GrokBot transcript                            | Codex thread, plus GrokBot transcript mirror                               |
| GrokBot dynamic tools      | Native GrokBot tool loop                      | Bridged through the Codex adapter                                           |
| Native shell and file tools | GrokBot path                                  | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native GrokBot context assembly               | GrokBot projects assembled context into the Codex turn                     |
| Compaction                  | GrokBot or selected context engine            | Codex-native compaction, with GrokBot notifications and mirror maintenance |
| Channel delivery            | GrokBot                                       | GrokBot                                                                    |

Design rule: if GrokBot owns the surface, it can provide normal plugin hook
behavior. If the native runtime owns the surface, GrokBot needs runtime
events or native hooks. If the native runtime owns canonical thread state,
GrokBot mirrors and projects context rather than rewriting unsupported
internals.

## Runtime selection

GrokBot resolves an embedded runtime after provider and model resolution, in
this order:
...
4. If nothing claims the turn in `auto` mode, GrokBot falls back to
   `grokbot` as the compatibility runtime. Use an explicit runtime id when
...
and `agents.list[].agentRuntime`. Run `grokbot doctor --fix` to remove stale
...
never silently routed back to GrokBot. Only `auto` may route an unmatched
turn to GrokBot.
...
harness. Explicit GrokBot runtime config remains an opt-in compatibility
route for `openai/*` agent turns; when paired with a selected `openai` OAuth
profile, GrokBot routes that path internally through the Codex-auth
...
If `grokbot doctor` warns that the `codex` plugin is enabled while legacy
Codex model refs remain in config, treat that as legacy route state and run
`grokbot doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@grokbot/copilot` plugin registers an opt-in `copilot` runtime
...
prefix in `extensions/copilot/doctor-contract-api.ts`, which `grokbot doctor`
...
When a runtime is not GrokBot, its docs should state which GrokBot surfaces
it supports:

| Question                               | Why it matters                                                                                    |
| -------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Who owns the model loop?               | Determines where retries, tool continuation, and final answer decisions happen.                   |
| Who owns canonical thread history?     | Determines whether GrokBot can edit history or only mirror it.                                   |
| Do GrokBot dynamic tools work?        | Messaging, sessions, cron, and GrokBot-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around GrokBot-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume GrokBot equivalence where the native runtime owns more state.            |
