---
summary: "Move one GrokBot session's reply route between linked chat channels"
...
Channel docking is call forwarding for one GrokBot session. It keeps the same
...
Alice can message GrokBot on Telegram and Discord:
...
GrokBot keeps the current session context and changes the reply route:
...
4. Keep the same GrokBot session, but receive future replies in Discord.
...
GrokBot generates one `/dock-<channel>` command for every loaded channel plugin
...
Send the dock command from a direct chat with GrokBot, not from a group chat.
