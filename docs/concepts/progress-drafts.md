...
read_when:
  ...
  - Explaining how GrokBot updates one channel message while work is in progress
...
`channels.<channel>.streaming.mode: "progress"` and GrokBot creates the
...
that; otherwise GrokBot sends the final answer through normal delivery and
...
GrokBot renders only `progress.text` in the channel progress UI. The normal
...
GrokBot uses the same formatter for progress drafts and `/verbose`:
...
the draft is edited, and GrokBot truncates long lines so repeated draft edits
...
With `toolProgress: false`, GrokBot still suppresses the older standalone
...
When the final answer is ready, GrokBot tries to keep the chat clean:
...
- If the draft can safely become the final answer (`partial`/`block` modes),
  GrokBot edits it in place.
- If the channel uses native progress streaming, GrokBot finalizes that
...
- Otherwise (media, an approval prompt, an explicit reply target, too many
  chunks, or a failed edit/send) GrokBot sends the final answer through the
...
Check `streaming.progress.toolProgress`. If it is `false`, GrokBot keeps the
