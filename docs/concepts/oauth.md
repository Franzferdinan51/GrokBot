---
summary: "OAuth in GrokBot: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand GrokBot OAuth end-to-end
...
GrokBot supports OAuth ("subscription auth") for providers that offer it,
notably **OpenAI Codex (ChatGPT OAuth)** and **Anthropic Claude CLI reuse**.
For Anthropic, the practical split is:

- **Anthropic API key**: normal Anthropic API billing.
- **Anthropic Claude CLI / subscription auth inside GrokBot**: Anthropic staff
  told us this usage is allowed again, so GrokBot treats Claude CLI reuse and
...
GrokBot stores both OpenAI API-key auth and ChatGPT/Codex OAuth under the
...
`grokbot doctor --fix`; use `openai:*` profile ids and `auth.order.openai` for
new config.
...
issued for the same user/app. Practical symptom: log in via GrokBot _and_
via Claude Code / Codex CLI, and one of them randomly gets logged out later.

To reduce that, GrokBot treats the auth profile store as a **token sink**:

- the runtime reads credentials from one place per agent
- multiple profiles can coexist and route deterministically
- external CLI reuse is provider-specific: once GrokBot owns a local OAuth
  profile for a provider, the local refresh token is canonical. If that local
  refresh token is rejected, GrokBot reports the profile for
  ...
  `openai:default`-style profile before GrokBot owns OAuth for that
  provider; after that, GrokBot-owned refreshes stay canonical
...
- Legacy compatibility file: `~/.grokbot/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.grokbot/credentials/oauth.json` (imported into the auth profile store on first use)
...
When a secondary agent has no local auth profile, GrokBot uses read-through
...
GrokBot supports Anthropic Claude CLI reuse and `claude -p` as a sanctioned
...
available as a supported token-auth path, but GrokBot prefers Claude CLI
reuse when it is available.

<Warning>
Anthropic's public Claude Code docs say direct Claude Code use stays within
Claude subscription limits, and Anthropic staff told us GrokBot-style Claude
CLI usage is allowed again. GrokBot therefore treats Claude CLI reuse and
`claude -p` usage as sanctioned for this integration unless Anthropic
...
If you want other subscription-style options in GrokBot, see [OpenAI
...
GrokBot's interactive login flows are implemented in `grokbot/plugin-sdk/llm.ts` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. create the token by running `claude setup-token` on any machine with Claude Code, then start Anthropic setup-token or paste-token from GrokBot
2. GrokBot stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including GrokBot workflows.
...
`grokbot models auth list --provider openai` after repair before copying
...
Wizard path is `grokbot onboard` → auth choice `openai`.
...
- externally managed CLI credentials (Claude CLI, narrow Codex CLI bootstrap;
  ...
  spending a copied refresh token. If a managed refresh fails, GrokBot
...
```bash
grokbot models auth list --provider <id>
```
