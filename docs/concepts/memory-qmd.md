...
alongside GrokBot. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** - if QMD is unavailable, GrokBot falls back to the
...
GrokBot creates a self-contained QMD home under
`~/.grokbot/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- GrokBot creates collections from your workspace memory files and any
...
- QMD continues to own its `index.sqlite`, YAML collection config, and model
  ...
  not GrokBot state tables. GrokBot-owned coordination lives only in SQLite:
  ...
  Runtime no longer creates QMD file-lock sidecars. `grokbot doctor --fix`
  removes retired sidecars only after proving their old process owner is stale.
  Upgrades are a clean cutover: stop and restart every GrokBot process that
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so GrokBot skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, GrokBot retries with `qmd query`.
- When `searchMode` is `query`, set `memory.qmd.rerank` to `false` to use
  ...
  GrokBot passes `--no-rerank` to the direct QMD CLI path and
  `rerank: false` to QMD's MCP query tool.
- With QMD releases that advertise multi-collection filters, GrokBot groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, GrokBot falls back to the builtin SQLite engine.
  ...
  `grokbot memory status` and one-shot CLI probes still recheck QMD
  directly.
...
GrokBot keeps the QMD search path compatible with both current and older QMD
installs.

On startup, GrokBot checks the installed QMD help text once per manager. If
the binary advertises support for multiple collection filters, GrokBot
...
Older QMD builds only accept one collection filter. When GrokBot detects one
...
process, so you can tune QMD globally without adding new GrokBot config:
...
collection under `~/.grokbot/agents/<id>/qmd/sessions/`. Setting only
...
GrokBot logs a warning with the derived channel and chat type so empty
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If GrokBot
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but GrokBot still reports
...
with `grokbot memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs GrokBot uses.

**Many QMD subprocesses during search?** Update QMD if possible. GrokBot
...
`memory.qmd.searchMode = "search"`. GrokBot treats that mode as
...
for the next startup reconciliation. GrokBot recreates stale managed
...
QMD traversal follows the underlying QMD scanner rather than GrokBot's
