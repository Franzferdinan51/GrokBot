---
summary: "Frequently asked questions about GrokBot setup, configuration, and usage"
read_when:
...
<Steps>
  ...
  <Step title="Tail the latest log">
    ...
    ```bash
    tail -f "$(ls -t /tmp/grokbot/grokbot-*.log | head -1)"
    ```
  ...
  <Step title="Gateway snapshot (WS-only)">
    ```bash
    grokbot health --json
    grokbot health --verbose   # shows the target URL + config path on errors
    ```
...
<AccordionGroup>
  <Accordion title="What is GrokBot, in one paragraph?">
  ...
  <Accordion title="Value proposition">
    GrokBot is not "just a Claude wrapper." It is a **local-first control plane** that runs a capable assistant on **your own hardware**, reachable from the chat apps you already use, with stateful sessions, memory, and tools - without handing your workflows to a hosted SaaS.
  ...
  <Accordion title="What are the top five everyday use cases for GrokBot?">
  ...
  <Accordion title="Can GrokBot help with lead gen, outreach, ads, and blogs for a SaaS?">
    ...
    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and platform policies, and review anything before it sends. Let GrokBot draft; you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    GrokBot is a **personal assistant** and coordination layer, not an IDE replacement. Use Claude Code or Codex for the fastest direct coding loop inside a repo. Use GrokBot for durable memory, cross-device access, and tool orchestration.
    ...
    Showcase: [https://grokbot.ai/showcase](https://grokbot.ai/showcase).
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put changes in `~/.grokbot/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.grokbot/grokbot.json`). Precedence: `<workspace>/skills` -> `<workspace>/.agents/skills` -> `~/.agents/skills` -> `~/.grokbot/skills` -> bundled -> `skills.load.extraDirs`, so managed overrides win over bundled skills without touching git. To install globally but limit visibility to some agents, keep the shared copy in `~/.grokbot/skills` and control visibility with `agents.defaults.skills` / `agents.list[].skills`. Only upstream-worthy edits should go out as PRs against the repo copy.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes: add directories via `skills.load.extraDirs` in `~/.grokbot/grokbot.json` (lowest precedence in the order above). `clawhub` installs into `./skills` by default, which GrokBot treats as `<workspace>/skills` on the next session. To limit visibility to certain agents, pair with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, GrokBot falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is exactly `NO_REPLY` / `no_reply` or `ANNOUNCE_SKIP`, GrokBot intentionally suppresses the announce instead of posting stale earlier progress.

    Debug: `grokbot tasks show <lookup>` where `<lookup>` is a task id, run id, or session key.
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    Debug:
    ```bash
    grokbot cron run <jobId>
    grokbot cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `grokbot skills` commands or drop skills into your workspace; the macOS Skills UI is not available on Linux. Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    grokbot skills search "calendar"
    grokbot skills search --limit 20
    grokbot skills install @owner/<skill-slug>
    grokbot skills install @owner/<skill-slug> --version <version>
    grokbot skills install @owner/<skill-slug> --force
    grokbot skills install @owner/<skill-slug> --global
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.grokbot.os` plus required binaries, and only load when eligible on the **Gateway host**. On Linux, `darwin`-only skills (`apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH)**. Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. GrokBot treats macOS-only skills as eligible when required binaries exist on the node; the agent runs them via the `nodes` tool. With "Always Ask," approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced)**. Keep the Gateway on Linux, but make the required CLI binaries resolve to SSH wrappers that run on a Mac, then override the skill to allow Linux so it stays eligible.
    ...
    3. Override the skill metadata (workspace or `~/.grokbot/skills`) to allow Linux:
       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "grokbot": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="How do I use my existing signed-in Chrome with GrokBot?">
    ...
    grokbot browser --browser-profile user snapshot
    ```
    ...
    Current limits on `existing-session` / `user` profiles versus the managed `grokbot` profile:
...
<AccordionGroup>
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    GrokBot validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor, so symlink-parent escapes fail closed even when the final path segment does not exist yet.
  ...
  <Accordion title="How does memory work?">
    GrokBot memory is Markdown files in the agent workspace: daily notes in `memory/YYYY-MM-DD.md`, curated long-term notes in `MEMORY.md` (main/private sessions only).

    GrokBot also runs a silent **pre-compaction memory flush** before compaction summarizes the conversation, reminding the model to write durable notes first. It only runs when the workspace is writable (read-only sandboxes skip it); disable with `agents.defaults.compaction.memoryFlush.enabled: false`. See [Memory](/concepts/memory).
...
<AccordionGroup>
  <Accordion title="Is all data used with GrokBot saved locally?">
    No: **GrokBot's own state is local**, but **external services still see what you send them**.

    - **Local by default**: sessions, memory files, config, and workspace live on the Gateway host (`~/.grokbot` plus your workspace directory).
  ...
  <Accordion title="Where does GrokBot store its data?">
    Everything lives under `$OPENCLAW_STATE_DIR` (default: `~/.grokbot`):

    | Path                                                               | Purpose                                                            |
    | ------------------------------------------------------------------ | ------------------------------------------------------------------ |
    | `$OPENCLAW_STATE_DIR/grokbot.json`                                 | Main config (JSON5)                                                 |
    ...
    | `$OPENCLAW_STATE_DIR/agents/<agentId>/agent/grokbot-agent.sqlite`  | Per-agent SQLite state, including session rows and transcripts      |
    | `$OPENCLAW_STATE_DIR/agents/<agentId>/sessions/`                    | Legacy session migration sources and archive/support artifacts      |

    Legacy single-agent path `~/.grokbot/agent/*` is migrated by `grokbot doctor`.

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate, configured via `agents.defaults.workspace` (default: `~/.grokbot/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These live in the **agent workspace**, not `~/.grokbot`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`, `MEMORY.md`, `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`. Lowercase root `memory.md` is legacy repair input only; `grokbot doctor --fix` can merge it into `MEMORY.md` when both exist.
    - **State dir (`~/.grokbot`)**: config, channel/provider state, auth profiles, sessions, logs, shared skills (`~/.grokbot/skills`).

    Default workspace is `~/.grokbot/workspace`, configurable:

    ```json5
    {
      agents: { defaults: { workspace: "~/.grokbot/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.grokbot` (credentials, sessions, tokens, encrypted secrets payloads). For a full restore, back up the workspace and state directory separately.

    Docs: [Agent workspace](/concepts/agent-workspace).

  </Accordion>

  <Accordion title="How do I completely uninstall GrokBot?">
    See [Uninstall](/install/uninstall).
  </Accordion>

  <Accordion title="Can agents work outside the workspace?">
    Yes. The workspace is the **default cwd** and memory anchor, not a hard sandbox. Relative paths resolve inside the workspace; absolute paths can access other host locations unless sandboxing is enabled. For isolation, use [`agents.defaults.sandbox`](/gateway/sandboxing) or per-agent sandbox settings. To make a repo the default working directory, point that agent's `workspace` at the repo root - the GrokBot repo itself is just source code, so keep the workspace separate unless you intentionally want the agent to work inside it.
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    GrokBot reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.grokbot/grokbot.json`). If the file is missing, it uses safe-ish defaults, including a default workspace of `~/.grokbot/workspace`.
  </Accordion>
  ...
  <Accordion title="Why do I need a token on localhost now?">
    GrokBot enforces gateway auth by default, including loopback. If no explicit auth path is configured, startup resolves to token mode and generates a runtime-only token for that startup, so local WS clients must authenticate. This blocks other local processes from calling the Gateway.

    Configure `gateway.auth.token`, `gateway.auth.password`, `OPENCLAW_GATEWAY_TOKEN`, or `OPENCLAW_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. You can also choose password mode, or `trusted-proxy` for identity-aware reverse proxies. For open loopback, set `gateway.auth.mode: "none"` explicitly. `grokbot doctor --generate-gateway-token` generates a token any time.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: always uses `All your chats, one GrokBot.`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    Grok can also reuse xAI OAuth from model auth (`grokbot onboard --auth-choice xai-oauth`).

    **Recommended**: `grokbot configure --section web` and pick a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, GrokBot auto-detects the first ready fetch fallback provider from available credentials; the official Firecrawl plugin provides that fallback.
    - Daemons read env vars from `~/.grokbot/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current GrokBot protects most accidental clobbers:

    - GrokBot-owned config writes validate the full post-change config before writing.
    - Invalid or destructive GrokBot-owned writes are rejected and saved as `grokbot.json.rejected.*`.
    - A direct edit that breaks startup or hot reload makes the Gateway fail closed or skip the reload; it does not rewrite `grokbot.json`.
    - `grokbot doctor --fix` owns repair, can restore last-known-good, and saves the rejected file as `grokbot.json.clobbered.*`.

    Recover:

    - Check `grokbot logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `grokbot.json.clobbered.*` or `grokbot.json.rejected.*` beside the active config.
    - Run `grokbot config validate` and `grokbot doctor --fix`.
    - Copy only the intended keys back with `grokbot config set` or `config.patch`.
    - No last-known-good or rejected payload: restore from backup, or re-run `grokbot doctor` and reconfigure channels/models.
  ...
  <Accordion title="Can the GrokBot browser run headless?">
    Yes:
...
<AccordionGroup>
  ...
  <Accordion title="Can two GrokBot instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic)**: run a script that calls the other Gateway with `grokbot agent --message ... --deliver`, targeting a chat where the other bot listens. If one bot is on a remote VPS, point your CLI at that remote Gateway via SSH/Tailscale (see [Remote access](/gateway/remote)):

    ```bash
    grokbot agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    grokbot gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does GrokBot load environment variables?">
    GrokBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory.
    - a global fallback `.env` from `~/.grokbot/.env` (`$OPENCLAW_STATE_DIR/.env`).

    Neither `.env` file overrides existing env vars. Provider credential and endpoint-routing keys are an exception for workspace `.env`: keys such as `GEMINI_API_KEY`, `XAI_API_KEY`, `MISTRAL_API_KEY`, or any key ending in `_ENDPOINT` (and other bundled-provider auth or endpoint env vars) are ignored from workspace `.env` and should live in the process environment, `~/.grokbot/.env`, or config `env`.
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.grokbot/.env` so they load even when the service does not inherit your shell env.
  ...
  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `grokbot models status` reports whether **shell env import** is enabled. "Shell env: off" does **not** mean your env vars are missing - it just means GrokBot will not load your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it will not inherit your shell environment. Fix by putting the token in `~/.grokbot/.env`, enabling `env.shellEnv.enabled: true`, or adding it to config `env` (applies only if missing), then restarting the gateway and rechecking:

    ```bash
    grokbot models status
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of GrokBot instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset GrokBot but keep it installed?">
    ```bash
    grokbot reset
    ```
    ...
    Onboarding also offers **Reset** if it detects an existing config; see [Onboarding (CLI)](/start/wizard). If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (default `~/.grokbot-<profile>`). Dev-only reset: `grokbot gateway --dev --reset` wipes dev config, credentials, sessions, and workspace.
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    If `HEARTBEAT.md` exists but is effectively empty (only blank lines, Markdown/HTML comments, ATX headings, fence markers, or empty list-item stubs), GrokBot skips the heartbeat run to save API calls. If the file is missing, the heartbeat still runs and the model decides what to do.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. GrokBot runs on **your own account** - if you are in the group, GrokBot can see it. By default, group replies are blocked until you allow senders (`groupPolicy: "allowlist"`).
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    grokbot directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does GrokBot not reply in a group?">
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth**: active sessions and transcripts live in the per-agent SQLite database; legacy/archive artifacts can still accumulate under `~/.grokbot/agents/<agentId>/sessions/`.
    - **Token cost**: more agents means more concurrent model usage.
    - **Ops overhead**: per-agent auth profiles, workspaces, and channel routing.

    Keep one **active** workspace per agent (`agents.defaults.workspace`), prune old sessions with `grokbot sessions cleanup` if disk grows (do not edit active SQLite state by hand), and use `grokbot doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  ...
  <Accordion title='Why does grokbot gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    "Running" is the **supervisor's** view (launchd/systemd/schtasks); the connectivity probe is the CLI actually connecting to the gateway WebSocket. Trust these lines from `grokbot gateway status`: `Probe target:` (the URL the probe used), `Listening:` (what is actually bound on the port), `Last gateway error:` (common root cause when the process is alive but the port is not listening).
  </Accordion>

  <Accordion title='Why does grokbot gateway status show "Config (cli)" and "Config (service)" different?'>
    ...
    ```bash
    grokbot gateway install --force
    ```

  </Accordion>

  <Accordion title='What does "another gateway instance is already listening" mean?'>
    GrokBot enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` ("another gateway instance is already listening").

    Fix: stop the other instance, free the port, or run with `grokbot gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run GrokBot in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `grokbot gateway` only starts when `gateway.mode` is `local` (or you pass an override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `grokbot dashboard` (prints + copies the dashboard URL, tries to open; shows an SSH hint if headless).
    - No token yet: `grokbot doctor --generate-gateway-token`.
    ...
    - Mismatch persists after the one retry: rotate/re-approve the paired device token:
      ```bash
      grokbot devices list
      grokbot devices rotate --device <id> --role operator
      ```
    ...
    - Still stuck: `grokbot status --all` plus [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    Recommended: `grokbot --profile <name> ...` per instance (auto-creates `~/.grokbot-<name>`), a unique `gateway.port` per profile config (or `--port` for manual runs), and a per-profile service with `grokbot --profile <name> gateway install`.

    Profiles also suffix service names: launchd `ai.grokbot.<profile>`, systemd `grokbot-gateway-<profile>.service`, Windows `GrokBot Gateway (<profile>)`. The unqualified `grokbot-gateway` systemd unit only exists for the default profile; the legacy pre-rename systemd unit name `clawdbot-gateway` is migrated automatically.
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```bash
    grokbot tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    File logs (structured): `/tmp/grokbot/grokbot-YYYY-MM-DD.log`. Set a stable path via `logging.file`; file log level via `logging.level`; console verbosity via `--verbose` and `logging.consoleLevel`.
    ...
    - macOS launchd stdout: `~/Library/Logs/grokbot/gateway.log` (profiles use `gateway-<profile>.log`; stderr is suppressed).
    - Linux: `journalctl --user -u grokbot-gateway[-<profile>].service -n 200 --no-pager`.
    - Windows: `schtasks /Query /TN "GrokBot Gateway (<profile>)" /V /FO LIST`.
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `grokbot gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart GrokBot?">
    Three Windows install modes:

    **1) Windows Hub local setup**: the native app manages a local app-owned WSL Gateway. Open **GrokBot Companion** from the Start menu or tray, then use **Gateway Setup** or the Connections tab.
    ...
    **2) Manual WSL2 Gateway**: the Gateway runs inside Linux.
    ...
    grokbot gateway restart
    ```
    If you never installed the service, start it in the foreground: `grokbot gateway run`.
    ...
    **3) Native Windows CLI/Gateway**: runs directly in Windows.
    ...
    grokbot gateway restart
    ```
    If you run it manually (no service): `grokbot gateway run`.
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    Usually means the UI lost the WebSocket connection. Check: is the Gateway running (`grokbot gateway status`)? Is it healthy (`grokbot status`)? Does the UI have the right token (`grokbot dashboard`)? If remote, is the tunnel/Tailscale link up?
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ```bash
    grokbot channels status
    grokbot channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. GrokBot already trims to the Telegram limit and retries with fewer commands, but some menu entries may still be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="How do I completely stop then start the Gateway?">
    ...
    In the foreground, stop with Ctrl-C, then `grokbot gateway run`.

    Docs: [Gateway service runbook](/gateway).

  </Accordion>

  <Accordion title="ELI5: grokbot gateway restart vs grokbot gateway">
    `grokbot gateway restart` restarts the **background service** (launchd/systemd). `grokbot gateway` runs the gateway **in the foreground** for this terminal session. Use the gateway subcommands if you installed the service; use the bare foreground run for a one-off.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must use structured media fields such as `media`, `mediaUrl`, `path`, or `filePath`. See [GrokBot assistant setup](/start/grokbot) and [Agent send](/tools/agent-send).

    ```bash
    grokbot message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose GrokBot to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults reduce risk:

    - Default behavior on DM-capable channels is **pairing**: unknown senders receive a pairing code and their message is not processed. Approve with `grokbot pairing approve --channel <channel> [--account <id>] <code>`. Pending requests are capped at **3 per channel**; check `grokbot pairing list --channel <channel> [--account <id>]` if a code did not arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `grokbot doctor` to surface risky DM policies.
  ...
  <Accordion title="Is GrokBot less safe because it uses TypeScript/Node instead of Rust/WASM?">
  ...
  <Accordion title="I saw reports about exposed GrokBot instances. What should I check?">
    ...
    Public binds without auth, open DMs/groups with tools, and exposed browser control are the findings to fix first. Details: [grokbot security audit](/gateway/security#grokbot-security-audit).

  </Accordion>

  <Accordion title="Are ClawHub skills and third-party plugins safe to install?">
    Treat third-party skills and plugins as code you are choosing to trust. ClawHub skill pages expose scan state before install, but scans are not a complete security boundary. GrokBot does not run built-in local dangerous-code blocking during plugin/skill install or update; use operator-owned `security.installPolicy` for local allow/block decisions.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code; their message is **not processed**. GrokBot only replies to chats it receives or to explicit sends you trigger.

    ```bash
    grokbot pairing approve whatsapp <code>
    grokbot pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    GrokBot blocks **cross-provider** messaging by default. If a tool call is bound to Telegram, it will not send to Discord unless you explicitly allow it - and this takes effect immediately, no gateway restart needed:
...
Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com/grokbot/grokbot/discussions).
