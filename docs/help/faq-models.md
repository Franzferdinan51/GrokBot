...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    ...
    you omit the provider, GrokBot tries an alias match first, then a unique
    ...
    provider no longer has the configured default model, GrokBot falls back
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `grokbot models set ...` (updates just model config)
    - `grokbot configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.grokbot/grokbot.json` directly
    ...
    restore from backup or run `grokbot doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `grokbot onboard`, choose `Ollama`, then `Local` or `Cloud + Local`
    ...
    manually: `grokbot models list`, then `grokbot models set ollama/<model>`.
  ...
  <Accordion title="If two providers expose the same model id, which one does /model use?">
    ...
    refs even though the model id matches — GrokBot does not silently switch
    ...
    to use fallback, GrokBot tries the requested provider/model first, then
  ...
  <Accordion title="Can I use GPT 5.5 for daily tasks and Codex 5.5 for coding?">
    ...
    - **Native Codex coding agent:** set `agents.defaults.model.primary` to
      `openai/gpt-5.5`. Sign in with `grokbot models auth login --provider
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M3"?'>
    If you're on an older GrokBot release, upgrade first (or run from source
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    Fix: run `grokbot agents add <id>` and configure auth in the wizard, or
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so GrokBot
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - Confirm where profiles live — current:
      `~/.grokbot/agents/<agentId>/agent/auth-profiles.json`; legacy:
      `~/.grokbot/agent/*` (migrated by `grokbot doctor`).
    - Confirm the Gateway loads your env var. `ANTHROPIC_API_KEY` set only in
      ...
      `~/.grokbot/.env` or enable `env.shellEnv`.
    - Confirm you're editing the right agent — multi-agent setups have
      multiple `auth-profiles.json` files.
    - Run `grokbot models status` to see configured models and provider
      auth state.
    ...
    **For "No credentials found for profile anthropic" (no email suffix):**
    ...
    - Use Claude CLI: run `grokbot models auth login --provider anthropic
      --method cli --set-default` on the gateway host.
    - Prefer an API key instead: put `ANTHROPIC_API_KEY` in
      `~/.grokbot/.env` on the gateway host, then clear any pinned order
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    ...
    switched to a Gemini shorthand), GrokBot tries it during fallback. No
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    on thinking blocks. GrokBot strips unsigned thinking blocks for Google
...
<AccordionGroup>
  <Accordion title="What is an auth profile?">
    ...
    Inspect saved profiles without dumping secrets: `grokbot models auth
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    GrokBot may skip a profile in a short **cooldown** (rate limits,
    timeouts, auth failures) or a longer **disabled** state
    (billing/insufficient credits). Inspect with `grokbot models status
    ...
    grokbot models auth order set --provider anthropic anthropic:work anthropic:default

    # Clear override (fall back to config auth.order / round-robin)
    grokbot models auth order clear --provider anthropic

    # Target a specific agent
    grokbot models auth order set --provider anthropic --agent main anthropic:default
    ```

    Verify what will actually be tried: `grokbot models status --probe`. A
  ...
  <Accordion title="OAuth vs API key - what is the difference?">
    - **OAuth / CLI login** often uses subscription access where the
      provider supports it. For Anthropic, GrokBot's Claude CLI backend
