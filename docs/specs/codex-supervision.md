...
summary: "Architecture and product boundary for supervising native Codex sessions from GrokBot."
read_when:
...
Codex supervision lets an GrokBot operator discover native Codex sessions and,
when safe, create a local branch through the normal GrokBot Chat surface.
Codex App Server remains the thread and model-loop owner. GrokBot supplies the
...
- native thread bindings to GrokBot sessions
...
Codex Desktop -> private stdio App Server -> user Codex home
                                             ^
GrokBot Codex plugin -> supervision App Server connection
...
Ordinary GrokBot Codex sessions -> managed agent-home stdio by default
  -> ordinary full harness threads -> GrokBot Chat and channel delivery
```
...
process-local; a thread unknown to GrokBot's supervision process is `notLoaded`
...
unbounded `thread/read`. GrokBot also rejects any serialized item page above
...
eventually discovered without importing them into GrokBot storage.

Catalog discovery is passive. Listing or reading metadata must not call
`thread/resume`, subscribe the GrokBot client to live thread requests, or
answer an approval.
...
```text
grokbot codex sessions [--search <text>] [--host <id>] [--limit <count>] [--cursor <cursor>] [--json] [gateway-options]
grokbot codex continue <thread-id> [--json] [gateway-options]
grokbot codex archive <thread-id> --confirm-no-other-runner [--json] [gateway-options]
```
...
2. Otherwise projects bounded user and assistant history through the source's
   ...
   GrokBot Chat and records a pending harness branch.
3. Stores the pending Codex-only model-lock policy, not a concrete model or
   ...
   returns the GrokBot `sessionKey`.
...
2. On that same connection, starts the canonical full Codex harness thread with
   `threadSource: "appServer"`, GrokBot's cwd, policy, config, environment, the
   full GrokBot harness tool surface, and exactly the model and provider
...
omit GrokBot model and provider overrides, so Codex restores the persisted pair.
If a separate native Codex control changes the canonical thread, GrokBot accepts
that native persisted selection. The outer GrokBot model and fallback chain
...
Deleting or resetting the GrokBot entry would otherwise discard the native
...
canonical harness thread on the first turn prevents GrokBot from becoming a
...
Atlas, or ChatGPT source remains eligible for both native and GrokBot catalogs.
...
known active GrokBot binding owner for the exact target or any non-archived
spawned descendant also rejects archive. GrokBot paginates Codex's experimental
...
`grokbot doctor --fix` migrates shipped `plugins.entries.codex-supervisor`
...
- audited handoff between Codex Desktop, CLI, and GrokBot
...
- Later resumes omit GrokBot model/provider overrides, preserve Codex's
  canonical persisted selection, accept separate native changes to that thread,
  and never substitute the outer GrokBot model or fallback chain.
