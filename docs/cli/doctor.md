---
summary: "CLI reference for `grokbot doctor` (health checks + guided repairs)"
read_when:
...
When Gateway status reports degraded SecretRef owners, doctor prints a **Secret runtime degradation** warning with every cold or stale owner, affected config path, redacted reason, and the `grokbot secrets reload` retry command.

When channel ingress events are dead-lettered, doctor names each affected channel account and points to [`grokbot channels dead-letters list`](/cli/channels#inbound-dead-letters) for inspection and recovery.
...
| ------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------- |
| Inspect                   | `grokbot doctor`                         | Human-oriented checks and guided prompts.                                       |
| Repair                    | `grokbot doctor --fix`                   | Applies supported repairs, using prompts unless non-interactive repair is safe. |
| Lint                      | `grokbot doctor --lint`                  | Read-only structured findings for CI, preflight, and review gates.              |
| Shared SQLite maintenance | `grokbot doctor --state-sqlite compact`  | Explicitly checkpoints, compacts, and verifies the canonical shared state DB.   |
| Session SQLite migration  | `grokbot doctor --session-sqlite <mode>` | Inspects, imports, validates, compacts, recovers, or restores session state.    |
...
grokbot doctor --lint --allow-exec
grokbot doctor --deep
grokbot doctor --fix
grokbot doctor --fix --non-interactive
grokbot doctor --generate-gateway-token
grokbot doctor --post-upgrade
grokbot doctor --post-upgrade --json
grokbot doctor --state-sqlite compact
grokbot doctor --state-sqlite compact --json
grokbot doctor --session-sqlite inspect --session-sqlite-all-agents
grokbot doctor --session-sqlite dry-run --session-sqlite-agent main --json
grokbot doctor --session-sqlite import --session-sqlite-all-agents
grokbot doctor --session-sqlite validate --session-sqlite-all-agents --json
grokbot doctor --session-sqlite compact --session-sqlite-all-agents
grokbot doctor --session-sqlite recover --github-issue
grokbot doctor --session-sqlite restore --session-sqlite-all-agents
```

For channel-specific permissions, use the channel probes instead of `doctor`:

```bash
grokbot channels capabilities --channel discord --target channel:<channel-id>
...
| `--github-issue`                | With `--session-sqlite recover`: prepare a sanitized grokbot/grokbot issue report; doctor creates it with `gh` after `--yes` or interactive confirmation.                             |
...
`grokbot doctor --lint` is read-only: no prompts, no repair, no config/state rewrites.
...
grokbot doctor --lint --only core/doctor/local-audio-acceleration --severity-min info
```
...
doctor --lint: ran 6 check(s), 1 finding(s)
  [warning] core/doctor/gateway-config gateway.mode - gateway.mode is unset; gateway start will be blocked.
    fix: Run `grokbot configure` and set Gateway mode (local/remote), or `grokbot config set gateway.mode local`.
```
  ...
  "findings": [
    {
      ...
      "path": "gateway.mode",
      "fixHint": "Run `grokbot configure` and set Gateway mode (local/remote), or `grokbot config set gateway.mode local`."
    }
  ]
}
```
...
`--severity-min` controls both which findings print and the exit threshold: `grokbot doctor --lint --severity-min error` can print nothing and exit `0` even when lower-severity `info`/`warning` findings exist.
...
`grokbot doctor --fix` against the same mounted state/config before restarting
the container normally.

## Legacy state migration

`grokbot doctor --fix` is the only owner for persistent file-to-SQLite migrations. It validates and claims each recognized source, writes and verifies canonical rows, records a migration receipt, then removes the retired source. Runtime code does not perform lazy imports or fallback reads.

This includes retired MCP OAuth files under `<state-dir>/mcp-oauth/*.json`. Stop the Gateway before repair. Doctor imports valid credentials into `<state-dir>/state/grokbot.sqlite`, preserves an existing canonical SQLite session when both stores exist, drops the obsolete persisted OAuth `state` value, and uses its receipt to prevent a recreated stale file from resurrecting logged-out credentials. Retired `.lock` sidecars fail closed: if Doctor reports a stale owner, verify that no older GrokBot process is running, remove that sidecar, and rerun Doctor.
...
`grokbot doctor --state-sqlite compact` is explicit offline maintenance for
the canonical shared state database at
`<state-dir>/state/grokbot.sqlite`. It does not accept an arbitrary database
path, is never invoked by normal Gateway operation, and is not part of
`grokbot doctor --fix`. The command acquires the same state ownership lock as
...
3. Requires a non-busy `wal_checkpoint(TRUNCATE)`. Stop any remaining GrokBot
...
running GrokBot build, or belongs to an agent database. Run
`grokbot doctor --fix` first for an older shared-state schema. Restore a
compatible backup or upgrade GrokBot for a newer schema.

## Session SQLite migration

GrokBot imports legacy session rows and transcript history into each agent's
SQLite database automatically during gateway startup and during
`grokbot doctor --fix`. `grokbot doctor --session-sqlite <mode>` is the
targeted inspection and validation tool for that migration. Current runtime
session rows live in
`~/.grokbot/agents/<agentId>/agent/grokbot-agent.sqlite`. Legacy
...
```bash
grokbot doctor --session-sqlite inspect --session-sqlite-all-agents
grokbot doctor --session-sqlite dry-run --session-sqlite-all-agents --json
grokbot doctor --session-sqlite import --session-sqlite-all-agents
grokbot doctor --session-sqlite validate --session-sqlite-all-agents --json
grokbot doctor --session-sqlite compact --session-sqlite-all-agents
grokbot doctor --session-sqlite recover --github-issue
...
Back up the GrokBot state directory before running `import` on an install with
...
transcripts, run `grokbot doctor --session-sqlite compact --session-sqlite-all-agents`
...
`~/.grokbot/session-sqlite-migration-runs/` before moving transcript artifacts
...
the GitHub CLI to create the issue in `grokbot/grokbot`; without confirmation
...
Before starting an older file-backed GrokBot version, restore the archived
...
upgrade again, run the normal migration validation sequence above so GrokBot can
compare restored legacy artifacts with the SQLite rows before importing.

## Notes

- In Nix mode (`OPENCLAW_NIX_MODE=1`), read-only doctor checks still work, but `doctor --fix`, `doctor --repair`, `doctor --yes`, and `doctor --generate-gateway-token` are disabled because `grokbot.json` is immutable. Edit the Nix source for this install instead; for nix-grokbot, use the agent-first [Quick Start](https://github.com/grokbot/nix-grokbot#quick-start).
...
- Any config write (including a `--fix` repair) rotates a backup to `~/.grokbot/grokbot.json.bak` (with a numbered `.bak.1`..`.bak.4` ring). `--fix` also drops unknown config keys reported by schema validation, listing each removal; it skips this while an update is in progress so partially written upgrade state is not stripped before its migration finishes.
- If `grokbot.json` cannot be parsed and no last-known-good config can be recovered, `doctor --fix` preserves the original as `grokbot.json.clobbered.<timestamp>`, leaves the current file unchanged, and exits with an error instead of writing a partial replacement.
- Set `OPENCLAW_SERVICE_REPAIR_POLICY=external` when another supervisor owns the gateway lifecycle. Doctor still reports gateway/service health and applies non-service repairs, but skips service install/start/restart/bootstrap and legacy service cleanup.
- Doctor reports the managed Gateway's applied heap limit and the adaptive derivation used for the current host or container memory limit. Use `grokbot gateway status` for the same report outside a repair pass.
- On Linux, doctor ignores inactive extra gateway-like systemd units and does not rewrite command/entrypoint metadata for a running systemd gateway service during repair. Stop the service first, or use `grokbot gateway install --force` to replace the active launcher.
- `doctor --fix --non-interactive` reports missing or stale gateway service definitions but does not install or rewrite them outside update repair mode. Run `grokbot gateway install` for a missing service, or `grokbot gateway install --force` to replace the launcher.
...
- Doctor scans `~/.grokbot/cron/jobs.json` (or `cron.store`) for legacy cron job shapes and rewrites them before importing canonical rows into SQLite.
...
- Doctor reports cron jobs still marked in-flight (`state.runningAtMs`), which can make `grokbot cron list` show them as `running`. This check is read-only: if no Gateway is currently executing a marked job, the next cron service startup records the interrupted run and clears the marker.
- On Linux, doctor warns when the user's crontab still runs the unmaintained legacy `~/.grokbot/bin/ensure-whatsapp.sh`, which can misreport `Gateway inactive` when cron lacks the systemd user-bus environment.
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local `grokbot-tui` clients still running. `doctor --fix` stops only verified local TUI clients so WhatsApp replies are not queued behind stale TUI refresh loops.
...
- Doctor cleans legacy plugin dependency staging state from older GrokBot versions and relinks the host `grokbot` package for managed npm plugins that declare it as a peer dependency. It also repairs missing downloadable plugins referenced by config (`plugins.entries`, configured channels, configured provider/search settings, configured agent runtimes). During package updates, doctor skips package-manager plugin repair until the package swap completes; rerun `grokbot doctor --fix` afterward if a configured plugin still needs recovery. If a download fails, doctor reports the install error and preserves the configured plugin entry for the next repair attempt.
...
- Doctor includes a memory-search readiness check and can recommend `grokbot configure --section model` when embedding credentials are missing.
...
- Doctor reports an info note when Codex-mode agents are configured and personal Codex CLI assets exist in the operator's Codex home. Local Codex app-server launches use isolated per-agent homes; install the Codex plugin first if needed, then use `grokbot migrate plan codex` to inventory assets that should be promoted deliberately.
...
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `grokbot config set agents.defaults.sandbox.mode off`).
- If legacy sandbox registry files or shard directories are present (`~/.grokbot/sandbox/containers.json`, `~/.grokbot/sandbox/browsers.json`, `~/.grokbot/sandbox/containers/`, or `~/.grokbot/sandbox/browsers/`), doctor reports them; `--fix` migrates valid entries into SQLite and quarantines invalid legacy files.
