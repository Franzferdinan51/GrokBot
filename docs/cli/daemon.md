---
summary: "CLI reference for `grokbot daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `grokbot daemon ...` in scripts
...
Legacy alias for Gateway service management. `grokbot daemon ...` maps to the same service-control commands as `grokbot gateway ...`. Prefer [`grokbot gateway`](/cli/gateway) for current docs and examples.

## Usage

```bash
grokbot daemon status
grokbot daemon install
grokbot daemon start
grokbot daemon stop
grokbot daemon restart
grokbot daemon uninstall
```
