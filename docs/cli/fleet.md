---
summary: "CLI reference for provisioning and managing isolated per-tenant GrokBot cells"
read_when:
...
`grokbot fleet` manages complete GrokBot instances called **cells**. Each cell has its own Gateway, state, credentials, channel accounts, container, and loopback-only host port. Use one cell for each tenant trust boundary; do not use one shared Gateway as a hostile multi-tenant boundary.
...
Fleet supports Docker and Podman. The default image is `ghcr.io/grokbot/grokbot:latest`.
...
The ID becomes part of the container name: `grokbot-cell-<tenant>`.
...
| ------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `--image <ref>`           | `ghcr.io/grokbot/grokbot:latest`    | Container image for the cell.                                                                  |
...
Create and upgrade accept digest-pinned image references such as `--image ghcr.io/grokbot/grokbot@sha256:<digest>`. Fleet passes the image reference through verbatim to Docker or Podman, which lets an operator keep a cell on immutable image bytes instead of a moving tag.
...
```bash
grokbot fleet status acme
grokbot fleet status acme --json
```
...
```bash
grokbot fleet logs acme
grokbot fleet logs acme --follow
grokbot fleet logs acme --tail 200
grokbot fleet logs acme --since 10m
```
...
```bash
grokbot fleet start acme
grokbot fleet stop acme
grokbot fleet restart acme
```
...
```bash
grokbot fleet upgrade acme --image ghcr.io/grokbot/grokbot:<version>
```
...
```bash
grokbot fleet stop acme
grokbot fleet backup acme --out ./acme.tgz
```

Restore that archive into the registered cell:

```bash
grokbot fleet restore acme --from ./acme.tgz
```
...
```bash
grokbot fleet doctor
grokbot fleet doctor acme --json
```
...
Cell state and auth-profile encryption keys use separate per-tenant host paths under the active GrokBot state directory:
