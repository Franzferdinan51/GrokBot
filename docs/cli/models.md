---
summary: "CLI reference for `grokbot models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
grokbot models list
grokbot models set <model-or-alias>
grokbot models set-image <model-or-alias>
grokbot models scan
```
...
`grokbot models status` shows the resolved default/fallbacks plus an auth overview. For plugin-owned agent runtimes such as Codex, it also checks whether the owning plugin is enabled and passed startup payload verification. A route with valid credentials but an unavailable runtime reports `status: unavailable` instead of `usable`; JSON output includes separate `authStatus`, `runtimeStatus`, and bounded runtime diagnostics. When provider usage snapshots are available, the OAuth/API-key status section includes provider usage windows and quota snapshots. Current usage-window providers: Anthropic, GitHub Copilot, Gemini CLI, OpenAI, MiniMax, Xiaomi, and z.ai. Usage auth comes from provider-specific hooks when available; otherwise GrokBot falls back to matching OAuth/API-key credentials from auth profiles, env, or config.
...
- `no_model`: provider auth exists, but GrokBot could not resolve a probeable model candidate for that provider.

For OpenAI ChatGPT/Codex OAuth troubleshooting, `grokbot models status`, `grokbot models auth list --provider openai`, and `grokbot config get agents.defaults.model --json` are the quickest way to confirm whether an agent has a usable `openai` OAuth profile for `openai/*` through the native Codex runtime. See [OpenAI provider setup](/providers/openai#check-and-recover-codex-oauth-routing).

### List

`grokbot models list` is read-only: it reads config, auth profiles, existing catalog state, and provider-owned catalog rows, but never rewrites `models.json`.
...
- If you omit the provider, GrokBot resolves the input as an alias first, then as a unique configured-provider match for that exact model id, and only then falls back to the configured default provider with a deprecation warning. If that provider no longer exposes the configured default model, GrokBot falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
By default GrokBot tries to probe tool and image support with live model calls. If no OpenRouter key is configured, the command falls back to metadata-only output and explains that `:free` models still require `OPENROUTER_API_KEY` for probes and inference.
...
```bash
grokbot models aliases list [--json] [--plain]
grokbot models aliases add <alias> <model-or-alias>
grokbot models aliases remove <alias>
```
...
```bash
grokbot models fallbacks list [--json] [--plain]
grokbot models fallbacks add <model-or-alias>
grokbot models fallbacks remove <model-or-alias>
grokbot models fallbacks clear
```

Manages `agents.defaults.model.fallbacks`. `grokbot models image-fallbacks list|add|remove|clear` manages the parallel `agents.defaults.imageModel.fallbacks` list with the same subcommand shape.

## Auth profiles

```bash
grokbot models auth add
grokbot models auth list [--provider <id>] [--json]
grokbot models auth login --provider <id>
grokbot models auth login --provider openai --profile-id openai:work
grokbot models auth login-github-copilot
grokbot models auth paste-api-key --provider <id>
grokbot models auth setup-token --provider <id>
grokbot models auth paste-token --provider <id>
grokbot models auth order get --provider <id>
grokbot models auth order set --provider <id> <profileIds...>
grokbot models auth order clear --provider <id>
```
...
`models auth login` runs a provider plugin's auth flow (OAuth/API key). Use `grokbot plugins list` to see which providers are installed. `login` accepts `--profile-id <id>` for providers that support named profiles during login (use this to keep multiple logins for the same provider separate), `--method <id>` to pick a specific auth method, `--device-code` as a shortcut for `--method device-code`, `--set-default` to apply the provider's recommended default model, and `--force` to remove existing profiles for that provider first (use when a cached OAuth profile is stuck or you want to switch accounts).
...
Use `grokbot models auth --agent <id> <subcommand>` to write auth results to a specific configured agent store. The parent `--agent` flag is honored by `add`, `list`, `login`, `paste-api-key`, `setup-token`, `paste-token`, `login-github-copilot`, and `order get`/`set`/`clear`.

For OpenAI models, `--provider openai` defaults to ChatGPT/Codex account login. Use `--method api-key` only when you want to add an OpenAI API-key profile, usually as a backup for Codex subscription limits. Run `grokbot doctor --fix` to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
grokbot models auth login --provider openai --set-default
grokbot models auth login --provider openai --method api-key
grokbot models auth paste-api-key --provider openai
grokbot models auth list --provider openai
```

Notes:

- `paste-api-key` accepts API keys generated elsewhere, prompts for the key value, and writes it to the default profile id `<provider>:manual` unless you pass `--profile-id`. In automation, pipe the key on stdin, for example `printf "%s\n" "$OPENAI_API_KEY" | grokbot models auth paste-api-key --provider openai`.
...
- Anthropic: `setup-token`/`paste-token` are supported GrokBot auth paths for `anthropic`, but GrokBot prefers reusing the Claude CLI (`claude -p`) on the host when it is available.
