---
summary: "CLI reference for `grokbot tasks` (background task ledger and Task Flow state)"
read_when:
  - You want to inspect, audit, or cancel background task records
  - You are documenting Task Flow commands under `grokbot tasks flow`
title: "`grokbot tasks`"
---

Inspect durable background tasks and Task Flow state. With no subcommand,
`grokbot tasks` is equivalent to `grokbot tasks list`.
...
```bash
grokbot tasks list [--runtime <name>] [--status <name>] [--json]
```
...
```bash
grokbot tasks flow list [--status <name>] [--json]
