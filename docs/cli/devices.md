---
summary: "CLI reference for `grokbot devices` (device pairing + token rotation/revocation)"
read_when:
...
### `grokbot devices approve [requestId] [--latest]`
...
```bash
grokbot devices approve
grokbot devices approve <requestId>
grokbot devices approve --latest
```

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), GrokBot supersedes the previous pending entry with a new `requestId`. Run `grokbot devices list` right before approval to get the current id.
</Note>

Approval behavior:

- If the device is already paired and requests broader scopes or role, GrokBot keeps the existing approval and creates a new pending upgrade request. Compare `Requested` vs `Approved` in `grokbot devices list`, or preview with `--latest`, before approving.
...
### `grokbot devices remove <deviceId>`

Remove one paired device entry.

```bash
grokbot devices remove <deviceId>
grokbot devices remove <deviceId> --json
```
...
### `grokbot devices rename --device <id> --name <label>`
...
```bash
grokbot devices rename --device <deviceId> --name "Kitchen Mac"
grokbot devices rename --device <deviceId> --name "Kitchen Mac" --json
```
...
### `grokbot devices clear --yes [--pending]`

Clear paired devices in bulk. Gated by `--yes`.

```bash
grokbot devices clear --yes
grokbot devices clear --yes --pending
grokbot devices clear --yes --pending --json
```

`--pending` also rejects all pending pairing requests.

### `grokbot devices rotate --device <id> --role <role> [--scope <scope...>]`

Rotate a device token for a role, optionally updating its scopes.

```bash
grokbot devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `grokbot devices revoke --device <id> --role <role>`

Revoke a device token for a role.

```bash
grokbot devices revoke --device <deviceId> --role node
```
...
3. Rotate the operator token for the affected device:
   ...
   ```bash
   grokbot devices rotate --device <deviceId> --role operator
   ```
...
The preview prints the exact `grokbot devices approve <requestId>` command; verify the details, then rerun that command with the request ID to approve it. For a remote gateway or explicit credentials, pass the same options while previewing and approving:

```bash
grokbot devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `grokbot devices list` first to confirm a pending request exists.
