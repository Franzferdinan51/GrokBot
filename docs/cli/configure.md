---
summary: "CLI reference for `grokbot configure` (interactive configuration prompts)"
read_when:
...
Use `grokbot onboard` or `grokbot setup` for the full guided first-run journey, `grokbot setup --baseline` for the baseline config/workspace only, and `grokbot channels add` when you only need channel account setup.

<Tip>
`grokbot config` with no subcommand opens the same wizard. Use `grokbot config get|set|unset` for non-interactive edits.
</Tip>
...
grokbot configure --section model --section channels
grokbot configure --section gateway --section daemon
```
...
<Note>
`grokbot configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `grokbot config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `grokbot models auth login --provider <id> --set-default` or `grokbot models set <model>` to intentionally change the default model.
</Note>
...
`grokbot configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:
