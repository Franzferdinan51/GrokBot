---
summary: "CLI reference for `grokbot promos` (list and claim promotional model offers)"
read_when:
...
```bash
grokbot promos list
grokbot promos claim <slug>
grokbot promos claim <slug> --api-key <key> --set-default
```
...
2. Validates the promotion's provider, auth choice, and declared plugin packages
   against your installed GrokBot version. Unknown ids or package mismatches are
...
3. Reuses your existing provider credentials when you have them. Otherwise it
   ...
   prompts, matching the `grokbot onboard` non-interactive flags; to keep the
...
`grokbot models set <model>`.

## Passive discovery in `models list`

`grokbot models list` also surfaces promotions without you asking ClawHub
directly:
...
- The first time a new offer is seen, a one-time notice points at
  `grokbot promos list`. Offers you have already listed or claimed are never
  announced again.
