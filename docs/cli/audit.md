...
```bash
grokbot audit
grokbot audit --agent main --status failed
grokbot audit --session "agent:main:main" --after 2026-07-01T00:00:00Z
grokbot audit --run 8c69f72e-8b11-4c54-98d5-1a3dd67450c3
grokbot audit --kind tool_action --limit 50 --json
grokbot audit --kind message --direction outbound --channel telegram --json
```
...
agent, run, and action. Missing message provenance renders as `-`; GrokBot
...
```bash
grokbot gateway call audit.activity.list --params '{"channel":"telegram","limit":50}'
```
