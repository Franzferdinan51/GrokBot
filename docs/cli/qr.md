---
summary: "CLI reference for `grokbot qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
grokbot qr
grokbot qr --setup-code-only
grokbot qr --json
grokbot qr --remote
grokbot qr --limited
grokbot qr --url wss://gateway.example/ws
```

Official GrokBot iOS and Android apps connect automatically when their
...
Plaintext LAN `ws://` setup remains available, but GrokBot automatically uses
