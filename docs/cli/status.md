---
summary: "CLI reference for `grokbot status` (diagnostics, probes, usage snapshots)"
read_when:
  - You want a quick diagnosis of channel health + recent session recipients
  - You want a pasteable "all" status for debugging
title: "grokbot status"
---
...
Plain `grokbot status` stays on the fast read-only path and marks memory as
...
`grokbot status --all`, `grokbot status --deep`, `grokbot security audit`,
and `grokbot memory status --deep`.
...
- Session status output separates `Execution:` from `Runtime:`. `Execution`
  ...
  whether the session is using `GrokBot Default`, `OpenAI Codex`, a CLI
...
- MiniMax's raw `usage_percent` / `usagePercent` fields are remaining quota,
  so GrokBot inverts them before display; count-based fields win when
...
- Update info surfaces in the Overview; if an update is available, status
  prints a hint to run `grokbot update` (see [Updating](/install/updating)).
