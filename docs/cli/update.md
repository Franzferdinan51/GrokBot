---
summary: "CLI reference for `grokbot update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `grokbot update` output or options
...
Update GrokBot and switch between stable/extended-stable/beta/dev channels.
...
grokbot update repair
grokbot update wizard
grokbot update --channel extended-stable
grokbot update --channel beta
grokbot update --channel dev
grokbot update --tag beta
grokbot update --tag main
grokbot update --dry-run
grokbot update --no-restart
grokbot update --yes
grokbot update --acknowledge-clawhub-risk
grokbot update --json
grokbot --update
```

`grokbot --update` rewrites to `grokbot update` (useful for shells and
launcher scripts).
...
| `--tag <dist-tag\|version\|spec>`                | Override the package target for this update only. It cannot be combined with an effective `extended-stable` channel, whose verified exact target is mandatory. For other package installs, `main` maps to `github:grokbot/grokbot#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install. |
...
| `--acknowledge-clawhub-risk`                     | Allow post-update plugin sync to continue past community ClawHub trust warnings without an interactive prompt. Without it, risky community releases are skipped and left unchanged when GrokBot cannot prompt. Official ClawHub packages and bundled plugin sources bypass this prompt.                                                     |

There is no `--verbose` flag. Use `--dry-run` to preview planned actions,
`--json` for machine-readable results, and `grokbot update status --json`
...
<Note>
In Nix mode (`OPENCLAW_NIX_MODE=1`), mutating `grokbot update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-grokbot, use the agent-first [Quick Start](https://github.com/grokbot/nix-grokbot#quick-start). `grokbot update status` and `grokbot update --dry-run` remain read-only.
</Note>
...
grokbot update status --timeout 10
```
...
`grokbot update` installed the new core package but post-core plugin sync,
managed npm plugin metadata, registry refresh, or doctor repair did not
converge.

```bash
grokbot update repair
grokbot update repair --channel beta
grokbot update repair --acknowledge-clawhub-risk
grokbot update repair --json
```
...
| `--acknowledge-clawhub-risk`                     | Same behavior as on `grokbot update`.                                                                                                                                                                                                                              |
| `--no-restart`                                   | Accepted for parity; repair never restarts the Gateway.                                                                                                                                                                                                             |

`update repair` runs `grokbot doctor --fix`, reloads the repaired config and
...
- `dev` -> ensures a git checkout (default `~/grokbot`, or
  `$OPENCLAW_HOME/grokbot` when `OPENCLAW_HOME` is set; override with
...
detached helper and exits, and that helper runs `grokbot update --yes --json`
...
healthy. If the plist is installed but launchd is not supervising it, GrokBot
...
`Gateway: restart failed: ...` with a manual `grokbot gateway restart` hint.
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `grokbot update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"`: GrokBot could not find a supervising
...
`grokbot status` and `grokbot status --all` show an `Update restart` row
...
<Steps>
  ...
  <Step title="Run doctor">
    `grokbot doctor` runs as the final safe-update check.
  </Step>
...
beta release, GrokBot falls back to the recorded default/latest spec and
reports a warning. For npm plugins, GrokBot also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `grokbot update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (for example an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `grokbot update repair` and `grokbot plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `grokbot update repair`. When a failed update leaves a managed plugin unusable, GrokBot disables its runtime entry and resets active slots without changing the operator-authored `plugins.allow` or `plugins.deny` policy.

After the per-plugin sync step, `grokbot update` runs a mandatory **post-core convergence** pass before the gateway restarts: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly declared `main` exists). Failures from this pass, and an invalid config snapshot, return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `grokbot update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `grokbot update repair` and `grokbot plugins inspect <id> --runtime --json`. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here (mirroring the `skipDisabledPlugins` policy used by the missing-payload check), so a stale disabled plugin record cannot block an otherwise valid update.
...
version. For default/`latest` intent, GrokBot does not query plugin
...
For package-manager installs, `grokbot update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: GrokBot installs the new package into a temporary npm prefix,
...
the updater approves only the candidate GrokBot lifecycle; transitive
dependency scripts remain blocked. GrokBot then swaps the clean package tree
...
plugin records aligned with the installed GrokBot build, while leaving full
plugin-command completion rebuilds to explicit
`grokbot completion --write-state` runs.

## Related

- `grokbot doctor` (offers to run update first on git checkouts)
