---
summary: "CLI reference for `grokbot hooks` (agent hooks)"
read_when:
...
Manage agent hooks (event-driven automations for commands like `/new`, `/reset`, and gateway startup). Bare `grokbot hooks` is equivalent to `grokbot hooks list`.
...
```bash
grokbot hooks list [--eligible] [--json] [-v|--verbose]
```
...
```bash
grokbot plugins install <package>        # npm by default
grokbot plugins install npm:<package>    # npm only
grokbot plugins install <package> --pin  # pin resolved version
grokbot plugins install <path>           # local directory or archive
grokbot plugins install -l <path>        # link a local directory instead of copying

grokbot plugins update <id>
grokbot plugins update --all
grokbot plugins update --dry-run
```

Hook packs install through the unified plugins installer/updater; `grokbot hooks install` / `grokbot hooks update` still work as deprecated aliases that print a warning and forward to the `plugins` commands.
...
- Bare specs and `@latest` stay on the stable track; if npm resolves to a prerelease, GrokBot stops and asks you to opt in explicitly (`@beta`, `@rc`, or an exact prerelease version).
...
- Install copies the pack into `~/.grokbot/hooks/<id>`, enables its hooks under `hooks.internal.entries.*`, and records the install under `hooks.internal.installs`.
- If a stored integrity hash no longer matches the fetched artifact, GrokBot warns and prompts before continuing; pass global `--yes` to bypass the prompt (for example in CI).
...
| command-logger        | `command`                                         | Logs command events to `~/.grokbot/logs/commands.log`                                             |
...
Enable any bundled hook with `grokbot hooks enable <hook-name>`. Full details, config keys, and defaults: [Bundled hooks](/automation/hooks#bundled-hooks).

### command-logger log file

```bash
tail -n 20 ~/.grokbot/logs/commands.log        # recent commands
cat ~/.grokbot/logs/commands.log | jq .          # pretty-print
grep '"action":"new"' ~/.grokbot/logs/commands.log | jq .   # filter by action
```
