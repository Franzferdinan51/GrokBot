---
summary: "CLI reference for `grokbot agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
grokbot agent --to +15555550123 --message "status update" --deliver
grokbot agent --agent ops --message "Summarize logs"
grokbot agent --agent ops --message-file ./task.md
grokbot agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
grokbot agent --session-key agent:ops:incident-42 --message "Summarize status"
grokbot agent --agent ops --session-key incident-42 --message "Summarize status"
grokbot agent --session-id 1234 --message "Summarize inbox" --thinking medium
grokbot agent --to +15555550123 --message "Trace logs" --verbose on --json
grokbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
grokbot agent --agent ops --message "Run locally" --local
```
...
- Slash commands (for example `/compact`) cannot run through `--message`. The CLI rejects them and points you at the first-class command instead (`grokbot sessions compact <key>` for compaction).
