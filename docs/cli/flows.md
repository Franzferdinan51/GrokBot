---
summary: "Redirect: flow commands live under `grokbot tasks flow`"
read_when:
  - You encounter `grokbot flows` in older docs or release notes
...
There is no top-level `grokbot flows` command. Durable TaskFlow inspection lives under `grokbot tasks flow`.

## Subcommands

```bash
grokbot tasks flow list   [--json] [--status <name>]
...
grokbot tasks flow list --status running
grokbot tasks flow list --json
grokbot tasks flow show flow_abc123
grokbot tasks flow show flow_abc123 --json
grokbot tasks flow cancel flow_abc123
```
