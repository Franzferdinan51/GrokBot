---
summary: "CLI reference for `grokbot uninstall` (remove gateway service + local data)"
read_when:
...
grokbot uninstall
grokbot uninstall --service --yes --non-interactive
grokbot uninstall --state --workspace --yes --non-interactive
grokbot uninstall --all --yes
grokbot uninstall --dry-run
```

## Notes

- Run `grokbot backup create` first for a restorable snapshot before removing
  state or workspaces.
