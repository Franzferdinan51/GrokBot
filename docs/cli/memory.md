---
summary: "CLI reference for `grokbot memory` (status/index/search/promote/promote-explain/rem-harness/rem-backfill)"
read_when:
...
```bash
grokbot memory status [--agent <id>] [--deep] [--index] [--fix] [--json] [--verbose]
```
...
```bash
grokbot memory index [--agent <id>] [--force] [--verbose]
```
...
```bash
grokbot memory search [query] [--query <text>] [--agent <id>] [--max-results <n>] [--min-score <n>] [--json]
```
...
```bash
grokbot memory promote [--agent <id>] [--limit <n>] [--min-score <n>] \
...
```bash
grokbot memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
grokbot memory rem-harness [--agent <id>] [--path <file-or-dir>] [--grounded] [--include-promoted] [--json]
```
...
```bash
grokbot memory rem-backfill --path <file-or-dir> [--agent <id>] [--stage-short-term] [--json]
grokbot memory rem-backfill --rollback [--rollback-short-term] [--json]
```
