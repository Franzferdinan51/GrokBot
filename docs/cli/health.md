---
summary: "CLI reference for `grokbot health` (gateway health snapshot via RPC)"
read_when:
...
grokbot health
grokbot health --json
grokbot health --timeout 2500
grokbot health --verbose
grokbot health --debug
```
...
- When outbound deliveries or inbound channel events are dead-lettered, text output reports their counts and oldest failure age. Inbound counts are grouped by channel account; inspect or recover individual events with [`grokbot channels dead-letters`](/cli/channels#inbound-dead-letters).

## Related

- [CLI reference](/cli)
- [`grokbot status`](/cli/status) — local diagnosis and channel probes without a full health snapshot
