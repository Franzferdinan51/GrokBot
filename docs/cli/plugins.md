---
summary: "CLI reference for `grokbot plugins` (init, build, validate, list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
grokbot plugins list [--enabled] [--verbose] [--json]
grokbot plugins search <query> [--limit <n>] [--json]
grokbot plugins install <path-or-spec> [--link] [--force] [--pin] [--marketplace <source>]
grokbot plugins inspect <id> [--runtime] [--json]
grokbot plugins inspect --all [--runtime] [--json]
grokbot plugins info <id>                    # alias for inspect
grokbot plugins enable <id>
grokbot plugins disable <id>
grokbot plugins uninstall <id> [--dry-run] [--keep-files] [--force]
grokbot plugins update <id-or-npm-spec> | --all [--dry-run]
grokbot plugins registry [--refresh] [--json]
grokbot plugins doctor
grokbot plugins init <id> [--name <name>] [--type tool|provider] [--directory <path>]
grokbot plugins build [--entry <path>] [--check]
grokbot plugins validate [--entry <path>]
grokbot plugins marketplace entries [--offline] [--feed-profile <name>] [--json]
grokbot plugins marketplace list <source> [--json]
grokbot plugins marketplace refresh [--feed-profile <name>] [--expected-sha256 <sha256>] [--json]
```
...
<Note>
In Nix mode (`OPENCLAW_NIX_MODE=1`), `grokbot.json` is immutable. `install`, `update`, `uninstall`, `enable`, and `disable` all refuse to run. Edit the Nix source for this install instead (`programs.grokbot.config` or `instances.<name>.config` for nix-grokbot), then rebuild. See the agent-first [Quick Start](https://github.com/grokbot/nix-grokbot#quick-start).
</Note>

<Note>
Bundled plugins ship with GrokBot. Some are enabled by default (for example bundled model providers, bundled speech providers, and the bundled browser plugin); others require `plugins enable`.

Native GrokBot plugins ship `grokbot.plugin.json` with an inline JSON Schema (`configSchema`, even if empty). Compatible bundles use their own bundle manifests instead.

`plugins list` shows `Format: grokbot` or `Format: bundle`. Verbose list/info output also shows the bundle subtype (`codex`, `claude`, or `cursor`) plus detected bundle capabilities.
</Note>

## Author

```bash
grokbot plugins init stock-quotes --name "Stock Quotes"
cd stock-quotes
...
argument is the plugin id; `--name` sets the display name. GrokBot uses the
...
`plugin:validate` that build then call `grokbot plugins build`/`validate`.

`plugins build` imports the built entry, reads its static tool metadata, writes
`grokbot.plugin.json`, and keeps `package.json`'s `grokbot.extensions` aligned.
...
```bash
grokbot plugins init acme-models --name "Acme Models" --type provider
cd acme-models
npm install
npm run build
npm test
...
`grokbot plugins build`/`validate`; those commands are for the tool
...
```bash
grokbot plugins search "calendar"                      # search ClawHub plugins
grokbot plugins install @grokbot/<package>            # trusted official catalog
grokbot plugins install <package>                       # arbitrary npm package
grokbot plugins install clawhub:<package>                # ClawHub only
grokbot plugins install npm:<package>                    # npm only
grokbot plugins install npm-pack:<path.tgz>               # local npm-pack tarball
grokbot plugins install git:github.com/<owner>/<repo>     # git repo
grokbot plugins install git:github.com/<owner>/<repo>@<ref>
grokbot plugins install <path>                            # local path or archive
grokbot plugins install -l <path>                         # link instead of copy
grokbot plugins install <plugin>@<marketplace>             # marketplace shorthand
grokbot plugins install <plugin> --marketplace <name>      # marketplace (explicit)
grokbot plugins install <package> --force                  # confirm source / overwrite existing
grokbot plugins install <package> --pin                    # pin resolved npm version
grokbot plugins install clawhub:<package> --acknowledge-clawhub-risk
grokbot plugins install <package> --dangerously-force-unsafe-install
```
...
<Warning>
Bare package names install from npm by default during the launch cutover, unless they match a bundled or official plugin id, in which case GrokBot uses that local/official copy instead of hitting the npm registry. Use `npm:<package>` when you deliberately want an external npm package instead. Use `clawhub:<package>` for ClawHub. Treat plugin installs like running code; prefer pinned versions.
</Warning>

<Warning>
ClawHub packages and GrokBot's bundled/official catalog are trusted install
...
`bundle-plugin` packages (not skills; use `grokbot skills search` for those).
...
summary, and an install hint such as `grokbot plugins install clawhub:<package>`.

<Note>
ClawHub is the primary distribution and discovery surface for most plugins. Npm
remains a supported fallback and direct-install path. GrokBot-owned
`@grokbot/*` plugin packages are published on npm again; see the current list
on [npmjs.com/org/grokbot](https://www.npmjs.com/org/grokbot) or the
...
<AccordionGroup>
  <Accordion title="Config includes and invalid-config repair">
    If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `grokbot.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

    If config is invalid during install, `plugins install` normally fails closed and tells you to run `grokbot doctor --fix` first. During Gateway startup and hot reload, invalid plugin config fails closed like any other invalid config; `grokbot doctor --fix` can quarantine the invalid plugin entry. The only documented install-time exception is a narrow bundled-plugin recovery path for plugins that explicitly opt into `grokbot.install.allowInvalidConfigRecovery`.

  </Accordion>
  <Accordion title="--force confirmation and reinstall vs update">
    `--force` confirms a non-ClawHub source without prompting. It does not bypass `security.installPolicy` or remaining install safety checks. When the plugin or hook pack is already installed, it also reuses the existing target and overwrites it in place. Use it after reviewing an arbitrary npm, local, archive, git, or marketplace source, or when intentionally reinstalling the same id. For routine upgrades of an already tracked npm plugin, prefer `grokbot plugins update <id-or-npm-spec>`.

    If you run `plugins install` for a plugin id that is already installed, GrokBot stops and points you at `plugins update <id-or-npm-spec>` for a normal upgrade, or at `plugins install <package> --force` when you genuinely want to overwrite the current install from a different source. Arbitrary sources still show the interactive provenance warning; noninteractive installs must pass `--force` after review. Trusted ClawHub and GrokBot-catalog sources do not need it. With `--link`, `--force` confirms the source but does not change the linked-path install mode.
  ...
  <Accordion title="--dangerously-force-unsafe-install">
    `--dangerously-force-unsafe-install` is deprecated and is now a no-op. GrokBot no longer runs built-in install-time dangerous-code blocking for plugin installs.
  ...
  <Accordion title="--acknowledge-clawhub-risk">
    Community ClawHub installs check the selected release's trust record before downloading. If ClawHub disables download for the release, reports malicious scan findings, or puts the release in a blocking moderation state (quarantined, revoked), GrokBot refuses it outright regardless of this flag. For non-blocking risky scan statuses or moderation states, GrokBot shows the trust details and asks for confirmation before continuing.

    Use `--acknowledge-clawhub-risk` only after reviewing the ClawHub warning and deciding to continue without an interactive prompt. Pending or stale (not-yet-clean) scan results warn but do not require acknowledgement. Official ClawHub packages and bundled GrokBot plugin sources bypass this release-trust check entirely.

  </Accordion>
  <Accordion title="Hook packs and npm specs">
    `plugins install` is also the install surface for hook packs that expose `grokbot.hooks` in `package.json`. Use `grokbot hooks` for filtered hook visibility and per-hook enablement, not package installation.

    Npm specs are **registry-only** (package name plus optional **exact version** or **dist-tag**). Git/URL/file specs and semver ranges are rejected. Dependency installs run in one managed npm project per plugin with `--ignore-scripts` for safety, even when your shell has global npm install settings. Managed plugin npm projects inherit GrokBot's package-level npm `overrides`, so host security pins apply to hoisted plugin dependencies too.
    ...
    Raw `@grokbot/*` specs that match bundled plugins resolve to the image-owned bundled copy before npm fallback. For example, `grokbot plugins install @grokbot/discord@2026.5.20 --pin` uses the bundled Discord plugin from the current GrokBot build instead of creating a managed npm override. To force the external npm package, use `grokbot plugins install npm:@grokbot/discord@2026.5.20 --pin`.

    Bare specs and `@latest` stay on the stable track. GrokBot date-stamped correction versions such as `2026.5.3-1` count as stable for this check. If npm resolves either form to a prerelease, GrokBot stops and asks you to opt in explicitly with a prerelease tag (`@beta`/`@rc`) or an exact prerelease version (`@1.2.3-beta.4`).

    For npm installs without an exact version (`npm:<package>` or `npm:<package>@latest`), GrokBot checks the resolved package metadata before install. If the latest stable package requires a newer GrokBot plugin API or minimum host version, GrokBot inspects older stable versions and installs the newest compatible release instead. Exact versions and explicit dist-tags stay strict: an incompatible selection fails and asks you to upgrade GrokBot or choose a compatible version.

    If a bare install spec matches an official plugin id (for example `diffs`), GrokBot installs the catalog entry directly. To install an npm package with the same name, use an explicit scoped spec (for example `@scope/diffs`).
  ...
  <Accordion title="Git repositories">
    ...
    Git installs clone into a temporary directory, check out the requested ref when present, then use the normal plugin directory installer, so manifest validation, operator install policy, package-manager install work, and install records behave like npm installs. Recorded git installs include the source URL/ref plus the resolved commit so `grokbot plugins update` can re-resolve the source later.

    After installing from git, use `grokbot plugins inspect <id> --runtime --json` to verify runtime registrations such as gateway methods and CLI commands. If the plugin registered a CLI root with `api.registerCli`, run that command directly through the GrokBot root CLI, for example `grokbot demo-plugin ping`.

  </Accordion>
  <Accordion title="Archives">
    Supported archives: `.zip`, `.tgz`, `.tar.gz`, `.tar`. Native GrokBot plugin archives must contain a valid `grokbot.plugin.json` at the extracted plugin root; archives that only contain `package.json` are rejected before GrokBot writes install records.
...
```bash
grokbot plugins install clawhub:grokbot-codex-app-server
grokbot plugins install clawhub:grokbot-codex-app-server@1.2.3
```

Bare npm-safe plugin specs install from npm by default during the launch cutover unless they match an official plugin id:

```bash
grokbot plugins install grokbot-codex-app-server
```

Use `npm:` to make npm-only resolution explicit:

```bash
grokbot plugins install npm:grokbot-codex-app-server
grokbot plugins install npm:@grokbot/discord@2026.5.20
grokbot plugins install npm:@scope/plugin-name@1.0.1
```

GrokBot checks the advertised plugin API / minimum gateway compatibility before install. When the selected ClawHub version publishes a ClawPack artifact, GrokBot downloads the versioned npm-pack `.tgz`, verifies the ClawHub digest header and the artifact digest, then installs it through the normal archive path. Older ClawHub versions without ClawPack metadata still install through the legacy package archive verification path. Recorded installs keep their ClawHub source metadata, artifact kind, npm integrity, npm shasum, tarball name, and ClawPack digest facts for later updates.
Unversioned ClawHub installs keep an unversioned recorded spec so `grokbot plugins update` can follow newer ClawHub releases; explicit version or tag selectors such as `clawhub:pkg@1.2.3` and `clawhub:pkg@beta` remain pinned to that selector.
...
```bash
grokbot plugins marketplace list <marketplace-name>
grokbot plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` to pass the marketplace source explicitly:

```bash
grokbot plugins install <plugin-name> --marketplace <marketplace-name>
grokbot plugins install <plugin-name> --marketplace <owner/repo>
grokbot plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
grokbot plugins install <plugin-name> --marketplace ./my-marketplace
```
...
<Tabs>
  ...
  <Tab title="Remote marketplace rules">
    For remote marketplaces loaded from GitHub or git, plugin entries must stay inside the cloned marketplace repo. GrokBot accepts relative path sources from that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path plugin sources from remote manifests.
  </Tab>
</Tabs>

For local paths and archives, GrokBot auto-detects:

- native GrokBot plugins (`grokbot.plugin.json`)
...
`~/.grokbot/extensions` or `<workspace>/.grokbot/extensions`; those
...
```bash
grokbot plugins install -l ./my-plugin
```
...
run `grokbot plugins enable <plugin-id>` or set
...
```bash
grokbot plugins list
grokbot plugins list --enabled
grokbot plugins list --verbose
...
run `grokbot plugins list --enabled --verbose` or
`grokbot plugins inspect <id>` with a listed plugin id to confirm the plugin
ids and copy trusted ids into `plugins.allow` in `grokbot.json`. When the
...
so GrokBot records install provenance.
...
`/app/extensions/synology-chat`. GrokBot discovers that mounted source overlay
...
- `grokbot plugins inspect <id> --runtime --json` shows registered hooks and diagnostics from a module-loaded inspection pass. Runtime inspection never installs dependencies; use `grokbot doctor --fix` to clean legacy dependency state or recover missing downloadable plugins that are referenced by config.
- `grokbot gateway status --deep --require-rpc` confirms the reachable Gateway URL/profile, service/process hints, config path, and RPC health.
...
Plugin install metadata is machine-managed state, not user config. Installs and updates write it to the shared SQLite state database under the active GrokBot state directory. The `installed_plugin_index` row stores durable `installRecords` metadata, including records for broken or missing plugin manifests, plus a manifest-derived cold registry cache used by `grokbot plugins update`, uninstall, diagnostics, and the cold plugin registry.

`plugins.installs` is a retired authored-config surface. Runtime and update commands read only the SQLite installed-plugin index. Run `grokbot doctor --fix` to import legacy config records into the index and remove the retired key before normal runtime use.

## Uninstall

```bash
grokbot plugins uninstall <id>
grokbot plugins uninstall <id> --dry-run
grokbot plugins uninstall <id> --keep-files
grokbot plugins uninstall <id> --force
```

`uninstall` removes plugin records from `plugins.entries`, the persisted plugin index, plugin allow/deny list entries, and linked `plugins.load.paths` entries when applicable. Unless `--keep-files` is set, uninstall also removes the tracked managed install directory, but only when it resolves inside GrokBot's plugin extensions root. If the plugin currently owns the `memory` or `contextEngine` slot, that slot resets to its default (`memory-core` for memory, `legacy` for context engine).
...
grokbot plugins update <id-or-npm-spec> --dry-run
grokbot plugins update @grokbot/voice-call
grokbot plugins update @acme/demo
grokbot plugins update grokbot-codex-app-server --acknowledge-clawhub-risk
grokbot plugins update grokbot-codex-app-server --dangerously-force-unsafe-install
```
...
<AccordionGroup>
  <Accordion title="Resolving plugin id vs npm spec">
    When you pass a plugin id, GrokBot reuses the recorded install spec for that plugin. That means previously stored dist-tags such as `@beta` and exact pinned versions continue to be used on later `update <id>` runs.

    During `update <id> --dry-run`, exact pinned npm installs stay pinned. If GrokBot can also resolve the package's registry default line and that default line is newer than the installed pinned version, the dry run reports the pin and prints the explicit `@latest` package update command to follow the registry default line.

    That targeted-update rule differs from the bulk `grokbot plugins update --all` maintenance path. Bulk updates still respect ordinary tracked install specs, but trusted official GrokBot plugin records can sync to the current official catalog target instead of staying on a stale exact official package. Use targeted `update <id>` when you intentionally want to keep an exact or tagged official spec untouched.

    For npm installs, you can also pass an explicit npm package spec with a dist-tag or exact version. GrokBot resolves that package name back to the tracked plugin record, updates that installed plugin, and records the new npm spec for future id-based updates.
  ...
  <Accordion title="--acknowledge-clawhub-risk on update">
    Community ClawHub-backed plugin updates run the same exact-release trust check as installs before downloading the replacement package. Use `--acknowledge-clawhub-risk` for reviewed automation that should continue when the selected ClawHub release has a risky trust warning. Official ClawHub packages and bundled GrokBot plugin sources bypass this release-trust prompt.
  </Accordion>
</AccordionGroup>
...
```bash
grokbot plugins inspect <id>
grokbot plugins inspect <id> --runtime
grokbot plugins inspect <id> --json
grokbot plugins inspect --all
```

Inspect shows identity, load status, source, manifest capabilities, policy flags, diagnostics, install metadata, bundle capabilities, and any detected MCP or LSP server support without importing plugin runtime by default. JSON output includes the plugin manifest contracts, such as `contracts.agentToolResultMiddleware` and `contracts.trustedToolPolicies`, so operators can audit trusted-surface declarations before enabling or restarting a plugin. Add `--runtime` to load the plugin module and include registered hooks, tools, commands, services, gateway methods, and HTTP routes. Runtime inspection reports missing plugin dependencies directly; installs and repairs stay in `grokbot plugins install`, `grokbot plugins update`, and `grokbot doctor --fix`.

Plugin-owned CLI commands are usually installed as root `grokbot` command groups, but plugins may also register nested commands under a core parent such as `grokbot nodes`. After `inspect --runtime` shows a command under `cliCommands`, run it at the listed path; for example a plugin that registers `demo-git` can be verified with `grokbot demo-git ping`.
...
```bash
grokbot plugins registry
grokbot plugins registry --refresh
grokbot plugins registry --json
```

The local plugin registry is GrokBot's persisted cold read model for installed plugin identity, enablement, source metadata, and contribution ownership. Normal startup, provider owner lookup, channel setup classification, and plugin inventory can read it without importing plugin runtime modules.

Use `plugins registry` to inspect whether the persisted registry is present, current, or stale. Use `--refresh` to rebuild it from the persisted plugin index, config policy, and manifest/package metadata. This is a repair path, not a runtime activation path.

`grokbot doctor --fix` also repairs registry-adjacent managed npm drift. If an orphaned or recovered `@grokbot/*` package under a managed plugin npm project or the legacy flat managed npm root shadows a bundled plugin, doctor removes that stale package and rebuilds the registry so startup validates against the bundled manifest. When an authoritative install record selects one managed generation but older flat or generation directories remain, doctor retires those stale trees for pruning after the gateway restarts. Doctor also relinks the host `grokbot` package into managed npm plugins that declare `peerDependencies.grokbot`, so package-local runtime imports such as `grokbot/plugin-sdk/*` resolve after updates or npm repairs.

## Marketplace

```bash
grokbot plugins marketplace entries
grokbot plugins marketplace entries --offline
grokbot plugins marketplace entries --json
grokbot plugins marketplace entries --feed-profile <name>
grokbot plugins marketplace entries --feed-url <url>
grokbot plugins marketplace list <source>
grokbot plugins marketplace list <source> --json
grokbot plugins marketplace refresh
grokbot plugins marketplace refresh --feed-profile <name>
grokbot plugins marketplace refresh --feed-url <url>
grokbot plugins marketplace refresh --expected-sha256 <sha256> --json
```

`plugins marketplace entries` lists entries from the configured GrokBot marketplace feed. By default it attempts the hosted feed and falls back to the latest accepted snapshot or bundled data. Use `--feed-profile <name>` to read a specific configured profile, `--feed-url <url>` to read an explicit hosted feed URL, and `--offline` to read the latest accepted snapshot without fetching the feed.

`plugins marketplace refresh` refreshes the configured hosted feed snapshot and reports whether GrokBot accepted hosted data, a hosted snapshot, or bundled fallback data. Use `--expected-sha256` when a caller needs the command to fail unless a fresh hosted payload matches a pinned checksum.
...
Marketplace refresh loads a hosted GrokBot marketplace feed and persists the
...
refreshes fail if GrokBot cannot persist the validated snapshot.
