---
summary: "CLI reference for `grokbot message` (send + channel actions)"
read_when:
...
```bash
grokbot message <subcommand> [flags]
```
...
`grokbot message` resolves channel SecretRefs before running the action,
...
```bash
grokbot message send --channel discord \
...
```bash
grokbot message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
grokbot message send --channel slack --target channel:C123 \
...
```bash
grokbot message send --channel telegram --target 123456789 --message "Open app:" \
  --presentation '{"blocks":[{"type":"buttons","buttons":[{"label":"Launch","webApp":{"url":"https://example.com/app"}}]}]}'
```

```bash
grokbot message send --channel telegram --target @mychat \
...
```bash
grokbot message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```bash
grokbot message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```bash
grokbot message poll --channel msteams \
  --target conversation:19:abc@thread.tacv2 \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi
```
...
```bash
grokbot message broadcast --targets <target...> [--channel all] [--message <text>] [--media <url>] [--dry-run]
```
