---
summary: "CLI reference for `grokbot path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
grokbot plugins enable oc-path
```
...
GrokBot state is spread across human-edited markdown, commented JSONC
...
`grokbot path` gives those callers a stable address instead of a one-off
...
Skip `grokbot path` for ordinary whole-file edits, rich config migrations, or
...
```bash
grokbot path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
grokbot path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
grokbot path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
grokbot path resolve 'oc://AGENTS.md/runtime-safety/grokbot-gateway'
```
...
```bash
grokbot path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
grokbot path resolve 'oc://gateway.jsonc/version'

# Wildcard search
grokbot path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
grokbot path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
grokbot path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
grokbot path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
grokbot path emit ./AGENTS.md
```
...
grokbot path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
grokbot path set 'oc://grokbot.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
grokbot path set 'oc://grokbot.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
grokbot path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
grokbot path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
grokbot path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
grokbot path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
grokbot path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
grokbot path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
grokbot path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
grokbot path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
grokbot path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
grokbot path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
grokbot path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ grokbot path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ grokbot path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ grokbot path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ grokbot path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ grokbot path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ grokbot path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ grokbot path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: grokbot.invoke
```

```bash
$ grokbot path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ grokbot path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
grokbot path resolve 'oc://AGENTS.md/tools/gh/risk' --human
grokbot path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
grokbot path find 'oc://AGENTS.md/tools/**/risk'
grokbot path find 'oc://session.jsonl/[event=action]/userId'
grokbot path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
grokbot path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
grokbot path set 'oc://gateway.jsonc/version' '2.0'
grokbot path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ grokbot path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
grokbot path emit ./AGENTS.md
grokbot path emit ./gateway.jsonc --json
```
...
`grokbot path` is TTY-aware: human-readable output on a terminal, JSON when
