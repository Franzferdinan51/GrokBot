...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an GrokBot Gateway.

`grokbot acp` speaks ACP over stdio for IDEs and forwards prompts to the Gateway over WebSocket, keeping ACP sessions mapped to Gateway session keys. It is a Gateway-backed ACP bridge, not a full ACP-native editor runtime: it focuses on session routing, prompt delivery, and streaming updates.

If you want an external MCP client to talk directly to GrokBot channel conversations instead of hosting an ACP harness session, use [`grokbot mcp serve`](/cli/mcp) instead.

## What this is not

`grokbot acp` means GrokBot acts as an ACP server: an IDE or ACP client connects to GrokBot, and GrokBot forwards that work into a Gateway session.

This is different from [ACP Agents](/tools/acp-agents), where GrokBot runs an external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to GrokBot: use `grokbot acp`
- GrokBot should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include GrokBot parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                     |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the GrokBot Gateway or agent instead.                                                                                                                          |
...
# Remote Gateway
grokbot acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
grokbot acp --url wss://gateway-host:18789 --token-file ~/.grokbot/gateway.token

# Attach to an existing session key
grokbot acp --session agent:main:main

# Attach by label (must already exist)
grokbot acp --session-label "support inbox"

# Reset the session key before the first prompt
grokbot acp --session agent:main:main --reset-session
```
...
grokbot acp client --server-args --url wss://gateway-host:18789 --token-file ~/.grokbot/gateway.token

# Override the server command (default: grokbot)
grokbot acp client --server "node" --server-args grokbot.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run GrokBot through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive `grokbot acp` over stdio with an ACP JSON-RPC client. Cover `initialize`, `session/new`, `session/list` with an absolute `cwd`, `session/resume`, `session/close`, duplicate close, and missing resume.
...
Avoid using `grokbot gateway call sessions.list` as the only ACP proof. That CLI path may request a fresh-token operator scope upgrade; ACP bridge correctness is proven by ACP stdio frames plus the Gateway `sessions.list` log.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want it to drive an GrokBot Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `grokbot acp` over stdio.

Example config (persisted):

```bash
grokbot config set gateway.remote.url wss://gateway-host:18789
grokbot config set gateway.remote.token <token>
```
...
```bash
grokbot acp --session agent:main:main
grokbot acp --session agent:design:main
grokbot acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see GrokBot plugin tools or selected built-in tools such as `cron`, enable the gateway-side ACPX MCP bridges instead of trying to pass per-session `mcpServers`. See [ACP Agents](/tools/acp-agents-setup#plugin-tools-mcp-bridge) and [GrokBot tools MCP bridge](/tools/acp-agents-setup#grokbot-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your GrokBot bot over ACP, use `acpx` with its built-in `grokbot` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx grokbot` at `grokbot acp`.
3. Target the GrokBot session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default GrokBot ACP session
acpx grokbot exec "Summarize the active GrokBot session state."

# Persistent named session for follow-up turns
acpx grokbot sessions ensure --name codex-bridge
acpx grokbot -s codex-bridge --cwd /path/to/repo \
  "Ask my GrokBot work agent for recent context relevant to this repo."
```

If you want `acpx grokbot` to target a specific Gateway and session key every time, override the `grokbot` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "grokbot": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 grokbot acp --url ws://127.0.0.1:18789 --token-file ~/.grokbot/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local GrokBot checkout, use the direct CLI entrypoint instead of the dev runner so the ACP stream stays clean:

```bash
env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 node grokbot.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client pull contextual information from an GrokBot agent without scraping a terminal.
...
In Zed, open the Agent panel and select "GrokBot ACP" to start a thread.
...
- `--server <command>`: ACP server command (default: `grokbot`).
- `--server-args <args...>`: extra arguments passed to the ACP server.
- `--server-verbose`: enable verbose logging on the ACP server.
- `--verbose, -v`: verbose client logging.
- `grokbot acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process, which can be used for context-specific shell/profile rules.
