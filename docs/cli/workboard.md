---
summary: "CLI reference for `grokbot workboard` cards, dispatch, and worker runs"
read_when:
...
`grokbot workboard` is the terminal surface for the bundled [Workboard plugin](/plugins/workboard). It lets an operator list cards, create a card, inspect one card, and ask the running Gateway to dispatch ready work into subagent worker runs.

Enable the plugin before using the command:

```bash
grokbot plugins enable workboard
...
```bash
grokbot workboard list [--board <id>] [--status <status>] [--include-archived] [--json]
grokbot workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
grokbot workboard show <id> [--json]
grokbot workboard move <id> --status <status> [--json]
grokbot workboard dispatch [--board <id>] [--max-starts <count>] [--admin] [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
grokbot workboard list
grokbot workboard list --board default --status ready
grokbot workboard list --json
```
...
```bash
grokbot workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
grokbot workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
grokbot workboard show 7f4a2c10
grokbot workboard show 7f4a2c10 --json
```
...
```bash
grokbot workboard move 7f4a2c10 --status review
grokbot workboard move 7f4a2c10 --status done --json
```
...
```bash
grokbot workboard dispatch
grokbot workboard dispatch --json
grokbot workboard dispatch --max-starts 10
grokbot workboard dispatch --admin
grokbot workboard dispatch --url http://127.0.0.1:18789 --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
Local `list`, `create`, `show`, and `move` commands operate on the local GrokBot state directory used by the current profile. Use `--dev` or `--profile <name>` on the top-level `grokbot` command when you need a different state root.
...
```bash
grokbot plugins inspect workboard --runtime --json
```
...
Then retry `grokbot workboard dispatch`. Data-only fallback is useful for local state cleanup, but worker runs need a live Gateway.
...
```bash
grokbot workboard list --status ready
```
