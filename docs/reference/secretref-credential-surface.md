...
- In scope: strictly user-supplied credentials that GrokBot does not mint or rotate.
...
### `grokbot.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- In `grokbot.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `grokbot doctor --fix` to migrate valid markers.
