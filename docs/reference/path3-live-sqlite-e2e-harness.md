...
`test/helpers/grokbot-test-instance.ts`, but the live proof path should inspect
...
SQLite before runtime reads begin. It must not run `grokbot doctor --fix`
...
`grokbot doctor --session-sqlite inspect` and
`grokbot doctor --session-sqlite validate` as diagnostic evidence. Those
...
`grokbot doctor --session-sqlite validate` is informational only in live mode.
