...
This is the full reference for `grokbot onboard`.
...
<Steps>
  <Step title="Reset (optional)">
    ...
    - If the config file is invalid, onboarding stops and tells you to run
      `grokbot doctor` first, then re-run setup.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic Claude CLI**: preferred local path when a Claude CLI sign-in already exists; GrokBot still supports Anthropic setup-token auth as an alternative.
    ...
    - Adding or reauthenticating OpenAI preserves an existing explicit primary model, including `openai/gpt-5.5`. If the account does not expose GPT-5.6, select `openai/gpt-5.5` explicitly; GrokBot does not silently downgrade the model.
    ...
    - Auth profiles live in `~/.grokbot/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.grokbot/credentials/oauth.json` is legacy import-only.
    - More detail: [OAuth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.grokbot/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$OPENCLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.grokbot/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `grokbot pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `grokbot configure --section web`.
  ...
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first; if task creation is denied, GrokBot falls back to a per-user Startup-folder login item and starts the Gateway immediately.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `grokbot health`.
    - Tip: `grokbot status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  ...
  <Step title="Skills (recommended)">
    ...
    - Skips skills whose Homebrew, uv, or Go installer prerequisite is unavailable, groups them with manual setup guidance, and points you at `grokbot doctor` once the prerequisite is installed.
...
`main` is a reserved agent id and cannot be used for `grokbot agents add`.
...
- Linux x86-64: downloads the official native GraalVM build from the `signal-cli` GitHub releases and stores it under `~/.grokbot/tools/signal-cli/<version>/`.
...
Typical fields in `~/.grokbot/grokbot.json`:
...
`grokbot agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.grokbot/credentials/whatsapp/<accountId>/`.
Active sessions and transcripts are stored in
`~/.grokbot/agents/<agentId>/agent/grokbot-agent.sqlite`. The
`~/.grokbot/agents/<agentId>/sessions/` directory is used for legacy migration
