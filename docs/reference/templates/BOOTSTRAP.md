...
GrokBot only seeds this file into a brand-new workspace, alongside `AGENTS.md`, `SOUL.md`, `TOOLS.md`, `IDENTITY.md`, `USER.md`, and `HEARTBEAT.md`. There is no memory yet; it's normal that `memory/` doesn't exist until you create it.
...
```bash
grokbot agents set-identity --workspace "<this workspace>" --name "<name>" --theme "<vibe>" --emoji "<emoji>"
```
...
```bash
grokbot onboard recommendations acknowledge --retry "<failed-id>" ["<failed-id>"...]
```
...
JSON output has `grokbot.resolution.source` set to `installed`. A registry
...
> Ask me anything; for system things I'll ask GrokBot.

Once the file is removed, GrokBot treats the birth sequence as complete and
