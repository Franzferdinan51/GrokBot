---
summary: "The @grokbot/ai npm package: reusable model transports, isolated runtimes, and host policy ports"
title: "@grokbot/ai package"
read_when:
  - You want to reuse GrokBot's model transports in another application
  - You are changing packages/ai or the AI transport host ports
  - You are reviewing what the grokbot release publishes to npm besides the root package
---

`@grokbot/ai` is the publishable library form of GrokBot's model execution
...
It publishes alongside the root `grokbot` package on every release, pinned to
the same version, with its own `npm-shrinkwrap.json` so its transitive
dependency tree is locked at install time. Installing `grokbot` installs the
matching `@grokbot/ai` automatically; library consumers can depend on it
directly without any GrokBot application code.
...
- **Host policy is injected, not bundled.** Request fetch guarding (for
  ...
  GrokBot installs its real implementations in its stream facade.
- **One event-stream identity.** `@grokbot/ai/event-stream` is the canonical
  `EventStream` constructor shared by GrokBot core, agent-core, and external
  consumers.
- **`internal/*` subpaths are not API.** They exist for the GrokBot
  application itself and carry no semver guarantee.
- Provider ids, credentials, model catalogs, retries, and failover remain
  application concerns. GrokBot layers those around this package; a library
...
| `./internal/*`   | GrokBot-internal, no semver guarantee                                         |
