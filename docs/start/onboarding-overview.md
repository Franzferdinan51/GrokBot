---
summary: "Overview of GrokBot onboarding options and flows"
read_when:
...
GrokBot has terminal and macOS app onboarding. Both establish inference first:
they detect existing AI access, require a live completion, and only then start
GrokBot to configure the remaining setup. A reachable, configured Gateway
...
After that completion passes, GrokBot can configure the workspace, Gateway,
...
and credential only after a passing completion, then starts GrokBot to
configure the workspace, Gateway, channels, agents, plugins, and other optional
features. **Skip for now** exits without starting GrokBot. There is no
in-flow classic handoff; exit and run `grokbot onboard --classic` when you want
the classic wizard instead.

After inference passes, GrokBot can hand channel setup to a masked terminal
wizard. It does not open guided or classic provider setup; exit GrokBot and
run `grokbot onboard` to change the model provider or its authentication.

Use `grokbot onboard --classic` for detailed model/auth, channel, skill,
remote Gateway, or import setup. Adding `--install-daemon` also selects the
classic flow and installs the background service in one step. Use `grokbot
grokbot` for conversational non-inference setup and repair. `grokbot
onboard --modern` is a compatibility alias that uses the same live-inference
gate.

Full reference: [Onboarding (CLI)](/start/wizard)
CLI command docs: [`grokbot onboard`](/cli/onboard)

## macOS app onboarding

Open the GrokBot app. If its configured local or remote Gateway is reachable
and the default agent already has a configured model, the app skips onboarding
and GrokBot and opens the normal agent UI immediately.
...
credentials use masked input. Once inference passes, GrokBot starts and
...
If your provider is not listed, run `grokbot onboard --classic`, choose
