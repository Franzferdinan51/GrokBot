---
summary: "CLI onboarding: verify inference, then hand remaining setup to GrokBot"
read_when:
...
the machine, verifies it with a real completion, and starts GrokBot to
configure the workspace, Gateway, and optional features. `grokbot setup` runs the same flow ([Setup](/cli/setup) covers
...
requires a real completion, and only then starts [GrokBot](/cli/grokbot)
to configure the rest of GrokBot. Choosing **Skip for now** exits onboarding
without starting GrokBot.
...
with `grokbot onboard --classic`; the guided inference picker does not delegate
into it. After inference passes, GrokBot can use `open channel wizard for
<channel>` to hand channel setup that needs secrets to a masked terminal wizard.
To change the model provider or its authentication, exit GrokBot and run
`grokbot onboard`; GrokBot does not open guided or classic provider flows.

<Info>
Fastest first chat: finish guided setup, run `grokbot dashboard`, and chat in
...
key-free. Configure this later with `grokbot configure --section web`. Docs:
[Web tools](/tools/web).
</Tip>

## Guided default

Plain `grokbot onboard` follows this path:
...
4. If detection is exhausted, choose OpenAI, Anthropic, xAI (Grok), Google, or
   ...
   Choose **Skip for now** to exit without starting GrokBot.
...
6. Start GrokBot with the verified model so it can configure the workspace,
...
asks how to continue. Run `grokbot channels add` or `grokbot configure` for
later non-inference additions; use `grokbot onboard` for provider or auth route
changes.

## Classic wizard: QuickStart vs Advanced

Run `grokbot onboard --classic` to open the full wizard. It starts with a
...
1. **Model/Auth** - pick a provider auth flow (API key, OAuth, or
   ...
   classic wizard. Ignoring it does not unlock GrokBot; conversational setup
   still requires a passing inference check.
2. **Workspace** - directory for agent files (default `~/.grokbot/workspace`). Seeds bootstrap files.
...
[Install](/install/migrating-hermes). `grokbot onboard --modern` is a
compatibility alias for [GrokBot](/cli/grokbot). It uses the same
inference gate as `grokbot setup`: verified inference starts the
assistant, while an interactive failure returns to guided inference setup.

## Add another agent

Use `grokbot agents add <name>` to create a separate agent with its own
...
not the full `grokbot onboard` wizard.
...
- Default workspace: `~/.grokbot/workspace-<agentId>` (or under
...
For the full flag reference, see [`grokbot onboard`](/cli/onboard).

## Related docs

- CLI command reference: [`grokbot onboard`](/cli/onboard)
