---
summary: "Get GrokBot installed and run your first chat in minutes."
read_when:
...
Install GrokBot, run onboarding, and chat with your AI assistant in about 5
...
/>
  ...
  <Step title="Run onboarding">
    ...
    steps and return later with `grokbot configure`.
...
```bash
mkdir -p "$HOME/.grokbot/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.grokbot/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run GrokBot as a service account or want custom paths:
