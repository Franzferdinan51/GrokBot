---
summary: "Configure native Codex plugins for Codex-mode GrokBot agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode GrokBot agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are configuring an existing workspace-directory Codex plugin
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode GrokBot agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the GrokBot turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. GrokBot does not translate
Codex plugins into synthetic `codex_plugin_*` GrokBot dynamic tools.
...
`codexPlugins` has no effect on GrokBot-provider runs, ACP conversation
...
```bash
grokbot migrate codex --dry-run --verify-plugin-apps
```
...
GrokBot does not call `plugin/install` or start authentication for a
`workspace-directory` plugin. Install, enable, and authenticate it in Codex
before adding or enabling the GrokBot policy. GrokBot keeps apps hidden when
...
GrokBot reports `marketplace_missing` for each enabled workspace plugin and
...
`enable`/`disable` write only to `~/.grokbot/grokbot.json`; they never edit
...
`auth_required`, reauthorize the app in Codex before enabling it in GrokBot.
For a `workspace-directory` entry, enabling it here changes only GrokBot
...
| Enabled    | Codex reports the plugin enabled, and GrokBot config allows it for Codex harness turns.                                           |
...
- During planning, GrokBot reads source Codex `plugin/read` details and
...
For `workspace-directory` plugins, setup happens outside GrokBot. GrokBot
...
disabled, inaccessible, or unauthenticated plugin exposes no apps; GrokBot
...
GrokBot reads Codex app inventory through app-server `app/list`, caches it
...
drops it, and GrokBot rebuilds it from the next `app/list` read.
...
A plugin app is exposed only when GrokBot can map it back to the configured
...
GrokBot injects a restrictive `config.apps` patch for the Codex thread:
...
get `open_world_enabled: true`. GrokBot does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: GrokBot returns a deterministic decline.
- `true`: GrokBot auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: GrokBot exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into GrokBot plugin
  approvals before returning the Codex approval response.
- `"ask"`: GrokBot uses the same Codex write/destructive gating as
  ...
  admitted app using `"ask"`, GrokBot selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  GrokBot; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                    |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in GrokBot.                                                      |
...
| `plugin_detail_unavailable`                       | GrokBot could not read plugin ownership details.                                                                                    | Inspect the target app-server's `plugin/list` and `plugin/read` responses.                                             |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | GrokBot schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known.  |
...
account. GrokBot can enable an accessible app for the thread even when the
...
`/new` or `/reset`. GrokBot does not repair or authenticate workspace plugins.
...
`remotePluginId`; GrokBot keeps owned apps hidden when that selector or the
...
not already active; install, enable, and authenticate it outside GrokBot.
...
Codex thread bindings keep the app config they started with until GrokBot
