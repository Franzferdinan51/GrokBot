---
summary: "Install and use Codex, Claude, and Cursor bundles as GrokBot plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how GrokBot maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

GrokBot can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** - content and metadata packs that
GrokBot maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native GrokBot plugins. Native plugins run
...
of requiring authors to rewrite them as native GrokBot plugins, GrokBot
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    grokbot plugins install ./my-bundle

    # Archive
    grokbot plugins install ./my-bundle.tgz

    # Claude marketplace
    grokbot plugins marketplace list <source>
    grokbot plugins install <plugin> --marketplace <source>
    ```
...
## What GrokBot maps from bundles

Not every bundle feature runs in GrokBot today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal GrokBot skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                        | Claude, Cursor |
| Hook packs    | GrokBot-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded GrokBot settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded GrokBot LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded GrokBot defaults                                     | Claude         |

#### Skill content

- Bundle skill roots load as normal GrokBot skill roots.
- Claude `commands/` roots are treated as additional skill roots.
- Cursor `.cursor/commands/` roots are treated as additional skill roots.

Claude markdown command files and Cursor command markdown both work through the
normal GrokBot skill loader.

#### Hook packs

Bundle hook roots work **only** when they use the normal GrokBot hook-pack
layout: `HOOK.md` plus `handler.ts` or `handler.js`. Today this is primarily
the Codex-compatible case.

#### MCP for embedded GrokBot

- Enabled bundles can contribute MCP server config.
- GrokBot merges bundle MCP config into the effective embedded GrokBot
  settings as `mcpServers`.
- GrokBot exposes supported bundle MCP tools during embedded GrokBot agent
...
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in GrokBot config. `grokbot mcp set` and `grokbot doctor --fix` normalize the common alias.
...
GrokBot registers bundle MCP tools with provider-safe names in the form
...
#### Embedded GrokBot settings

Claude `settings.json` is imported as default embedded GrokBot settings when
the bundle is enabled. GrokBot sanitizes shell override keys before applying
them:

- `shellPath`
- `shellCommandPrefix`

#### Embedded GrokBot LSP

- Enabled Claude bundles can contribute LSP server config.
- GrokBot loads `.lsp.json` plus any manifest-declared `lspServers` paths.
- Bundle LSP config is merged into the effective embedded GrokBot LSP
  defaults.
- Only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `grokbot plugins inspect <id>`.

### Detected but not executed

These are recognized and shown in diagnostics, but GrokBot does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit GrokBot best when they use skill roots and GrokBot-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded GrokBot settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded GrokBot
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded GrokBot LSP defaults
...
GrokBot checks for native plugin format first:

1. `grokbot.plugin.json` or a valid `package.json` with `grokbot.extensions` - treated as a **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) - treated as a **bundle**

If a directory contains both, GrokBot uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `grokbot plugins install` and ship everything
  they need in the installed plugin directory.
- GrokBot-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `grokbot doctor --fix` removes stale local bundled-plugin install records
...
- GrokBot does **not** load arbitrary bundle runtime modules in-process.
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `grokbot plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded GrokBot settings from `settings.json` are supported. GrokBot does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    GrokBot hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
