...
resumes omit GrokBot model and provider overrides so Codex restores the
...
GrokBot binding owns the exact target or one of its non-archived spawned
descendants. GrokBot follows Codex's descendant pagination and fails closed on
...
`grokbot doctor --fix` to migrate the old entry, endpoint definitions, policy
...
For ordinary harness turns, GrokBot starts the managed Codex binary shipped
...
enables native Computer Use. If no desktop app bundle is installed, GrokBot
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after GrokBot builds its inherited environment.                                                                                                                                                                                                                                                             |
...
| `loopDetectionPreToolUseRelay`                | `true`                                                 | Install the Codex `PreToolUse` subprocess used only for GrokBot loop detection and its explicit no-policy marker. Set `false` to reduce per-tool process fan-out. Before-tool plugin hooks and trusted-tool policy still install their required relay.                                                                                                                                         |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active GrokBot sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows GrokBot sandbox egress.                                                                                                                                                                                       |
...
contract. When enabled, GrokBot also sets `features.network_proxy.enabled` and
`default_permissions` in the Codex thread config so the generated permission
profile can start Codex-managed networking. GrokBot generates a
collision-resistant `grokbot-network-<fingerprint>` profile name from the
...
GrokBot treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before GrokBot builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent. When native
Codex plugins are configured, GrokBot uses the connected app-server's plugin
...
still the authoritative inventory and metadata source, but GrokBot policy
...
plugins via `plugin/install` and refreshes inventory. Only connect GrokBot to
remote app-servers that are trusted to accept GrokBot-managed plugin installs
...
unattended GrokBot turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, GrokBot treats the implicit default as guardian
...
When an GrokBot sandbox is active, the local Codex app-server process still
runs on the Gateway host. GrokBot therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the GrokBot sandbox
backend. Shell access is exposed through GrokBot sandbox-backed dynamic tools
such as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

<Note>
On Docker-backed GrokBot sandbox hosts (`agents.defaults.sandbox.mode` set to
a Docker backend), `grokbot doctor` probes whether the host allows the
...
Ubuntu/AppArmor hosts. Fix the reported host namespace policy for the GrokBot
...
The stable default is fail-closed: active GrokBot sandboxing disables native
...
to try Codex's remote environment support with GrokBot's sandbox backend.
...
When the flag is on and the current GrokBot session is sandboxed, GrokBot
...
GrokBot-owned environment. If the app-server cannot register the environment,
...
the loopback exec-server unless it is running on the same host, so GrokBot
...
1. An explicit GrokBot Codex auth profile for the agent.
2. The app-server's existing account in that agent's Codex home.
3. For local stdio app-server launches only, `CODEX_API_KEY`, then
   `OPENAI_API_KEY`, when no app-server account is present and OpenAI auth is
   still required.

When GrokBot sees a ChatGPT subscription-style Codex auth profile (OAuth or
...
Stdio app-server launches inherit GrokBot's process environment by default.
GrokBot owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's GrokBot state. That keeps Codex
config, accounts, plugin cache/data, and thread state scoped to the GrokBot
...
GrokBot skips its auth-profile bridge for the app-server. Verified owner
...
continuing it in GrokBot; independent Codex processes do not coordinate
...
unrelated thread not owned by another GrokBot Chat require
`allowWriteControls`. Neither option bypasses a locked binding.

GrokBot does not rewrite `HOME` for normal local app-server launches.
Codex-run subprocesses such as `grokbot`, `gh`, `git`, cloud CLIs, and shell
...
In the default agent scope, GrokBot plugins and GrokBot skill snapshots
still flow through GrokBot's own plugin registry and skill loader; personal
Codex `~/.codex` assets do not. If you have useful Codex CLI skills or
plugins from a Codex home that should become part of an isolated GrokBot
agent, inventory them explicitly:

```bash
grokbot migrate codex --dry-run
grokbot migrate apply codex --yes
```
...
GrokBot removes `CODEX_HOME` and `HOME` from this list during local launch
...
`grokbot` namespace with `deferLoading: true`. GrokBot normally does not
...
When a finite runtime allowlist disables native Code Mode, GrokBot sends an
empty execution-environment selection. In that direct, unsandboxed case,
GrokBot keeps its policy-filtered `exec` and `process` tools as the shell
fallback. Runtime allowlists and `codexDynamicToolsExclude` still apply.

Most remaining GrokBot integration tools, such as messaging, media, cron,
...
`sessions_spawn` remains available for explicit GrokBot or ACP delegation.
...
tool, are grouped under `openclaw_direct`. GrokBot adds that namespace to
...
Dynamic tool budgets are capped at 600000 ms. On timeout, GrokBot aborts the
...
releases the GrokBot session lane so follow-up chat messages are not queued
...
post-tool idle budget: after GrokBot returns an `item/tool/call` response,
...
GrokBot best-effort interrupts the native turn and releases the session
...
GrokBot upgrades the bundled `@openai/codex` version or when a deployment
...
If discovery fails or times out, GrokBot uses a bundled fallback catalog:
...
The app-server catalog can report `ultra`; GrokBot reasoning controls currently
...
GrokBot does not write synthetic Codex project-doc files or depend on Codex
fallback filenames for persona files, because Codex fallbacks only apply when
`AGENTS.md` is missing.

For GrokBot workspace parity, the Codex harness forwards the other
...
- `SOUL.md`, `IDENTITY.md`, and `USER.md` are forwarded as **turn-scoped**
  ...
  user profile.
- The compact loaded GrokBot skills list is also forwarded as turn-scoped
...
- `BOOTSTRAP.md`, when present, is forwarded as GrokBot turn input reference
  context.
