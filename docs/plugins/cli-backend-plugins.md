...
read_when:
  ...
  - You need to map a third-party CLI into GrokBot's text fallback runner
---

CLI backend plugins let GrokBot call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points GrokBot at the plugin runtime module              |
| Manifest ownership   | `grokbot.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `grokbot.runtimeExtensions` pointing at the
  ...
  <Step title="Declare backend ownership">
    ```json grokbot.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through GrokBot",
    ...
    `cliBackends` is the runtime ownership list; it lets GrokBot auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "grokbot/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how GrokBot should launch and parse the CLI:
...
| `modelAliases`                                            | Map GrokBot model ids to CLI-native ids                                          |
| `sessionArg` / `sessionArgs`                              | How to pass a session id                                                          |
| `sessionMode`                                             | `always`, `existing`, or `none`                                                   |
| `sessionIdFields`                                         | JSON fields GrokBot reads from CLI output                                        |
...
| `defaultAuthProfileId`             | Prefer a specific GrokBot auth profile                                     |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into GrokBot's loopback MCP tool bridge                                |
| `ownsNativeCompaction`             | Backend owns its own compaction - GrokBot defers                           |
...
command. GrokBot hashes the bounded complete installed package tree, including
...
`sideQuestionToolMode: "disabled"`; otherwise GrokBot fails closed when BTW
...
GrokBot calls it once with the final fresh or resume argv and fails closed when
...
### `ownsNativeCompaction`: opting out of GrokBot compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so GrokBot's safeguard summarizer never runs
...
over-budget session can stay over budget or go stale (GrokBot no longer
rescues it):
...
CLI backends do not receive GrokBot tools by default. If the CLI can consume
...
"always-on"` so GrokBot can fail closed when a caller requires no native
...
```bash
grokbot plugins inspect acme-cli --runtime --json
grokbot agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `grokbot.extensions` and built runtime entries for published packages</Check>
<Check>`grokbot.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
