...
[`grokbot plugins install`](/cli/plugins).
...
export OPENCLAW_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/grokbot-codex-2026.5.8.tgz",
  "grokbot-web-search": "npm:@grokbot/web-search@2026.5.8"
}'
```
...
When a setup-time flow installs a plugin whose id appears in the map, GrokBot
...
- Overrides do not inherit official trusted-source status. Even when the
  catalog entry normally represents an GrokBot-owned package, an override is
...
OPENCLAW_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/grokbot-codex-2026.5.8.tgz"}' \
pnpm grokbot onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$OPENCLAW_STATE_DIR/npm/projects" -path '*/node_modules/@grokbot/codex/package.json' -print
grep -R '"@grokbot/codex"' "$OPENCLAW_STATE_DIR/npm/projects"/*/package-lock.json
```
