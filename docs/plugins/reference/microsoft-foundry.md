---
summary: "Adds Microsoft Foundry model provider support to GrokBot."
read_when:
...
Adds Microsoft Foundry model provider support to GrokBot.

## Distribution

- Package: `@grokbot/microsoft-foundry`
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. GrokBot refreshes Microsoft Foundry runtime tokens through
...
GrokBot uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so GrokBot
...
deployment through onboarding or include model metadata so GrokBot can verify
