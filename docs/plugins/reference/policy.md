...
The Policy plugin contributes doctor health checks for policy-managed GrokBot
...
posture, ingress/channel access posture, data-handling posture, and GrokBot config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
GrokBot settings and workspace declarations as evidence, and reports drift
through `grokbot policy check` and `grokbot doctor --lint`. A clean policy
check emits policy, evidence, findings, and attestation hashes that operators
can record for audit.

`grokbot policy compare --baseline <file>` compares one policy file to another
