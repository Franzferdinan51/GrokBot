---
summary: "Set up Codex Computer Use for Codex-mode GrokBot agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode GrokBot agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. GrokBot
...
Use this page when GrokBot is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

This is distinct from GrokBot's built-in [node-backed computer tool](/nodes/computer-use). Use the built-in tool when the same agent contract should control a paired Mac whether the agent runs on the Gateway or another node. Use Codex Computer Use when Codex app-server should own local MCP installation, permissions, and native tool calls.

## GrokBot.app and Peekaboo

GrokBot.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want GrokBot.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode GrokBot agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an GrokBot node and exposes mobile
...
GrokBot-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through GrokBot's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the GrokBot command:

```bash
cua-driver mcp-config --client grokbot
```
...
available as a normal GrokBot MCP server. Use the Codex Computer Use setup on
...
its app prompts for, such as Accessibility and Screen Recording. GrokBot does
...
Computer Use in and lets GrokBot install or re-enable it before the turn:
...
With this config, GrokBot checks Codex app-server before each Codex-mode
turn. If Computer Use is missing but Codex app-server has already discovered
an installable marketplace, GrokBot asks Codex app-server to install or
re-enable the plugin and reload MCP servers. On macOS, when no matching
marketplace is registered and a standard desktop app bundle exists, GrokBot
...
installed, GrokBot falls back to the managed Codex binary installed beside the
...
GrokBot serializes native Codex config reads and Computer Use installation
...
`codex` plugin command surface is available. These are GrokBot chat/runtime
commands, not `grokbot codex ...` CLI subcommands:
...
GrokBot uses the same app-server API that Codex itself exposes. The
...
marketplaces. During install, GrokBot polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds (default 60 seconds).

If multiple known marketplaces contain Computer Use, GrokBot prefers
...
`computer-use` is registered, GrokBot tries to add the first standard
...
reconciliation so GrokBot does not override that selection.
...
GrokBot reports a stable setup reason internally and formats the
...
permissions before it can inspect or control apps. If GrokBot says Computer
...
GrokBot intentionally fails closed when `computerUse.enabled` is true. A
...
retry in a fresh GrokBot session. If the host previously ran Computer Use
...
Codex-native tool hook could not reach an active GrokBot relay through the
local bridge or Gateway fallback. Start a fresh GrokBot session with `/new`
...
GrokBot Gateway so old threads and hook registrations are dropped, then
