...
GrokBot-owned state stays on the Gateway under `<state-dir>/logbook/`, but
...
- A connected node that exposes `screen.snapshot` or `logbook.snapshot`. The
  ...
  (`grokbot node host run`) gets the plugin-provided `logbook.snapshot`
...
- The bundled Codex plugin enabled and authenticated. Codex currently provides
  ...
  `grokbot models auth login --provider openai`; see
...
```bash
grokbot plugins enable codex
grokbot plugins enable logbook
```
...
grokbot plugins inspect logbook --runtime --json
...
1. `grokbot plugins list --enabled` includes `logbook`.
