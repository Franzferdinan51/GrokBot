...
read_when:
  - You want an GrokBot agent to join a Microsoft Teams meeting
  - You are configuring Chrome, BlackHole, or SoX for Teams meeting talk-back
title: "Microsoft Teams meetings plugin"
---

The `teams-meetings` plugin joins Microsoft Teams links as a guest in the GrokBot Chrome profile. It accepts work links under `teams.microsoft.com/l/meetup-join/...` and consumer links under `teams.live.com/meet/...`. It does not create meetings, dial in, call Microsoft Graph, or record meetings.
  ...
  plugins: {
    entries: {
      "teams-meetings": {
        ...
        config: {
          defaultMode: "agent",
          chrome: { guestName: "GrokBot Agent" },
        },
      },
    },
  },
...
```bash
grokbot teamsmeetings setup
grokbot teamsmeetings join 'https://teams.microsoft.com/l/meetup-join/...'
```
...
| Mode         | Behavior                                                                    |
| ------------ | --------------------------------------------------------------------------- |
| `agent`      | Realtime transcription consults the configured GrokBot agent; TTS replies. |
...
Transcribe mode enables Teams live captions after admission and captures speaker-attributed caption rows. The `transcript` action returns the bounded caption buffer for the active GrokBot meeting session.
