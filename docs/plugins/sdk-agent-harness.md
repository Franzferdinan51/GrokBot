...
An **agent harness** is the low level executor for one prepared GrokBot agent
...
runtime and the normal GrokBot provider transport is the wrong abstraction:
...
- a model runtime that needs its own resume id in addition to the GrokBot
  session transcript
...
Core still forwards a compatible, explicitly selected or ordered GrokBot auth
...
Verified GrokBot continuations also pass `params.expectedRuntimeArtifact`.
...
The prepared attempt also includes `params.runtimePlan`, an GrokBot-owned
policy bundle for runtime decisions that must stay shared across GrokBot and
native harnesses:
...
Harnesses may use the plan for decisions that need to match GrokBot behavior,
...
all of them before dispatch. Implicit selection uses GrokBot if no plugin can
own the full set; an explicit or persisted plugin selection fails closed.

## Register a harness

**Import:** `grokbot/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "grokbot/plugin-sdk/agent-harness";
...
Delegates receive only work admission and embedded execution. GrokBot requires
...
GrokBot chooses a harness after provider/model resolution:
...
4. If no registered harness matches, GrokBot uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, GrokBot does not
...
GrokBot. The harness then claims that provider in `supports(...)`.
...
- app-server request: GrokBot sends the bare model id to Codex and lets the
...
behavior stay on GrokBot. Plaintext official HTTP endpoints are rejected. Older `codex/gpt-*`
...
blocks older or unversioned servers, so GrokBot only runs against the protocol
...
seam is for async tool-result transforms that must run before GrokBot or
...
`grokbot/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so GrokBot's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; GrokBot does not infer it from assistant prose. The helper
...
`grokbot/plugin-sdk/agent-harness-runtime` after they finalize an attempt. It
dispatches the portable `agent_end` hook and GrokBot's research capture
...
user-input helpers from `grokbot/plugin-sdk/agent-harness-runtime` to format
the prompt, deliver it through GrokBot's blocking reply path, and normalize
...
`grokbot/plugin-sdk/agent-harness-tool-runtime`. It owns
...
The bundled `codex` harness is the native Codex mode for embedded GrokBot
...
refs should be repaired with `grokbot doctor --fix`, and legacy `codex/*`
model refs remain compatibility aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. GrokBot still owns the chat channel,
...
By default, GrokBot uses `auto` provider/model runtime policy: registered
...
token. Keep that binding explicitly associated with the GrokBot session, and
keep mirroring user-visible assistant/tool output into the GrokBot
transcript.

The GrokBot transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in GrokBot harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so GrokBot
can clear it when the owning GrokBot session is reset.

## Tool and media results

Core constructs the GrokBot tool list and passes it into the prepared
...
outputs on the same delivery path as GrokBot-backed runs.
...
GrokBot-tool media. On `message_tool_only` routes, this narrow provenance lets
...
outcome accumulator. A harness that executes GrokBot dynamic tools or native
...
- Report `outcome: "success"` or `outcome: "failure"`. Include the structured
  ...
  display text.
- Use `nativeMutation` only for native tools that do not use an GrokBot tool
  definition. Supply protocol-owned mutation and replay facts there; do not
  copy GrokBot's mutation classifier into the harness.
...
without terminal reports, GrokBot cannot preserve mutating-tool failure truth
