---
summary: "Manage GrokBot plugins from the Control UI or CLI"
read_when:
...
rules, and edge cases, see [`grokbot plugins`](/cli/plugins).
...
configured Control UI base path. For example, a base path of `/grokbot` uses
`/grokbot/settings/plugins`. The page has two tabs:
...
- **Discover** is the store: featured plugins included with GrokBot, official
...
or **Disable**. Workboard, for example, is included with GrokBot and disabled
...
OAuth-backed MCP connectors still need a one-time `grokbot mcp login <name>`
...
`plugins list` is a cold inventory check: what GrokBot can discover from
...
an install hint (`grokbot plugins install clawhub:<package>`) per result.

## Enable and disable plugins

```bash
grokbot plugins enable <plugin-id>
grokbot plugins disable <plugin-id>
```
...
grokbot plugins install clawhub:<package>@1.2.3
grokbot plugins install clawhub:<package>@beta

# Install from npm.
grokbot plugins install npm:<package>
grokbot plugins install npm:@scope/grokbot-plugin@1.2.3
grokbot plugins install npm:@grokbot/codex

# Install from a local npm-pack artifact.
grokbot plugins install npm-pack:<path.tgz>

# Install from git or a local development checkout.
grokbot plugins install git:github.com/acme/grokbot-plugin@v1.0.0
...
name matches a bundled or official plugin id, in which case GrokBot uses
that local/official copy instead. Use `clawhub:`, `npm:`, `git:`, or
`npm-pack:` for deterministic source selection. GrokBot's bundled and official
...
ClawHub, or hook-pack install, use `grokbot plugins update` instead. With
...
```bash
grokbot plugins update <plugin-id>
grokbot plugins update <npm-package-or-spec>
grokbot plugins update --all
grokbot plugins update <plugin-id> --dry-run
```
...
`grokbot plugins update --all` is the bulk maintenance path. It still
respects ordinary tracked install specs, but trusted official GrokBot
...
```bash
grokbot plugins update @scope/grokbot-plugin@beta
grokbot plugins update @scope/grokbot-plugin
```
...
See [`grokbot plugins`](/cli/plugins#update) for the exact fallback and
pinning rules.

## Uninstall plugins

```bash
grokbot plugins uninstall <plugin-id> --dry-run
grokbot plugins uninstall <plugin-id>
grokbot plugins uninstall <plugin-id> --keep-files
```
...
| ----------- | --------------------------------------------------------------------------- | -------------------------------------------------------------- |
| ClawHub     | You want GrokBot-native discovery, scan summaries, versions, and hints     | `grokbot plugins install clawhub:<package>`                   |
| git         | You want a branch, tag, or commit from a repository                         | `grokbot plugins install git:github.com/<owner>/<repo>@<ref>` |
...
| npm pack    | You are proving a local package artifact through npm install semantics      | `grokbot plugins install npm-pack:<path.tgz>`                 |
| npmjs.com   | You already ship JavaScript packages or need npm dist-tags/private registry | `grokbot plugins install npm:@acme/grokbot-plugin`           |
...
ClawHub is the primary public discovery surface for GrokBot plugins. Publish
...
Native npm plugins must ship a plugin manifest (`grokbot.plugin.json`) plus
`package.json` metadata before publishing:

```json package.json
{
  "name": "@acme/grokbot-plugin",
  "version": "1.0.0",
  "type": "module",
  "grokbot": {
...
grokbot plugins install npm:@acme/grokbot-plugin
grokbot plugins install npm:@acme/grokbot-plugin@beta
grokbot plugins install npm:@acme/grokbot-plugin@1.0.0
```
...
- [Building plugins](/plugins/building-plugins) shows the full plugin
  package shape (including `grokbot.plugin.json`) and first publish
  workflow.
