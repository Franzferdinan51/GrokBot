...
read_when:
  - You want an GrokBot agent to join a Google Meet call
  - You want an GrokBot agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

The `google-meet` plugin joins explicit Meet URLs on behalf of an GrokBot agent. It is deliberately narrow:
...
- `mode: "agent"` (default) transcribes participant speech with a realtime provider, routes it to the configured GrokBot agent, and speaks the answer with regular GrokBot TTS. `mode: "bidi"` lets a realtime voice model answer directly. `mode: "transcribe"` joins observe-only with no talk-back.
...
```bash
grokbot googlemeet setup
grokbot googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
grokbot googlemeet setup --transport chrome-node --mode transcribe
```
...
```bash
grokbot googlemeet setup --transport twilio
```
...
```bash
grokbot googlemeet create --transport chrome-node --mode agent
grokbot googlemeet create --no-join
```
...
- **`browser`**: used without OAuth credentials. GrokBot opens `https://meet.google.com/new` on the pinned Chrome node and waits for Google to redirect to a real meeting-code URL; the GrokBot Chrome profile on that node must already be signed in to Google. Join and create both reuse an existing Meet tab (or an in-progress `.../new` / Google account prompt tab) before opening a new one; tab matching ignores harmless query strings like `authuser`.
...
```bash
grokbot googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
This only applies to API-created rooms, so OAuth must be configured. If you authenticated before this option existed, rerun `grokbot googlemeet auth login --json` after adding the `meetings.space.settings` scope to your OAuth consent screen.
...
```bash
grokbot googlemeet test-listen <meet-url> --transport chrome-node
```
...
Local Chrome joins through the signed-in GrokBot browser profile and needs `BlackHole 2ch` for the mic/speaker path. A single BlackHole device is enough for a first smoke test but can echo; use separate virtual devices or a Loopback-style graph for clean duplex audio.
...
| Runs where           | What                                                                                            |
| -------------------- | ----------------------------------------------------------------------------------------------- |
| Gateway host         | GrokBot Gateway, agent workspace, model/API keys, realtime provider, Google Meet plugin config |
| Parallels macOS VM   | GrokBot CLI/node host, Chrome, SoX, BlackHole 2ch, a Chrome profile signed in to Google        |
...
grokbot node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  grokbot node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```

Use the same flag when installing as a LaunchAgent (it is process environment, stored in the LaunchAgent environment when present on the install command, not an `grokbot.json` setting):

```bash
OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  grokbot node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
grokbot node restart
```
...
```bash
grokbot googlemeet test-speech https://meet.google.com/abc-defg-hij
```
...
If `chromeNode.node` is omitted, GrokBot auto-selects only when exactly one connected node advertises both `googlemeet.chrome` and browser control; pin `chromeNode.node` (node id, display name, or remote IP) when several capable nodes are connected.
...
| `No connected Google Meet-capable node`                  | Run `grokbot node run` in the VM, approve pairing, and run `grokbot plugins enable google-meet` and `grokbot plugins enable browser` there. Confirm `gateway.nodes.allowCommands` includes `googlemeet.chrome` and `browser.proxy`.                              |
...
| Chrome opens but cannot join                             | Sign in to the browser profile in the VM, or keep `chrome.guestName` set. Guest auto-join uses GrokBot browser automation through the node browser proxy; point the node's `browser.defaultProfile` (or a named existing-session profile) at the profile you want. |
| Duplicate Meet tabs                                      | Leave `chrome.reuseExistingTab: true`. GrokBot activates an existing tab for the same URL, and creation reuses an in-progress `.../new` or Google account prompt tab, before opening another.                                                                      |
| No audio                                                 | Route Meet mic/speaker through the virtual audio path used by GrokBot; use separate virtual devices or Loopback-style routing for clean duplex audio.                                                                                                              |

## Install notes

The Chrome talk-back default uses two external tools that GrokBot does not bundle or redistribute; install them as host dependencies through Homebrew:
...
SoX is licensed `LGPL-2.0-only AND GPL-2.0-only`; BlackHole is GPL-3.0. If you build an installer or appliance that bundles BlackHole with GrokBot, review BlackHole's upstream licensing or get a separate license from Existential Audio.
...
Opens the Meet URL through GrokBot browser control and joins as the signed-in GrokBot browser profile. On macOS, the plugin checks for `BlackHole 2ch` before launch and, if configured, runs an audio bridge health/startup command before opening Chrome. For local Chrome, pick the profile with `browser.defaultProfile`; `chrome.browserProfile` is passed to `chrome-node` hosts instead.

```bash
grokbot googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
grokbot googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Chrome mic/speaker audio routes through the local GrokBot audio bridge. If `BlackHole 2ch` is not installed, the join fails with a setup error instead of joining without an audio path.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an GrokBot agent. Be brief.",
...
Provide Twilio credentials through environment to keep secrets out of `grokbot.json`:
...
grokbot plugins list | grep -E 'google-meet|voice-call'
grokbot googlemeet setup
```
...
`meetings.space.created` is required by `spaces.create`. `meetings.space.readonly` resolves Meet URLs/codes to spaces. `meetings.space.settings` lets GrokBot pass `SpaceConfig` settings such as `accessType` during API room creation. `meetings.conference.media.readonly` is for Meet Media API preflight and media work; Google may require Developer Preview enrollment for actual Media API use. `calendar.events.readonly` is only needed for `--today`/`--event` calendar lookup. `drive.meet.readonly` is only needed for `--include-doc-bodies` export. If you only need browser-based Chrome joins, skip OAuth entirely.
...
```bash
grokbot googlemeet auth login --json
```
...
grokbot googlemeet auth login --json --manual
```
...
```bash
grokbot googlemeet doctor --oauth --create-space --json
```

Prove read access to an existing space:

```bash
grokbot googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
grokbot googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```

A `403` from these checks usually means the Meet REST API is disabled, the refresh token is missing the required scope, or the Google account cannot access that space. A refresh-token error means rerun `grokbot googlemeet auth login --json` and store the new `oauth` block.

No OAuth is needed for the browser fallback; Google auth there comes from the signed-in Chrome profile on the selected node, not GrokBot config.
...
```bash
grokbot googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
grokbot googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

After Meet has created conference records:

```bash
grokbot googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
grokbot googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
grokbot googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
grokbot googlemeet latest --today
grokbot googlemeet calendar-events --today --json
grokbot googlemeet artifacts --event "Weekly sync"
grokbot googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
grokbot googlemeet latest --meeting https://meet.google.com/abc-defg-hij
grokbot googlemeet artifacts --conference-record conferenceRecords/abc123 --json
grokbot googlemeet attendance --conference-record conferenceRecords/abc123 --json
```

Close the room for an API-created space:

```bash
grokbot googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```

Calls `spaces.endActiveConference` and requires OAuth with the `meetings.space.created` scope for a space the authorized account can manage. Accepts a Meet URL, meeting code, or `spaces/{id}` and resolves it to the API space resource first. This is separate from `googlemeet leave`: `leave` stops GrokBot's local/session participation; `end-active-conference` asks Google Meet to end the active conference for the space.

Write a readable report:

```bash
grokbot googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
grokbot googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
grokbot googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
grokbot googlemeet setup --transport chrome-node --mode transcribe
grokbot googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```
...
| `OPENCLAW_GOOGLE_MEET_CLIENT_SECRET`, `OPENCLAW_GOOGLE_MEET_ACCESS_TOKEN`, `OPENCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` | Optional; same fallback names without the `GROKBOT_` prefix also work |
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the GrokBot browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the GrokBot browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
Creating joins by default, but Chrome/Chrome-node still needs a signed-in Google profile to join through the browser; if signed out, GrokBot reports `manualActionRequired: true` or a browser fallback error and asks the operator to finish Google login before retrying.
...
The common Chrome agent path only needs the plugin enabled, BlackHole, SoX, a realtime provider key, and a configured GrokBot TTS provider:
...
| `chrome.guestName`                | `"GrokBot Agent"`                       | Shown on the signed-out Meet guest screen                                                                                                                                                                         |
...
| `realtime.agentId`                | `"main"`                                 | GrokBot agent id used for `openclaw_agent_consult`                                                                                                                                                               |
...
An `grokbot doctor --fix` migration exists for the legacy `realtime.provider: "google"` shape: it moves that intent to `realtime.voiceProvider: "google"` plus `realtime.transcriptionProvider: "openai"` when those fields are not already set.
...
For Chrome transports, `leave` keeps a reused user-owned tab open after clicking Meet's Leave call button. Tabs opened by GrokBot are closed after departure.
...
| `speechReady` / `speechBlockedReason` / `speechBlockedMessage`        | Whether managed Chrome speech is allowed now; `speechReady: false` means GrokBot did not send the intro/test phrase   |
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured GrokBot agent | Normal GrokBot TTS runtime            | You want "my agent is in the meeting" behavior        |
...
```bash
grokbot googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
grokbot googlemeet setup
grokbot nodes status --connected
grokbot nodes invoke \
  --node parallels-macos \
...
- `grokbot logs --follow` shows DTMF TwiML served before realtime TwiML, then a realtime bridge with the initial greeting queued.
...
```bash
grokbot plugins list | grep google-meet
grokbot googlemeet setup
```
...
```bash
grokbot plugins enable google-meet
grokbot plugins enable browser
OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  grokbot node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
- **API creation**: `oauth.clientId` and `oauth.refreshToken` (or matching `OPENCLAW_GOOGLE_MEET_*` env vars) are present, and the refresh token was minted after create support was added; older tokens may lack `meetings.space.created`, so rerun `grokbot googlemeet auth login --json`.
- **Browser fallback**: `defaultTransport: "chrome-node"` and `chromeNode.node` point at a connected node with `browser.proxy` and `googlemeet.chrome`; the GrokBot Chrome profile on that node is signed in and can open `https://meet.google.com/new`.
...
- **Audio-choice interstitial**: if Meet shows "Do you want people to hear you in the meeting?", leave the tab open. GrokBot should click **Use microphone** or (create-only) **Continue without microphone** and keep waiting for the generated URL; if it cannot, the error should mention `meet-audio-choice-required`, not `google-login-required`.

### Agent joins but does not talk

```bash
grokbot googlemeet setup
grokbot googlemeet doctor
```

Use `mode: "agent"` for the STT -> GrokBot agent -> TTS path, `mode: "bidi"` for the direct realtime voice fallback. `mode: "transcribe"` intentionally starts no talk-back bridge. For observe-only debugging, run `grokbot googlemeet status --json <session-id>` after participants speak and check `captioning`, `transcriptLines`, `lastCaptionText`. If `inCall` is true but `transcriptLines` stays `0`, Meet captions may be disabled, no one has spoken since the observer was installed, the Meet UI changed, or live captions are unavailable for the meeting language/account.

`googlemeet test-speech` always checks the realtime path and reports whether bridge output bytes were observed for that invocation. If `speechOutputVerified` is false and `speechOutputTimedOut` is true, the realtime provider may have accepted the utterance but GrokBot did not see new output bytes reach the Chrome audio bridge.
...
```bash
grokbot googlemeet recover-tab
grokbot googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
grokbot googlemeet setup --transport twilio
grokbot voicecall setup
grokbot voicecall smoke
```

`voicecall smoke` is readiness-only by default. Dry-run a specific number:

```bash
grokbot voicecall smoke --to "+15555550123"
```

Only add `--yes` to intentionally place a live outbound call:

```bash
grokbot voicecall smoke --to "+15555550123" --yes
```
...
- `grokbot googlemeet doctor <session-id>`: confirm the delegated Twilio call ID, whether DTMF was queued, and whether the intro greeting was requested.
- `grokbot voicecall status --call-id <id>`: confirm the call is still active.
- `grokbot voicecall tail`: confirm Twilio webhooks are arriving at the Gateway.
- `grokbot logs --follow`: look for the Twilio Meet sequence: Google Meet delegates the join, Voice Call stores and serves pre-connect DTMF TwiML, Voice Call serves realtime TwiML for the Twilio call, then Google Meet requests intro speech with `voicecall.speak`.
- Re-run `grokbot googlemeet setup --transport twilio`; a green setup check is required but does not prove the meeting PIN sequence is correct.
...
- If the participant joins but you do not hear the greeting, check `grokbot logs --follow` for the post-DTMF `voicecall.speak` request and either media-stream TTS playback or the Twilio `<Say>` fallback. If the transcript still shows "enter the meeting PIN", the phone leg has not joined the Meet room yet, so participants will not hear speech.
