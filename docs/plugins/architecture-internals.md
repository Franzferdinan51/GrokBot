...
At startup, GrokBot does roughly this:
...
The manifest is the control-plane source of truth. GrokBot uses it to:
...
derived set is empty, GrokBot keeps the scope empty instead of widening to
...
GrokBot does not cache plugin discovery results or direct manifest registry
...
GrokBot still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, GrokBot calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that GrokBot no longer calls, such as
...
| _(built-in model lookup)_         | GrokBot tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared GrokBot refreshers                                                                                          |
...
that still runs on GrokBot's normal inference loop.

`resolveUsageAuth` decides whether GrokBot should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of GrokBot's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as GrokBot adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  schemas while GrokBot owns the provider/runtime boundary.
...
- GrokBot only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "GrokBot plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope. The default surface (`gatewayRuntimeScopeSurface: "write-default"`) is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) and any non-trusted-proxy auth method get a single `operator.write` scope, even if the caller sends `x-grokbot-scopes`
  - `trusted-proxy` callers without an explicit `x-grokbot-scopes` header also keep the legacy `operator.write`-only surface
  - `trusted-proxy` callers that do send `x-grokbot-scopes` get the declared scopes instead
  - a route can opt into `gatewayRuntimeScopeSurface: "trusted-operator"` to always honor `x-grokbot-scopes` for identity-bearing auth modes (falling back to the full CLI default scope set when the header is absent)
...
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, opt into `trusted-operator` scope surface, require an identity-bearing auth mode, and document the explicit `x-grokbot-scopes` header contract.
...
Use narrow SDK subpaths instead of the monolithic `grokbot/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                            | Purpose                                      |
| ---------------------------------- | -------------------------------------------- |
| `grokbot/plugin-sdk/plugin-entry` | Plugin registration primitives               |
| `grokbot/plugin-sdk/channel-core` | Channel entry/build helpers                  |
| `grokbot/plugin-sdk/core`         | Generic shared helpers and umbrella contract |
...
<Info>
`grokbot/plugin-sdk/channel-lifecycle`, small channel helper facades,
`grokbot/plugin-sdk/config-runtime`, and `grokbot/plugin-sdk/infra-runtime`
...
External plugins should only import `grokbot/plugin-sdk/*` subpaths. Never
...
`grokbot/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape GrokBot writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to GrokBot's
...
- if both `catalog` and `discovery` are registered, GrokBot uses `catalog`
  and emits a warning
...
- Read-only command paths such as `grokbot status`, `grokbot status --all`,
  `grokbot channels status`, `grokbot channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `grokbot.extensions`:
...
Security guardrail: every `grokbot.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `grokbot plugins install` installs plugin dependencies with a
...
Optional: `grokbot.setupEntry` can point at a lightweight setup-only module.
When GrokBot needs setup surfaces for a disabled channel plugin, or
...
Optional: `grokbot.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let GrokBot load the
...
Channel plugins can advertise setup/discovery metadata via `grokbot.channel` and
install hints via `grokbot.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@grokbot/nextcloud-talk",
  "grokbot": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@grokbot/nextcloud-talk",
...
Useful `grokbot.channel` fields beyond the minimal example:
...
GrokBot can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.grokbot/mpm/plugins.json`
- `~/.grokbot/mpm/catalog.json`
- `~/.grokbot/plugins/catalog.json`

Or point `OPENCLAW_PLUGIN_CATALOG_PATHS` (or `OPENCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "grokbot": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `grokbot.install` block. The
...
This is how GrokBot stays opinionated without becoming hardcoded to one
