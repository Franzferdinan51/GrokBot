---
summary: "Create your first GrokBot plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new GrokBot plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend GrokBot without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the GrokBot repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
grokbot plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because GrokBot discovers
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect GrokBot to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through GrokBot model fallback.
  </Card>
...
```json grokbot.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to GrokBot",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    appear in `contracts.tools` so GrokBot can discover ownership without
  ...
  <Step title="Register the tool">
    ...
    `defineChannelPluginEntry` from `grokbot/plugin-sdk/core` instead.
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    grokbot plugins inspect my-plugin --runtime --json
    ```

    If the plugin registers a CLI command, run that command too and confirm
    output, for example `grokbot demo-plugin ping`.

    For a bundled plugin in this repository, GrokBot discovers source-checkout
  ...
  <Step title="Test the package install">
    ...
    `grokbot.extensions` at built JavaScript like `./dist/index.js`, and make
    ...
    grokbot plugins install npm-pack:/tmp/<plugin-package>.tgz --force
    grokbot plugins inspect my-plugin --runtime --json
    ```

    `npm-pack:` uses GrokBot's managed per-plugin npm project, so it catches
  ...
  <Step title="Install">
    ...
    ```bash
    grokbot plugins install clawhub:your-org/your-plugin
    ```
...
plugin is enabled. Optional tools need explicit user opt-in before GrokBot
...
operator, installed plugin code, or a modified GrokBot runtime. Sensitive
...
aligned with `api.registerTool(..., { optional: true })` so GrokBot can avoid
...
`grokbot/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `grokbot` metadata</Check>
<Check>**grokbot.plugin.json** manifest is present and valid</Check>
...
1. Watch [grokbot/grokbot](https://github.com/grokbot/grokbot/releases) releases (`Watch` > `Releases`). Beta tags look like `v2026.3.N-beta.1`. You can also follow [@grokbot](https://x.com/grokbot) on X for release announcements.
