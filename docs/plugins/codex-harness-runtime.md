...
read_when:
  ...
  - You are changing plugin behavior across GrokBot and Codex harness turns
---
...
continuation, and native compaction. GrokBot owns channel routing, session
files, visible message delivery, GrokBot dynamic tools, approvals, media
...
GrokBot compatibility route keeps the normal GrokBot system prompt even when
it uses Codex-flavored OpenAI auth or transport.

GrokBot starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and GrokBot agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. Lightweight
GrokBot runs (for example cron) still suppress project-doc loading.

GrokBot developer instructions cover GrokBot runtime concerns: source-channel
delivery, GrokBot dynamic tools, ACP delegation, adapter context, and the
...
Most GrokBot dynamic tools use the searchable `grokbot` namespace. Tools
...
When an GrokBot session is attached to an existing Codex thread, the next
...
Supervised bindings are the exception. The GrokBot model picker stays locked,
...
model-difference warning; the outer GrokBot model and fallback chain never
...
not resume a thread, subscribe GrokBot to its live events, or answer its
approvals.
...
On the same supervision connection, GrokBot starts the canonical
...
resumed. The canonical thread has the full GrokBot harness tool surface;
...
GrokBot allows archive for a local `idle` or `notLoaded` row only after explicit
...
prove that another process is not using the thread. GrokBot blocks a known
...
contract: the agent replies normally and GrokBot posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable GrokBot tool
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| GrokBot plugin hooks                 | GrokBot                 | Product/plugin compatibility across GrokBot and Codex harnesses.   |
| Codex app-server extension middleware | GrokBot bundled plugins | Per-turn adapter behavior around GrokBot dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

GrokBot does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, GrokBot injects
...
so Codex's app-server reviewer and GrokBot's approval bridge handle real
...
controls; they are not exposed as GrokBot plugin hooks in the v1 contract.

For GrokBot dynamic tools, GrokBot executes the tool after Codex asks for
the call, so plugin and middleware behavior runs in the harness adapter. For
Codex-native tools, Codex owns the canonical tool record; GrokBot can mirror
...
matching app-server approval. If an GrokBot `before_tool_call` hook returns
...
request for the same tool use, GrokBot opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through GrokBot approvals when
...
notifications and GrokBot adapter state, not native Codex hook commands.
...
debugging. They do not invoke GrokBot plugin hooks.
...
| GrokBot channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                    |
| GrokBot dynamic tools                        | Supported                                                                        | Codex asks GrokBot to execute these tools, so GrokBot stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | GrokBot projects GrokBot-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. GrokBot disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around GrokBot-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through GrokBot after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | GrokBot records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but GrokBot does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. GrokBot owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms GrokBot-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | GrokBot can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | GrokBot does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | GrokBot can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, GrokBot only returns explicit allow or deny
...
request, GrokBot remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through GrokBot's plugin approval
...
resolve the same gateway record before GrokBot returns the app-server answer.
...
default `messages.queue.mode: "steer"`, GrokBot batches steer-mode chat
...
that case, GrokBot waits for the active run to finish before starting the
...
harness, GrokBot also calls Codex app-server `feedback/upload` for relevant
...
GrokBot session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, GrokBot does not print those Codex ids
...
belongs to Codex app-server. GrokBot does not run preflight compaction for
Codex turns, replace Codex compaction with context-engine compaction, or fall
back to GrokBot or public OpenAI summarization when native compaction cannot
be started. GrokBot keeps a transcript mirror for channel history, search,
...
GrokBot keeps the request and shared-client lease open until Codex emits the
...
timeout, GrokBot requests a native turn interrupt. The lease and per-thread
...
period, GrokBot retires the connection before releasing the fence. Remote
...
compaction is Codex's job; GrokBot only starts native compaction for manually
requested triggers.

When a context engine requests Codex thread-bootstrap projection, GrokBot
...
Codex reasoning or plan records when the app-server emits them. GrokBot
...
not rewrite Codex-native tool result records. It only applies when GrokBot
writes an GrokBot-owned session transcript tool result.

## Media and delivery

GrokBot continues to own media delivery and media provider selection. Image,
...
through the normal GrokBot delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, GrokBot forwards that exact file through the normal reply-media
