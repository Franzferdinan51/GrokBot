---
summary: "Bundled `oc-path` plugin: ships the `grokbot path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`grokbot path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the GrokBot repo under
...
- **Local automation**: shell scripts resolve or update one workspace value
  with `grokbot path … --json` instead of carrying separate markdown, JSONC,
...
grokbot path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
grokbot path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `grokbot` CLI** on the host where you
...
Plugin metadata lives in `extensions/oc-path/grokbot.plugin.json`:
...
lazily the first time you run `grokbot path …`, so installs that never use
...
state. Bare `grokbot path` invocations work immediately on the same host;
the CLI loads the plugin on demand.

Disable with:

```bash
grokbot plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `grokbot path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
- [`grokbot path` CLI reference](/cli/path)
