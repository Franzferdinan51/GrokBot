...
read_when:
  - You want to place an outbound voice call from GrokBot
...
Voice calls for GrokBot via a plugin: outbound notifications, multi-turn
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        grokbot plugins install @grokbot/voice-call
        ```
      </Tab>
...
| `store`                         | unset        | Overrides the default `$OPENCLAW_STATE_DIR/voice-calls` path (normally `~/.grokbot/voice-calls`). |
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    release (`2026.6.0`), so run `grokbot doctor --fix` to rewrite committed
...
- Voice Call exposes the shared `openclaw_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal GrokBot tools.
...
configured GrokBot agent without paying a full agent-consult round trip on
...
`openclaw_agent_consult` still run the full GrokBot agent and should be used
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `grokbot doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
grokbot voicecall call --to "+15555550123" --message "Hello from GrokBot"
grokbot voicecall start --to "+15555550123"   # alias for call
grokbot voicecall continue --call-id <id> --message "Any questions?"
grokbot voicecall speak --call-id <id> --message "One moment"
grokbot voicecall dtmf --call-id <id> --digits "ww123456#"
grokbot voicecall end --call-id <id>
grokbot voicecall status --call-id <id>
grokbot voicecall tail
grokbot voicecall latency                      # summarize turn latency from logs
grokbot voicecall expose --mode funnel
```
...
Provider signatures are checked against the public URL GrokBot reconstructs
...
Use `grokbot logs --follow` for the live phase trace. A healthy Twilio Meet
...
`grokbot voicecall tail` still shows persisted call records; useful for
...
- `grokbot logs --follow` shows realtime TwiML served, the realtime bridge started, and the initial greeting queued.
