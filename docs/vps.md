---
summary: "Run GrokBot on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for GrokBot
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the GrokBot Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install GrokBot on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep GrokBot bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/grokbot-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Service env for a stable startup path: `OPENCLAW_NO_RESPAWN=1` and
  `NODE_COMPILE_CACHE=/var/tmp/grokbot-compile-cache`
...
The standard `grokbot onboard --install-daemon` path installs a systemd user
unit; edit it with:
...
`sudo systemctl edit grokbot-gateway.service`.
