...
as a dependency installer or package-script runner, but it cannot run GrokBot
because it does not provide `node:sqlite`.

## Desktop companion

The GrokBot Linux companion is a Tauri desktop app for a local Gateway. It:

- installs the GrokBot CLI and managed Node runtime when they are missing; release builds install the stable channel automatically, while development builds ask for the channel first
...
[GitHub release](https://github.com/grokbot/grokbot/releases) for the tag,
named `GrokBot-<version>-amd64.deb` and `GrokBot-<version>-amd64.AppImage`,
with a `SHA256SUMS.linux-app.txt` checksum file next to them. Download the
`.deb` and install it with `sudo apt install ./GrokBot-<version>-amd64.deb`,
...
`grokbot-linux-companion` artifact for pull requests touching the app and for
...
Linux Canvas uses two cooperating processes. `grokbot node run` remains the single Gateway node connection; the bundled `linux-canvas` plugin forwards `canvas.*` calls to the running desktop app over a user-only Unix socket. The app owns one on-demand WebView window, including the bundled A2UI renderer and action bridge back to the agent.

The plugin is enabled by default. It advertises Canvas only when the desktop socket exists at `$XDG_RUNTIME_DIR/grokbot-canvas.sock`, or `/tmp/grokbot-canvas-$UID.sock` when `XDG_RUNTIME_DIR` is unavailable. Disable it with `plugins.entries.linux-canvas.enabled: false`. On a headless Linux server without the desktop app, Canvas is not advertised.
...
2. `npm i -g grokbot@latest`
3. `grokbot onboard --install-daemon`
...
The bundled Linux Node plugin gives the CLI `grokbot node` service device capabilities without requiring the desktop app. Commands are advertised to the Gateway only when their capability is enabled and the required local tool exists.
...
Configure the plugin in `grokbot.json`:
...
grokbot configure   # select "Gateway service" when prompted
```
...
`grokbot gateway install` renders a systemd **user** unit by default. Full
...
(`~/.config/systemd/user/grokbot-gateway[-<profile>].service`):

```ini
[Unit]
Description=GrokBot Gateway (profile: <profile>, v<version>)
...
Hand-written units do not inherit the adaptive heap sizing that `grokbot gateway install` writes for managed Gateway services. Prefer the managed installer, or set an explicit heap limit in the custom supervisor after accounting for native-memory headroom.
...
sessions and channel connections, so GrokBot biases transient child
processes to be killed first when possible.

For eligible Linux child spawns, GrokBot wraps the command in a short
...
- GrokBot-launched browser/Chrome processes (via the plugin SDK process runtime)
