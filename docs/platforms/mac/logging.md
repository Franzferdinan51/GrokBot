---
summary: "GrokBot logging: rolling diagnostics file log + unified log privacy flags"
read_when:
...
- Location: `~/Library/Logs/GrokBot/diagnostics.jsonl`.
...
## Enable for GrokBot (`ai.grokbot`)

Write the plist to a temp file first, then install it atomically as root:

```bash
cat <<'EOF' >/tmp/ai.grokbot.plist
...
EOF
sudo install -m 644 -o root -g wheel /tmp/ai.grokbot.plist /Library/Preferences/Logging/Subsystems/ai.grokbot.plist
```
...
- Remove the override: `sudo rm /Library/Preferences/Logging/Subsystems/ai.grokbot.plist`.
