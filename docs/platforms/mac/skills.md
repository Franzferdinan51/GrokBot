...
The macOS app surfaces GrokBot skills via the gateway; it does not parse skills locally.
...
- Requirements come from `metadata.grokbot.requires` in each `SKILL.md`.

## Install actions

- `metadata.grokbot.install` defines install options (brew/node/go/uv/download).
...
- The app stores keys in `~/.grokbot/grokbot.json` under `skills.entries.<skillKey>`.
