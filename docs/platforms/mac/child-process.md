...
enables the launchd service via the external `grokbot` CLI (no embedded
...
- The app installs a per-user LaunchAgent labeled `ai.grokbot.gateway` (or
  `ai.grokbot.<profile>` when using `--profile`/`OPENCLAW_PROFILE`).
...
Replace the label with `ai.grokbot.<profile>` when running a named profile.
...
`~/.grokbot/disable-launchagent`, so the app only attaches to an already
