...
Windows Hub publishes independently from the GrokBot CLI and Gateway. Download
the latest stable Hub installer from the
[Windows Hub releases page](https://github.com/grokbot/grokbot-windows-node/releases/latest)
or directly via `releases/latest/download`:

- [OpenClawCompanion-Setup-x64.exe](https://github.com/grokbot/grokbot-windows-node/releases/latest/download/OpenClawCompanion-Setup-x64.exe)
- [OpenClawCompanion-Setup-arm64.exe](https://github.com/grokbot/grokbot-windows-node/releases/latest/download/OpenClawCompanion-Setup-arm64.exe)

If a link above 404s, visit the [Windows Hub releases page](https://github.com/grokbot/grokbot-windows-node/releases)
and open the newest stable Windows Hub release. Regular GrokBot stable releases
also mirror a pinned, release-validated Windows Hub build; that mirror can lag a
newer standalone Hub release.

After install, launch **GrokBot Companion** from the Start menu or system
...
Windows Hub can register as an GrokBot node so the agent can use declared
...
without a running GrokBot Gateway.
...
For terminal-first use, install GrokBot from PowerShell:
...
the readable `gateway.cmd` script in the GrokBot state dir but launches it
through a generated `gateway.vbs` WScript wrapper, so the background Gateway
does not open a visible console window. If task creation is denied, GrokBot
...
```powershell
grokbot onboard --non-interactive --skip-health
grokbot gateway run
```
...
Then install GrokBot inside WSL with the Linux quickstart:
...
```bash
systemctl --user is-enabled grokbot-gateway.service
systemctl --user status grokbot-gateway.service --no-pager
```
...
Check Task Manager for `GrokBot.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If not, launch **GrokBot Companion** from
the Start menu.
