...
summary: "How GrokBot structures the built-in agent runtime: code layout, boundaries, resource manifests, and runtime selection."
---

GrokBot owns the built-in agent runtime. Runtime code lives under `src/agents/`, model/provider transport lives under `src/llm/`, and plugin-facing contracts are exposed through `grokbot/plugin-sdk/*` barrels.
...
| `packages/agent-core/`              | Reusable agent core (`@grokbot/agent-core`): agent loop, harness types, messages, compaction helpers, prompt templates, skills, and session storage contracts.                                                           |
| `src/agents/runtime/`               | GrokBot facade that wires `@grokbot/agent-core` to the plugin SDK LLM runtime and re-exports it plus local proxy utilities.                                                                                             |
| `src/agents/agent-tools*.ts`        | GrokBot-owned tool definitions, parameter schemas, tool policy, before/after tool-call adapters, and host/sandbox edit tools.                                                                                            |
...
Core calls the built-in runtime through GrokBot modules and SDK barrels; no external agent framework packages remain. Plugins use documented `grokbot/plugin-sdk/*` entrypoints and do not import `src/**` internals.
...
- The built-in runtime id is `grokbot`. The legacy alias `pi` normalizes to `grokbot`; `codex-app-server` normalizes to `codex`.
...
- `auto` selects a registered plugin harness that supports the effective provider route, otherwise the built-in GrokBot runtime. A provider or model prefix alone never selects a harness.
- OpenAI may select `codex` implicitly only for an exact official HTTPS Platform Responses or ChatGPT Responses route with no authored request override. Completions adapters, custom endpoints, and routes with authored request behavior stay on `grokbot`; plaintext official HTTP endpoints are rejected. See [OpenAI implicit agent runtime](/providers/openai#implicit-agent-runtime).
...
- [GrokBot agent runtime workflow](/grokbot-agent-runtime)
